...
*   **Delivery-state measurement**: what launch monitors can and cannot infer about the delivery state the fitting pipeline predicts. (See: [Launch Monitor Technology](technology-launch-monitors.html))
*   **Heavy-hit decoupling**: why fitting shapes the pre-impact delivery state — muscle action cannot push through the collision window. (See: [Multibody Impact Coupling and the Heavy Hit Hypothesis](technology-heavy-hit-impact-coupling.html))
*   **Ground reaction measurement**: force plates and pressure insoles supplying the base constraints of fitting simulations. (See: [Force Measurement Technology](technology-force-measurement.html))
*   **Reference points**: club delivery data must name the reference point its twists and wrenches are expressed about. (See: [The Reference-Point Problem](reference-point-problem.html))
*   **Vendor measurement claims**: per-vendor sensor and parameter capabilities for the inputs the fitting document consumes. (See: [Launch Monitor Vendor Reference](launch-monitor-vendor-reference.html))
*   **Model interchange**: the simulation engines that consume the fitting and delivery-trajectory wires. (See: [Drake Models](../models/models-drake.html), [MuJoCo Models](../models/models-mujoco.html), [OpenSim Models](../models/models-opensim.html), and [Model Repositories](../repositories/repositories-models.html))
:::
