...
$$
where $a_{\text{drift}}(x) = -M^{-1}(q,\eta) \left( C(x) \dot{q}_{\text{sys}} + G(q,\eta) + F_s(\eta,\dot{\eta}) \right)$.
The terms constituting $f(x)$ are:
1.  **Kinematic rates:** The state variables $\dot{q}, \dot{\eta}$ are treated as independent of the input $u$ in the instantaneous state-space formulation. While $u$ obviously causes future changes in velocity over time, at any fixed instant $t$, the current state $x(t)$ is independent of the current input $u(t)$, meaning the instantaneous partial derivative is zero: $\frac{\partial \dot{q}}{\partial u} = 0$.
...
=
\begin{bmatrix}
0 \\ \left[-\frac{\partial M^{-1}}{\partial u} h(x) - M^{-1} \frac{\partial h(x)}{\partial u}\right]_{1:n} \\ 0 \\ \left[-\frac{\partial M^{-1}}{\partial u} h(x) - M^{-1} \frac{\partial h(x)}{\partial u}\right]_{n+1:n+m}
\end{bmatrix}.
$$
...
$$\tau_{\text{drift}}(t)
  =
  M_{qq}(q,\eta)\,[a_{\text{drift}}(x(t))]_{1:n}.$$
