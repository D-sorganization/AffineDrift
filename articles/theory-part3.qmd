...
Having established that the drift field is invariant to input, we have satisfied the necessary condition for a rigorous classification. We can now assert that every Newton of force acting on the system belongs to exactly one causal category: it is either a static configurational load, a dynamic velocity load, or an active input force. The following taxonomy operationalizes this logic into a set of definitions computable from standard inverse dynamics.
