...
In Parts I and II, we constructed the ZTCF and ZVCF baselines on the premise that the passive dynamics $f(x)$ remain well-defined even when the input $u$ changes. This property—*drift invariance*—is the logical fulcrum of the entire framework. If the passive behavior of the system were to shift implicitly when the golfer applied torque (e.g., via unmodeled physiological coupling), the counterfactuals would lose their causal validity. This section formalizes the drift invariance condition and demonstrates why the mechanical assumptions made in Part I guarantee it holds.
