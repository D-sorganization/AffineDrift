  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        Part 3 tackles a subtle but critical question: Does the "passive" nature of the swing change when you try harder? (Spoiler: No.)
      </p>

      <div class="laymans-item">
        <h3>The "Invariant" Drift</h3>
        <p>
          We prove a property called <strong>Drift Invariance</strong>. It means that the laws of physics governing the club's passive motion (gravity, inertia) don't change just because you apply more muscle force&mdash;<em>as long as your grip stiffness does not change</em>. If you suddenly clench the grip mid-swing (or go slack), the effective stiffness of the hand&ndash;club interface changes, and that <em>does</em> alter the passive dynamics. The invariance is a claim about the math of applying a given torque, not a claim that your body's stiffness profile is fixed.
        </p>
        <div class="analogy">
          <strong>Analogy:</strong> Think of a car on a road. The road's slope and the car's weight are the "drift" factors. Whether you gently tap the gas or floor it (input), the road and the car's weight remain the same. The "drift" is independent of your "input." (What this analogy doesn't capture: if you could somehow add weight to the car by pressing the pedal harder, the analogy would break&mdash;which is exactly what a mid-swing grip-stiffness change does.)
        </div>
      </div>
      ...
      <div class="laymans-item">
        ...
        <div class="key-takeaway">
          This system lets us freeze a swing at one instant and say, "50% of the force acting right now comes from momentum (Drift), and 50% comes from the player (Input)." The split describes the forces at that instant; it does not carve the finished swing into a drift half and a player half.
        </div>
      </div>
...
Recall the control-affine form $\dot{x} = f(x) + g(x)u$ established in Part 1, where $f$ encodes drift and $g$ encodes control directions [@isidori1995nonlinear]. In Part 2 we constructed two counterfactual baselines: the **Zero Torque Counterfactual (ZTCF)**, which integrates the drift field $f(x)$ to expose velocity-dependent drift (see [canonical ZTCF definitions](zero-torque-counterfactual.html#sec-ztcf-canonical)), and the **Zero Velocity Counterfactual (ZVCF)**, which freezes velocity to isolate configuration loads. Both tools rely on the subtraction $\tau_{\text{input}} = \tau_{\text{total}} - \tau_{\text{drift}}$.
...
In Part 2, we defined the ZTCF and ZVCF as tools to isolate passive dynamics from active input. These constructions rely on the algebraic separation of the equations of motion into a drift term $f(x)$ and an input term $g(x)u$.
...
If applying torque altered the drift field $f(x)$—by implicitly modifying the effective inertia or stiffness matrix—then our baselines would be moving targets, dependent on the very variable we seek to isolate.
...
::: {.callout-important}
## Standing Hypothesis of Proposition 1 --- Constant (Input-Independent) Impedance
...
*Proof.* Recall the definition of $f(x)$ derived in Part 1:
$$
f(x) = \begin{bmatrix} \dot{q} \\ [a_{\text{drift}}(x)]_{1:n} \\ \dot{\eta} \\ [a_{\text{drift}}(x)]_{n+1:n+m} \end{bmatrix},
\qquad
a_{\text{drift}}(x) = -M^{-1}(q,\eta) \left( C(x) \dot{q}_{\text{sys}} + G(q,\eta) + F_s(\eta,\dot{\eta}) \right),
$$
...
$$f(x) =
  \begin{bmatrix}
    \dot{q} \\ [a_{\text{drift}}(x)]_{1:n} \\ \dot{\eta} \\ [a_{\text{drift}}(x)]_{n+1:n+m}
  \end{bmatrix}$$
...
::: {.callout-note}
## Applied Hand-Path Evidence

The [proximal-to-distal hand-path article](proximal-distal-energy-transfer.html#force-along-the-hand-path)
defines the matching force, impulse, power, and work estimands. Its new numerical
results remain withheld until a compact evidence snapshot is pinned to an exact
...
This subtraction is valid precisely because of the affine structure derived in Part 1. Recall that the input acceleration is given by $A_{\text{input}}(x)u$. The 'Input Force' defined here is the dynamic realization of that acceleration term, scaled by the system mass matrix [@featherstone2008rigid]. It captures not just the direct drive on the joints, but also the forces induced by the inertial coupling ratio ($\Gamma$) acting on the flexible shaft. Thus, $\tau_{\text{input}}$ represents the complete mechanical footprint of the active control.

### Category 4: Trajectory-Mediated Differences
...
In simulation (as developed in [Part 4](theory-part4.html) and [Part 5](theory-part5.html)), these categories enable:
...
This taxonomy completes the theoretical framework, providing a model-based map from observed forces to the mechanical categories used here. However, the exactness of that map is bounded by the fidelity of the model itself. While the decomposition is algebraically exact within the model, its application to reality is constrained by the simplifications made in Part 1. We now catalog these limitations to define the scope of validity for the causal claims made on this site.
...
we derived a clean separation between passive drift forces and torque-driven input forces. The model's affine structure enabled two mathematically rigorous counterfactual tools—the ZTCF and the ZVCF—which isolate the contributions of inertia, configuration, shaft deformation, and applied torques.

The resulting taxonomy classifies generalized forces into configuration drift, velocity drift, input forces, and trajectory-mediated differences. This provides a principled basis for interpreting torques and hand forces at any instant of the swing. Within the modeling assumptions stated earlier, the decomposition is analytically exact and reproducible. It offers a mechanical explanation of how the swing evolves over time, distinguishing forces that arise "on their own" from those produced directly by applied torques.
...
- **Part 4: Mathematical Appendices.**
  These appendices provide the supporting derivations. They contain the detailed block matrix inversion proofs and modal approximation steps that justify the affine separation for flexible multibody systems.

- **Part 5: Numerical Validation (Simulink).**
  This section provides the **numerical stress test**. By implementing the full forward dynamics in a stiff solver environment, Part 5 tests whether the delicate counterfactual subtractions remain stable under integration error and whether the implementation preserves the algebraic separation developed on the page.
...
The framework developed in this manuscript provides a new mechanical viewpoint for interpreting forces in the golf swing. The technical details are presented for scientific completeness, but the practical impact is straightforward. The approach separates forces that arise naturally from the motion of the body and club (passive drift forces) from forces the golfer must actively create through muscular torque (input forces). This distinction is essential for understanding how players produce clubhead speed, how they sequence their movements, and why some patterns are more efficient than others.
...
For the mathematical reader interested in the rigorous derivation of the control-affine form from first principles, we refer to the detailed derivations collected in the Appendices (Part 4). Those appendices provide the **block matrix inversion proofs** that justify the clean separation of rigid and flexible dynamics. They also show that the modal approximation of the shaft preserves the linearity of the torque input—the condition on which the Force Taxonomy above depends.
