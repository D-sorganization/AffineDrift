...
::: abstract
This article provides a comprehensive catalog of the classic sources of nonlinearity in mechanical and biomechanical systems, with explicit attention to how each interacts with the control-affine framework $\dot{x} = f(x) + G(x)u$. While nonlinearities abound—from configuration-dependent mass matrices to friction, saturation, and biological force-velocity relationships—the fundamental principle of *force superposition at the level of Newton's second law* still holds. This article examines where each nonlinearity enters the equations of motion (drift versus input), whether it preserves affine structure, how it affects the validity of the zero-torque counterfactual (ZTCF), and its practical implications for decomposing dynamics into drift and input components. We provide rigorous mathematical treatment alongside concrete examples, with special attention to the golf swing, locomotion, and robotic manipulation.
:::
...
$$
where $\tau_u = G(q)u$, $\tau_C = -C(q,\dot{q})\dot{q}$, and $\tau_g = -g(q)$ are the input, Coriolis, and gravitational torques. At a **fixed state**, $M(q)^{-1}$ is a fixed linear operator, so the force-induced accelerations superpose **exactly for any $M(q)$** — constant or not:
$$
\ddot{q} = \ddot{q}_u + \ddot{q}_g + \ddot{q}_C, \qquad
\ddot{q}_u = \tilde{G}(q)u,\;\; \ddot{q}_g = -M(q)^{-1}g(q),\;\; \ddot{q}_C = -M(q)^{-1}C(q,\dot{q})\dot{q}.
$$
The reason the two-term sum $\ddot{q}_u + \ddot{q}_g$ misses the total is that it **omits the Coriolis acceleration** $\ddot{q}_C$, *not* that $M(q)$ varies. (Where $M(q)$ does matter is across *different* states: the gain $\tilde{G}(q) = M(q)^{-1}G(q)$ is state-dependent, so superposition is local to a fixed state, not global.)

**Implication for ZTCF:** The ZTCF is based on comparing $u = 0$ (passive drift) versus $u \neq 0$ (active input) at fixed state. The mass matrix does not invalidate this decomposition; it means only that the effective input gain $\tilde{G}(q) = M(q)^{-1}G(q)$ is state-dependent. The passive drift $f(x) = f(q, \dot{q})$ still represents "what happens with $u = 0$," and the input $G(x)u$ still represents "the linear response to control" at that instant.
...
**Does it preserve affinity?** The equations of motion written in angular velocity $\omega$ remain affine in torque:
$$
...
**Static friction** is the maximum friction force that can resist motion before sliding begins. A **breakaway (threshold) torque** $\tau_{\text{break}}$ characterizes it: below that torque, the joint does not move.
...
**Implication for ZTCF:** Static friction complicates the ZTCF. The passive drift at $u = 0$ includes stiction, which "holds" the joint in place. But stiction also modifies the response to small perturbations. Separating friction effects from intrinsic dynamics requires careful experimental design, or high-frequency dither.
...
The **sign function discontinuity** at $\dot{q} = 0$ is a key feature. Set-valued analysis, which admits the sign function directly, or regularization handles it mathematically.
...
**Implication for ZTCF:** Contact adds hybrid dynamics to the system. The ZTCF must account for the possibility of transitioning in and out of contact. In biomechanics (walking, running, landing), contact is episodic, so the ZTCF decomposition applies within contact and swing phases separately.
...
**Deadzone** is a region of input values around zero that produces no output. It is common in actuators with friction or threshold-based operation.
...
**Does it preserve affinity?** Hysteresis introduces state that $(q, \dot{q})$ alone does not capture. The full state must include the "history" of deformation or an internal state variable. With that variable in place, an extended state space can carry hysteresis — at the cost of complicating the system significantly.
...
$$
where $\sigma$ is a width parameter. More often, models use a **piecewise linear** form:
...
**Does it preserve affinity?** No, if we treat the muscle force as the control input. The relationship $F(u) = F_{\text{max}}(L) \cdot u$ where $F_{\max}$ depends on length $L$ is **state-dependent input gain**, but the nonlinearity is multiplicative, not additive. It amounts to a state-dependent modification of $G(x)$.
...
**Hill's force-velocity relationship** describes how muscle force depends on contraction velocity. As the muscle contracts faster, the force decreases. The canonical Hill hyperbola $(F + a)(v + b) = (F_0 + a)\,b$, solved for $F$, is
$$
F(v) = \frac{F_0\,b - a\,v}{v + b},
$$
where $v$ is shortening velocity (positive for contraction), $F_0$ is isometric force, and $a, b$ are shape parameters. The curvature constant is tied to the maximum shortening velocity by $a = F_0\,b / v_{\max}$, which fixes the two physical limits: $F(0) = F_0$ (isometric) and $F(v_{\max}) = 0$ (force vanishes at maximum shortening speed). Equivalently, in normalized form, $F(v) = F_0\,(v_{\max} - v)/(v_{\max} + k\,v)$ with $k = a/F_0 \approx 4$.

The force-velocity relationship has several regimes:
- **Isometric** ($v = 0$): Force is maximum ($= F_0$).
- **Shortening** ($v > 0$): Force decreases hyperbolically with velocity, reaching zero at $v = v_{\max}$.
...
This is a challenge for the affine model because stiffness is typically considered a property of the joint, not part of the control input. But in muscle-actuated systems, co-contraction actively controls stiffness.
...
This effect is **history-dependent**: the enhanced force depends on the pre-stretch magnitude, rate, and timing. State $(q, \dot{q}, a)$ alone cannot capture it; the model must add further state variables such as fiber length and passive elastic strain.
...
**Implication for ZTCF:** SSC effects are problematic for a simple ZTCF because the passive dynamics (when $u = 0$) are not a true equilibrium; they depend on prior motion history. A generalized ZTCF can account for this by treating the passive dynamics as a dynamical system with its own history, not a simple counterfactual.
...
**Does it preserve affinity?** If temperature is treated as a slow, quasi-static parameter, the system remains approximately affine at each instant. If temperature dynamics are fast and coupled to the mechanical system, the full state space must include them.
...
5. **Elastic:** The golfer has series elastic elements (tendons, passive elastic tissues), and the club shaft is flexible.
