...
**Does it preserve affinity?** The canonical body-frame Euler equations are affine in torque:
$$
I_b \dot{\omega}_b + \omega_b \times (I_b \omega_b) = \tau_b,
$$
where $I_b$ is constant in the body frame, $\omega_b$ is the angular velocity expressed in body coordinates, and $\tau_b$ is the applied torque in body coordinates. The gyroscopic term $\omega_b \times (I_b \omega_b)$ is state-dependent but input-independent, so affinity is preserved.

**Note on world-frame formulation:** If one instead writes the equation in the world frame using the time-varying inertia tensor $I_{\text{world}}(t) = R(t) I_b R(t)^\top$, then an additional term $\dot{I}_{\text{world}} \omega$ must be included:
$$
I_{\text{world}} \dot{\omega} + \dot{I}_{\text{world}} \omega = \tau - \omega \times (I_{\text{world}}\omega).
$$
Omitting $\dot{I}_{\text{world}}$ in the world-frame equation gives an inconsistent result; the body-frame form is preferred precisely because $I_b$ is constant and this term vanishes. However, when integrating to recover orientation (using Euler angles or another parameterization), the kinematic equations introduce state-dependent nonlinearity in the orientation update.
