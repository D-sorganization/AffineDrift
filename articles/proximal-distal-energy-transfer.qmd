...
::: {.callout-warning}
## Evidence Pin Pending

The hand-path drift/control extension is not yet published as evidence. Its
[local evidence snapshot](../data/proximal_distal_energy_transfer/hand_path_attribution_snapshot.json)
is deliberately marked `awaiting_upstream_merge`: it contains no source commit,
figures, or numerical findings. Those fields will remain withheld until the
canonical analysis tracked by
[D-sorganization/UpstreamDrift#8470](https://github.com/D-sorganization/UpstreamDrift/issues/8470)
merges to remote `main`, after which selected artifacts will be copied here and
verified by SHA-256. The conceptual definitions below are available for review;
they are not a substitute for the pending results.
:::
...
## Planned Model Ladder and Reporting Contract

The pending extension uses three declared model tiers: an exact planar double
pendulum, a one-arm mobile-hub/three-link model, and a mechanically closed
two-arm model. At every reported interface it must verify total = drift +
control for force, couple, path projection, power, impulse, and work. Joint
tables will retain signs and phase windows rather than reducing vectors to one
norm. Shares will be undefined near a zero denominator or strong cancellation.
For the two-hand tier, resultant club wrench and differential/internal loading
will be reported separately because net club motion does not identify a unique
allocation between the hands.
...
The new hand-path evidence remains withheld under the fail-closed snapshot at
the top of this page. Its later publication requires an exact merged source
commit, artifact hashes, regeneration commands, and the offline check
`python3 scripts/check_proximal_distal_evidence.py --require-pinned`.
