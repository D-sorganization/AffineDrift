...
This section formalizes the limitations of classical inverse dynamics. It then explains how the control-affine framework, together with the Zero Torque Counterfactual (ZTCF) and Zero Velocity Counterfactual (ZVCF), solves the detective's dilemma by separating passive from active contributions.
...
$$
\tau_{\mathrm{drift}}(x) := C(q,\dot{q})\dot{q} + g(q) + \tau_{\mathrm{passive}},
\qquad
\tau_{\mathrm{inertial}} := M(q)\ddot{q},
$$

so that $\tau_{\mathrm{ID}} = \tau_{\mathrm{inertial}} + \tau_{\mathrm{drift}}$ (matching the decomposition in [inverse-dynamics](inverse-dynamics.html) and [drift-components-wrench-double-pendulum](drift-components-wrench-double-pendulum.html)). The *active* contribution is the residual after removing the drift the zero-input (ZTCF) trajectory would require:
...
Thus, classical inverse dynamics cannot unambiguously estimate muscular effort in a drift-dominated system such as the golf swing. Even a model-perfect net input torque remains compatible with different muscle recruitment, co-contraction, and passive-impedance states.
...
The ZVCF therefore isolates the configuration-dependent passive component of generalized forces. Recovering the **pure** input means subtracting the *full* drift, $F_{\mathrm{input}} = F_{\mathrm{total}} - F_{\mathrm{drift}}$ (the drift the ZTCF provides at the branch instant), **not** the ZVCF. Subtracting the ZVCF leaves the velocity-dependent drift (Coriolis/centrifugal) in place, so $F_{\mathrm{total}} - F_{\mathrm{ZVCF}}$ equals $F_{\mathrm{input}} + F_{\mathrm{vel.drift}}$, not input alone. In the downswing the velocity-dependent drift is large, so this distinction matters.
...
The [proximal-to-distal hand-path study](proximal-distal-energy-transfer.html#force-along-the-hand-path) applies this distinction to the equivalent grip force reported in golf inverse dynamics. Its new numerical attribution remains fail-closed until an exact source commit and artifact hashes are pinned.

## Causal Interpretation of Inverse Dynamics in Affine Systems

Because drift is the causal residue of all prior states, the generalized forces required to reproduce a motion necessarily encode the system's history. Classical inverse dynamics at time $t$ therefore conflates contributions from past passive dynamics with present-time active inputs. The affine decomposition clarifies this causality by separating drift from input forces, allowing torque inference to be read against the system's geometry and dynamic history.
