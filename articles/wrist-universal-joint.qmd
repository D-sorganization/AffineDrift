  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        Why does gripping in the fingers vs. the palm matter? It's not just about comfort—it changes the physics of how your wrist transmits power.
      </p>

      <div class="laymans-item">
        <h3>The Wrist is a Universal Joint</h3>
        <p>
          Your wrist acts like the universal joint in a car's driveshaft. It can bend in two directions (up/down, left/right) but can't twist (rotate like a screwdriver). This inability to twist forces torque to "leak" through the joint in predictable ways.
        </p>
        <div class="analogy">
          <strong>Analogy:</strong> Think of a U-joint connecting a truck's engine to its wheels. It transmits power even when bent. Your wrist does the same, transmitting arm rotation to the club without you actively twisting it.
        </div>
      </div>
      ...
      <div class="laymans-item">
        ...
        <p>
          This leaked torque (called "constraint torque") is powerful but messy—you can't really control it.
        </p>
        <ul>
            <li><strong>Finger Grip:</strong> Aims this messy torque at the <em>swing path</em> (helping speed).</li>
            <li><strong>Palm Grip:</strong> Aims this messy torque at the <em>clubface rotation</em> (causing wild hooks/slices).</li>
        </ul>
        <div class="key-takeaway">
          Gripping in the fingers acts like a mechanical filter, channeling the body's chaotic power into speed while keeping the clubface stable.
        </div>
      </div>
    ...
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> We acknowledge the biological complexity. However, soft tissue compliance acts primarily as a damper, not an actuator. While it may soften the <em>magnitude</em> of constraint torques, it does not alter the <em>topology</em> of the transmission. The fundamental lack of a third actuated degree of freedom at the radiocarpal joint remains the dominant constraint.
        </div>
      </div>
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> This highlights a valid trade-off. Single-plane golfers likely accept higher <em>sensitivity</em> to torque variance (High Beta-axis projection) in exchange for reduced <em>input variance</em> (simplified kinematics). Our model quantifies the mechanical cost they must overcome, explaining why such techniques often require unique equipment (e.g., jumbo grips, stiff shafts) to dampen the amplified sensitivity.
        </div>
      </div>
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> While the axis is actuated, the control loop matters. The reaction time to sense a constraint perturbation and reflexively counter it (>100ms) is too slow for the downswing impact interval. Thus, while players do apply <em>feedforward</em> torque, they cannot use feedback to cancel the high-frequency variability of constraint torques. Geometry (grip angle) remains the primary filter for high-frequency noise.
        </div>
      </div>
...
*   $\hat{z}$: Pronation/Supination axis (Constrained at the wrist joint)

::: {.callout-note}
## Constraint torque vs. transmitted torque — terminology

In this derivation the $\hat{z}$ axis is **kinematically constrained at the wrist joint**: no wrist muscle directly actuates rotation about $\hat{z}$. The reaction force that enforces this constraint is the *constraint torque* (a Lagrange multiplier, $\tau_{c,z} = (\mathbf{J}_c^\top \boldsymbol{\lambda})_z$). This is the correct sense of "constraint torque."

It is separate from **transmitted torques**: torques generated by *proximal* actuators (e.g., the pronator teres and supinator muscles at the proximal radioulnar joint) that arrive at the wrist as an external load through the kinematic chain. Forearm pronation/supination falls in the transmitted-torque category — it is actuated proximally, not produced by the wrist's kinematic constraint. Both types act on the $\hat{z}$ axis and are unmodifiable by the wrist alone, but they have distinct physical origins. The remainder of this section derives the constraint torque in the Lagrange-multiplier sense; see @sec-constraint-vs-transmitted for the practical distinction between the two.
:::

The kinematic constraint of the wrist joint dictates that no relative rotation occurs about the local $\hat{z}$ axis **at the wrist**. Consequently, the wrist muscles cannot directly actuate rotation about this axis:
$$
...
### The Wrist as a Torque Transmission Pathway {#sec-constraint-vs-transmitted}
...
The constraint-torque mechanism of the universal joint provides an elegant pathway for this transmission. However, it is important to distinguish two conceptually distinct torques that appear at the wrist's constrained ($\hat{z}$) axis:

- **Constraint torques** (Lagrange-multiplier forces): passive reaction torques that enforce the kinematic constraint $\Phi(\mathbf{q}) = 0$. They do no work and cannot be independently commanded. These are the $\boldsymbol{\lambda}$ terms in the constrained equations of motion.
- **Transmitted torques**: actively generated torques originating from proximal actuators—specifically, the supinator and pronator teres muscles acting at the proximal radioulnar joint—that arrive at the wrist as an external load via the kinematic chain. Forearm pronation/supination torque falls in this category: it is actuated proximally and *transmitted* to the hand segment, not produced by the kinematic constraint itself.

Both types appear on the constrained axis and both are unmodifiable by the wrist joint alone. The practical consequence is the same—the wrist has no mechanism to filter or modulate either—but the physical origin differs and should not be conflated.

However, this passive transmission comes with a critical caveat: because neither constraint torques nor transmitted proximal torques can be directly controlled at the wrist, they inherit all the variability present in the proximal segments that generate them. In other words, the torques acting on the club's rotational axis depend on the motion of other segments; any rotation or torque in the forearm directly affects what arrives at the wrist, and the wrist has no degrees of freedom to modulate or filter those contributions — it is forced to transmit whatever it receives.

This creates a control challenge: the wrist transmits both constraint torques and proximally actuated transmitted torques to the club, but has no mechanism to filter or modulate either contribution. Whatever variability exists in forearm motion, trunk rotation, or their interaction will be transmitted directly through the wrist.
...
Consider a torque perturbation with standard deviation $\sigma_{\tau} = 5$ N·m arriving at the wrist's constrained axis (either as a constraint torque or a proximally transmitted torque) during a short correction window $\Delta t \sim 5\ \text{ms}$ near impact. For a finger grip directing this into the alpha axis ($I_\alpha \approx 0.005\ \text{kg·m}^2$):

$$
\begin{align}
\sigma_{\alpha} &= \frac{\sigma_{\tau}}{I_{\alpha}} = \frac{5\ \text{N·m}}{0.005\ \text{kg·m}^2} = 1{,}000\ \text{rad/s}^2 \\
\sigma_{\omega_{\alpha}} &\approx \sigma_{\alpha} \cdot \Delta t = 1{,}000 \times 0.005 = 5\ \text{rad/s}
\end{align}
$$

This angular velocity variation in the swing plane contributes to clubhead speed variation but has minimal effect on face angle near impact. ($5\ \text{rad/s} \approx 286\ \text{°/s}$, modest relative to the ${\sim}2{,}000\ \text{°/s}$ swing-plane angular velocity.)

For a palm grip directing the same torque into the beta axis ($I_\beta \approx 0.00015\ \text{kg·m}^2$):

$$
\begin{align}
\sigma_{\beta} &= \frac{\sigma_{\tau}}{I_{\beta}} = \frac{5\ \text{N·m}}{0.00015\ \text{kg·m}^2} = 33{,}333\ \text{rad/s}^2 \\
\sigma_{\omega_{\text{face}}} &\approx \sigma_{\beta} \cdot \Delta t = 33{,}333 \times 0.005 = 167\ \text{rad/s}
\end{align}
$$

The accumulated face-angle deviation over the same 5 ms window is:

$$
\sigma_{\text{face angle}} \approx \tfrac{1}{2}\,\sigma_{\beta}\,(\Delta t)^2
= \tfrac{1}{2} \times 33{,}333 \times (0.005)^2
\approx 0.42\ \text{rad} \approx 24°
$$

::: {.callout-note}
## Order-of-magnitude check
A 24° face-angle standard deviation is still large enough to confirm the qualitative claim that beta-axis torque uncertainty has a catastrophic effect on face angle. The 33:1 inertia ratio ($I_\alpha / I_\beta \approx 33$) means the face-axis angular acceleration is 33× larger for the same torque, so even sub-millisecond perturbations matter. A more conservative $\sigma_\tau = 0.5\ \text{N·m}$ (within typical proprioceptive noise estimates) yields $\sigma_{\text{face angle}} \approx 2.4°$—still operationally significant for shot direction.
:::

This example illustrates how identical torque variability produces dramatically different outcomes depending on grip orientation.
