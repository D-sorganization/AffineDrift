...
Frames and constants follow the [roll-model article](putting-roll-models.html): $x$ along the intended start line, $y$ up, $z$ right; ball and hole dimensions are the openly published equipment and Rules specifications; stimp $S$ maps to rolling deceleration $a = 5.49/S$ m/s². Claims about specific software below are confined to this project's own repositories, which are public and linkable; commercial simulators are discussed only in terms of the modelling choices any implementation must make, since their internals are not publicly documented and this site does not cite what cannot be verified.
:::

## 1. The Ball Model: A Hybrid ODE With Two Phases and an Event

Strip the [roll-model derivation](putting-roll-models.html) to its computational skeleton and what remains is a **hybrid dynamical system** — two smooth vector fields separated by a discrete transition:
...
The putting vertical of the project's [Tools golf suite](https://github.com/D-sorganization/Tools) implements the model of §1–§2 as specified by the [roll-model derivation](putting-roll-models.html), as part of the broader shared-tools putting simulation program:
...
- **Interactive web companion** — the [Rate of Closure Impact Explorer's putting tab](https://d-sorganization.github.io/rate-of-closure-explorer/) (live site) runs the same roll model in the browser under the same parity discipline, including an "Import green…" action that loads a heightfield green — including greens authored in the project's `putting_green` engine — rather than only the built-in example surfaces.
...
The putting material now forms a closed loop with the impact-mechanics corpus. The [impact article](impact-mechanics-and-ball-flight.html) establishes the collision physics and the face-dominance of the start line; the [reference-point article](reference-point-problem.html) explains why delivered direction must name its reference point — a caution that applies verbatim to putter deliveries, where a degree of start-line error is a missed three-metre putt; the [rotation-induced-spin supplement](rotation-induced-spin.html) derives the sphere impulse partition whose surface-side twin is the skid phase; the [roll-model article](putting-roll-models.html) carries that partition across the turf to the hole; and this article turns the assembled physics into software with a stated error budget. The unifying habit is the same one the [technology section](../pages/technology.html) applies to instruments: say what is measured, what is derived, and what is assumed — and keep the three lists separate.

Two companions round out the loop from the putter side: [secondary-axis stability](secondary-axis-stability.html) constrains the inertial design of the delivery state this software simulates, and the [strokes-gained critique](strokes-gained-limitations.html) prices what the simulated shots are worth in score terms.

## References

- Companion derivation: [The Physics of the Rolling Putt](putting-roll-models.html) — all physical constants and closed forms used here, with their assumption ledger.
...
*Implementation companion to [The Physics of the Rolling Putt](putting-roll-models.html).*
