...
This is the **Euclidean-orthogonal** projector onto $\operatorname{null}(\mathbf{J}_c)$ — orthogonal with respect to the standard inner product on $\mathbb{R}^n$.

::: {.callout-important}
## Euclidean vs. kinetic-energy-weighted projector

For constrained rigid-body dynamics the physically meaningful projector uses the **kinetic-energy (mass) metric** $M(q)$:

$$
P_M = I - M^{-1} \mathbf{J}_c^\top \bigl(\mathbf{J}_c M^{-1} \mathbf{J}_c^\top\bigr)^{-1} \mathbf{J}_c
$$

$P_M$ is $M$-orthogonal: it projects forces orthogonally with respect to the kinetic-energy inner product $\langle u, v \rangle_M = u^\top M\, v$, and therefore correctly decomposes joint torques into constraint-space and null-space components in a dynamically consistent sense. The Euclidean projector $P$ above is used here for algebraic clarity when deriving the null-space basis $N$; in dynamic equations (e.g., the reduced-order equations of motion) $P_M$ should be used instead. See Khatib (1987) and Siciliano & Khatib (2016, §3.9) for the kinetic-energy-weighted formulation.
:::
...
The reduced system has $n - m$ second-order ODEs compared to $n + m$ equations in the DAE formulation. The constraint forces $\boldsymbol{\lambda}$ have been completely eliminated.
...
$$
    \underbrace{\ddot{\mathbf{q}}}_{\text{acceleration}} = \underbrace{-M^{-1}(\mathbf{c} + \mathbf{g})}_{\text{drift } \mathbf{a}_0(\mathbf{q}, \dot{\mathbf{q}})} + \underbrace{M^{-1} P_M}_{\text{control effectiveness } B(\mathbf{q})} \boldsymbol{\tau}_{\text{muscle}}
$$

Here $P_M = I - M^{-1}\mathbf{J}_c^\top(\mathbf{J}_c M^{-1}\mathbf{J}_c^\top)^{-1}\mathbf{J}_c$ is the **kinetic-energy-weighted** null-space projector (Khatib 1987; Siciliano & Khatib 2016). Using the Euclidean projector $P$ in this equation would misallocate the inertia weighting and give physically incorrect force decompositions.

Key observations:
1. The null space projector $P_M$ determines which muscle forces are *dynamically effective* (i.e., produce acceleration in unconstrained directions)
...
Elite golfers are often described as having an "effortless" swing. From a control-theoretic perspective, this reflects **timed and shaped muscle torques**: the golfer does not minimise $\|u\|$ but instead times torque application to work *with* $\mathbf{f}(\mathbf{x})$, amplifying drift contributions rather than fighting them. The control input $u$ remains non-trivial in absolute magnitude throughout the downswing.

$$
    \dot{x} = \mathbf{f}(\mathbf{x}) + g(\mathbf{x})\,u, \quad \|u\| \text{ non-trivial, timed to align with drift}
$$

::: {.callout-important}
## No $\|u\| \to 0$ claim

The drift-dominance argument (high DCR in the late downswing) does **not** imply that muscle torques vanish or become negligible in absolute terms. It implies that the *marginal ability to alter the trajectory* via additional torque diminishes — because drift acceleration has grown large relative to the control authority. Absolute torque levels remain substantial (consistent with working estimates of roughly 15/85 Nm passive/active ratios in the late downswing). The Drift–Control Ratio is a ratio of *magnitudes*, not a claim that active input is zero.
:::
...
The control effectiveness matrix $B(\mathbf{q}) = M^{-1} P_M$ (using the kinetic-energy-weighted projector $P_M$; see definition above) determines how muscle forces translate to clubhead acceleration. Key insights:
...
The twist-space view of the same decomposition — generalized forces acting as wrenches on the constraint-consistent subspace — is developed in the companion [Screw Theory Reference](screw-theory-reference.html).
    ...
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          ...
          <h3>The "Soft Constraint" Problem</h3>
        </div>
        <p class="critic-argument">
          The mathematical model assumes the closed loop formed by the hands on the grip is a rigid constraint ($\Phi(q) = 0$). In reality, the hands are compliant: skin deforms, grip pressure varies, and the hands can slide slightly. Skeptics argue that treating this as a hard constraint creates a misleadingly sharp "Null Space." In truth, the system operates in a "fuzzy" constraint cloud where small deviations are possible, potentially altering the available degrees of freedom.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> This is a valid physical observation. However, the <em>stiffness</em> of the constraint (bone and high-friction grip) is orders of magnitude higher than the voluntary torque capability of the wrists in orthogonal directions. While the constraint is technically "soft" (compliant), it functions as a "hard" guide for the primary motion. We model the ideal structure to identify the <em>dominant</em> kinematic pathways, acknowledging that compliance adds a layer of "stiffness orthogonal to the null space."
        </div>
      </div>

      <div class="critic-item">
        <div class="critic-perspective">
          <span class="critic-label">Motor Control:</span>
          <h3>Do Brains Calculate Null Spaces?</h3>
        </div>
        <p class="critic-argument">
          The article presents the Null Space Projector ($P$) as a computational tool. Critics from the ecological psychology camp argue that the human brain does not perform singular value decomposition (SVD) or matrix projection in real-time. Suggesting that the brain "projects torque onto the null space" imputes a computational strategy to a biological system that likely learns these synergies through trial-and-error reinforcement learning or dynamic primitives, not linear algebra.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> We agree that the brain does not explicitly compute matrix inverses. The Null Space is a <em>descriptive</em> mathematical tool for us as analysts, not a <em>prescriptive</em> algorithm for the brain. It describes the physical manifold that the brain discovers. The "path of least resistance" that a golfer learns <em>is</em> the Null Space; our mathematics simply provides a precise language to quantify what the athlete feels intuitively.
        </div>
      </div>
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          ...
          <h3>Is 26 DOF Enough?</h3>
        </div>
        <p class="critic-argument">
          The simplified 26-DOF model aggregates complex joints like the spine and shoulder girdle into simple hinges or ball joints. Critics argue that critical "slop" or "play" in the shoulder complex (scapular gliding) and spinal warping provides essential degrees of freedom that a rigid-body chain misses. By ignoring these micro-motions, the model may overstate the constraint dimensionality reduction, missing the subtle adjustments elite golfers use to square the face.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> All biomechanical models are approximations. While adding more DOFs (e.g., $n=100$) would increase the dimension of the null space, the <em>fundamental topology</em> of the loop closure remains. The principle of drift-control separation via projection holds regardless of $n$. We choose 26 DOF as the "minimum viable complexity" to demonstrate the phenomenon without drowning in parameter identification errors.
        </div>
      </div>
