...
Launch monitor data is the primary evidence base for almost everything written about the modern golf swing. The documentation behind it is scattered across vendor blogs, help-center articles, sticker-guide PDFs, user manuals and patent filings. Much of it is undated, some of it contradictory, and a fair amount of it marketing wearing a technical vocabulary.
...
|---|---|---|
| **TrackMan** | Geometric center of the clubhead (within ~6 mm of CG) | [Club data definitions](https://www.trackman.com/blog/golf/club-data-definitions) |
...
| **GEARS** | Not stated for path; *does* report speed at both face center and impact point | [Club/ball metrics](https://www.gearssports.com/articles/gears-golf-club-ball-metrics/) |
...
For a driver, two systems using those different reference points can therefore disagree by roughly the 3° example TrackMan gives, with the face-center path reading more out-to-in. That component of disagreement is definitional rather than an accuracy error, although the realized difference depends on club geometry and motion.
...
TrackMan references **path and attack angle at the geometric center**, but **face angle and dynamic loft at the contact point**:
...
**Does this build a slice into the numbers?** The mechanism is real. If a player is coached to a zero reported path on a geometric-center system, the contact point --- which is what strikes the ball --- travels roughly 3° more out-to-in than the number they are chasing. But **no vendor makes that claim**, and it should not be attributed to one. Treat it as an inference from TrackMan's own quantities, with the caveat that players calibrate to ball flight rather than to numbers, and that a competing draw-side mechanism (the face continuing to close through contact) partly offsets it. The full treatment is in [Impact Mechanics, Part VI](impact-mechanics-and-ball-flight.qmd).
...
Uneekor's older sticker guide is blunt about its dependency: *"CLUB DATA CANNOT BE READ IF THE BALL IS COVERING ONE OF THE TWO STICKERS. CLUB DATA WILL BE ONLY SHOWN IF BOTH STICKERS ARE VISIBLE."* That statement should not be generalized to the current range. Uneekor's January 2026 Club AI announcement advertises sticker-free club data, while the current EYE XO2 product page still says reflective club stickers are required. The public documentation therefore conflicts as of 2026-08-03; check marker requirements against the exact hardware, software package and data set you are using. Marked-*ball* requirements for spin are a separate issue.
...
A useful tell: on FlightScope, **model and software tier** unlock club path, rather than the sensor supplying it --- which is itself evidence of derivation.
...
| Help Center: Club Path | [support.trackmangolf.com](https://support.trackmangolf.com/hc/en-us/articles/39724275421211-Parameters-Club-Path-Tee-to-Green) | "...geometric center at the time of maximum compression" |
| Help Center: Face Angle | [support.trackmangolf.com](https://support.trackmangolf.com/hc/en-us/articles/39724466444059-Parameters-Face-Angle-Tee-to-Green) | "...at the center-point of contact... at the time of maximum compression" |
| Help Center: D Plane Tilt | [support.trackmangolf.com](https://support.trackmangolf.com/hc/en-us/articles/39726207070875-Parameters-D-Plane-Tilt-Tee-to-Green) | The geometry-only spin axis, explicitly excluding gear effect |
...
**#7 (Oct 2010), "Ten Fundamentals"** — the origin of most TrackMan rules of thumb. It carries the 85/15 claim in **both** planes; see [Impact Mechanics, §IV.3](impact-mechanics-and-ball-flight.qmd) for why only the vertical one survives measurement. It also gives "SPIN LOFT = DYNAMIC LOFT − ANGLE OF ATTACK" as a definition rather than the approximation TrackMan's current blog calls it, and spin decay as "typically 4% for each second". Its attack-angle distance study reports that at 90 mph club speed, −5° attack angle optimizes to 191 yards carry versus 214 yards at +5°.
...
FlightScope markets the technology as "Fusion Tracking" --- radar combined with camera. Model and package tier gate the availability of club path, which is itself evidence of derivation rather than sensing.
...
- **It contradicts itself on the reference point.** Horizontal swing plane uses *"the club's geometric center"*; club speed uses *"the club's centre of mass."* Those are different points on a clubhead, used interchangeably without acknowledgement. **FlightScope gives face angle no clubhead reference point at all** --- only the external target line.
- **Its own parameter pages contradict "measured, not calculated."** Marketing says *"actual club and ball measurements - no calculations."* The club path page says *"it **calculates** the club path"*; angle of attack, dynamic loft, face-to-path, swing planes and spin axis all say "calculates."
- **It calls the system "lighting-agnostic"** in the blog, against a FAQ requiring *"a minimum of 300 lux to capture accurate Face Impact Location data."*
...
The same document is unusually careful elsewhere, and worth reading for its caveats. It notes that impact loft is *"the inclination of this flat plane, **not** the actual loft at the impact point for curved face clubs such as a driver"* --- i.e. it does not correct for bulge and roll. It adds that the face plane itself *"is a 2D plane described by the placement of the fiducials on the club face."* Where the operator sticks the markers therefore defines the coordinate frame, and Foresight publishes no positional tolerance for that placement.
...
The patent, **[US11844990B2](https://patents.google.com/patent/US11844990B2/en)** (Full Swing Golf Inc), is the real technical document. It describes four or more receive antennas in a deliberately **non-uniform array**, with one antenna placed at least 1.5× farther out to extend the angular-resolution baseline. Sequential CW radar supplies Doppler velocity and FMCW supplies range, and a deep-learning stage estimates the trajectory. Radar only --- no cameras in the claims.
...
Documentation has been **deleted**. Ernest Sports rebuilt the site, and the entire manuals library, FAQ and Tour Plus manual now 404; the Wayback Machine never captured the manual PDF, which is therefore permanently lost. No measured-versus-calculated table has ever existed, and no Ernest Sports document anywhere defines club path or face angle, though the products advertise both.

Marketing asserts "zero estimation," but two archived statements undercut it. The first explains that outdoor failures lose "the rotation of the ball and club data" because "the background is the same color as the golf ball," which ties spin and club data to the cameras rather than the radar. The second, a 2014 FAQ, concedes that a radar-only model "does not measure launch and spin however... reacts to various launch and spin conditions" — a calibration table, not a measurement.

::: {.callout-note}
## Similar Portable Radars, Opposite Advice on Marked Balls

FlightScope **requires** aluminum stickers on the ball for indoor spin: *"Aluminum stickers are needed in an indoor environment in order for the X3C, Mevo+, Mevo Gen2, and Mevo to accurately measure ball spin."*
...
Contact lasts roughly 0.45 ms. At the cited pre-impact closure rate, the difference between reporting face angle at first contact and at maximum compression can therefore be on the order of **a degree**. The precise effect on ball flight depends on speed, spin, launch conditions and aerodynamic modeling.
...
### III.3 Nobody Actually Measures the Center of Gravity
...
So the reference-point taxonomy above describes what vendors *report*, not always what they *sense*. TrackMan reconstructs a geometric center from the head's radar silhouette; optical systems track markers on the face. Both then map to a declared reference point through a model.
...
Among the studies located in searches through 2026-08-03, **[Leach, Forrester, Mears & Roberts (2017)](https://doi.org/10.1016/j.measurement.2017.08.009)**, *Measurement* **112**, 125–136, is the traceable validation study of consumer launch monitors rather than a comparison against another launch monitor. The authors tested TrackMan Pro IIIe and Foresight GC2+HMT simultaneously against a GOM Inspect reference reconstructed from four Photron cameras at 5,400 Hz, with calibration traceable to PTB Germany, over 240 shots from 8 golfers using driver, 7-iron and wedge.

They characterized the reference itself first: 0.06 mph in ball velocity, 0.01° in launch angle, 14 rpm in spin. So the disagreements below are real.
...
A device that declines to report on a third of shots is doing something different from a device that reports on all of them. The authors also cautioned that the simultaneous multi-system setup, including tripods, lighting and possible interference, was not necessarily optimal for either commercial device, so these non-return rates should not be treated as universal field rates.
:::
...
**There is no TrackMan article about slicing.** The sitemap contains 122 blog URLs and **none** contains "slic"; a Help Center search returns nothing on the topic. The closest published material is:
