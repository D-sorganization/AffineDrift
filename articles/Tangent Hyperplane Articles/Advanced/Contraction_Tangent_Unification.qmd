...
description: "This article establishes a profound connection between two seemingly disparate frameworks in nonlinear control: contraction theory (Lohmiller & Slotine,..."
...
System (@eq-ctu-dynamics) is contracting in a region $\mathcal{D} \subset \mathbb{R}^n$ with rate $\lambda > 0$ if there exists a uniformly positive definite metric $\mathbf{M}(\mathbf{x}, t) = \mathbf{\Theta}^\top(\mathbf{x}, t) \mathbf{\Theta}(\mathbf{x}, t)$ such that:
$$
...
If system (@eq-ctu-dynamics) is contracting, then:
...
Choose $\mathbf{Q} = \text{diag}(16, 1)$, $\mathbf{R} = 1$. Solving the ARE analytically with $A = \begin{bmatrix}0&1\\0&0\end{bmatrix}$, $B = \begin{bmatrix}0\\1\end{bmatrix}$:
$$
\mathbf{S}_\infty = \begin{bmatrix} 12 & 4 \\ 4 & 3 \end{bmatrix}
$$ {#eq-double-integrator-riccati}

Optimal gain:
$$
\mathbf{K} = \mathbf{R}^{-1} \mathbf{B}^\top \mathbf{S}_\infty = \begin{bmatrix} 4 & 3 \end{bmatrix}
...
**Proposed formulation**: A **stochastic contraction metric** $\mathbf{S}(t)$ for the mean-field dynamics satisfying:
$$
...
> **Note on non-standard formulation:** The quadratic noise term $\tfrac{1}{2} \mathbf{S} \mathbf{G} \mathbf{\Sigma} \mathbf{G}^\top \mathbf{S}$ does not appear in the standard stochastic LQR/Riccati equation, which has an additive noise term of the form $+\mathbf{G} \mathbf{\Sigma} \mathbf{G}^\top$ (from the Itô correction). The equation above is a heuristic extension motivated by contraction-theoretic robustness analysis; a rigorous derivation from the stochastic Lyapunov/contraction framework would require careful application of Itô's lemma. This equation should be treated as an illustrative proposal rather than a derived result.

The qualitative interpretation holds regardless: noise terms tend to **oppose** contraction—the CNS must work harder to maintain stability in the presence of variability.
...
- **Free space**: Dynamics (@eq-ctu-robot-dynamics)
