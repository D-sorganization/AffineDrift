...
$$
where $W_c$ is the controllability Gramian. The eigenvalues of $W_c$ in planar directions shrink as $\mathcal{O}(1/\text{DCR}^2)$, while axial eigenvalues remain $\mathcal{O}(1)$. This produces an ellipsoid with aspect ratio $\sim \text{DCR}$—a pancake whose thickness-to-width ratio matches the DCR.
