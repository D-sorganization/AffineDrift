...
While the DCR provides a scalar magnitude of the problem—telling us *that* control is lost—the Lie bracket analysis reveals the *geometry* of that loss. It shows us exactly which directions in the state space become inaccessible. As the drift vector field $f(x)$ lengthens, it does not merely push the system faster; it curves the reachable manifold, bending the "straight lines" of control input into complex involutions that the golfer cannot unwind in the available time.
