...
Its practical cost is noise sensitivity: the axis position is computed by dividing by $\lVert\boldsymbol{\omega}\rVert^2$, so when angular velocity is small the axis location becomes ill-conditioned — structurally the same pathology as centre of pressure at low vertical load. The standard mitigations are to gate on a minimum angular velocity and to estimate the axis over a finite interval rather than instantaneously. For golf this caveat bites precisely at transition, where the club reverses direction and angular velocity passes through small values.
