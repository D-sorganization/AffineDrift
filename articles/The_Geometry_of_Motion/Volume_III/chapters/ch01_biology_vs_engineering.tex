...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Bones are not rigid.} Long bones (femur, humerus, tibia) have measurable
        compliance under dynamic loading \cite{nigg2007biomechanics}. A golf club shaft
        deflects by several centimeters and several degrees during a swing; a human femur
        deflects by millimeters during running. The deflection is small in absolute terms
        but can have significant effects on end-effector accuracy.

  \item \textbf{Joints are not revolutes.} The knee, for example, combines rolling,
        sliding, and rotation in a coupled manner that changes with flexion angle
        \cite{walker1972quantitative,komistek2003invivo}. The glenohumeral (shoulder)
        joint permits motion in three rotational axes but also allows several millimeters
        of translation. The intervertebral joints of the spine have six degrees of
        freedom each, all coupled through the disc and ligament mechanics.
  ...
  \item \textbf{Sensors are noisy, delayed, and multimodal.} Proprioceptors (muscle
        ...
        significant noise and delays of 30--100~ms \cite{scott2004optimal}. The central
        nervous system must fuse information from proprioceptive, visual, and vestibular
        sources to estimate the body's state.
...
For the human body during locomotion, the long bones satisfy $\omega_k \gg \omega_c$---bone
compliance can be neglected. For the spine and for soft tissues, the separation is less
clean, and dynamic flexibility effects may be significant.

\subsection{Muscle Force Production Is State-Dependent}

The most striking difference between engineered and biological actuators is that muscle
...
\begin{enumerate}
  ...
  \item \textbf{Contraction velocity} (force-velocity relationship): concentric
        ...
        (lengthening) contractions can produce forces approximately 30--50\% above
        isometric, depending on the specific model (see
        \cite{thelen2003adjustment,millard2013flexing} and Chapter~3).
...
\begin{clinicalbox}{Why This Matters for the Golf Swing}{golf-muscle}
  During the downswing, the wrist extensors are rapidly lengthening (eccentric
  contraction) while the shoulder muscles are shortening (concentric). The force
  capacity at the wrist is therefore \emph{enhanced} by the force-velocity
  relationship, while that at the shoulder is \emph{diminished}. An engineering
  model that assigns constant torque limits to each joint completely misses this
  asymmetry, which is fundamental to the kinetic chain mechanism of the swing.
\end{clinicalbox}
...
joint degrees of freedom \cite{Zatsiorsky2002}. Even for a simple task like reaching for a cup (which
...
of the key biomechanical properties that any model must capture.
...
\begin{itemize}
  \item Maximum isometric force $F_0$ (ranging from 10~N for small hand muscles to
        over 6000~N for the quadriceps)
  \item Optimal fiber length $\ell_0$ (the length at which maximum isometric force is produced)
  \item Maximum contraction velocity $v_0$ (typically 5--15 fiber lengths per second)
  \item Tendon slack length $\ell_T^s$ (the length at which the tendon begins to bear load)
  \item Pennation angle $\alpha_0$ (the angle between muscle fibers and the line of action,
        ranging from $0^\circ$ for parallel-fibered muscles to $30^\circ$ or more for
        pennate muscles like the quadriceps)
\end{itemize}

These properties have been systematically catalogued for major musculoskeletal models
\cite{Delp2007, Arnold2010}.
...
\begin{laymansbox}{Mathematical Reminder: The Expanded Manifold}{expanded_manifold_reminder}
