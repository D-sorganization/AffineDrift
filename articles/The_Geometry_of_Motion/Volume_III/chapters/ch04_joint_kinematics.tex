...
\begin{tikzpicture}[scale=0.85, >=Stealth]
  ...
  \node at (0, 3.5) {\textbf{Engineering Revolute}};
  \draw[thick, fill=gray!20] (0, 0.5) circle (0.8);
  \draw[->, thick, blue] (0, 0.5) -- (0.8, 0.5) node[right] {1-DOF axis (fixed)};

  \node at (4, 3.5) {\textbf{Biological Joint}};
  \draw[thick] (4, 0) ellipse (1.2 and 0.6);
  \draw[thick] (4, 1) ellipse (1.0 and 0.5);
  \draw[->, thick, red] (4, 0.5) -- (4.8, 0.8) node[right, font=\small] {ISA migrates};
  \node[font=\tiny, gray, text width=2cm, align=center] at (4, -0.8) {Coupled rotation\\+ translation};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
  ...
  \item The tibia undergoes coupled \textbf{internal rotation} of approximately $15^\circ$
...
flexion. The position and velocity of the ISA are key outputs of kinematic analysis
\cite{Crisco2000, Wretenberg1995}. The four-bar linkage model provides a simplified but
insightful mechanical analog for understanding coupled knee kinematics \cite{OConnor1989}:
...
def knee_isa_trajectory(
    ...
    roll_slide_ratio : float
        ...
        Pure rolling = 1.0, pure sliding = 0.0.
    ...
    """
    R_condyle = 0.030  # approximate femoral condyle radius (m)
...
gross simplification: quantitative measurements show the humeral head translates during
abduction \cite{Karduna2000, Dickerson2005}, and the instantaneous center of rotation
shifts continuously. The instantaneous screw axis of the shoulder is not fixed and
migrates through the joint during arm motion \cite{Halder2000}.

The shoulder complex involves four joints:
\begin{enumerate}
  \item \textbf{Glenohumeral} (GH): primary ball-and-socket, 3 DOF rotation + $\sim$3~mm translation
