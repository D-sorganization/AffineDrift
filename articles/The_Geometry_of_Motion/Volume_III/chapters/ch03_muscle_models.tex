...
muscle thermodynamics and muscle energetics \cite{Hill1938, Hill1950}, is the standard
lumped-parameter model used in biomechanical simulation. It represents the musculotendon
unit as a combination of contractile, elastic, and damping elements. Modern implementations
include refinements by \cite{Thelen2003} and \cite{millard2013flexing}.
...
\begin{definition}{Active Force-Length Curve}{fl-curve}
  ...
  \end{equation}
  where $\gamma \approx 0.45$ \cite{Thelen2003} controls the width of the
  curve. Maximum force ($f_L = 1$) occurs at optimal fiber length ($\tilde{\ell} = 1$).
  Force decreases for both shorter and longer fiber lengths, reaching near-zero at
  approximately $\tilde{\ell} = 0.5$ and $\tilde{\ell} = 1.5$.
\end{definition}

\begin{quote}
  \textit{A note on the numerical parameters.} The values $\gamma = 0.45$,
  $A_f = 0.25$, $k^{PE} = 4.0$, and $\varepsilon_0 = 0.6$ used throughout this
  chapter are the default parameters recommended by Thelen
  \cite{Thelen2003} for the ``generic'' adult Hill-type muscle, and
  also serve as the baseline for the Millard--Uchida formulation
  \cite{millard2013flexing}. They are calibrated against mid-thigh muscles of
  young adults and should be interpreted as a starting point, not as universal
  constants. Real muscles exhibit systematic variation with age, sex, training
  status, and fiber-type composition, and subject-specific tuning is almost
  always required for quantitative predictions.
\end{quote}
...
class MuscleParameters:
    ...
    # Defaults below are the Thelen (2003) generic-adult values.
    # See: Thelen, D.G. (2003) "Adjustment of muscle mechanics
    # model parameters to simulate dynamic contractions in older
    # adults," J. Biomech. Eng. 125(1), 70-77 [Thelen2003].
    gamma: float = 0.45  # Force-length curve width (Thelen 2003)
    Af: float = 0.25     # Force-velocity shape factor (Thelen 2003)
    kPE: float = 4.0     # Passive elastic stiffness (Thelen 2003)
    e0: float = 0.6      # Passive strain at F0 (Thelen 2003)
...
def muscle_force(
    ...
    # Pennation angle varies with fiber length under the
    # constant-thickness assumption: sin(alpha) = sin(alpha0) * l0_M / l_M
    sin_alpha = np.sin(params.alpha0) * params.l0_M / max(l_M, 1e-6)
    cos_alpha = np.sqrt(max(0.0, 1.0 - sin_alpha ** 2))
...
biceps = MuscleParameters(
    F0=624.3,        # N (Holzbaur et al. 2005, upper-extremity model)
    l0_M=0.1157,     # m
...
\begin{principle}{The Force-Velocity Asymmetry}{fv-asymmetry}
  The force-velocity relationship characterizes how muscle force depends on contraction
  velocity \cite{Hill1938, Hill1950}:
  \begin{itemize}
    \item \textbf{Concentric (shortening) contraction}: force \emph{decreases} with
          ...
          muscle produces zero force. The relationship is well-approximated by a
          hyperbolic curve.
    \item \textbf{Eccentric (lengthening) contraction}: force \emph{increases} beyond
          the maximum isometric force. The magnitude of enhancement is muscle and fiber-type
          dependent, documented in \cite{Thelen2003} and similar references.
    \item \textbf{Isometric ($v = 0$)}: force equals the product of activation and
          the force-length relationship.
  \end{itemize}
  The asymmetry between concentric and eccentric force production is a fundamental property
  of biological muscle that is absent in engineered linear actuators.
\end{principle}
...
\cite{Winters1984}. The asymmetry between activation and deactivation time constants
...
\begin{principle}{Tendons as Energy Storage Elements}{tendon-energy}
  Biological tendons are not simply rigid force transmitters. They are viscoelastic elements
  that can store and return elastic strain energy. The energy storage capacity of a tendon
  is determined by its stiffness and the strain it undergoes during muscle contraction.
  This energy storage changes the energetics and the effective impedance of the muscle-tendon
  unit compared to a model with infinitely stiff tendons. The physiological consequence is
  that tendons enable more efficient movement by recovering elastic energy that would otherwise
  be dissipated, as documented in \cite{Alexander1991, Alexander2002}.
\end{principle}
