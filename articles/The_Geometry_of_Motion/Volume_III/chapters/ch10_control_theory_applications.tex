...
\begin{tikzpicture}[scale=0.9, >=Stealth, node distance=2.2cm]
  % CNS / Goal
  \node[draw, thick, fill=blue!10, minimum width=2.2cm, align=center] (cns) {CNS\\(Goal)};

  % Neural command
  \node[draw, thick, fill=green!10, minimum width=2.2cm, align=center, right of=cns] (neural) {Neural\\Excitation $\bm{u}(t)$};

  % Muscle dynamics
  \node[draw, thick, fill=green!10, minimum width=2.2cm, align=center, right of=neural] (muscle) {Muscle\\Dynamics};

  % Skeletal dynamics
  \node[draw, thick, fill=green!10, minimum width=2.2cm, align=center, right of=muscle] (skeletal) {Skeletal\\Dynamics};

  % Measurement
  \node[draw, thick, fill=orange!10, minimum width=2.2cm, align=center, below of=skeletal] (measure) {Proprioceptive\\Feedback};
...
\end{tikzpicture}
\caption{Biological feedback control loop: CNS goal signals drive neural excitations through muscle and skeletal dynamics, with proprioceptive feedback closing the loop.}
\label{fig:ch10:control-loop}
\end{figure}
\begin{equation}
...
def forward_dynamics_simulation(
    ...
    def dynamics(t: float, state: np.ndarray) -> np.ndarray:
        ...
        # Activation dynamics
        u = excitation_func(t)
        tau_act = 0.015  # Illustrative placeholder; replace with a cited model value.
        tau_deact = 0.050  # Illustrative placeholder; replace with a cited model value.
        adot = np.where(
            u > a,
            (u - a) / tau_act,
            (u - a) / tau_deact
