...
\epigraph{%
  The human body is a high-dimensional articulated system with
  changing contacts and compliant tissues. Rigid-body dynamics is
  a useful approximation, but never the whole story.
}{}
...
\begin{center}
\begin{tikzpicture}[scale=0.85, >=Stealth, node distance=1.5cm]
  % Open chain
  \node[draw, circle, thick] (base1) at (0, 2) {0};
  \node[draw, circle, thick] (j1) at (1.5, 2) {1};
  \node[draw, circle, thick] (j2) at (3, 2) {2};
  \node[draw, circle, thick] (j3) at (4.5, 2) {3};

  \draw[thick] (base1) -- (j1) -- (j2) -- (j3);

  \node[below] at (2.25, 1.5) {\textbf{Open Chain} (arm reaching)};

  % Closed chain
  \node[draw, circle, thick] (base2) at (0, -1) {0};
  \node[draw, circle, thick] (j4) at (1.5, -1) {1};
  \node[draw, circle, thick] (j5) at (3, -1) {2};
  \node[draw, circle, thick] (ground) at (3, -2.5) {Ground};

  \draw[thick] (base2) -- (j4) -- (j5) -- (ground);
  \draw[thick, dashed] (ground) -- (base2) node[midway, below] {contact};

  \node[below] at (2.25, -3.2) {\textbf{Closed Chain} (gait stance phase)};
\end{tikzpicture}
\end{center}
...
\begin{equation}
  ...
  = \sum_{i=1}^{p} \bm{r}_i(\bm{q}) F_{\text{muscle},i},
  \label{eq:ch5:net-torque}
\end{equation}
where $\bm{r}_i(\bm{q}) \in \Reals^n$ is the $i$-th column of $R$, giving the moment
arm vector of muscle $i$ across all joints. This moment-arm mapping is the
standard bridge between muscle models and generalized coordinates in
musculoskeletal simulation \cite{Zajac1989,Delp2007}.
...
mechanical coupling between joints that is not captured by the simple
one-actuator-per-joint abstraction common in introductory robotics.
...
\begin{principle}{The Coordination Function of Bi-articular Muscles}{biarticular}
  ...
  \begin{enumerate}
    \item \textbf{Energy transfer}: they can transport mechanical energy from one joint to
          another without requiring a separate distal actuator. When the proximal joint
          extends (muscle shortening at the proximal end) and the distal joint flexes
          (muscle lengthening at the distal end), mechanical power can be redistributed
          from proximal to distal coordinates \cite{Zajac1989,Neptune2001}. The metabolic
  ...
  \noindent
  \textbf{Control-Theoretic Perspective:} The matrix $R(\bm{q})$ can be read as a
  state-dependent input map from muscle forces to generalized torques. Bi-articular
  muscles may create dependencies among its columns, which is one reason the effective
  control directions in a musculoskeletal model can be lower-dimensional than the raw
  muscle count. Geometric control language is useful for describing that reduction
  \cite{AgrachevSachkov2004}, but the specific structure of $R(\bm{q})$ remains
  anatomy- and model-dependent.
\end{principle}
...
R = np.array([
    # shoulder, elbow
    [0.05,  0.03,  -0.025,  0.0],   # shoulder moment arms (m)
    [0.0,   0.04,  -0.020,  0.03],  # elbow moment arms (m)
]).T  # Shape: (4 muscles, 2 joints) -> transpose to (2, 4)
R = R.T  # Now (2 joints, 4 muscles)
...
nervous system to modulate joint \emph{impedance} independently of net joint
\emph{torque}. This is better understood as a biologically compliant analogue of
impedance control rather than a literal duplicate of a rigid torque source
\cite{Zajac1989,MillardEvans2013}.

Position-dependent stiffness modulation gives a useful control vocabulary for
discussing state-dependent actuation. Geometric control theory describes how
state-dependent inputs change closed-loop structure \cite{AgrachevSachkov2004},
but the actual stiffness law in biomechanics still has to come from muscle,
tendon, and contact modeling rather than from control theory alone.
