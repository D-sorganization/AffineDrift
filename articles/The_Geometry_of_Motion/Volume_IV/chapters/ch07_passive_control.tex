...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw (1, 0) -- (1.5, 1) node[anchor=south, font=\tiny] {swing};
  \draw (1, 0) -- (0.5, 1) node[anchor=south, font=\tiny] {stance};

  % Hip
  \draw (1, 1) circle (0.12);
  \draw (1, 1) node[anchor=center, font=\tiny] {hip};

  % Ground
  \draw (-0.2, 0) -- (2.2, 0);
  \draw (0.5, -0.15) node[anchor=north, font=\tiny] {slope};

  % Forces
  \draw[->, thick, blue] (1, 0) -- (1, -0.5) node[anchor=north, font=\tiny] {gravity};

  % Energy flow
  \draw (3, 1.3) node[anchor=center, font=\small\bf] {Energy Flow};
  \draw (3, 0.9) node[anchor=west] {Gravitational PE};
  \draw (3, 0.6) node[anchor=west] {$\Rightarrow$ Kinetic energy};
  \draw (3, 0.3) node[anchor=west] {$\Rightarrow$ Angular momentum};

  % Title
  \draw (0.75, 2.2) node[anchor=center, font=\small\bf] {Passive Dynamic Walking};
  \draw (0.75, 1.9) node[anchor=center, font=\tiny] {(McGeer, 1990)};
\end{tikzpicture}
