...
\begin{tikzpicture}[scale=0.85]
  % Timeline
  \draw (0, 0) -- (8, 0);
  \draw (0, -0.2) node[anchor=north] {Baseline};
  \draw (3.5, -0.2) node[anchor=north] {Adaptation};
  \draw (7.5, -0.2) node[anchor=north] {Washout};

  % Error curve (schematic)
  \draw[blue, thick] (0, 1) -- (1.5, 1) -- (2, 3) -- (3.5, 1.2) -- (4.5, 0.3) -- (5.5, 0.1) -- (8, 0.8);

  % Labels
  \draw (0.75, 0.5) node[anchor=center, font=\tiny] {minimal};
  \draw (2.75, 3.3) node[anchor=center, font=\tiny] {early};
  \draw (4, 0.8) node[anchor=center, font=\tiny] {adaptation};
  \draw (7.2, 1.2) node[anchor=center, font=\tiny] {aftereffect};

  % Dashed line (zero)
  \draw[dashed] (0, 0) -- (8, 0);

  % Y-axis
  \draw[->, thick] (-0.3, 0) -- (-0.3, 3.5) node[anchor=east, rotate=90] {Error};

  % Title
  \draw (4, 4.2) node[anchor=center, font=\small\bf] {Visuomotor Adaptation Curve};
  \draw (4, 3.8) node[anchor=center, font=\tiny] {(Reaching with visual rotation)};
\end{tikzpicture}
