...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw (1.25, 2.4) node[anchor=center] {Desired};
  \draw (1.25, 2.1) node[anchor=center, font=\tiny] {Effect};

  % Action representation
  \draw (3, 2) rectangle (4.5, 2.8);
  \draw (3.75, 2.4) node[anchor=center] {Action};
  \draw (3.75, 2.1) node[anchor=center, font=\tiny] {Code};

  % Motor command
  \draw (5.5, 2) rectangle (7, 2.8);
  \draw (6.25, 2.4) node[anchor=center] {Motor};
  \draw (6.25, 2.1) node[anchor=center, font=\tiny] {Command};

  % Sensory effect (bottom)
  \draw (5.5, 0.5) rectangle (7, 1.3);
  \draw (6.25, 0.9) node[anchor=center] {Sensory};
  \draw (6.25, 0.6) node[anchor=center, font=\tiny] {Effect};

  % Arrows: forward links
  \draw[->, thick] (2, 2.4) -- (3, 2.4) node[anchor=south, midway, font=\tiny] {encode};
  \draw[->, thick] (4.5, 2.4) -- (5.5, 2.4) node[anchor=south, midway, font=\tiny] {forward};

  % Feedback: sensory consequence
  \draw[->, thick, dashed] (6.25, 2) -- (6.25, 1.3) node[anchor=west, midway, font=\tiny] {sensory};

  % Title
  \draw (3.75, 3.3) node[anchor=center, font=\small\bf] {Ideomotor Principle};
  \draw (3.75, 3.0) node[anchor=center, font=\tiny] {(Theory of Event Coding)};

  % Label
  \draw (8.5, 1.3) node[anchor=west, font=\tiny] {Desired effect drives};
  \draw (8.5, 0.9) node[anchor=west, font=\tiny] {action planning;};
  \draw (8.5, 0.5) node[anchor=west, font=\tiny] {prediction error};
  \draw (8.5, 0.1) node[anchor=west, font=\tiny] {adjusts execution};
\end{tikzpicture}
