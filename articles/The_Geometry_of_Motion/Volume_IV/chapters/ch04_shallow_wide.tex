...
\epigraph{%
  ...
  It is a massively parallel, notably shallow one.
...
\begin{neurobox}{Neural Timing Constraints}{timing}
  Measured values from neurophysiology~\cite{HodgkinHuxley1952,Kandel2013}:
  \begin{itemize}
...
The cerebral cortex has a notably consistent six-layered architecture across
mammalian species~\cite{Mountcastle1997}. This constraint applies uniformly to M1 and
...
\begin{insight}{The Architecture Is the Algorithm}{arch-is-algo}
  ...
  \begin{enumerate}
    \item \textbf{Width}: approximately 16 billion cortical neurons in
          humans~\cite{Azevedo2009}, each receiving on the order of $10^4$ synaptic
          inputs~\cite{Kandel2013}. The total computational capacity is in the connections,
          not the depth.
...
\emph{This architectural choice has far-reaching consequences.} While a deep network might
...
\begin{tikzpicture}[scale=0.95]
  ...
  \draw (0.4, 1.85) node[align=center, anchor=center, font=\tiny] {Input};

  % Deep network (left)
  \draw (0.4, 3.5) node[align=center, anchor=center, font=\small\bf] {Deep: 10 layers $\times$ 50 wide};
  \foreach \i in {1, ..., 10} {
  ...
  \draw (6, 1.5) node[align=center, anchor=west, font=\tiny] {Latency: 50 ms};

  % Output layer
  \draw (8, 1.5) rectangle (8.8, 2.2);
  \draw (8.4, 1.85) node[align=center, anchor=center, font=\tiny] {Output};

  % Wide network (right)
  \draw (0.4, -0.2) node[align=center, anchor=center, font=\small\bf] {Wide: 2 layers $\times$ 2000 wide};
  \fill[blue, opacity=0.2] (1, -1) rectangle (4.5, -0.4);
  \draw (2.75, -0.7) node[align=center, anchor=center, font=\small] {Hidden layer (2000 neurons)};
  \draw (6, -0.7) node[align=center, anchor=west, font=\tiny] {Latency: 10 ms};

  % Arrow showing advantage
  \draw[->, thick, red] (6.2, 1.3) -- (6.2, -0.3);
  \draw (6.5, 0.5) node[align=center, anchor=west, font=\small, color=red] {5x faster};
\end{tikzpicture}
...
Muscle synergies provide direct evidence for dimensionality reduction. d'Avella and Bizzi
showed that the muscle patterns of natural frog hindlimb behaviours decompose into a small
set of synergies, some shared across behaviours and some specific to
one~\cite{dAvellaBizzi2005}. How far that conclusion depends on the decomposition used is a
separate question, and Tresch and colleagues addressed it directly by running several matrix
factorization algorithms against the same simulated and experimental data: the algorithms
broadly agree on the synergies they recover, which is what makes the finding a property of
the data rather than of the method~\cite{Tresch2006}. The muscle activation pattern of $p$
