...
\noindent
Geometrically, $\text{UCM}(\bm{q}_0)$ is the kernel of the task map's differential. Where $J_h$
has full rank $k$, the implicit function theorem makes $h$ a submersion near $\bm{q}_0$, so the
level set $h^{-1}(\bm{y}_0)$ is a smooth $(n-k)$-dimensional manifold whose tangent space at
$\bm{q}_0$ is exactly the UCM. Motion along that manifold changes the configuration without
changing the task variable.

It is worth being explicit that this is a statement about the \emph{task map}, not about the
dynamics. The UCM is a redundancy of $h$, not an uncontrollable subspace of the control system:
a configuration can be fully controllable and still have a large uncontrolled manifold, because
controllability asks what states are reachable while the UCM asks which state changes the task
cannot see. Conflating the two makes the hypothesis sound like a claim about what the nervous
system \emph{cannot} do, when it is a claim about what it need not bother to correct.
...
This has been experimentally confirmed across many motor tasks \cite{ScholzSchoner1999,TodorovJordan2002}.
...
\begin{itemize}
  ...
  \item Stage 3: exploit the drift dynamics $f(\state)$ by designing trajectories that
        use passive mechanics (Volume~II, Chapter~5). Agrachev \& Sachkov
        \cite{AgrachevSachkov2004} develop the geometric theory of reachable sets in
        Chapter~8; what a system can reach depends on its drift as much as on its inputs,
        which is why releasing a degree of freedom can enlarge the reachable set without
        adding an actuator.
\end{itemize}
