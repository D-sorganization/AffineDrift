...
To fix this, Roy Featherstone formalized **Spatial Vector Algebra**. Rather than writing Newton's equations as two separate 3D systems (one for $F = ma$ and one for $\tau = I\alpha + \omega \times I\omega$), Spatial Algebra unifies them into a single, breathtakingly elegant 6D format [@featherstone2010guide6d; @featherstone2008rigid].
...
Just as mass $m$ resists linear acceleration, and a $3 \times 3$ inertia tensor $I$ resists angular acceleration, standard 6D Spatial Algebra unifies them into a single $6 \times 6$ **Spatial Inertia Matrix** $I_s$.
