...
\begin{enumerate}[leftmargin=2cm, label=**A\arabic*.**]
  \item There exists a meaningful equilibrium $\state^*$ where $\bm{f}(\state^*, \control^*) = \bm{0}$.
  \item The system's purpose is to *reach and remain at* this equilibrium.
...
\begin{itemize}[leftmargin=1.5cm]
  ...
  \item A gymnast performing a floor routine---the motion *is* the performance.
...
\begin{enumerate}[label=(\roman*)]
  \item **High dimensionality.** The human body has over 200 degrees of freedom.
        Even a simplified musculoskeletal model of the golf swing uses 20--40
        generalized coordinates.
  \item **Extreme nonlinearity.** Joint torques couple through the full rigid-body
        dynamics, including Coriolis and centrifugal terms that dominate at high
        angular velocities.
  \item **Underactuation.** Once the club is released in the downswing, the
        ...
        active torque.
  \item **Time-criticality with flexible timing.** The swing must produce the
  ...
  \item **Repeatability over optimality.** A touring professional does not
...
\begin{enumerate}[label=(\alph*)]
  \item **Path tracking**: keep $\state(t)$ close to the reference path
        $\traj^*(s(t))$, and
  \item **Timing control**: regulate $\dot{s}$ to achieve the desired
...
\begin{itemize}[leftmargin=1.5cm]
  \item The **tangential component**: where the system is *along* the
        trajectory (measured by $s$).
  \item The **transverse component**: how far the system is *from* the
...
\item **(Conceptual.)** Consider a simple pendulum $\ddot{\theta} + \sin\theta = u$.
  \begin{enumerate}[label=(\alph*)]
...
\item **(Mathematical.)** For the system $\dot{x}_1 = x_2$,\;
...
\item **(Design.)** A simplified 2-DOF golf swing model has coordinates
...
\item **(Philosophical.)** A thermostat, an autopilot, and a dancer's body
...
motion from what is merely a choice of clock.
  ...
  \begin{itemize}[leftmargin=1cm]
    \item **Intrinsic**: properties of the curve itself---its shape,
      curvature, how it bends and twists through state space. These are
      independent of parameterization.
    \item **Extrinsic**: properties that depend on how the curve is
...
tracking difficulty.
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item The required authority grows as the *square* of the speed.
...
\begin{itemize}[leftmargin=1.5cm]
  \item A **tangential component** $\dot{v}\, \tangent$ that changes the
    speed along the curve.
  \item A **normal component** $v^2\kappa\, \normal$ that ``steers'' the
...
control problem:
  ...
  \begin{enumerate}[label=**(\Alph*)**]
    \item **Path design**: Choose the geometric curve $\traj^*(s)$ in state
      space that achieves the task (e.g., delivers the clubhead to the ball with
      the correct velocity vector).
    \item **Timing design**: Choose the timing law $s(t)$ that balances
...
local approximations that are useful for controller design.
  ...
  \begin{enumerate}[label=(\roman*)]
    \item The **osculating line** is the tangent line: the best linear
      approximation.
    \item The **osculating circle** lies in the $(\tangent, \normal)$ plane,
      has radius $R = 1/\kappa$, and center at $\traj^*(s_0) + R\,\normal$.
      It is the best second-order (circular) approximation.
    \item The **osculating plane** is the plane spanned by $\tangent$ and
      $\normal$. Torsion measures how fast the curve leaves this plane.
    \item The **osculating sphere** is the best third-order approximation
...
This result has a direct implication: \emph{the stability of trajectory
...
\begin{corollary}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item *Compact*: $p - 1$ scalar functions instead of $n$ coordinate
      functions.
    \item *Intrinsic*: independent of coordinate choice.
    \item *Timing-independent*: the trajectory shape is fully determined
...
\item **(Computation.)** Compute the curvature and torsion of the curve
  $\traj(t) = (t, t^2, t^3)$ at $t = 0$ and $t = 1$. At which point is the
  curve ``harder to track'' at constant speed?

\item **(Arc length.)** The logarithmic spiral $\traj(t) = e^{at}(\cos t,\;
...
\item **(Frenet--Serret.)** For the curve $\traj(s) = (\cos(s/\sqrt{2}),\;
...
\item **(Path--timing separation.)** A robot end-effector must trace the
...
\item **(Conceptual.)** Consider two trajectories in $\Reals^4$ with
...
\item **(Numerical project.)** Using motion capture data (or simulated data
...
``error'' is that it is slightly ahead of or behind schedule.
  ...
  \begin{enumerate}[label=(\roman*)]
    \item **Transverse deviations**: the system drifts *away from*
      the trajectory path. This is a genuine control failure.
    \item **Tangential deviations**: the system is ahead of or behind
...
\begin{theorem}[Floquet Stability Criterion]\label{thm:floquet}
  A periodic orbit is:
  \begin{enumerate}[label=(\roman*)]
    \item **Orbitally stable** if all Floquet multipliers satisfy
      $|\mu_k| \leq 1$, with multipliers on the unit circle being
      semisimple.
    \item **Asymptotically orbitally stable** if all Floquet multipliers
      satisfy $|\mu_k| < 1$ (all strictly inside the unit circle).
    \item **Orbitally unstable** if any Floquet multiplier satisfies
      $|\mu_k| > 1$.
  \end{enumerate}
\end{theorem}
...
\begin{itemize}[leftmargin=1.5cm]
  ...
  \item The *phase-dependent* nature of $\bm{Q}(t)$ is essential:
...
\begin{enumerate}[label=**Layer \arabic*.**, leftmargin=2cm]
  \item **Feedforward** $\control^*(t)$: computed offline from inverse
    dynamics, provides $\sim$90\% of the total control effort.
  \item **Projection**: measures the current state $\state(t)$,
    projects onto the orbit to find $s^*$, and computes the transverse
    deviation $\bm{\xi}$.
  \item **Transverse feedback** $\delta\control = \bm{K}_\transverse(s^*)\, \bm{\xi}$:
...
\begin{enumerate}[label=**4.\arabic*.**, leftmargin=1.5cm]

\item **(Conceptual.)** Explain why a pendulum swinging on its limit
...
\item **(Computation.)** For the Van der Pol oscillator
...
\item **(Transverse linearization.)** For the system
...
\item **(Numerical project: Transverse LQR.)** For the double pendulum
  from Exercise~3.4:
  \begin{enumerate}[label=(\alph*)]
...
\item **(Design.)** A walking robot with period-1 gait has Floquet
...
\item **(Philosophical.)** Compare the transverse LQR approach
...
\begin{enumerate}[label=(\roman*)]
  \item **Inertial coupling** ($M_{21}\ddot{\theta}_1$): Accelerating or decelerating the arms immediately exerts a force on the club.
  \item **Velocity coupling** ($C_2$): This term contains the centrifugal forces. As the arms swing rapidly, centrifugal force naturally pulls the club outward.
\end{enumerate}
...
\begin{itemize}
  \item During the early downswing, the club's zero dynamics are *unstable*. If the club gets slightly out of position, the centrifugal force tends to pull it further off the optimal plane.
  \item During the late downswing (the release phase), the club's zero dynamics become *strongly stable*. The massive acceleration $\ddot{\theta}_2 \gg 0$ acts like a stiff restoring spring. The centrifugal force aligns the club perfectly with the arms. 
\end{itemize}
...
\begin{enumerate}[label=**5.\arabic*.**, leftmargin=1.5cm]

\item **(Annihilator construction.)** A cart-pole system has state $\bm{q} = (x, \theta)^\top$, where $x$ is the cart position and $\theta$ is the pole angle. The motor applies horizontal force to the cart.
  \begin{enumerate}
...
\item **(Passive pumping.)** In a playground swing, the person swinging cannot exert an external torque on the swing setup (there is no motor at the top pivot). By what mechanism (which dynamics term) does the swinger add energy to the system by leaning back and forth? Relate this to the golf wrist release.

\item **(Zero Dynamics Stability.)** Consider a generic system partitioned into $\ddot{q}_a = v_a$ and $\ddot{q}_u + c q_u = q_a$. Let the reference trajectory be $q_a^*(t) = \sin(t)$.
  \begin{enumerate}
...
\begin{enumerate}[label=**6.\arabic*.**, leftmargin=1.5cm]

\item **(Free Terminal Time Formulation.)** Explain how one sets up an OCP where final time $t_f$ is a decision variable in a fixed-grid Direct Collocation solver. (Hint: consider scaling time $t = \tau \cdot t_f$, where $\tau \in [0, 1]$). Show how the dynamic constraint $\dot{\state} = \bm{f}$ changes under this substitution.

\item **(Kinematic Goals vs Setpoints.)** You are optimizing a baseball bat swing. Your state is $(\theta, \dot{\theta})$. Assume the ball arrives at the plate between $t=0.5$ and $t=0.6$ seconds. Formulate the path constraint and the terminal manifold constraint $\psi$ assuming you want maximum $\dot{\theta}$ when $\theta = \pi/2$, but the exact time of impact within that window does not matter.

\item **(Signal-Dependent Noise Penalty.)** Human muscle torque output $\tau$ has variance $\sigma^2 = k |\tau|^2$. Write a modified integral cost function $L(\state, \tau)$ that penalizes the introduction of variance into the system. Interpret what this does to the optimal trajectory shape.
