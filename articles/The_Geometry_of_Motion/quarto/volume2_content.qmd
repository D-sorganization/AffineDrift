...
Many introductory control treatments begin with a system, an error signal, a
reference point, and a controller whose role is to reduce that error. This is a
useful starting point, especially for regulation problems, but it can obscure
tasks where the object of interest is a time-varying motion rather than a final
state.

But consider the golf swing.

A skilled golfer does not steer the clubhead toward the ball in the same way a
thermostat regulates a room around $72^\circ$F. The relevant object is a
coordinated, whole-body trajectory through a high-dimensional configuration
space. The clubhead reaches impact because the motion has been shaped so that
its path, orientation, and velocity pass through a small region of state space at
the right time.


  **For many movement tasks, control is trajectory regulation.**
  A setpoint or equilibrium is one important control objective, but it is not the
  only one. For locomotion, manipulation, sport motion, and many robotic tasks,
  the relevant object is a *trajectory*---a curve through state space that the
  system is asked to follow, and around which disturbances are rejected.
  Performance is then measured by trajectory error, timing, robustness, energy,
  and task-specific tolerances rather than only by convergence to a point.


This is not merely a philosophical reframing. It changes the optimization
problem, the stability definition, and the criteria used to decide whether the
controlled motion succeeded.

The purpose of this volume is to develop control ideas from this
trajectory-first perspective. Setpoint regulation remains a useful special case,
but the emphasis here is on systems whose task is to move through state space
while staying inside acceptable error bounds.
