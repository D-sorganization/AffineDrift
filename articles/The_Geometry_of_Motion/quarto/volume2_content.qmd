...
\begin{tikzpicture}[scale=1.0]
  ...
  \begin{scope}[shift={(-4.5,0)}]
    \node[font=\bfseries] at (0, 2.8) {Classical View};
    ...
    \filldraw[accentred] (0.8, 0.6) circle (3pt)
      node[right, font=\bfseries, accentred] {~target $\state^*$};
    ...
    \node[font=, text width=3.5cm, align=center] at (0, -2.5)
      {All trajectories convergeto a single point.};
  \end{scope}

  % Our picture
  \begin{scope}[shift={(4.5,0)}]
    \node[font=\bfseries] at (0, 2.8) {Trajectory View};
    \draw[-{Stealth[length=3mm]}, thick, gray] (-1.8, 0) -- (1.8, 0)
      node[right, font=] {$x_1$};
    \draw[-{Stealth[length=3mm]}, thick, gray] (0, -1.8) -- (0, 1.8)
      node[above, font=] {$x_2$};
    ...
    \node[trajgreen!80!black, font=\bfseries] at (1.2, 1.3)
      {$\traj^*(t)$};
    ...
    \node[font=, text width=3.5cm, align=center] at (0, -2.5)
      {System tracks a movingreference through state space.};
  \end{scope}
...
Combining the trajectory tube (Definition~@def-tube) with the
...
\varepsilon(s) = \begin{cases}
    \varepsilon_{\text{loose}} & s \in [0,\, 0.3] \quad \text{(address \& backswing)}, 
    \varepsilon_{\text{loose}} - (\varepsilon_{\text{loose}} - \varepsilon_{\text{tight}})
      \dfrac{s - 0.3}{0.4} & s \in [0.3,\, 0.7] \quad \text{(transition \& downswing)}, 
    \varepsilon_{\text{tight}} & s \in [0.7,\, 0.8] \quad \text{(impact zone)}, 
...
\begin{tikzpicture}[xscale=8, yscale=2.5]
  ...
  \draw[-{Stealth[length=3mm]}, thick] (0, 0) -- (1.08, 0)
    node[right, font=] {phase $s$};
  \draw[-{Stealth[length=3mm]}, thick] (0, 0) -- (0, 1.3)
    node[above, font=] {tube width $\varepsilon(s)$};
...
\begin{tikzpicture}[scale=1.0]
  ...
  \begin{scope}[shift={(-4,0)}]
    \node[font=\bfseries, chapblue] at (0, 2.2) {Low curvature};
    \draw[very thick, trajgreen, decoration={markings,
  ...
  \begin{scope}[shift={(4,0)}]
    \node[font=\bfseries, accentred] at (0, 2.2) {High curvature};
...
### Construction in $\Reals^n$
  ...
  first $p \leq n$ derivatives $\{\traj'(s), \traj”(s), ..., \traj^{(p)}(s)\}$
  are linearly independent at each point. The **Frenet--Serret frame**
  $\{\frenet_1(s), \frenet_2(s), ..., \frenet_p(s)\}$ is defined by
  applying the Gram--Schmidt process to these derivatives:
  \begin{align}
    \frenet_1 &= \traj' = \tangent, \label{eq:frenet_e1} 
    \tilde{\frenet}_k &= \traj^{(k)} - \sum_{j=1}^{k-1}
      \inner{\traj^{(k)}}{\frenet_j}\, \frenet_j, \qquad
    \frenet_k = \frac{\tilde{\frenet}_k}{\norm{\tilde{\frenet}_k}},
    \quad k = 2, ..., p. \label{eq:frenet_ek}
  \end{align}
  The frame vectors $\frenet_1, ..., \frenet_p$ form an orthonormal set at
...
$$ {#eq-frenet_serret}
  ...
  and $\kappa_3, ..., \kappa_{p-1}$ are the
...
stretching. The curvatures $\kappa_1, ..., \kappa_{p-1}$ are the
...
$\kappa_3, ..., \kappa_{p-1}$ measure increasingly subtle geometric
...
\begin{remark}{The Curvature Signature}{curv_signature}
  ...
  ..., \kappa_{p-1}(s))$ as functions of arc length constitutes the
...
\begin{tikzpicture}[scale=1.0]
  ...
  \begin{scope}[shift={(-4.5, 0)}]
    ...
    \node[font=\bfseries] at (1.5, -0.7) {Three timing laws};
  \end{scope}
  ...
  \begin{scope}[shift={(4, 0)}]
    ...
    \node[font=\bfseries] at (0.3, -0.8) {Same spatial path};
    % Speed markers
...
frame $\{\frenet_1(s), ..., \frenet_p(s)\}$, any state $\state$ near the
...
the nearest point on the reference), and $\xi_2, ..., \xi_n$ are the
...
$$
  ...
  $\bm{\xi} = (\xi_2, ..., \xi_n)$ are the transverse (cross-trajectory)
...
equations @eq-frenet_serret), fictitious forces appear in the transverse
...
$$ {#eq-transverse_lin}
  ...
  $\kappa_1(s), ..., \kappa_{p-1}(s)$. The term $v^2\bm{K}(s)$ represents
  the geometric coupling between the curve's shape and the transverse stability.
\end{proposition}

This result has a direct implication: *the stability of trajectory
...
\begin{theorem}[Fundamental Theorem of Curves]\label{thm:fundamental}
  Let $\kappa_1(s), ..., \kappa_{p-1}(s)$ be smooth functions on
  $[0, L]$ with $\kappa_k(s) > 0$ for $k = 1, ..., p-2$. Then there
  exists a curve $\traj: [0, L] \to \Reals^n$, parameterized by arc length,
  whose generalized curvatures are exactly $\kappa_1, ..., \kappa_{p-1}$.
...
2. Compute $\kappa_1(s)$ using @eq-curvature_general_n.
...
\begin{tikzpicture}[scale=1.1]
  ...
  \node[font=\bfseries, chapblue] at (-2.2, 0.5) {$\manifold$};

  % Point p
  \coordinate (P) at (0.5, 0.7);
  \filldraw[accentred] (P) circle (2.5pt);
  \node[below left, font=\bfseries, accentred] at (P) {$p$};

  % Tangent plane
  \draw[fill=darkgold!10, opacity=0.7, thick, darkgold!60]
    (-0.8, 1.8) -- (1.8, 2.5) -- (2.2, 0.5) -- (-0.4, -0.2) -- cycle;
  \node[font=, darkgold!80!black] at (1.8, 1.0) {$\tangentspace{p}$};

  % Tangent vector
  \draw[-{Stealth[length=3.5mm]}, very thick, accentred]
    (P) -- ++(1.0, 0.8) node[above right, font=] {$v \in \tangentspace{p}$};
  ...
  \node[trajgreen!80!black, font=] at (-1.7, 0.4) {$\gamma(t)$};
\end{tikzpicture}
...
individual joint manifolds (modulo kinematic constraints):
  ...
  $\manifold_1, ..., \manifold_k$, the total configuration space is
...
\begin{tikzpicture}[scale=0.9, every node/.style={font=}]
  ...
  \node[font=, align=left] at (5.2, 1.8)
    {$\configspace = \SOthree \times \SOthree$};
  \node[font=, align=left] at (5.2, 1.1)
    {$\phantom{\configspace = {}} \times\;\Sone \times \Sone \times \Sone$};
  \node[font=, align=left] at (5.2, 0.2)
    {dim $= 3 + 3 + 1 + 1 + 1 = 9$};
  \node[font=, accentred, align=center] at (5.2, -0.4)
    {**Not $\Reals^9$!**};
\end{tikzpicture}
...
on each tangent space.
  ...
  $\tangentspace{p}$. In local coordinates $\bm{q} = (q_1, ..., q_n)$, the
...
The remarkable fact is that the geodesic equation @eq-geodesic is
...
but the coordinate-free form @eq-euler_lagrange_manifold makes clear that:
...
$$ {#eq-pd_so3}
  ...
  The na\"{\i}ve controller has singularities; @eq-pd_so3 does not.
  The na\"{\i}ve controller produces torques proportional to angle differences
  (which can be misleading near $\pm\pi$); @eq-pd_so3 produces
...
2. A planar double pendulum (two revolute joints in 2D).
3. A spherical pendulum (a point mass on a rigid rod, free to swing
  in all directions).
4. A spacecraft with two reaction wheels (the body rotates freely in
  3D; each wheel adds one internal DOF).

  For each, identify whether $\Reals^n$ coordinates would have singularities.
- **(Euler angle singularity.)** For the ZYX Euler angle representation
...
- **(Logarithmic error.)** For two rotations
...
- **(Geodesics.)** For a double pendulum with masses $m_1, m_2$ and
  lengths $l_1, l_2$:
...
2. The manifold PD controller from @eq-pd_so3 for the
  same task.
...
- **(Golf swing configuration space.)** For the 9-DOF golf swing
  model of @eq-golf_config:
...
\begin{tikzpicture}[scale=1.0]
  % --- Lyapunov view ---
  \begin{scope}[shift={(-4.5,0)}]
    \node[font=\bfseries, chapblue] at (0, 3.0) {Lyapunov View};
    ...
    % Verdict
    \node[font=\bfseries, accentred] at (0, -2.5) {“Unstable”};
  ...
  % --- Orbital view ---
  \begin{scope}[shift={(4.5,0)}]
    \node[font=\bfseries, trajgreen!80!black] at (0, 3.0) {Orbital View};
    \draw[-{Stealth[length=2.5mm]}, thick, gray] (-2, 0) -- (2, 0)
      node[right, font=] {$x_1$};
    \draw[-{Stealth[length=2.5mm]}, thick, gray] (0, -2) -- (0, 2.5)
      node[above, font=] {$x_2$};
    ...
    % Verdict
    \node[font=\bfseries, trajgreen!80!black] at (0, -2.5) {“Stable”};
...
\begin{tikzpicture}[scale=1.15]
  ...
  \node[trajgreen!70!black, font=\bfseries] at (3.4, 0.1) {$\orbit$};

  % Point on orbit
  \coordinate (P) at (0, 1.65);
  \filldraw[trajgreen!70!black] (P) circle (2.5pt);
  \node[above, font=, trajgreen!70!black] at (0, 1.85) {$\traj^*(s^*)$};

  % Tangent vector
  \draw[-{Stealth[length=3.5mm]}, very thick, chapblue]
    (P) -- ++(1.2, -0.05);
  \node[chapblue, font=\bfseries] at (1.5, 1.85) {$\bm{t}(s^*)$};

  % Transverse hyperplane (shown as a line in 2D)
  \draw[thick, darkgold, dashed] ($(P) + (-0.1, -1.8)$) -- ($(P) + (0.1, 1.8)$);
  \node[darkgold, font=\bfseries, rotate=85] at (-0.5, 2.8) {$\Sigma(s^*)$};

  % Actual state
  \coordinate (X) at (0.3, 0.2);
  \filldraw[accentred] (X) circle (2.5pt);
  \node[right, font=\bfseries, accentred] at (0.4, 0.2) {$\state(t)$};

  % Transverse deviation
  \draw[-{Stealth[length=2.5mm]}, thick, accentred]
    (P) -- ($(P) + (0.05, -1.45)$);
  \node[accentred, font=] at (-0.7, 0.5) {$\bm{\xi}$};

  % Phase annotation
  \draw[thin, gray, |->] (-3, -1.6) -- (3, -1.6) node[right, font=] {$s$};
  \draw[thin, gray, dashed] (0, -1.6) -- (P);
  \filldraw[gray] (0, -1.6) circle (1.5pt);
  \node[font=, gray, below] at (0, -1.6) {$s^*$};
\end{tikzpicture}
...
\begin{theorem}[Transverse Linearization~[@Shiriaev2010; @ManchesterSlotine2017]]\label{thm:transverse_lin}
...
$$ {#eq-transverse_linear}
  ...
  \begin{align}
    \Amat_\transverse(s) &= \bm{P}(s) \left[
      ...
      {\norm{\dot{\traj}^*}^2} \cdot
      \frac{\partial \bm{f}}{\partial \state}\bigg|_{\traj^*, \control^*}
    \right] \bm{P}(s), \label{eq:A_transverse} 
...
\begin{remark}{Understanding the Transverse $\Amat_\transverse$}{A_trans_explain}
  Equation @eq-A_transverse has a clear structure:
...
\end{remark}
  ...
  @eq-transverse_linear (with $\delta\control = \bm{0}$) is
  ...
  *linear* problem on the $(n{-}1)$-dimensional transverse space.
  ...
  @eq-transverse_linear, we achieve orbital stability of the
...
$\{\bm{n}_1(s), ..., \bm{n}_{n-1}(s)\}$ for $\Sigma(s)$---the Frenet--Serret
...
$$
  ...
  The eigenvalues $\mu_1, ..., \mu_{n-1}$ of $\monodromy$ are the
...
@eq-transverse_linear using standard LTV techniques, then map
...
The terminal-value Riccati equation @eq-riccati naturally produces
...
\begin{theorem}[Riccati Funnel]\label{thm:riccati_funnel}
  Let $\bm{S}(t)$ be the solution of @eq-riccati with terminal
...
\begin{tikzpicture}[
  block/.style={draw, rounded corners, minimum height=1.2cm,
    minimum width=3.5cm, font=, align=center, thick},
  ...
  \draw[arrow] (plant) -- ++(2, 0) node[right, font=] {$\state(t)$}
    |- (7.5, -2) -| (-3, -2) |- (proj.west);
  \draw[arrow] (proj) -- (fb);
...
If we lack the authority to command arbitrary accelerations, the classical setpoint perspective sees this as a fundamental failing. A setpoint controller wants to cancel the nonlinearities, feedback-linearize the system, and dictate the exact motion. Underactuation prevents this brute-force cancellation. 
...
$$
\begin{aligned}
  M_{11}\ddot{\theta}_1 + M_{12}\ddot{\theta}_2 + C_1 + g_1 &= u_1, 
  M_{21}\ddot{\theta}_1 + M_{22}\ddot{\theta}_2 + C_2 + g_2 &= 0.
\end{aligned}
$$ {#eq-pend-underactuation}

Equation @eq-pend_unactuated is the **unactuated dynamics** or **passive constraint**. It dictates the motion of the club. Notice that we cannot directly command $\ddot{\theta}_2$. However, we *can* influence it through the coupling terms:
...
\begin{tikzpicture}[scale=1.2]
  ...
  \filldraw[chapblue!50!black] (0, 0) circle (4pt) node[above left, black, font=] {Shoulder ($u_1$)};
  
  % Club (Link 2)
  \draw[thick, accentred, line width=1mm] (2, -2) -- (1, -4);
  \filldraw[accentred!50!black] (2, -2) circle (3pt) node[right, black, font=, xshift=2pt] {Wrist (passive)};
  \filldraw[gray!80!black] (1, -4) circle (4pt) node[below, black, font=] {Clubhead};

  % Angles
  \draw[dashed, gray] (0, 0) -- (0, -2);
  \draw[-{Stealth[length=2mm]}, gray] (0, -1) arc (270:315:1);
  \node[font=] at (0.3, -1.2) {$\theta_1$};

  \draw[dashed, gray] (2, -2) -- (2, -4);
  \draw[-{Stealth[length=2mm]}, gray] (2, -3) arc (270:245:1);
  \node[font=] at (1.8, -3.2) {$\theta_2$};
...
For control design, this means we cannot arbitrarily select a path in $\statespace$ and expect a controller to track it. The reference trajectory *must* live in the restricted subspace defined by @eq-unactuated_dynamics. The trajectory optimization algorithms in Chapter~6 will explicitly enforce this constraint.
...
If we force the actuated coordinates to track a reference trajectory (e.g., using a high-gain PD controller, $\bm{v}_a = \ddot{\bm{q}}_a^* - K_d \dot{\tilde{\bm{q}}}_a - K_p \tilde{\bm{q}}_a$), the unactuated coordinates are left to evolve according to @eq-zero_dynamics. The stability of the resulting motion depends entirely on whether @eq-zero_dynamics is a stable differential equation when forced by $\bm{q}_a^*$. This leads us to the concept of the *zero dynamics*.
...
2. What is the input matrix $\Bmat$?
3. Construct a left annihilator $\Bmat^\perp$.
4. Derive the unactuated dynamics constraint. What physical conservation law does this represent when gravity is ignored?
...
$$
  ...
  constitutes a valid **dynamic funnel** if the set is positively invariant under the closed-loop dynamics. That is, if $\state(0) \in \mathcal{F}(0)$, then $\state(t) \in \mathcal{F}(t)$ for all $t > 0$.
...
We can reframe Theorem @thm-funnel_invariance using the S-procedure. To guarantee that $\dot{V}(\state, t) < 0$ whenever $V(\state, t) = 1$, we require:
...
However, if we introduce the Stochastic OCP (via Covariance Steering) from Chapter 6 and apply the realistic premise of SDN, the optimizer faces a sharp tradeoff.
