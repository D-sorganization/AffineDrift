...
2. **Extreme nonlinearity.** Joint torques couple through the full rigid-body
        ...
        angular velocities.
3. **Underactuation.** Once the club is released in the simplified downswing
        model, the wrist hinge is treated as low-torque or passive.
        Clubhead speed at impact is strongly shaped by passive coupling terms,
        but the size of that contribution depends on the model, grip constraint,
        and measured swing data.
...
\begin{example}[From Setpoint Thinking to Trajectory Thinking]\label{example:golf_shift}
...
This decomposition is not naturally represented by a simple setpoint framework,
where time and space are usually fused together.
...
This funnel profile encodes the modeling hypothesis that precision demands are
highest near impact and lower during setup. The controller allocates authority
where the stated terminal constraint is most sensitive, rather than maintaining
uniform precision everywhere; this distinction is easy to obscure in a pure
fixed-setpoint framing.
...
correction to handle physical disturbances. A control design that treats everything as feedback is asking the
feedback loop to do disproportionately more work than necessary, ignoring the baseline deterministic dynamics of the swing.
...
alone could not produce within the chosen actuator model. In a simplified golf
model, a passive wrist release and centrifugal coupling can contribute to
clubhead acceleration and face closure; whether they square the clubface in a
real swing depends on grip mechanics, shaft dynamics, impact geometry, and
validation against measured swings.
...
\begin{description}[leftmargin=1.5cm, labelwidth=1.2cm, font=\bfseries\color{chapblue}]
  ...
  \item[Ch.~7] **Funnel Synthesis.**
    Designing time-varying tubes with certified invariance under stated model,
    controller, and disturbance assumptions. Sums-of-squares programming for
    funnel computation. Robust funnels under model uncertainty.
  ...
  \item[Ch.~11] **Case Study: The Complete Golf Swing.**
    A planned application of the theory to a declared musculoskeletal model of
    the golf swing, from trajectory optimization through funnel synthesis to
...
2. **(Design.)** A simplified 2-DOF golf swing model has coordinates
   ...
   3. Discuss why a setpoint controller applied only at impact ($s = 0.7$)
   may be inadequate or misleading for a trajectory-native task.
...
\begin{example}[The Circle: Three Parameterizations]\label{example:circle_param}
...
\begin{remark}[Arc Length in High Dimensions]\label{remark:high_dim_arc}
  ...
  metric \cite{doCarmo1992}. When the configuration manifold carries such a
  ...
  \cite{BulloLewis2004}. That statement is about configuration velocities
...
\begin{example}[Curvature of an Elliptical Trajectory]\label{example:ellipse_curv}
...
\begin{example}[Frenet--Serret for a 3D Helix]\label{example:helix}
...
\begin{example}[Curvature Cost in the Golf Downswing]\label{example:golf_curv_cost}
...
\begin{remark}[The Curvature Signature]\label{remark:curv_signature}
...
\begin{example}[Timing the Downswing]\label{example:timing_downswing}
...
\begin{example}[Curvature Signature of a Golf Swing]\label{example:golf_signature}
...
\begin{example}[The Circle $\Sone$]\label{example:circle_manifold}
...
\begin{example}[The Human Shoulder]\label{example:shoulder}
...
\begin{warning}[Euler Angle Catastrophe in Golf Modeling]\label{warning:euler_golf}
...
\begin{example}[Inertia-Weighted Distance in the Swing]\label{example:inertia_distance}
...
\begin{example}[Geodesic Curvature of the Downswing]\label{example:downswing_geod_curv}
...
\begin{remark}[Why Not Just Subtract Quaternions?]\label{remark:quat_subtract}
...
\begin{example}[PD Control on $\SOthree$]\label{example:pd_so3}
...
\begin{example}[Limit Cycles: The Prototype]\label{example:limit_cycle}
...
\begin{theorem}[Transverse Linearization~\cite{HauserHindman1995,Shiriaev2010,ManchesterSlotine2017}]\label{thm:transverse_lin}
...
\begin{remark}[Understanding the Transverse $\Amat_\transverse$]\label{remark:A_trans_explain}
...
\begin{example}[Transverse Linearization of a Planar System]\label{example:planar_transverse}
...
\begin{remark}[The Missing Multiplier]\label{remark:missing_multiplier}
...
\begin{example}[Floquet Analysis of a Walking Gait]\label{example:walking_floquet}
...
\begin{example}[Amplification Profile of the Golf Swing]\label{example:golf_amplification}
...
\begin{remark}[Why Not Just Use Standard LQR?]\label{remark:why_not_lqr}
...
When a gymnast swings on a high bar, their center of mass rotates around the bar, but they have no motor at the bar---that "joint" is a passive pin constraint. When a human walks, the foot acting as the pivot is unactuated during the single-support phase; we cannot create torque from a point contact. In a simplified golf model, the wrist can be treated as a low-torque or passive hinge over part of the release. The club's angular acceleration is shaped by passive coupling between the arms and club, not by a direct wrist motor in that model. The assumption is useful, but it is not a claim that every real wrist torque is zero.
...
If we force the actuated coordinates to track a reference trajectory (e.g., using a high-gain PD controller, $\bm{v}_a = \ddot{\bm{q}}_a^* - K_d \dot{\tilde{\bm{q}}}_a - K_p \tilde{\bm{q}}_a$), the unactuated coordinates are left to evolve according to \eqref{eq:zero_dynamics}. The stability of the resulting motion depends entirely on whether \eqref{eq:zero_dynamics} is a stable differential equation when forced by $\bm{q}_a^*$. This leads us to the concept of the *zero dynamics*.
...
\begin{remark}[Why Good Swings Feel Effortless]\label{remark:effortless}
...
Underactuation shifts the modeling task from forcing every coordinate independently to shaping the interaction between actuators, gravity, inertia, and constraints. In Chapter 6, numerical trajectory optimization is used to search for such motions by solving optimal-control problems that explicitly respect the $\Bmat^\perp$ constraint and the stated actuator limits.
...
\begin{example}[The Golf Swing OCP]\label{example:golf_ocp}
...
## Beyond LQR: The Need for Certified Invariance
...
If the derivative of the boundary function points inward everywhere on the boundary, trajectories may circulate inside the funnel, but they can never escape it. For a moving target problem, we want the spatial volume of $\mathcal{F}(t)$ to shrink as $t \to t_f$, terminating in a tight bounded region at impact.
  ...
  A computed funnel is an analysis tool for repeatability. Under the stated model, controller, and disturbance assumptions, it defines a certified set of initial conditions. It does not by itself prove successful ball striking in an unmodeled human swing.


## Sums-of-Squares (SOS) Programming


Proving that $\dot{V} < 0$ on the boundary $V=1$ for a general nonlinear function $\bm{f}$ is famously difficult. We have an infinite number of states on the boundary to check! We circumvent this by restricting our dynamics $\bm{f}$ algorithms to polynomials (via Taylor expansion or substitution) and leveraging **Sums-of-Squares (SOS) Programming**~\cite{Boyd1994, MajumdarTedrake2017}.
...
Let us return to the double-pendulum golf model. The reference trajectory $\traj^*(t)$ from Chapter 6 gives nominal kinematics. The transverse LQR from Chapter 4 gives a feedback matrix $\bm{K}(t)$ designed to stabilize selected deviations. The goal is to compute a conservative funnel around this sequence and state the model assumptions under which the closed-loop system satisfies the invariance certificate.
...
\begin{example}[Volume Maximization via SDP]\label{example:vol_max}
...
With $\traj^*(s)$ computed, we analyze the trajectory using **Transverse Linearization** (Chapter 4) [@HauserHindman1995; @Shiriaev2010]. We ignore errors along the tangent of the path only within the chosen phase model and construct a time-varying LQR feedback matrix to regulate selected transverse deviations.

Finally, we compute candidate bounds for this stability region using **Sums-of-Squares Programming** (Chapter 7) [@MajumdarTedrake2017]. The SDP supplies a boundary function $V(\state, t) = 1$ that certifies invariance under the polynomial model, controller, and disturbance assumptions used in the computation.
