...
This book develops control theory from that trajectory-first perspective. It treats
the classical setpoint framework as a special---and often misleading---case of a
richer theory, and it builds the mathematical tools needed to design controllers for
systems whose purpose is to *move*.
...
A setpoint is the degenerate case where $\traj^*(t) = \state^*$ for all $t$
...
::: {#exm-golf_shift}
**From Setpoint Thinking to Trajectory Thinking**
...
A setpoint framework does not represent this decomposition naturally, because it
fuses time and space together.
...
This splits the state into two components:
...
\end{equation}
For the golf swing, empirical studies indicate the feedforward component accounts for approximately 90\% of
the total joint torques [@Winter2009]. The feedback controller provides the remaining 10\% of
correction to handle physical disturbances. A control design that treats everything as feedback asks the
feedback loop to do far more work than necessary. It ignores the baseline deterministic dynamics of the swing.
...
alone could not produce. This is how the golf swing works in a double-pendulum model:
the wrist hinge releases passively, and the centrifugal acceleration of the downswing
squares the clubface without a dedicated wrist torque.
...
increasingly sophisticated layers:
  ...
  - **Ch.~4** **Orbital Stability and Transverse Linearization.**
  ...
  - **Ch.~5** **Underactuation and Passive Dynamics.**
  ...
  - **Ch.~8** **Phase-Variable Control** [@WesterveltGrizzle2007].
  ...
  - **Ch.~9** **Stochastic Trajectories and Motor Variability.**
    Signal-dependent noise in biological actuators. The minimum-variance theory of human movement [@TodorovJordan2002]. Why optimal trajectories are smooth [@FlashHogan1985].

  - **Ch.~10** **Learning to Move.**
  ...
  - **Ch.~11** **Case Study: The Complete Golf Swing.**
...
This book replaces the classical vocabulary with a trajectory-centered one:

| **Classical Term** | **Trajectory-First Term** |
| --- | --- |
...
1. **(Conceptual.)** Consider a simple pendulum $\ddot{\theta} + \sin\theta = u$.
  1. **(Mathematical.)** For the system $\dot{x}_1 = x_2$,\;
   ...
   stability.
2. **(Design.)** A simplified 2-DOF golf swing model has coordinates
...
3. **(Philosophical.)** A thermostat, an autopilot, and a dancer's body
...
Why not work in coordinates? Because coordinate-based descriptions mix up
properties of the *curve* with properties of the *parameterization*.
A slow practice swing and a full-speed
...
Many different parameterizations describe the same geometric curve.
If $\traj(\lambda)$ is a parameterized curve and
...
arc-length derivatives of the frame vectors can be written using only the frame
itself and a set of scalar functions called **generalized curvatures**.
...
**Curvature Cost in the Golf Downswing**
  ...
  Mid-downswing, $v \approx 25$\,m/s, with a conceptual reversal curvature
  $\kappa \approx 2$\,rad/m estimated as a representative value from elite motion
  capture data [@Lephart2007]. Using $a_\perp = \kappa v^2$, the centripetal
  acceleration demand is
  \begin{equation}
    a_\perp \approx 2 \times 25^2 = 1250\;\text{m/s}^2 \approx 127\;g.
  \end{equation}
  (These specific kinematics are subject to large variance and act here as an
  illustrative upper bound.) Muscular torque alone does not meet a demand of this
  size; in the model, the *passive coupling* in the kinematic chain supplies the
  rest [@Neumann2010]---the underactuation that the setpoint framework treats as a
  constraint. The geometry of the trajectory creates the inter-segmental forces
...
**The Curvature Signature**
  ...
  **curvature signature** of the trajectory. By the fundamental theorem
  ...
  This has an implication for motor control: a golfer who reproduces the
  curvature signature of a swing reproduces its spatial path up to a rigid
  motion, whatever the timing.
:::
...
**Timing the Downswing**
  ...
  This offers one reading of what golfers do at the top of the swing. The
  "pause at the top"---the brief deceleration between backswing and
  downswing---need not be aesthetic affectation. Under the model it is the timing
  law respecting the curvature speed limit at the point of maximum path curvature.
:::
...
This result has a practical implication: *the stability of trajectory
...
A trajectory is not just "a function of time." It is a geometric object with
intrinsic structure, and that structure determines how hard the trajectory is to
control and what the controller must supply. In Chapter 3, we will see that
...
not a specific Apollo~13 operational event. Modern spacecraft avoid
Euler angles for this reason.
...
A manifold is a space where you can use coordinates *locally*, but where no
single coordinate system works *globally*. You need an atlas of overlapping
charts, the way a road atlas needs multiple pages to cover a country.
...
4. **Parallel transport replaces vector addition.** To compare
    ...
    Subtracting velocity vectors at different times is mathematically
    meaningless.
...
The geodesic equation \eqref{eq:geodesic} is
...
2. **Manifold curvature**: the Christoffel symbol terms
  ...
  This gives a geometric account of the "free speed" of the golf swing:
...
The configuration spaces of mechanical systems have geometry, and that
geometry matters for control.
...
converge to an equilibrium?* We argued in Chapter 1 that this is the
...
**Limit Cycles: The Prototype**
  ...
  are all modeled as limit cycles. The golf swing is a transient trajectory (not
...
For the golf swing, orbital stability---not Lyapunov stability---is the
appropriate framework. In the models of this chapter, the passive transverse
dynamics can contract toward impact. A transverse LQR controller then tightens
the resulting funnel, using phase-varying gains matched to the demands of each
phase of the motion.
...
In classical robotics, the goal is often full actuation: one motor for every degree of freedom. If a robot has six joints, it has six motors. This fully actuated paradigm allows the system to follow almost any smooth trajectory in the configuration space, subject only to actuator saturation limits. The control problem is algebraically simple because the mapping from torques to joint accelerations is invertible.

But nature does not build fully actuated systems, and neither do athletes.

When a gymnast swings on a high bar, their center of mass rotates around the bar, but they have no motor at the bar---that "joint" is a passive pin constraint. When a human walks, the foot acting as the pivot is unactuated during the single-support phase; we cannot create torque from a point contact. And crucially for our running example, a standard model of the golf downswing treats the wrist as a free hinge once the golfer releases the club. In that model the club accelerates to over 100 miles per hour, *driven by the passive physics* of the arms pulling the grip rather than by muscular torque applied at the wrist.
  ...
  The system is **underactuated** if $m < n$, meaning there are fewer actuators than degrees of freedom. In this case, the matrix $\Bmat$ is not full rank (it maps a lower-dimensional control space into the $n$-dimensional configuration space).


If we lack the authority to command arbitrary accelerations, the classical setpoint perspective sees this as a failing. A setpoint controller wants to cancel the nonlinearities, feedback-linearize the system, and dictate the exact motion. Underactuation prevents this brute-force cancellation. 
...
Equation \eqref{eq:pend_unactuated} is the **unactuated dynamics** or **passive constraint**. It dictates the motion of the club. Notice that we cannot directly command $\ddot{\theta}_2$. However, we *can* influence it through the coupling terms:
1. **Inertial coupling** ($M_{21}\ddot{\theta}_1$): Accelerating or decelerating the arms immediately exerts a force on the club.
2. **Velocity coupling** ($C_2$): This term contains the centrifugal forces. As the arms swing rapidly, centrifugal force naturally pulls the club outward.
...
\end{equation}
  ...
  We define a full-rank matrix $\Bmat^\perp(\bm{q}) \in \Reals^{(n-m) \times n}$, called the **left annihilator** of $\Bmat$, such that:
  \begin{equation}
...
We obtain a phase-dependent picture rather than a universal law:
...
- During the late downswing (the release phase), the club's zero dynamics may become strongly attracting in the simplified model. Large club acceleration can act like a restoring term, but the degree of alignment depends on the selected parameters, grip model, and disturbance assumptions.
...
1. **(Annihilator construction.)** A cart-pole system has state $\bm{q} = (x, \theta)^\top$, where $x$ is the cart position and $\theta$ is the pole angle. The motor applies horizontal force to the cart.
  1. **(Passive pumping.)** In a playground swing, the person swinging cannot exert an external torque on the swing setup (there is no motor at the top pivot). By what mechanism (which dynamics term) does the swinger add energy to the system by leaning back and forth? Relate this to the golf wrist release.
2. **(Zero Dynamics Stability.)** Consider a generic system partitioned into $\ddot{q}_a = v_a$ and $\ddot{q}_u + c q_u = q_a$. Let the reference trajectory be $q_a^*(t) = \sin(t)$.
   1. Assume we enforce $q_a = q_a^*$ perfectly. Write the zero dynamics for $q_u$.
   2. Under what conditions on the parameter $c$ are the zero dynamics stable? Is it possible to have a trajectory that passes through both stable and unstable regimes of zero dynamics?
...
In Chapter 1, we reframed our control objective from "reaching a destination" to "tracking a trajectory." Up to this point, we have assumed that this optimal reference trajectory $\traj^*(t)$ is given to us by an oracle. This chapter answers the obvious next question: *Where does this reference come from?*
...
**The Golf Swing OCP**
  ...
  **Initial Guess:** If we seed the NLP solver with a naive guess (e.g., constant shoulder torque), the club speed will be low.
...
| Passive Exploitation | Solvers optimize underactuated topologies by exploiting the coupling matrices ($M_{21}, C_2$). |
...
Proving that $\dot{V} < 0$ on the boundary $V=1$ for a general nonlinear function $\bm{f}$ is famously difficult. We have an infinite number of states on the boundary to check! We circumvent this by restricting our dynamics $\bm{f}$ to polynomials (via Taylor expansion or substitution) and leveraging **Sums-of-Squares (SOS) Programming** [@Boyd1994; @MajumdarTedrake2017].

A polynomial $p(x)$ is a Sum of Squares if it can be written as $p(x) = \sum_{i} q_i(x)^2$ for some polynomials $q_i(x)$. If a polynomial is SOS, it is globally non-negative. Testing if a polynomial is SOS turns out to be equivalent to solving a Semidefinite Program (SDP)—a convex optimization problem that can be solved efficiently.
...
This construction is defensible only after the phase variable's domain, monotonicity condition, and reset behavior are specified. This is the same practical issue that appears in maneuver regulation, transverse linearization, and hybrid zero dynamics [@HauserHindman1995; @WesterveltGrizzle2007; @Shiriaev2010].
...
When the virtual constraints are satisfied exactly ($y = 0$), the $n$-dimensional state space collapses into a much smaller dynamical system that evolves strictly along the manifold $\mathcal{Z}$. This is the **Zero Dynamics** of the system.

For systems that undergo discrete impacts (a foot hitting the ground, or a golf club striking a ball), the system evolves via continuous differential equations until an impact surface is triggered. An instantaneous jump map---a reset dictated by conservation of momentum---then applies.
...
When a roboticist specifies a torque of $50\text{ Nm}$, the motor delivers $50\text{ Nm}$ with a tiny, constant variance. Biological actuators do not work this way. Human muscles generate force by recruiting discrete motor units. As higher forces are required, larger, more fast-twitch oriented motor units are recruited. This physiology creates a control challenge: **motor noise is proportional to the command signal**.


  A control input $\control(t)$ applied by a biological system is realized as a stochastic variable $\tilde{\control}(t)$:
  \begin{equation}
    \tilde{\control}(t) = \control(t) + \bm{W} \control(t) \cdot \bm{\epsilon}(t)
  \end{equation}
  where $\bm{W}$ is a scaling matrix representing the coefficient of variation, and $\bm{\epsilon}(t) \sim \mathcal{N}(0, I)$ is white noise. That is, the variance of the applied force grows with the square of the ordered force: $\text{Var}(\tilde{u}) = c u^2$.


This complicates deterministic optimal control. If a computed trajectory requires large muscle commands, signal-dependent noise can increase endpoint variance. The magnitude of that effect depends on the noise coefficient, the controller, and the sensitivity of the trajectory.
...
However, if we introduce the Stochastic OCP (via Covariance Steering) from Chapter 6 and apply the realistic premise of SDN, the optimizer faces a tradeoff.
...
3. It widens its "funnel" during the transition phase, where tight feedback would amplify motor noise and degrade the impact constraints.
...
When faced with a new task (e.g., striking the ball on an uneven lie), the system searches its library for the closest matching funnel. By interpolating between established funnels using techniques like LQR-Trees [@Tedrake2009] or simple spatial blending, the system can generate a stable, feasible motion without online optimization.
