...
::: {.callout-note}
  ...
  Mathematically, two independent degrees of freedom have become linearly dependent—they merged into the identical physical rotation axis. A degree of freedom is lost during the calculation. If you attempt to control the plane as it passes through this "Singularity," your matrices become non-invertible and the computation breaks down on a physical impossibility.

:::


Because our control theory and geometric motion algorithms rely on continuous trajectories twisting through all possible spatial angles (like chaotic golf club arcs or throwing motions), we must abandon Euler angles. We cannot afford mathematical singularities.
...
Because $\hat{x}_b, \hat{y}_b,$ and $\hat{z}_b$ are all length $1$ (unit magnitude) and mutually orthogonal (all $90^\circ$ apart), the columns are an orthonormal basis. The strict mathematical definitions that guarantee this orthonormality are:
...
::: {.callout-note}
  ...
  If a Configuration Space $\configspace$ describes only a rigid body rotating around a point, we say $\configspace = \SO$.
...
$\SO$ prevents Gimbal Lock and provides pure, continuous rotation matrices. But tracking 9 entries inside a $3 \times 3$ grid (only 3 of which are independent) is computationally inefficient for an engine updating a 15-DOF human model at 1,000 Hertz.

To solve this, William Rowan Hamilton invented **Quaternions** ($\mathcal{H}$) in 1843. Rather than tracking a full Cartesian body frame $R \in \SO$, any 3D rotation can be described as a combination of precisely two things:
...
By enforcing that the sum of the squares of these 4 variables equals 1 ($q_0^2 + q_1^2 + q_2^2 + q_3^2 = 1$), a Unit Quaternion provides a singularity-free, fast algebraic mapping of $\SO$.
...
To merge Translation and Rotation into a single unified control framework for physical bodies mapping the full world, we must upgrade from $3 \times 3$ Rotation Matrices to the $4 \times 4$ space of $\SE$.
