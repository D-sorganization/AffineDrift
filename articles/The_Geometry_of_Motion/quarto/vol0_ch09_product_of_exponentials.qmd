---
pagetitle: "The Product of Exponentials Formula"
description: "Ordered rigid-chain kinematics, compatible Jacobians, task-dependent singularities and the links between golf delivery, force limits and uncertainty."
css:
...
## Historical Context and the Choice of Representation

[]{#abandoning-denavit-hartenberg}
[]{#sec-poe_history}
[]{#laymans-context_box}

Denavit–Hartenberg (D-H) parameters and PoE are alternative descriptions of serial-chain geometry, not competing laws of motion. The original D-H construction provides a systematic relationship between neighboring link frames [@dh1955]. Murray, Li and Sastry and Lynch and Park develop Lie-group and screw-based kinematics [@Murray1994; @Lynch2017]. Both approaches remain useful; neither removes the need to calibrate axes, choose coordinate signs or define the tool.
...
### A Common Contract Before Calculation
...
## Screws and Twists: A Geometric Foundation
[]{#sec-screw_foundation}
...
## The Forward Kinematics Equation: Space-Frame PoE
...
[]{#subsec-poe_derivation}

One derivation sets the most distal joint first, then works toward the base. Joint $n$ moves the tool to $E_nM$. Moving joint $n-1$ next carries that entire downstream assembly, giving $E_{n-1}E_nM$. Continuing toward joint 1 gives [the space-frame formula](#eq-poe_space_frame). This is a construction of a configuration, not a requirement that a golfer activate joints in that temporal order.
...
On narrow screens, scroll the figure or [open it at full size](../figures/poe_transported_axis.svg).

::: {#fig-product_exponentials}
::: {.aba-diagram-scroll role="region" aria-label="A planar two-link arm at home and after a 45-degree first-joint rotation; scroll horizontally" tabindex="0"}
<img src="../figures/poe_transported_axis.svg" alt="A planar two-link arm at home and after a 45-degree first-joint rotation. The second joint moves from (1, 0) to (0.707, 0.707) meters; its positive z direction stays perpendicular to the page in this planar example.">
:::

An Upstream Rotation Carries the Downstream Axis. The Home Axis Is a Constant Model Parameter; the Current Axis Passes Through the Moved Elbow.
:::

## Body-Frame PoE: Motion Relative to the Tool
[]{#sec-body_frame_poe}
[]{#subsec-poe_conversion}

Define constant home axes in the home tool frame by
[]{#eq-adjoint_conversion}
...
## Velocity Kinematics and the Spatial Jacobian
[]{#sec-spatial_jacobian}
[]{#sec-velocity_kinematics}
...
### Body Jacobian and the Adjoint Relationship
[]{#sec-body_jacobian}
[]{#eq-jacobian_adjoint}
...
### Point Velocity and Numerical Verification
[]{#subsec-jacobian_numerical}
...
## Comparison With D-H: A Complete Planar Example
[]{#sec-dh_limitations}
[]{#sec-poe_vs_dh}
...
### PoE Parametrization and Independent Evaluation
[]{#subsec-poe_example}
...
### Computational Advantages and Limits
[]{#subsec-poe_advantages}
...
## Prismatic Joints and Mixed Chains
[]{#sec-prismatic_joints}
...
### Example: Cartesian Translation
[]{#subsec-cartesian_example}
...
## Worked Example: A Spatial 3R Robot
[]{#sec-worked_example_3r}
...
### Home Pose and Screw Axes
[]{#subsec-home_config}
[]{#subsec-spatial_screw_axes}
...
### Numerical Evaluation at a Test Configuration
[]{#subsec-spatial_3r_eval}
...
## Singularities Belong to a Declared Task
[]{#sec-singularities}
[]{#subsec-singularity_types}
...
### A Two-Link Counterexample to Task Conflation
...
### Detection and Damped Inversion
[]{#subsec-singularity_handling}
...
## Workspace, Manipulability and Force Duality
[]{#sec-workspace_manipulability}

[]{#def-reachable_workspace}

Given admissible joint set $\mathcal Q$, the reachable pose set is
[]{#eq-reachable_workspace}
...
### Declare the Metric Before the Ellipsoid
[]{#def-manipulability}[]{#def-velocity_ellipsoid}
...
### Static Force Bounds Use the Dual Map
[]{#def-force_ellipsoid}[]{#laymans-manipulability_duality}
...
### Numerical Example: The Planar 2R Arm
[]{#ex-manipulability_2r}
...
On narrow screens, scroll the figure or [open it at full size](../figures/poe_velocity_ellipses.svg).

::: {#fig-poe_velocity_ellipses}
::: {.aba-diagram-scroll role="region" aria-label="Two velocity ellipses with identical axis scales; scroll horizontally" tabindex="0"}
<img src="../figures/poe_velocity_ellipses.svg" alt="Two velocity ellipses with identical axis scales. At a 45-degree elbow angle the semiaxes are 2.073 and 0.341 meters per second; at 0.1 radians they are 2.233 and 0.0447. Both use a unit Euclidean joint-rate ball in radians per second.">
:::

Position-Velocity Ellipses for Equal Unit-Length Links. Equal Plot Scales Reveal the Strong Anisotropy Already Present at 45 Degrees and Its Increase Near Extension.
:::

## Redundancy and Null-Space Motion
[]{#subsec-null_space_motion}

[]{#def-null_space_projector}

Using the Moore–Penrose inverse defined by the SVD,
[]{#eq-null_space_projector}
...
## Inverse Kinematics With a Consistent Pose Error
...
### Residual Derivatives and Convergence
...
## Connecting Geometry to the Golf Swing and Humanoid Control
...
[]{#sec-ch9_summary}[]{#sec-ch9_further_reading}
...
Murray, Li and Sastry [@Murray1994] and Lynch and Park [@Lynch2017] provide the underlying kinematic and Lie-group treatment. Denavit and Hartenberg [@dh1955] give the classical alternative parameterization. Use the derivations and independent examples here to reconcile conventions when moving between texts; an apparent disagreement can arise from transform direction or six-vector ordering, but an incompatible product or matrix dimension remains an actual error.
...
Unless stated otherwise, use a planar 2R arm with positive link lengths $L_1=1.5$ m and $L_2=1$ m, angular-first twists and the home pose along positive $x$. Declare which task each Jacobian describes.
 ...
 1. **Forward Kinematics.** Construct its space PoE and evaluate $q=(60,-30)$ degrees. Check the endpoint $(1.616025,1.799038)$ m by direct link geometry.
 1. **D-H Comparison.** Using the stated standard convention, include both link translations and verify the same pose. Explain how a terminal tool transform would enter another frame assignment.
 1. **Axis Identification.** For a positive $y$ revolute axis through $(2,0,1)$, find $v=(-1,0,2)$ and verify that points on the axis remain fixed under its exponential.
 1. **Cartesian Robot.** Derive the $6\times3$ spatial Jacobian of three orthogonal sliders. Explain why zero six-dimensional manipulability does not mean this mechanism is singular everywhere.
 1. **Two Different Jacobians.** At $(45,0)$ degrees, compute $J_s$ and $J_p$. Show that their ranks differ. Repeat with a position-plus-yaw task.
 1. **Manipulability.** With $q_1=0$, plot position manipulability as $q_2$ ranges from zero to $\pi$. Show why rotating $q_1$ with $q_2=0$ leaves it zero.
 1. **Singular Directions.** At full extension, identify the infeasible first-order radial velocity and a finite joint change that produces inward displacement at second order.
 1. **Body PoE.** Compute both $B_i=\operatorname{Ad}_{M^{-1}}S_i$ and verify the body product at a nonzero configuration. Deliberately reverse the factors and quantify the error.
 1. **Position IK.** Solve the target $(2,0.5)$ m on both elbow branches. Compare the analytic solutions with a damped Cartesian iteration from several initial guesses; report failure reasons as well as successes.
 1. **Velocity.** At $(30,60)$ degrees with rates $(1,-0.5)$ rad/s, verify endpoint velocity $(-1.25,1.299038)$ m/s and angular velocity $0.5$ rad/s. Reconstruct the point velocity from $V_s$.
 1. **Mixed Chain.** Construct a prismatic $z$ joint followed by a revolute $y$ joint, stating the home location of the second axis and the tool offset. Explain why joint types alone do not determine $M$ or all screw coordinates.
 1. **Numerical Jacobian.** Compare analytic, central-difference and independent geometric derivatives across decreasing step sizes. Include a noncommuting spatial example so an ordering error cannot hide.
 1. **Orientation Task.** Derive the planar yaw Jacobian $(1,1)$, its null direction and the corresponding point motion. Explain why zero yaw change does not mean a stationary tool.
 1. **Redundancy.** For a planar 3R arm with a position-only task, construct an SVD null-space projector. Compare an Euler step with repeated integration/reprojection and measure task drift. Then add yaw to the task.
 1. **Workspace and Load.** Plot the 2R position workspace under stated joint bounds. At an extended configuration, explain the ideal radial force null direction and identify physical load limits omitted by the torque-ball model.
