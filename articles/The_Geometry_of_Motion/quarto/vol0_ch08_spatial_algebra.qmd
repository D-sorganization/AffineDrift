...
# Spatial Vector Algebra and Inertia {#sec-spatial-algebra}
## Featherstone's Spatial Algebra


In @sec-so3 and @sec-screw, we unified translation and rotation into 6-dimensional Twists ($\twist$) and Wrenches ($\wrench$). While the geometry of the Screw Axis inherently links them, operating on these 6-dimensional vectors using standard 3D linear algebra creates a bloated, confusing mess of cross products and offsets.
...
This notation, $\mathrm{ad}_{\twist}$ for motion and $\mathrm{ad}_{\twist}^*$ for force, is used consistently in @sec-rnea, @sec-spatial-algebra, and @sec-aba. It is distinct from the familiar 3-vector bracket $[\cdot]_\times$, which acts only on $\Reals^3$.
...
The spatial-algebra identities above give a fixed-cost ($O(1)$) matrix multiplication for each link's Coriolis contribution. However, assembling the full $n \times n$ Coriolis matrix $C(\bm{q}, \dot{\bm{q}})$ for a serial chain is still $O(n^2)$ because there are $n^2$ entries to compute. The RNEA (Chapter 7) deliberately avoids assembling $C$ at all, computing $C(\bm{q},\dot{\bm{q}})\dot{\bm{q}}$ in a single $O(n)$ sweep via the spatial Newton-Euler recursion.
:::
...
This chapter uses the **Featherstone / Lynch-Park convention** [@featherstone2008rigid, §2.13], [@lynch2017modern, §8.2]: the off-diagonal blocks are $m[\bm{c}]$ (top-right) and $m[\bm{c}]^T = -m[\bm{c}]$ (bottom-left). Some references (e.g., Murray-Li-Sastry) write these blocks with the opposite sign; the matrix is symmetric either way because $[\bm{c}]^T = -[\bm{c}]$.
:::
...
## Exercises {#sec-spatial-algebra-exercises}

**8.1 (Computational)** Using the `cylinder_spatial_inertia` function from the text, compute the spatial inertia of a 3 kg rod (radius 3 cm, length 40 cm) with its center of mass at 20 cm. Verify it is symmetric and positive-definite. See [@featherstone2008rigid, Ch. 2].

**8.2 (Conceptual)** Explain why the Spatial Inertia Matrix $\mathcal{I}_s$ is $6 \times 6$ and what each quadrant represents physically. What does positive-definiteness of $\mathcal{I}_s$ mean in physical terms?

**8.3 (Computational)** For a spatial velocity $\twist = [0, 0, 1; 1, 0, 0]^T$ and inertia from Exercise 8.1, compute the velocity-product wrench $\bm{p} = \operatorname{ad}_{\twist}^T (\mathcal{I}_s \twist)$. Verify units are consistent.

**8.4 (Conceptual)** The RNEA (see @sec-rnea) avoids assembling the full $n \times n$ Coriolis matrix $C(\bm{q}, \dot{\bm{q}})$. Explain why assembling $C$ is $O(n^2)$ even though each per-link contribution via spatial algebra is $O(1)$.
