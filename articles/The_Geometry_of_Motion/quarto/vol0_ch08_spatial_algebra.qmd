...
This chapter uses the **Featherstone / Lynch-Park convention** [@featherstone2008rigid, §2.13], [@lynch2017modern, §8.2]: the off-diagonal blocks are $m[m{c}]$ (top-right) and $m[m{c}]^T = -m[m{c}]$ (bottom-left). Some references (e.g., Murray-Li-Sastry) write these blocks with the opposite sign; the matrix is symmetric either way because $[m{c}]^T = -[m{c}]$.
:::
...
**8.3 (Computational)** For a spatial velocity $	wist = [0, 0, 1; 1, 0, 0]^T$ and inertia from Exercise 8.1, compute the velocity-product wrench $m{p} = \operatorname{ad}_{	wist}^T (\mathcal{I}_s 	wist)$. Verify units are consistent.

**8.4 (Conceptual)** The RNEA (see @sec-rnea) avoids assembling the full $n 	imes n$ Coriolis matrix $C(m{q}, \dot{m{q}})$. Explain why assembling $C$ is $O(n^2)$ even though each per-link contribution via spatial algebra is $O(1)$.
