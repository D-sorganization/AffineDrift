...
1. **Controllability:** Can we drive the state anywhere we want using the control input $\control$? Test: the **controllability matrix** $\mathcal{C} = [B, AB, A^2B, ..., A^{n-1}B]$ must have rank $n$.

2. **Observability:** Can we reconstruct the full state from output measurements? Test: the **observability matrix** $\mathcal{O} = [C^\top, (CA)^\top, (CA^2)^\top, ...]^\top$ must have rank $n$.
...
**1.10 (Challenge)** A system has $A = \begin{bmatrix}-1&00&-2\end{bmatrix}$, $B = \begin{bmatrix}10\end{bmatrix}$, $C = [0, 1]$. Is it controllable? Is it observable? What does this mean physically — which states can you drive, and which can you measure?
