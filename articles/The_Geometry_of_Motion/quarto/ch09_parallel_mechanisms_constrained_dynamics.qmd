...
A standard example is a wheel that rolls on a plane without lateral slip. For heading $\theta$ and axle position $(x,y)$, the lateral constraint is $-\sin\theta\,\dot{x}+\cos\theta\,\dot{y}=0$. This distribution is nonintegrable: admissible motions do not remain on a fixed lower-dimensional configuration submanifold. In contrast, rolling restricted to a straight track can give the integrable relation $x-r\varphi=\mathrm{constant}$, where $r$ is the wheel radius and $\varphi$ its spin angle. This chapter focuses on **holonomic constraints**; the nonholonomic case requires a velocity-distribution treatment rather than the level-set construction below.
