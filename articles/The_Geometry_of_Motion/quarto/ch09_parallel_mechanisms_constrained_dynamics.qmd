...
This chapter unifies configuration manifolds, Lagrangian mechanics on submanifolds, and optimal control under constraints. The geometry reveals why and how complex motions decompose, where redundancy lives, and how to design controls that respect the hidden structure [@briot2015parallel_robots; @roithmayr2016kanes_method].
...
\begin{align}
  \ell_1 \cos\theta_1 + \ell_2 \cos\theta_2 &= \ell_3 \cos\theta_3 + \Delta x, \\
  \ell_1 \sin\theta_1 + \ell_2 \sin\theta_2 &= \ell_3 \sin\theta_3 + \Delta y.
\end{align}
...
$$J_c = \begin{bmatrix}
  -\ell_1 \sin\theta_1 & -\ell_2 \sin\theta_2 & \ell_3 \sin\theta_3 \\
  \ell_1 \cos\theta_1 & \ell_2 \cos\theta_2 & -\ell_3 \cos\theta_3
\end{bmatrix},$$
...
The Christoffel symbols express the Coriolis-centrifugal acceleration:
...
The dynamics of the shape variables reduce to a system of lower dimension. This matters for the golf swing: the total angular momentum is conserved (a cyclic coordinate). The swing dynamics then reduce to the shape changes of the arms, with the rotation of the torso following deterministically.
...
where $\alpha, \beta > 0$ are tuning parameters. The correction terms act as a feedback law: when the constraint is violated, the correction grows and drives the state back onto the manifold. This reduces the effective index to 1 and suppresses exponential growth of constraint violations due to numerical integration error.
...
The drift field $f(q, \dot{q})$ encapsulates both the Christoffel symbols (geodesic curvature) and the gradient of the potential. This connection reveals that the Coriolis and centrifugal terms, though they appear as "fictitious forces" in a coordinate-dependent view, are manifestations of the curvature of the configuration manifold.
...
This is a linear feedback law in the costate. For a golf swing with bounded actuation (e.g., $u_i \in [0, 1]$ for muscle activation), the control clips at the boundaries, giving a mixed bang-singular control.
