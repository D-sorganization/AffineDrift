...
def convert_tex_to_qmd(input_file, output_file):
    ...
    for line in lines:
        if line.startswith(r'\chapter{'):
            ...
            out_lines.append(f'\n### {title}\n')
        elif line.startswith(r'\begin{principle}') or line.startswith(r'\begin{definition}') or line.startswith(r'\begin{keyidea}'):
            out_lines.append(f'\n::: {{.callout-note}}\n')
        elif line.startswith(r'\end{principle}') or line.startswith(r'\end{definition}') or line.startswith(r'\end{keyidea}'):
            out_lines.append(f'\n:::\n')
        else:
...
convert_tex_to_qmd(os.path.join(base_dir, 'Volume_II', 'main.tex'), os.path.join(base_dir, 'quarto', 'volume2_content.qmd'))
