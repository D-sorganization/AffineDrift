%% ==========================================================================
%%  Chapter 4: Contraction Theory and Metric Certificates
%% ==========================================================================
...
Contraction theory answers one of the most practical questions in engineering:
**if I make a small mistake, will the system recover?**

Think about pouring water into a funnel. No matter where the water enters the
funnel's rim, it all converges to the same narrow exit. The funnel is "contracting"---it
squeezes all paths toward a single trajectory. A system with this property is forgiving:
small errors in timing, position, or force all wash out over time.

The opposite is a system that *diverges*---like balancing a marble on top of a hill.
The slightest nudge sends it rolling away, and the error grows. Such systems are
unforgiving and require constant, precise correction.

This chapter develops the mathematics to distinguish funnels from hilltops, and
everything in between. The key tool is a **metric**---a local ruler that measures
distances between trajectories. If that ruler says distances are shrinking, the
system is contracting. The beauty of the approach is that finding the right ruler
*proves* the system is stable, without having to simulate every possible starting
condition.
:::
...
Classical Lyapunov stability, as developed in the late 1800s and formalized throughout
the twentieth century, addresses a foundational question: *does the system return
...
systems: the behavior of interest is often a *moving trajectory*, not a rest point.

A robot executing a reaching task, a vehicle tracking a path, a biological motor system
performing a skilled movement---these systems are intrinsically *trajectory-focused*.
...
Contraction theory reframes the question: **do all nearby trajectories converge
...
Slotine** in 1998~[@LohmillerSlotine1998]. Their key insight was to study how *tangent
vectors* between trajectories evolve under the flow. If all such vectors shrink in a
...
\begin{intuitionbox}
Think of contraction this way: if you release a handful of rubber ducks into a river,
they initially spread out. But as the river narrows through a gorge, they are squeezed
together. By the time they exit the gorge, they are traveling in a tight group regardless
of where you released them. The gorge is a contracting region. Contraction theory
formalizes this “gorge property” for any dynamical system, using a metric that plays
the role of the river's width.
\end{intuitionbox}


### Demidovich's Condition (1961): An Historical Precursor


The mathematical seeds of contraction theory are rooted in work by **Demidovich**
in 1961~[@Demidovich1961], who studied conditions for *global asymptotic stability*
based on the symmetric part of the Jacobian. Demidovich showed that if the symmetric part
of the system's Jacobian is uniformly negative definite, all nearby solutions converge
...
contraction in the *Euclidean metric* (identity matrix). The modern framework
...
- **Incremental stability and synchronization**: studying how coupled oscillators,
  networked systems, and consensus algorithms achieve agreement.
- **Control design**: synthesis of feedback laws that guarantee contraction, leading
  to robust and verifiable controllers.
- **Learning-based control**: verification of neural network controllers via metric
  composition and convex optimization.
- **Robotics and autonomous systems**: design of feedback laws that are tolerant to
...
a perturbation? Contraction theory asks a more general question: do *all* nearby
...
*moving trajectory*---a robot executing a task, a vehicle following a path, a
...
The variational dynamics (@ch-variational) are
...
Let $\mat{M}(\x,t) \succ 0$ be a smooth, positive-definite **metric tensor** and
...
The system~@eq-ch4-system is **contracting** on a forward-invariant region
...
condition is called a **matrix inequality** because it compares two matrices in the
...
If~@eq-ch4-contraction-LMI holds uniformly on $\mathcal{D}$, then for any two
...
\begin{proof}
**Step 1: Geodesic Parametrization and Tangent Vector Evolution.**
...
Substituting $\dot{\dx} = \mat{A}\dx$ from @eq-ch4-tangent-evolution:
\begin{align}
...
By the contraction condition @eq-ch4-contraction-LMI:
...
Taking square roots yields the geodesic decay @eq-ch4-geodesic-decay.

**Step 5: Metric Sandwich Bound.**

To translate the geodesic bound into a Euclidean norm bound, we use the metric sandwich
assumption @eq-ch4-metric-bounds. For any vector $\dx$:
...
Taking square roots yields @eq-ch4-euclidean-decay. \qed
\end{proof}
...
## Worked Example: A Simple Linear System {#sec-ch4-example}
...
For this to satisfy the contraction condition with rate $\lambda$ we need:

$$
\begin{bmatrix} -4 & 1  1 & -6 \end{bmatrix} + 2\lambda \mat{I}
= \begin{bmatrix} -4+2\lambda & 1  1 & -6+2\lambda \end{bmatrix} \preceq 0.
$$


The matrix inequality $\mat{A}^\T\mat{M} + \mat{M}\mat{A} \preceq -2\lambda\mat{M}$ is a
*matrix* (positive-semidefinite) inequality, **not** an elementwise comparison.
The off-diagonal entry $1$ does not need to satisfy $1 \le 0$ individually; what matters is
whether the matrix above is negative-semidefinite as a whole.
Its eigenvalues are $-5 \pm \tfrac{\sqrt{2}}{2} + 2\lambda$, so for $\lambda < 1.79$
they are both negative.
Hence $\mat{M} = \mat{I}$ *does* certify contraction at any rate $\lambda < 1.79$;
the erroneous infeasibility claim in some presentations of this example should be ignored.

The symmetric part $\sym(\mat{A}) = \tfrac{1}{2}(\mat{A}^\T + \mat{A})$ has
eigenvalues approximately $-1.79$ and $-3.21$, so the tightest rate certified by
$\mat{M} = \mat{I}$ is $\lambda = 1.79$ rad/s directly---**no additional
factor of $1/2$** is needed, since $\sym(\mat{A}) = \tfrac{1}{2}(\mat{A}+\mat{A}^\T)$
already incorporates the scaling.

\begin{designbox}
In practice, for linear systems, one solves the LMI @eq-ch4-example-LMI using
...
## Partial Contraction: Hierarchical Structure {#sec-ch4-partial}
...
scenario is called **partial contraction**.
...
1. The reduced system for $\x_2$ is globally contracting with rate $\lambda_2 > 0$
  in a metric $\mat{M}_2(\x_2,t)$.
2. For any fixed trajectory $\x_2(t)$, the $\x_1$-subsystem driven by $\x_2(t)$ is
...
If $\x_2$ is contracting, then by Theorem~@thm-ch4-convergence, perturbations
...
## Control Contraction Metrics: Design and Duality {#sec-ch4-ccm}
...
Pre- and post-multiply @eq-ch4-CCM-nonlinear by $\mat{W}$ and use $\mat{M} = \mat{W}^{-1}$:
...
This is a matrix inequality in $\mat{W}$ and $\mat{K}$ that is *bilinear* in the control
...
control effort). This condition is *linear* in $\mat{W}$ and can be enforced at a collection
...
@eq-ch4-discrete-contraction-condition. Iterating yields $V_k \leq \rho^{2k} V_0$.
\end{proof}
...
in detail in @ch-duality. Both frameworks certify stability and convergence,
...
## Numerical Example: Van der Pol Oscillator {#sec-ch4-vdp}
...
where $\alpha, \beta, \gamma > 0$ are constants to be determined. This metric *amplifies*
...
### Why the Pure-Diagonal Ansatz Fails (and What Does Work)

Before quoting a numerical value for $\gamma$, a subtle but important point is
worth making explicit. The $(1,1)$ block of $\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}}$
with the ansatz $\mat{M} = \diag(\alpha,\; \beta(1+\gamma x_1^2))$ and constant $\alpha$ is
identically zero, because $\mat{A}_{11} = 0$, $\dot{m}_1 = 0$, and the $(1,1)$ cross-terms
vanish. The strict LMI $\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}} \preceq -2\lambda\mat{M}$
then requires $0 \leq -2\lambda\alpha$, which is impossible for any $\lambda>0$ and $\alpha>0$.

So no *constant-diagonal* metric of the form above certifies strict contraction at
a positive rate---regardless of $\gamma$. The intuitive picture in
the box above is correct (amplifying the velocity direction where
$x_1$ is small compensates for the expanding nonlinear term), but it needs a
richer parameterization to turn into a certificate: either an
off-diagonal metric or a position-dependent $m_1(x_1)$. The value
$\gamma \approx 1.5$ quoted in the classical Lohmiller--Slotine Van der Pol
treatment~[@LohmillerSlotine1998] comes from such an extended ansatz; the
diagonal--constant-$m_1$ form in the "solution strategy" above is a teaching
simplification that reveals structure but does not itself prove contraction.

::: {.callout-warning}
**Pedagogical note.** The earlier edition of this section quoted
$\gamma \approx 1.5$ as though it followed from the diagonal ansatz
in~@eq-ch4-vdp-metric-ansatz. It does not. The Lohmiller--Slotine
result uses a metric with additional structure (either an off-diagonal term
or a non-constant $m_1$) that the diagonal ansatz cannot represent. Readers
who want to reproduce $\gamma \approx 1.5$ should consult~[@LohmillerSlotine1998]
or the extended-ansatz treatment cited there. The code cell below verifies
the negative result for the simple ansatz and is included so the claim is
checkable rather than asserted.
:::

### Reproducing the Infeasibility Check Numerically

The cell below discretizes the state space on a $41 \times 41$ grid covering the
Van der Pol limit cycle ($|x_1| \leq 2$, $|x_2| \leq 3$), forms the contraction LMI
at each grid point using the simple ansatz $\mat{M} = \diag(\alpha,\;\beta(1+\gamma x_1^2))$,
and bisects over $\gamma$ for the smallest $\gamma$ that makes every sampled LMI feasible
with $\lambda = 0.3$, $\epsilon = 0.5$, $\alpha = 1$, $\beta = 1.2$:

“`{python}
#| label: vdp-contraction-lmi
#| echo: true
#| eval: false

import numpy as np

def vdp_lmi_violation(x1, x2, eps, lam, alpha, beta, gamma):
    """Max eigenvalue of A^T M + M A + M_dot + 2 lambda M at (x1, x2).
    Non-positive => LMI is satisfied at this point."""
    A = np.array([
        [0.0,                         1.0],
        [-1.0 - 2.0 * eps * x1 * x2,  eps * (1.0 - x1**2)],
    ])
    m2 = beta * (1.0 + gamma * x1**2)
    M = np.diag([alpha, m2])
    Mdot = np.diag([0.0, 2.0 * beta * gamma * x1 * x2])
    LMI = A.T @ M + M @ A + Mdot + 2.0 * lam * M
    return np.max(np.linalg.eigvalsh((LMI + LMI.T) / 2.0))

def smallest_feasible_gamma(eps=0.5, lam=0.3, alpha=1.0, beta=1.2,
                            n_grid=41, x1_max=2.0, x2_max=3.0):
    xs1 = np.linspace(-x1_max, x1_max, n_grid)
    xs2 = np.linspace(-x2_max, x2_max, n_grid)
    X1, X2 = np.meshgrid(xs1, xs2, indexing="ij")
    lo, hi = 0.0, 5.0
    for _ in range(60):
        mid = 0.5 * (lo + hi)
        worst = max(
            vdp_lmi_violation(X1[i, j], X2[i, j],
                              eps, lam, alpha, beta, mid)
...
gamma_star = smallest_feasible_gamma()
print(f"Smallest feasible gamma for the diagonal ansatz "
      f"at lambda=0.3, eps=0.5: {gamma_star:.3f}")
# The bisection saturates at the upper bound 5.000, confirming that
# no finite gamma in [0, 5] makes the diagonal-constant-m1 LMI feasible.
“`

Running the cell returns $\gamma^{\star} = 5.000$ (the upper bound of the bisection),
confirming that no finite $\gamma$ closes the $(1,1)$ gap---the LMI is *uniformly*
infeasible for the simple ansatz. The actual Lohmiller--Slotine metric uses either a
non-constant $m_1$ or an off-diagonal coupling; see~[@LohmillerSlotine1998]
for the worked-out case.
...
metric $\mat{M}$, then the perturbed system exhibits **bounded-input bounded-state**
(BIBS) behavior and **incremental input-to-state stability** (ISS).

\begin{theorem}[Disturbance Rejection Bound]

Suppose $\dot{\x} = f(\x,t)$ is contracting with rate $\lambda$ in metric $\mat{M}$
satisfying the metric bounds @eq-ch4-metric-bounds. Consider the perturbed system
@eq-ch4-perturbed-system with $\norm{d(t)} \leq D$.
...
The ultimate bound @eq-ch4-ultimate-bound-steady shows that:


- **Higher contraction rate $\lambda$ ** $\implies$ **smaller steady-state error**.
  A fast-contracting system naturally rejects disturbances more aggressively.
- **Metric condition number ** $\kappa = \overline{m}/\underline{m}$ enters
  the bound. A well-conditioned metric (close to isotropic) improves robustness.
- **Design trade-off**: increasing $\lambda$ may require larger control effort
...
**Input-to-state stability** (ISS) is a classical robustness property: bounded inputs
lead to bounded states. However, ISS alone does not constrain how different trajectories
relate to each other.

**Incremental ISS** (or differential ISS) is a stronger property: bounded input differences
lead to bounded state differences. Contraction *implies* incremental ISS, but not vice versa.

\begin{proposition}[Contraction Implies Incremental ISS]

If the system $\dot{\x} = f(\x) + G(\x)\uvec$ is contracting with rate $\lambda$ in
metric $\mat{M}$ (with metric bounds @eq-ch4-metric-bounds), then for any two
...
@tbl-ch4-lyapunov-contraction provides a comprehensive comparison of Lyapunov
stability, Lyapunov asymptotic stability, incremental stability, and contraction.



: Comparison of stability concepts: Lyapunov stability certifies return to an
equilibrium, while contraction certifies convergence of all nearby trajectories to
each other, independent of a particular equilibrium. {#tbl-ch4-lyapunov-contraction}
...
1. **Muscle impedance (passive contraction):** Skeletal muscles exhibit intrinsic
...
2. **Stretch reflex (active contraction):** The spinal stretch reflex provides
...
3. **Co-contraction (metric shaping):** When a person stiffens a joint by
...
contraction (Theorem~@thm-ch4-partial) applies. The kinematic chain
$\text{torso} \to \text{shoulder} \to \text{elbow} \to \text{wrist} \to \text{club}$
exhibits a natural partitioning:


- The proximal segments (torso, shoulder) are heavily muscled and contract rapidly.
  Their contraction rate $\lambda_{\text{proximal}}$ is high.
- The distal segments (wrist, club) have less muscular authority. Their contraction
  rate $\lambda_{\text{distal}}$ is lower, and the club shaft (being passive) does
  not contract at all without feedback.
- Partial contraction theory guarantees that if the proximal subsystem contracts,
...
(see forward reference to @ch-counterfactuals below on counterfactuals
and the ZTCF/ZVCF decomposition). The muscular torques become negligible compared
to centrifugal and Coriolis forces. In this regime, the contraction properties of the
*drift field* determine whether the motion is stable:


1. If the drift Jacobian has eigenvalues with negative real parts (natural damping),
  the uncontrolled trajectory is contracting. This is the “natural stability”
  of the late downswing.
2. If the drift Jacobian has eigenvalues with positive real parts (instability),
...
3. The contraction rate of the drift determines the “forgiveness” of the swing:
...
(with $\mat{A} = \partial f/\partial \x$, matching the canonical form
of~@eq-ch4-contraction-LMI) must be augmented to account for the noise.
...
Consider two solutions $\x_1(t), \x_2(t)$ of~@eq-ch4-sde with different initial
...
The bound~@eq-ch4-stochastic-bound shows that contraction dominates at large
...
(@ch-duality). If an observer is contracting with rate $\lambda$, and the
...
*tube* around the nominal trajectory: the set of all states that are guaranteed
...
- A contracting controller with rate $\lambda \geq -\dot\varphi / \varphi$
  guarantees that trajectories remain within the funnel.
- The adaptive gain $k(t) = 1/(\varphi(t) - \|e(t)\|)$ from classical funnel
...
1. **Historical foundations:** Contraction theory evolved from Demidovich's
...
2. **The contraction condition** (Def.~@def-ch4-contraction) is a matrix
  inequality on the Jacobian and metric:
  $\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}} \preceq -2\lambda\mat{M}$.
3. **Exponential convergence** (Thm.~@thm-ch4-convergence): If the condition
...
4. **Metric freedom**: The choice of $\mat{M}$ is flexible. Many nonlinear systems
...
5. **Worked example** (Sec.~@sec-ch4-example): A linear system with Jacobian
  $\mat{A} = [[-2,1],[0,-3]]$ is contracting in the identity metric at rate $\lambda \approx 1.79$.
6. **Partial contraction** (Sec.~@sec-ch4-partial): Not all directions must
  contract uniformly. Hierarchical decomposition (Thm.~@thm-ch4-partial)
  enables contraction in one subsystem to drive convergence in another.
7. **Control contraction metrics (CCM)** (Sec.~@sec-ch4-ccm): For controlled
...
8. **Discrete-time contraction** (Sec.~@sec-ch4-discrete): The condition
...
9. **Numerical metric synthesis**: The Van der Pol oscillator (Sec.~@sec-ch4-vdp)
  demonstrates how parameterized metrics can be found computationally to achieve
  contraction despite nonlinearity.
10. **Robustness margins**: The contraction rate $\lambda$ directly bounds
  disturbance rejection. For $\norm{d(t)} \leq D$, the ultimate error bound
  is $\sqrt{\overline{m}/({\lambda\underline{m}})} \cdot D$ (Thm.~@thm-ch4-disturbance-rejection).
11. **ISS relationship**: Contraction implies incremental ISS, which is strictly
  stronger than standard ISS. Contraction certifies trajectory synchronization, not
  just input-output boundedness.
12. **Lyapunov connection**: Contraction implies Lyapunov stability, but the
  converse does not hold. Contraction is a stronger, trajectory-focused property
  (Prop.~@prop-ch4-contraction-lyapunov).


In the next chapter, we turn to **input-output analysis** and transfer functions,
...
1. **Contraction of a linear system.**
...
6. **Stochastic contraction bound.**
  ...
  $\sigma^2 / (2\lambda)$, consistent with the bound in Theorem~@thm-ch4-stochastic-contraction.
7. **Tube computation.**
