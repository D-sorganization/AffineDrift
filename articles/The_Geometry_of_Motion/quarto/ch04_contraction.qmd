---
title: "Contraction Theory and Metric Certificates"
---
...
system is contracting. The beauty of the approach: finding the right ruler
*proves* the system is stable, without simulating every possible starting
condition.
:::
...
**Lohmiller and Slotine** formalized the modern contraction theory for nonlinear systems
in 1998 [@LohmillerSlotine1998]. Their key insight: study how *tangent
...
condition is a special case of modern contraction theory: it corresponds to
...
- **Control design**: synthesizing feedback laws that guarantee contraction, yielding
  robust and verifiable controllers.
- **Learning-based control**: verifying neural network controllers via metric
  composition and convex optimization.
- **Robotics and autonomous systems**: designing feedback laws that tolerate
...
(iii) the metric bounds translating between geodesic and Euclidean distance.
...
be $\lambda = |\lambda_{\max}(\sym(\mat{A}))| = 1.79$ rad/s directly---**no additional
...
::: {.theorem}
**Partial Contraction via Hierarchical Decomposition**
...
::: {.example}
**Mechanical System: Position Synchronization via Velocity Contraction**
...
After expanding and simplifying (using $\mat{M} = \mat{W}^{-1}$ and
...
::: {.theorem}
**Discrete-Time Exponential Convergence**
...
For linear discrete-time systems, the discrete contraction metric relates to
the solution of a discrete-time algebraic Riccati equation (DARE), which
Chapter~\ref{ch:riccati} treats in detail. Both frameworks certify stability and convergence,
...
\end{equation}
where $m_1, m_2 > 0$ are smooth functions. The special structure of the Van der Pol system
motivates the diagonal ansatz (position and velocity are somewhat decoupled).
...
The solution shows how contraction metrics can *adapt* to the nonlinear structure
...
::: {.proposition}
**Contraction Implies Incremental ISS**
...
Table~\ref{tbl:ch4:lyapunov-contraction} compares Lyapunov
...
: Comparison of stability concepts: Lyapunov stability certifies return to an equilibrium, while contraction certifies convergence of all nearby trajectories to each other, independent of a particular equilibrium. {#tbl-tbl_ch4_lyapunov-contraction}
...
::: {.proposition}
**Contraction Implies Lyapunov Stability**
...
The theory of contraction metrics applies naturally and powerfully to the
...
2. **Stretch reflex (active contraction):** The spinal stretch reflex provides
   fast ($\sim40$ ms) feedback that resists perturbations. This is a feedback controller
...
$\mat{M}(\x(t))$ that is optimally adapted to the swing dynamics. Moving from
novice to expert means, in part, learning the optimal contraction metric.
:::
...
::: {.theorem}
**Stochastic Contraction --- adapted from Pham, Tabuada, and Slotine**
...
2. **The contraction condition** (Def.~\ref{def:ch4:contraction}) is a matrix
   inequality on the Jacobian and metric:
   $\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}} \preceq -2\lambda\mat{M}$.
3. **Exponential convergence** (Thm.~\ref{thm:ch4:convergence}): If the condition
...
5. **Worked example** (Sec.~\ref{sec:ch4:example}): A linear system with Jacobian
   $\mat{A} = [[-2,1],[0,-3]]$ is contracting in the identity metric at rate $\lambda \approx 1.79$.
6. **Partial contraction** (Sec.~\ref{sec:ch4:partial}): Not all directions must
...
7. **Control contraction metrics (CCM)** (Sec.~\ref{sec:ch4:ccm}): For controlled
...
8. **Discrete-time contraction** (Sec.~\ref{sec:ch4:discrete}): The condition
...
9. **Numerical metric synthesis**: The Van der Pol oscillator (Sec.~\ref{sec:ch4:vdp})
