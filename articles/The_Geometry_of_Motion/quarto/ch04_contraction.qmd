...
in 1961~\cite{Demidovich1961}, who studied conditions for *global asymptotic stability*
...
\begin{itemize}
\item **Incremental stability and synchronization**: studying how coupled oscillators,
      networked systems, and consensus algorithms achieve agreement.
\item **Control design**: synthesis of feedback laws that guarantee contraction, leading
      to robust and verifiable controllers.
\item **Learning-based control**: verification of neural network controllers via metric
      composition and convex optimization.
\item **Robotics and autonomous systems**: design of feedback laws that are tolerant to
...
The system~\eqref{eq:ch4:system} is **contracting** on a forward-invariant region
...
\emph{matrix} (positive-semidefinite) inequality, \textbf{not} an elementwise comparison.
...
Hence $\mat{M} = \mat{I}$ \emph{does} certify contraction at any rate $\lambda < 1.79$;
...
$\mat{M} = \mat{I}$ is $\lambda = 1.79$ rad/s directly---\textbf{no additional
...
in detail in Chapter~\ref{ch:duality}. Both frameworks certify stability and convergence,
...
treatment~\cite{LohmillerSlotine1998} comes from such an extended ansatz; the
...
in~\eqref{eq:ch4:vdp-metric-ansatz}. It does not. The Lohmiller--Slotine
...
who want to reproduce $\gamma \approx 1.5$ should consult~\cite{LohmillerSlotine1998}
...
non-constant $m_1$ or an off-diagonal coupling; see~\cite{LohmillerSlotine1998}
...
\begin{itemize}
\item **Higher contraction rate $\lambda$ ** $\implies$ **smaller steady-state error**.
      A fast-contracting system naturally rejects disturbances more aggressively.
\item **Metric condition number ** $\kappa = \overline{m}/\underline{m}$ enters
      the bound. A well-conditioned metric (close to isotropic) improves robustness.
\item **Design trade-off**: increasing $\lambda$ may require larger control effort
...
\begin{enumerate}
\item **Muscle impedance (passive contraction):** Skeletal muscles exhibit intrinsic
...
\item **Stretch reflex (active contraction):** The spinal stretch reflex provides
...
\item **Co-contraction (metric shaping):** When a person stiffens a joint by
...
(see forward reference to Chapter~\ref{ch:counterfactuals} below on counterfactuals
...
of~\eqref{eq:ch4:contraction-LMI}) must be augmented to account for the noise.
...
\begin{enumerate}
\item **Historical foundations:** Contraction theory evolved from Demidovich's
...
\item **The contraction condition** (Def.~\ref{def:ch4:contraction}) is a matrix
      inequality on the Jacobian and metric:
      $\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}} \preceq -2\lambda\mat{M}$.

\item **Exponential convergence** (Thm.~\ref{thm:ch4:convergence}): If the condition
...
\item **Metric freedom**: The choice of $\mat{M}$ is flexible. Many nonlinear systems
...
\item **Worked example** (Sec.~\ref{sec:ch4:example}): A linear system with Jacobian
      $\mat{A} = [[-2,1],[0,-3]]$ is contracting in the identity metric at rate $\lambda \approx 1.79$.

\item **Partial contraction** (Sec.~\ref{sec:ch4:partial}): Not all directions must
...
\item **Control contraction metrics (CCM)** (Sec.~\ref{sec:ch4:ccm}): For controlled
...
\item **Discrete-time contraction** (Sec.~\ref{sec:ch4:discrete}): The condition
...
\item **Numerical metric synthesis**: The Van der Pol oscillator (Sec.~\ref{sec:ch4:vdp})
      demonstrates how parameterized metrics can be found computationally to achieve
      contraction despite nonlinearity.

\item **Robustness margins**: The contraction rate $\lambda$ directly bounds
...
\item **ISS relationship**: Contraction implies incremental ISS, which is strictly
      stronger than standard ISS. Contraction certifies trajectory synchronization, not
      just input-output boundedness.

\item **Lyapunov connection**: Contraction implies Lyapunov stability, but the
...
\begin{enumerate}
\item **Contraction of a linear system.**
...
\item **Discrete-time contraction.**
...
\item **Stochastic contraction bound.**
