...
Do we need to know acceleration ($\ddot{x}$) as part of the state? According to Newton's Second Law: $F = m\ddot{x}$. Acceleration is not an independent property of the train; the external forces applied at that instant drive it. Therefore, if we know the forces (the Control Inputs, $\control$), we can compute the acceleration. The state itself is fully defined by position and velocity.
...
When a dynamic system (like an airplane or a robotic arm) moves through physical space over time, its entire sprawling motion collapses to a single, unified **1-dimensional curve** snaking through $n$-dimensional state space. This curve is called the system's **Trajectory** $\traj$.


### Standard Form Differential Equations


State space converts complex, higher-order physical differential equations into simple, first-order vector math. That conversion is the source of its power.
...
Whether you are controlling a single gear or a 15-DOF musculoskeletal golf swing (as discussed in Volume II), you are trying to shape the trajectory $\state(t)$ by manipulating the vector field $\bm{f}$ with your control inputs $\control$.


## Summary


State space decouples physics from geometry. We no longer treat a system as physical objects bumping into each other in 3D space. We treat it as a single point—a cursor—moving through hyper-dimensional coordinates based on a vector field. 
