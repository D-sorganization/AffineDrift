---
pagetitle: "The Articulated Body Algorithm"
description: "Complete tree-dynamics derivation, independently checked three-link mechanics, contact and impact distinctions, and rigorous golf-swing interpretation."
css:
...
On narrow screens, scroll the diagram horizontally or [open the full-size diagram](../figures/articulated_body_passes.svg).

::: {#fig-aba_passes}
::: {.aba-diagram-scroll role="region" aria-label="Three articulated-body passes; scroll horizontally" tabindex="0"}
<img src="../figures/articulated_body_passes.svg" alt="Three ordered passes: outward motion and bias calculation, inward joint elimination including both inertia and bias, and outward recovery of joint and body accelerations. All passes use consistent parent-to-child motion transforms.">
:::

Three Passes Through a Rigid-Body Tree: Compute Known Motion and Bias, Eliminate Joint Accelerations Inward, Then Recover Accelerations Outward.
:::
...
[]{#forward-dynamics}
[]{#sec-forward_dynamics_problem}

### Problem Formulation
[]{#subsec-fd_formulation}
...
### What the Complexity Comparison Means
[]{#subsec-why_not_invert}
...
## Articulated Inertia: The Key Insight

[]{#featherstones-on-solution-articulated-inertia}
[]{#sec-articulated_inertia}

### A Free Hinge Changes the Apparent Load
[]{#subsec-inertia_intuition}
...
### Mathematical Definition and Frames
[]{#subsec-articulated_inertia_math}
...
### Eliminating One Joint Acceleration
[]{#subsec-constraint_derivation}
...
## The Articulated Body Algorithm: Three Passes

[]{#the-three-passes-of-aba}
[]{#sec-aba_three_passes}
...
### Pass 1: Outward Velocity and Bias Calculation

[]{#outward-pass-velocity-and-bias-forces}
[]{#subsec-pass1_outward}

#### Spatial Velocity Propagation
[]{#subsubsec-velocity_propagation}
...
### Pass 2: Inward Subtree Elimination

[]{#inward-pass-articulated-inertias}
[]{#subsec-pass2_inward}

#### Leaf Boundary Condition
[]{#subsubsec-leaf_inertia}
...
### Pass 3: Outward Acceleration Recovery

[]{#outward-pass-joint-accelerations}
[]{#subsec-pass3_outward}

#### Fixed-Base Acceleration and Gravity
[]{#subsubsec-base_acceleration}
...
[]{#subsubsec-joint_acceleration}

For each body in outward order, compute
[]{#eq-aba_joint_acceleration}
...
## Why the Tree Calculation Is Linear-Time
[]{#sec-aba_complexity}
...
## Floating Base: A Six-Degree-of-Freedom Root
[]{#sec-floating_base}


### The Root Articulated Solve
[]{#subsec-floating_base_aba}
...
### Contact Does Not Automatically Fix the Base
[]{#subsec-contact_constraints}
...
## Floating-Base Humanoids and Momentum
[]{#sec-floating_base_humanoid}


### Configuration Coordinates and Generalized Velocity
[]{#subsec-virtual_joint}
...
### Centroidal Momentum and Whole-Body Dynamics
[]{#subsec-centroidal_momentum}


Keep the angular-first ordering and define momentum about the total center of mass $c$:
[]{#eq-centroidal_momentum}
...
[]{#sec-friction_motor_inertia}

### Viscous Joint Friction
[]{#subsec-joint_friction}
...
### An Ideal Reflected Armature Model
[]{#subsec-motor_inertia}
...
## ABA and Composite Rigid-Body Methods
[]{#sec-aba_vs_crba}
...
## Choosing Forward or Inverse Dynamics
[]{#sec-fd_id_decision}

### When the Acceleration Is Prescribed
[]{#subsec-use_id}
...
### When the Applied Loads Are Specified
[]{#subsec-use_fd}
...
## Constraint Handling: Contact and Joint Limits
[]{#sec-constraints}
...
[]{#sec-gaussian_elimination_connection}
...
## Worked Example: A Fully Specified Three-Link Arm
[]{#sec-worked_example_aba}
...
### Pass 2: Articulated Inertias and Residual Efforts
[]{#subsec-pass2_example}
...
### Pass 3 and an Independent Check
[]{#subsec-pass3_example}
...
In NumPy, a one-dimensional vector's transpose is still one-dimensional. To form $U_iU_i^T$, call `np.outer` with the vector as both arguments. For a one-dimensional array, `U @ U.T` instead returns a scalar, which can silently broadcast into an incorrect matrix update.

### Connection to Simulation Frameworks
[]{#subsec-simulation_frameworks}

Different simulators use different combinations of recursive dynamics and constraint solvers. MuJoCo's documented computation path forms its mass matrix with CRB and uses sparse $L^TDL$ factorization and back-substitution [@MuJoCoComputationReview]. It must not be described as using ABA for every timestep. Pinocchio provides ABA and analytical dynamics derivatives, along with other algorithms [@PinocchioSoftwareReview]. These statements identify documented methods; they do not rank accuracy or runtime. A reproducible comparison must identify software versions, model parameters, contact and integration settings, hardware and the actual quantity timed.

## Contact Forces and Impact Dynamics
[]{#sec-contact_impact}
...
## Contact Dynamics and Complementarity
[]{#sec-contact_lcp}


### Non-Penetration and Sustained Contact
[]{#subsec-complementarity}
...
### Friction and Contact Wrenches
[]{#subsec-friction_cone}
...
### The Frictionless Acceleration LCP
[]{#subsec-lcp_formulation}



For a chosen set of closed, zero-normal-velocity contacts, write
[]{#eq-constrained_dynamics}
...
### Time-Stepping and Numerical Order
[]{#subsec-time_stepping}
...
## From Coupled Dynamics to a Golf-Swing Argument
...
[]{#sec-ch10_further_reading}

Featherstone's original formulation and later treatment provide the recursive dynamics foundation [@Featherstone1983; @featherstone1987]. His Spatial_v2 documentation provides inspectable reference algorithms and coordinate-transform conventions [@FeatherstoneSpatialReview]. MuJoCo's computation documentation and Pinocchio's software description document distinct implementation choices [@MuJoCoComputationReview; @PinocchioSoftwareReview]. The calculations here are derived explicitly and checked independently; consulting these sources does not substitute for stating the model and experiment used in a golf argument.
...
1. **Articulated and Composite Inertia:** Use three cylinders with $m_i=1$ kg, $L_i=1$ m, radius $0.05$ m and $q=(0.2,-0.4,0.3)$ rad. Calculate every articulated inertia with the distal joints free, and every composite inertia with them locked. Explain the difference without treating a singular point-mass inertia as a generic rigid body.
1. **Motor Inertia:** Add $0.04$ kg m$^2$ of ideal armature to joint 1 of the worked example. Recalculate all accelerations. Derive the sensitivity using the rank-one change in $M$, and explain why distal accelerations can change even though their own armatures are unchanged.
1. **Floating Base:** Describe the extra root quantities and solve for a thirty-joint floating-base mechanism. Separate its additional bounded-size root work from the effects of adding contact constraints. Do not infer an exact runtime ratio from the degree-of-freedom count.
1. **Repeated Right-Hand Sides:** At fixed $q$ and $\dot q$, compare reuse of a dense mass factorization and an articulated factorization for one hundred torque vectors. State preparation, per-solve and storage costs separately. Explain which quantities must be refreshed if $\dot q$ or $q$ changes.
1. **Bias Verification:** Restrict the worked model to its first two links at $q=(0.4,-0.7)$ and $\dot q=(0.8,-0.5)$. Calculate velocity forces from the spatial recursion and from Christoffel terms of the Cartesian kinetic-energy mass matrix. Compare gravity separately to avoid confusing force conventions.
1. **Singular Pivots:** Prove $d_i>0$ for a nonzero $S_i$ and positive-definite $I_i^A$ without armature. Explain how massless idealizations, degenerate coordinates or nonphysical parameters can defeat this condition. Why is replacing a zero pivot by zero acceleration not a valid solution?
1. **Joint Damping:** Add coefficients $(0.2,0.1,0.03)$ N m s/rad to the worked example. Verify $\tau^{\mathrm{eff}}=\tau-B\dot q$ before the solve and that the damping power is nonpositive. Compare against the incorrect post-solve acceleration subtraction.
1. **Gravity Simulation:** Construct four identical cylinders of length $0.3$ m, mass $1$ kg and radius $0.025$ m, initially at $q=(0.4,-0.2,0.3,-0.1)$ with zero rates. Simulate gravity-only motion with a fixed proximal base. State the integrator, refine the timestep, and monitor total mechanical energy; a fixed-base pendulum chain is not whole-body ballistic free fall.
...
1. **Integration Order:** Compare a first-order velocity-first method with a suitable second-order method on a smooth problem. Verify observed convergence against a refined reference. Explain when the words symplectic and Verlet apply, and why an impact can invalidate a smooth-step order argument.
1. **Matrix Formulation:** Build a specified ten-link cylinder chain and compare ABA with a positive-definite linear solve using its assembled $M$ and bias. Report residuals and conditioning; do not explicitly invert $M$ merely to apply it to one vector.
1. **Floating-Base Reactions:** For a specified humanoid model, choose either a point-foot or a finite-patch contact model. Write the root and contact equations together. State the rank, friction and unilateral conditions required for the computed reaction, and explain what measured contact data would remove from the unknowns.
1. **Redundancy:** Choose a task Jacobian $J$ for the worked model. Examine the map $JM^{-1}\delta\tau$ and construct a torque perturbation with zero immediate task acceleration when such a null space exists. Explain why that does not imply zero state change or unchanged delivery at a later time.
1. **Constrained Projection:** Let $M\succ0$ and full-row-rank $J$ be fixed, and require $Ja+b_c=0$. Minimize $(a-a_{\mathrm{free}})^TM(a-a_{\mathrm{free}})/2$ under that constraint. Derive

    $$
    a=a_{\mathrm{free}}-M^{-1}J^T(JM^{-1}J^T)^{-1}(Ja_{\mathrm{free}}+b_c).
    $$

    Explain why this is a mass-weighted closest acceleration, not an arbitrary Euclidean minimum-norm forward solution, and discuss redundant constraints separately.
