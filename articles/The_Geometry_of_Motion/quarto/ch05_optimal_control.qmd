...
The theory of optimal control grew out of dynamic programming and the calculus of
variations. Tracing that path shows how modern methods like DDP fit into the broader
landscape.
...
whatever the initial state and the first decision, the remaining decisions must themselves
constitute an optimal policy with regard to the state that the first decision produces.
...
This interpretation carries a guarantee: DDP inherits Newton's *local quadratic convergence*.
The guarantee rests on two conditions---the second-order sufficient conditions at a local
minimum, and an exact re-linearization. The second holds automatically, since we use the exact
...
Given a nonlinear cost function $\ell_k(\x_k,\uvec_k)$ and a nominal trajectory $\{\bar{\x}_k, \bar{\uvec}_k\}$,
we expand to second order around this trajectory. Define perturbations $\dx_k = \x_k - \bar{\x}_k$ and
$\du_k = \uvec_k - \bar{\uvec}_k$. The Taylor expansion is:
\begin{align}
\ell_k(\bar{\x}_k + \dx_k, \bar{\uvec}_k + \du_k) &\approx \ell_k(\bar{\x}_k, \bar{\uvec}_k)
...
are second derivatives, all evaluated at $(\bar{\x}_k, \bar{\uvec}_k)$. Because only the relative
cost change matters (optimality is insensitive to constant offsets), we drop the zero-order term
and define the **quadratic cost matrices**:
\begin{align}
...
(e.g., effort-penalizing costs that depend on both position and force). Whether to keep
$\mat{N}_k$ is what separates *full DDP* from *iLQR*, discussed in Section~\ref{sec:ch5:ilqr-vs-ddp}.
...
\end{equation}
where $\mat{A}_k = \frac{\partial f}{\partial \x}\big|_{(\bar{\x}_k,\bar{\uvec}_k)}$ and
$\mat{B}_k = \frac{\partial f}{\partial \uvec}\big|_{(\bar{\x}_k,\bar{\uvec}_k)}$ are the Jacobians of the
...
Substituting the optimal gain back into the value function yields an equivalent, more numerically
stable form of the Riccati update, and it does so for both full DDP and iLQR. After algebraic manipulation:
\begin{equation}
...
This form is useful for verification, and it shows what the gain does. The term $\mat{K}_k^\T\mat{R}_k\mat{K}_k$
is the input cost the feedback law incurs; the last term propagates the cost-to-go through the
closed-loop dynamics $\mat{A}_k - \mat{B}_k\mat{K}_k$.
...
::: {.callout-note}
## Differential Dynamic Programming (Full)
...
However, this is impractical for large trajectory problems. DDP avoids the full Newton Hessian by exploiting
*a special structure in the problem*. The Hessian splits into local blocks along the trajectory, so solving the
KKT conditions locally, at each step via Bellman's principle, recovers the global solution.
...
The theorem says that DDP is not a heuristic approximation scheme but a structured Newton
method with guaranteed local convergence properties. The quadratic convergence rate means that each iteration
(near the optimum) roughly doubles the number of correct digits, so the final approach is fast.
...
than quadratic. However, iLQR is often faster overall because each iteration costs less, trading
convergence rate against cost per iteration.
:::
...
We use $\theta = 0$ at the **upright (inverted) equilibrium**. With this convention,
gravity acts to increase $|\theta|$ (destabilizing), giving $+g\sin\theta$. If $\theta = 0$
were the downward (stable) equilibrium, the sign would be $-g\sin\theta$ (restoring force).
The goal is to stabilize $\theta = 0$ (the inverted position) using the control torque $\tau$.
:::
...
1. **Swing-up phase** ($k = 0, \ldots, T_1 - 1$): Drive the pendulum to the top with minimal energy.
   \begin{equation}
...
2. **Balancing phase** ($k = T_1, \ldots, T-1$): Maintain balance near the top with minimal control effort.
   \begin{equation}
...
: DDP Convergence for Inverted Pendulum Swing-Up ($T = 100$ steps, $T_1 = 50$) {#tbl-ch5_ddp-convergence}
...
: Sample DDP Feedback Gains During Balancing Phase {#tbl-ch5_ddp-gains}
...
With these gains, the closed-loop system tracks the nominal trajectory despite small disturbances,
keeping the balancing robust.
...
$\uvec_{\text{des}} = \bar{\uvec}_k + \alpha\,\du_k^{(\text{ff})} + \mat{K}_k(\x_k - \bar{\x}_k)$, clamp it:
\begin{equation}
...
\begin{align}
\tilde{\x}_k &= \frac{\x_k}{x_{\max}}, \quad \tilde{\uvec}_k = \frac{\uvec_k}{u_{\max}}, \label{eq:ch5:bryson-norm}\\
...
You can systematize the tuning with a grid search or with tools like the Ziegler--Nichols method (adapted to LQR).
...
The negative sign and backward time direction reflect how $\mat{S}(t)$ evolves: *backward* in time from
the terminal cost.
...
\begin{equation}
\uvec_k = \bar{\uvec}_k + \alpha\,\du_k^{(\text{ff})} + \mat{K}_k(\x_k - \bar{\x}_k),
...
the shifted trajectory from time $t_k$. Concretely, if $\{\bar{\x}_j, \bar{\uvec}_j\}_{j=k}^{k+N}$
was optimal at $t_k$, the warm-start for $t_{k+1}$ is:
\begin{equation}
\bar{\x}_j^{(0)} = \bar{\x}_{j+1}^{\text{prev}}, \quad
\bar{\uvec}_j^{(0)} = \bar{\uvec}_{j+1}^{\text{prev}}, \quad j = k+1, \ldots, k+N-1,
...
6. **Cost weight selection.**
   ...
   Plot the resulting trajectories and control efforts. Explain why a large $\mat{Q}$
