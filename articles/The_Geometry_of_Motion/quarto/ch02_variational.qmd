---
title: "Variational Dynamics and the Moving Frame"
css: ../../../css/variational-chapter.css
---
...
## First Variation Along a Feasible Motion

### State, Input and Reference
...
The first-variation equation is exact for the derivative $\xi$. Under the stated local smoothness and boundedness conditions,
$$
...
### The Taylor Remainder and Its Mixed Term
...
## Time Ordering and the Peano--Baker Series
...
## Variation of Constants and Outcome Sensitivity

### Derivation of the Forced Solution
...
### Impact Time Is an Additional Variable
...
## Finite Error and the Limits of Linear Prediction
...
## Discrete Flow and Its Derivative

### Zero-Order-Held Inputs
...
### A Singular-Safe Linear Formula
...
## A Checked Pendulum Example

### Model and Analytic Transition
...
### Numbers, Units and Impulse Interpretation
...
## Parameter Sensitivity and Adjoint Gradients
...
### Discrete Adjoint and Backpropagation
...
## Volume, Distance and Stability Are Different
...
## Control Authority Over a Horizon
...
## Moving Frames and Geometric Meaning
...
### Integrating What the Task Requires
...
### Stiffness Is Not the Same as Loss of Information
...
### QR Propagation With Correct Product Order
...
### Rotations and Variational Integrators
...
## What This Establishes for the Swing
...
1. **Ordered Propagation.** Compute both orders of the two nilpotent interval generators above. Compare with $\exp(A_1+A_2)$ and identify the commutator term responsible for the discrepancy.
1. **A Moving Stable Spectrum.** Differentiate $\Phi(t,0)=R(t)\exp((D-\Omega)t)$ for the rotating example and verify its initial condition. Show that one solution grows although every frozen eigenvalue is negative.
1. **Pendulum Limits.** Using $m=\ell=1$ and $g=10$, compare the nonlinear pendulum and first variation from angles 0.1, 0.05 and 0.025 rad over 0.5 s. State the norm and integration tolerance; investigate the special odd-symmetry error order at this equilibrium.
1. **Bounded Pulses.** Replace the ideal torque impulse by a rectangular pulse with the same angular impulse. Vary duration and enforce a declared torque limit. Separate the short-pulse approximation from actuator feasibility.
1. **The Initial-State Gradient.** Reproduce the scalar adjoint check with $x(0)=p$, then repeat with fixed $x(0)$. Identify the changed boundary term and verify gradients across several finite-difference steps.
1. **A Short-Horizon Task.** Derive the double-integrator Gramian and amplitude bounds. Compare the achievable terminal position and velocity corrections as $T$ decreases; explain why full rank does not imply a uniformly easy correction.
1. **Volume Versus Distance.** For $A=\operatorname{diag}(1,-2)$, compute the determinant, singular values and response from each coordinate direction. Repeat for a damped pendulum at its upright equilibrium.
1. **A QR Product.** Implement the interval factorization and compare the correct left accumulation with the reversed order using noncommuting propagators. Monitor reconstruction error and factor magnitudes as the horizon grows.
1. **Symplecticity Versus Stability.** Verify the unit-oscillator symplectic identity and modified quadratic for symplectic Euler. Compare $h=0.5$, 2 and 3, including the repeated-eigenvalue case. Do not infer stability from determinant 1.
1. **A Golf Output Contract.** Choose one pre-impact outcome, declare its units, frame, event and retained actuator states, and write its first-order input kernel. Describe the measurements needed to test a timing claim and a finite intervention that could falsify it.
