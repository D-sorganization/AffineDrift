...
# Exponential Coordinates and Matrix Logarithms {#sec-exp-coords}
...
In @sec-so3 and @sec-screw, we established that physical orientations live in the Special Orthogonal Group $\SO$, and complete configurations live in the Special Euclidean Group $\SE$. 
...
The key algebraic identity for a unit-axis skew matrix is $[\hat{\bm{\omega}}]^3 = -[\hat{\bm{\omega}}]$
(proved by direct computation using $\|\hat{\bm{\omega}}\|=1$). Applying this repeatedly gives:

$$[\hat{\bm{\omega}}]^{2k+1} = (-1)^k [\hat{\bm{\omega}}], \qquad [\hat{\bm{\omega}}]^{2k+2} = (-1)^k [\hat{\bm{\omega}}]^2, \qquad k = 0,1,2,\ldots$$
...
e^{[\hat{\bm{\omega}}]\theta}
  = I
  + \sum_{k=0}^{\infty}\frac{(-1)^k\,\theta^{2k+1}}{(2k+1)!}[\hat{\bm{\omega}}]
  + \sum_{k=0}^{\infty}\frac{(-1)^k\,\theta^{2k+2}}{(2k+2)!}[\hat{\bm{\omega}}]^2.
$$
...
## Exercises {#sec-exp-coords-exercises}

**6.1 (Computational)** Implement the three-branch `matrix_log_SO3` and verify it round-trips with Rodrigues' formula on $\theta \in \{0, \pi/6, \pi/2, \pi - 10^{-10}\}$.

**6.2 (Computational)** Implement the SE(3) exponential using the closed-form $G(\hat{\bm{\omega}},\theta)$ formula. Compare runtime vs `scipy.linalg.expm` on $10^4$ random screws.

**6.3 (Conceptual)** Give a $2\times 2$ example with $A,B\in\mathfrak{so}(2)$ where the BCH approximation $A+B$ fails at first order and quantify the error.

**6.4 (Conceptual)** Explain why SLERP traces a geodesic on $\SO$ while linear Euler-angle interpolation does not. Connect to the exponential map. See [@lynch2017modern, §3.3].
