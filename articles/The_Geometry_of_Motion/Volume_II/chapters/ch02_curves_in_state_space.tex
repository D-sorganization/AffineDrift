...
\begin{laymansbox}{State Space and Motion}{state_space}
...
properties of the \emph{parameterization}. A slow practice swing and a
full-speed swing may trace a similar path in configuration space, but they do
not trace the same curve in full state space because the velocity components
are different. The geometric tools we develop here allow us to say precisely
which claims are about the configuration path, which claims are about timing,
and which claims require the full state trajectory.
...
\begin{principle}{Arc Length as the Canonical Clock}{arc_clock}
  ...
  describe the spatial \emph{path} of the body through configuration space.
...
\begin{principle}{Trajectory Derivatives and Input Rank}{dynamic_rank}
...
For the one-input, two-coordinate spring system in Chapter~1, the unforced
...
\begin{remark}{The Orbit Theorem and Achievable Curves}{orbit_theorem}
  A profound connection exists between the curvature of a trajectory and the \emph{control vector fields} that generate it. The \textbf{Orbit Theorem} (also known as \textbf{Chow's Theorem}), a central result in geometric control theory as presented by Agrachev and Sachkov \cite{AgrachevSachkov2004}, states that the orbit generated by iteratively flowing along the control vector fields $\bm{f}_i$ and their Lie brackets $[\bm{f}_i, \bm{f}_j] = \frac{\partial \bm{f}_i}{\partial \state}\bm{f}_j - \frac{\partial \bm{f}_j}{\partial \state}\bm{f}_i$ determines the space of achievable curves. For control-affine systems $\dot{\state} = \bm{f}(\state) + \sum u_i\bm{g}_i(\state)$, higher-order Lie brackets correspond to higher derivatives of the trajectory in the direction perpendicular to the primary control inputs. The curvature of a trajectory encodes which Lie bracket relationships are being exploited: a highly curved trajectory requires the activation of higher-order commutators in the control Lie algebra. This is the geometric signature of how underactuated systems achieve complex motions despite having fewer inputs than degrees of freedom.
\end{remark}
...
This result has a far-reaching implication: \emph{the stability of trajectory
