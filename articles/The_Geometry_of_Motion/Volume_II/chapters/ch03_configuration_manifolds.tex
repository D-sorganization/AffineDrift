...
\index{configuration manifold}\index{smooth manifold}\index{chart}%
\index{atlas}\index{tangent bundle}\index{cotangent bundle}%
\index{SO(3)@$\SOthree$}\index{SE(3)@$\SEthree$}%
\index{Lie group}\index{Lie algebra}\index{exponential map}%
\index{logarithm map}\index{generalized coordinates}%
\index{Riemannian metric}\index{mass matrix}\index{geodesic}
...
\begin{laymansbox}{Angles Are Not Numbers}{angles_not_numbers}
...
\begin{laymansbox}{Mathematical Reminder: The Foundations Re-applied}{vol0_reminder_manifolds}
...
\begin{enumerate}
    \item \textbf{Manifold ($\mathcal{Q}$):} The curved, global "position" space of the robot.
    \item \textbf{Tangent Space ($T_q\mathcal{Q}$):} The perfectly flat, linear "velocity" space attached to one specific position.
    \item \textbf{Tangent Bundle ($T\mathcal{Q}$):} The total \textbf{State Space} combining all positions and all their velocities.
\end{enumerate}
...
\begin{remark}{Lie Algebras and Controllability on Manifolds}{lie_algebra_control}
  The Lie algebra of vector fields is a central object in geometric control theory, but it should be read as a local accessibility tool rather than a universal reachability certificate. Agrachev and Sachkov \cite{AgrachevSachkov2004} and Bullo and Lewis \cite{BulloLewis2004} show how the distribution generated by control vector fields and their iterated Lie brackets characterizes directions that can be produced infinitesimally, under the stated smoothness, rank, and constraint assumptions. For a simplified golf-swing model, the control vector fields may represent shoulder, elbow, hip, or wrist torques, while their brackets represent inertial coupling terms that are not directly commanded. A full-rank Lie closure supports an accessibility claim for the model; it does not by itself prove that every configuration is reachable under torque bounds, joint limits, contact constraints, fatigue, or a finite downswing time.
\end{remark}
