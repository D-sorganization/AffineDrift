...
A remarkable feature of underactuated optimal control is the existence of \textbf{abnormal extremals}---optimal trajectories where the maximum principle is satisfied in a degenerate way (the costate is proportional to the velocity). These do not appear in standard optimal control with full actuation, but are pervasive in underactuated systems. As explained by Agrachev and Sachkov \cite{AgrachevSachkov2004}, abnormal extremals correspond precisely to trajectories where the control must be ``singular''---switching instantaneously or taking on a specific form not derivable from the regular Hamiltonian maximization.

In a simplified swing model, the club's rapid acceleration during release may correspond to an abnormal or near-singular arc. The control may switch or saturate to exploit inertial coupling, but this is a model prediction rather than a universal property of golf swings. Recognizing possible abnormal extremals is important for trajectory optimization because standard numerical solvers can miss them unless the problem formulation allows switching, saturation, and active-set changes.
