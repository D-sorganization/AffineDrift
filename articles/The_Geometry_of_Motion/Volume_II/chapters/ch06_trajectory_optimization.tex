...
\begin{remark}{Pontryagin Maximum Principle and Optimal Control}{pontryagin_ocp}
  While direct collocation provides a practical numerical algorithm, the theoretical foundation for trajectory optimization comes from the \textbf{Pontryagin Maximum Principle} \cite{AgrachevSachkov2004}. This principle states that along an optimal trajectory, a Hamiltonian function $\mathcal{H}(\state, \control, \bm{\lambda}) = L(\state, \control) + \bm{\lambda}^\top \bm{f}(\state, \control)$ must be maximized with respect to $\control$ at each instant, where the costate $\bm{\lambda}(t)$ evolves backward according to adjoint dynamics. For underactuated systems, \textbf{bang-bang} structures can emerge when the Hamiltonian is linear in $\control$, and \textbf{singular arcs} can occur when higher-order conditions are needed to determine $\control$. A simplified golf-release model may show a control structure resembling a singular arc, but that is a hypothesis about the chosen model and bounds, not a direct prediction of measured wrist muscle activation.
\end{remark} 
where $\tilde{\state}$ and $\tilde{\control}$ are the polynomial approximations. 
