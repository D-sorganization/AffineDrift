...
\index{trajectory optimization}\index{optimal control problem}\index{OCP}%
\index{direct collocation}\index{multiple shooting}\index{NLP}%
\index{moving target OCP}\index{Pontryagin maximum principle}%
\index{Hamiltonian!in optimal control}\index{costate}\index{adjoint dynamics}%
\index{bang-bang control}\index{singular arc}%
\index{covariance steering}\index{signal-dependent noise}%
\index{stochastic OCP}\index{linear covariance analysis}
...
\end{align*}
where $\bm{w}$ is a large vector of decision variables. How we discretize the integrals and differential equations defines the transcription method. We will focus on two modern approaches: \textbf{Direct Shooting}~\cite{bock1984multiple} and \textbf{Direct Collocation}~\cite{hargraves1987direct}.
...
\end{equation}
where $\bm{w} \sim \mathcal{N}(0, \Sigma_w)$ represents signal-dependent motor noise (a known property of human muscle actuation, where noise scales proportionally to the applied torque)~\cite{HarrisWolpert1998}.

To solve this in a collocation framework without massive Monte Carlo simulations, we use \textbf{Covariance Steering}~\cite{chen2015optimal} or \textbf{Linear Covariance Analysis}. We augment the state vector $\state(t)$ with its local covariance matrix $\bm{\Sigma}(t)$. The dynamics of $\bm{\Sigma}(t)$ are propagated alongside the nominal trajectory using the linearized Jacobian matrices along the path. 
