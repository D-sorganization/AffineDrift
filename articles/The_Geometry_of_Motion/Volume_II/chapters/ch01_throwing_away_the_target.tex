...
\index{trajectory tracking}\index{setpoint control}\index{control is motion}%
\index{trajectory tube}\index{tube-invariance}\index{funnel}%
\index{golf swing}\index{underactuation}\index{passive dynamics}%
\index{limit cycle}\index{Lyapunov stability}\index{reachable set}%
\index{attainable set}\index{Orbit Theorem}\index{signal-dependent noise}
...
Open any classical control textbook~\cite{kuo2002learning} and you will find, within the first few pages, a
...
In this volume, we develop control theory from this trajectory-first
perspective, showing that the classical setpoint framework is a special (and often
limiting) case of a much richer theory, and building the mathematical tools needed
...
\begin{definition}{Trajectory Tube}{tube}
  ...
  function $\varepsilon: [0,T] \to \Reals_{>0}$, the \textbf{trajectory tube}~\cite{MajumdarTedrake2017} is the
  time-varying set
  \begin{equation}
...
\begin{enumerate}[label=(\roman*)]
  \item \textbf{High dimensionality.} The human body has over 200 degrees of freedom~\cite{Winter2009,Zatsiorsky2002}.
