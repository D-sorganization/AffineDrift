...
perspective. The classical setpoint framework is the correct tool for regulation
problems---holding a system at a desired operating point---and remains one of the
great achievements of control engineering. However, for systems whose primary purpose
is to \emph{move}, the trajectory-tracking framework provides a natural generalization
that contains setpoint regulation as a special case.
...
\begin{remark}{Tubes as Reachable Sets}{reachability}
  The trajectory tube $\tube(t)$ is intimately related to the concept of \textbf{reachable sets} and \textbf{attainable sets} from geometric control theory. These concepts form the mathematical foundation of trajectory-first control and are rigorously developed in the modern treatment by Agrachev and Sachkov \cite{AgrachevSachkov2004}. A reachable set at time $t$ is the collection of all states that can be reached from the initial condition $\state(0)$ in exactly time $t$, using admissible controls. The funnel-shaped tube narrows as time progresses precisely because the reachable sets, when constrained by the terminal manifold $\psi(\state(t_f)) = \bm{0}$, force convergence toward the target. Agrachev and Sachkov's framework for characterizing these sets through the orbit of vector fields generated by the control Lie algebra provides the theoretical underpinning for understanding why certain motions are ``accessible'' and others are not.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Extreme nonlinearity.} Joint torques couple through the full rigid-body
        ...
        angular velocities.
  \item \textbf{Underactuation.} Once the club is released in the simplified
        downswing model, the wrist hinge is treated as low-torque or passive.
...
\begin{example}{From Setpoint Thinking to Trajectory Thinking}{golf_shift}
  ...
  \textbf{Setpoint approach} (inappropriate for this task): Define a target state
  $\state^* = (\bm{q}_{\text{impact}}, \dot{\bm{q}}_{\text{impact}})$ and design a
  controller to drive $\state(t) \to \state^*$. This is ill-suited to a golf swing
  because the system should not \emph{stop} at $\state^*$. Impact is a waypoint, not
  a destination. The club must be accelerating \emph{through} the ball, not
  decelerating toward it.
...
\begin{remark}{The Pontryagin Maximum Principle}{pontryagin}
  The mathematical foundation for solving trajectory optimization problems with terminal constraints (such as the moving goal in the golf swing) comes from the \textbf{Pontryagin Maximum Principle}, a cornerstone of geometric control theory. As developed rigorously in Agrachev and Sachkov \cite{AgrachevSachkov2004}, this principle transforms the infinite-dimensional problem of optimizing $J[\state(\cdot), \control(\cdot)]$ into a two-point boundary value problem via the introduction of costate variables (adjoint variables) and a Hamiltonian function. The principle states that along an optimal trajectory, the Hamiltonian $\mathcal{H} = L(\state, \control) + \bm{\lambda}^\top \bm{f}(\state, \control)$ is maximized with respect to $\control$ at each instant, where $\bm{\lambda}(t)$ evolves backward in time according to adjoint dynamics. This formalism unifies our treatment of both the path optimization (determining $\traj^*$) and the feedback stabilization (computing funnels around $\traj^*$) within a single geometric framework. Chapter 6 will apply these principles explicitly to synthesize the moving-target golf swing trajectory.
\end{remark}
...
\item \textbf{(Design.)} A simplified 2-DOF golf swing model has coordinates
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Discuss why a setpoint controller applied only at impact ($s = 0.7$)
