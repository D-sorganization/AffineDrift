...
\begin{laymansbox}[Context \& Use Case: Smooth Interpolation]
...
\textbf{All Variables Defined:} $se(3)$ represents the flat tangent velocity space, $SE(3)$ represents the curved position manifold, and $\hat{\boldsymbol{\omega}}$ is the unit rotation axis.
\end{laymansbox}
...
This flat tangent space is called the \textbf{Lie Algebra}, mathematically denoted as $so(3)$ for pure rotations and $se(3)$ for complete spatial poses (Twists). While the \emph{Lie Group} represents "Where am I?", the \emph{Lie Algebra} represents "In what exact direction and how fast am I moving?".
...
\begin{equation}
  [\mathbf{r}] = [\hat{\boldsymbol{\omega}}]\theta = \begin{bmatrix} 0 & -r_z & r_y \\ r_z & 0 & -r_x \\ -r_y & r_x & 0 \end{bmatrix} \in so(3)
\end{equation}

This skew-symmetric matrix literally \emph{is} the Lie Algebra $so(3)$. The Matrix Exponential physically wraps this flat tangent vector around the curved surface of $SO(3)$ to produce our final Rotation Matrix. Because calculating an infinite Taylor series ($e^A = I + A + A^2/2! + \dots$) is too slow for computers, we use \textbf{Rodrigues' Formula}:

\begin{equation}
   \mathbf{R} = \exp([\hat{\boldsymbol{\omega}}]\theta) = \mathbf{I} + \sin\theta [\hat{\boldsymbol{\omega}}] + (1 - \cos\theta)[\hat{\boldsymbol{\omega}}]^2
\end{equation}

This exact same exponential mapping scales up effortlessly to the full $4 \times 4$ spatial Twists ($\mathcal{V} \in se(3)$) we learned about in Chapter 5. By passing a Twist and a time step into the Matrix Exponential, we get the exact 3D rigid body transformation:
\begin{equation}
...
We must perform the exact inverse operation: the \textbf{Matrix Logarithm}. The Matrix Logarithm takes a curved element from the Lie Group ($\mathbf{R} \in SO(3)$) and perfectly flattens it back onto the tangent space to extract the axis-angle representation.
