\chapter[Exponential Coordinates and Matrix Logarithms]{Exponential Coordinates\\and Matrix Logarithms}
...
\section{Historical Context and the Mathematical Question}
\label{sec:historical_context_lie}
\label{laymans:context_box}

Rodrigues' rotation formula appeared in 1840 \cite{rodrigues1840}, before Sophus Lie's later theory of continuous transformation groups. The formula is therefore not a consequence that Rodrigues rediscovered after Lie. Modern robotics texts develop rigid-motion calculations using this group structure; Murray, Li and Sastry and Lynch and Park provide systematic treatments \cite{Murray1994,Lynch2017}. The derivations below are included so that the conventions can be checked without relying on historical priority claims.
...
\begin{equation}
 ...
 =8\sin^2(\theta/2).
 \label{eq:rotation_chordal_cost}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:exponential_angular_frames}
\end{equation}
...
\section[Exponential Coordinates and Axis-Angle Representation]{Exponential Coordinates\\and Axis-Angle Representation}
...
\section{Rodrigues' Formula and Its Derivation}
...
\begin{equation}
 ...
 \label{eq:rodrigues}
\end{equation}
More generally, $K^{2k+1}=(-1)^kK$ for $k\geq0$, and $K^{2k}=(-1)^{k-1}K^2$ for $k\geq1$. The even-power coefficient is $1-\cos\theta$; its sign does not change through rearrangement. For nonzero $\phi$, the equivalent expression is $I+(\sin\theta/\theta)\Phi+((1-\cos\theta)/\theta^2)\Phi^2$, with continuous limits at zero. Rodrigues' formula is valid for every real angle, not just a principal interval.
...
\subsection{Numerical Stability and Branch Choice}
...
For a proper rotation, $\operatorname{atan2}(\|(R-R^T)^\vee\|/2,(\operatorname{tr}R-1)/2)$ is an alternative principal-angle formula. It avoids the direct inverse-cosine endpoint calculation but still leaves axis extraction to be handled near a half turn. A stable implementation can recover a unit quaternion using a well-scaled component and then obtain angle and axis with a half-angle formula. The worked implementation delegates that rotation calculation to SciPy and validates the input first. It does not silently repair an invalid matrix.
...
\begin{equation}
 ...
 \label{eq:exponential_se3_block}
\end{equation}
...
\begin{equation}
 ...
 \end{aligned}
 \label{eq:exponential_jacobian_inverse}
\end{equation}
...
\section{Coordinate Rates, Jacobians, and Conditioning}
\label{sec:exponential_differential}
...
\begin{equation}
 ...
 \label{eq:exponential_coordinate_rates}
\end{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{../figures/exponential_branches.pdf}
\caption{A Half-Turn Branch Cut Is Different From a Differential Singularity. A Smooth Rotation About a Fixed Axis Crosses the Shortest-Vector Cut, While the Chosen Local Jacobian Remains Nonsingular.}
...
\begin{equation}
 ...
 \label{eq:exponential_bch_second}
\end{equation}
...
\subsection{Higher-Order Terms and Their Domain}
...
Use \texttt{scipy.linalg.expm} for a matrix exponential. NumPy's elementwise exponential computes a different operation. For a general matrix, scaling and squaring reduces its norm, evaluates a suitable rational approximation, then repeatedly squares the result. As a simple illustration, the Pad\'e approximant of order $[2/2]$ is
\begin{equation}
 e^A\approx
 (I-A/2+A^2/12)^{-1}(I+A/2+A^2/12).
 \label{eq:exponential_pade}
\end{equation}
...
For $SO(3)$ and $SE(3)$, structure-specific formulas are compact. Near zero, evaluate the coefficients of $J_l$ using series: $1/2-\theta^2/24+\theta^4/720$ and $1/6-\theta^2/120+\theta^4/5040$. Away from zero, writing $1-\cos\theta=2\sin^2(\theta/2)$ reduces cancellation. In array code, $\Phi^2$ means \texttt{Phi @ Phi}, not \texttt{Phi ** 2}, which squares entries independently. An apparently plausible rotation block cannot validate an incorrect translation block.

\section{Geodesics on SO(3): Metric and Shortest Paths}
\label{sec:geodesics}
\index{Geodesics}
...
\section{SLERP and Its Physical Limits}
\label{sec:slerp}
\index{SLERP}
...
\begin{equation}
 ...
 \label{eq:exponential_free_rotation}
\end{equation}
...
\section{Reading a Golf Swing Through These Coordinates}
\label{sec:exponential_golf}
...
\begin{equation}
 ...
 \label{eq:exponential_delivery_sensitivity}
\end{equation}
...
Lynch and Park and Murray, Li and Sastry develop the rigid-motion conventions used here \cite{Lynch2017,Murray1994}. The Modern Robotics rotation and rigid-motion exponential demonstrations provide accessible derivations \cite{ModernRoboticsRotationExpReview,ModernRoboticsRigidExpReview}. Sol\`a, Deray and Atchuthan collect left/right Jacobians and $SE(3)$ formulas; their six-vector convention is translation-first, so block order must be changed when comparing to this chapter \cite{SolaMicroLieReview}. SciPy documents the numerical behavior of \texttt{expm}, \texttt{Rotation.from\_matrix} and \texttt{Slerp} \cite{SciPyExpmReview,SciPyRotationMatrixReview,SciPySlerpReview}. These mathematical and software references do not independently establish empirical claims about golf technique.

\section{Exercises}
\label{sec:exercises_ch6}

\begin{enumerate}[itemsep=3pt,topsep=4pt]
\item \textbf{Rodrigues' Formula Verification:} Compute a positive $45$-degree rotation about $(1,1,1)/\sqrt3$. Check orthogonality, determinant and the fixed axis. State the frame in which the axis is expressed.

\item \textbf{A Quarter-Turn Logarithm:} For $R=\begin{bmatrix}0&-1&0\\1&0&0\\0&0&1\end{bmatrix}$, find a shortest rotation vector and its skew matrix. Give one longer logarithm with the same exponential.

\item \textbf{Trace and the Half Turn:} Derive $\operatorname{tr}R=1+2\cos\theta$. Explain why it remains valid at $\pi$, why the skew-based axis formula fails there, and how $R+I$ recovers an axis line at exactly $\pi$.

\item \textbf{Skew Powers:} Prove $K^2=uu^T-I$ and $K^3=-K$ for a unit vector $u$. List $K^4$ through $K^7$ and recover the signs in Rodrigues' series.

\item \textbf{A Finite First-Order Step:} Expand $e^{\theta K}-(I+\theta K)$ through degree two. Show that $I+\theta K$ is not exactly orthogonal when $\theta\ne0$.

\item \textbf{Rigid-Motion Exponential:} Compute $\exp\widehat\xi$ for the integrated coordinate $\xi=(0,0,1,1,0,0)$, using radians and metres. Identify an offset rotation axis and verify that a point on it is fixed.

\item \textbf{Pure Translation and Duration:} For constant twist $(\omega,v)=(0,0,0,1,0,0)$ in radians/s and m/s, find $T(t)$ from the identity. Explain the difference between $J_l(0)=I$ and the factor $t$ in its translation.

\item \textbf{Inverse and Branches:} Prove that $-\widehat\xi$ is a logarithm of $(e^{\widehat\xi})^{-1}$. State when shortest branches give $\operatorname{Log}(T^{-1})=-\operatorname{Log}(T)$ and construct the pure-half-turn counterexample to an unconditional single-valued identity.

\item \textbf{Second-Degree BCH:} With $A=[a]_\times$, $B=[b]_\times$, derive the coefficient of $\epsilon^2$ in $\operatorname{Log}(e^{\epsilon A}e^{\epsilon B})$. Explain how reversing order changes it and why this does not quantify a pair of $90$-degree rotations accurately.

\item \textbf{SLERP Velocity:} Differentiate $R(t)=R_0\exp((t/\tau)[\phi_B]_\times)$. Calculate $R^T\dot R$ and $\dot RR^T$. Explain why differentiating a general rotation vector instead requires a Jacobian.

\item \textbf{Metric Normalization:} Compute the rotation-angle distance from identity to a $120$-degree rotation about $(1,1,1)/\sqrt3$. What length would an unnormalized Frobenius tangent norm assign to the same path?

\item \textbf{Small-Angle Error:} At $\theta=0.001$ rad about $z$, compute the Frobenius error of $I+\theta K$ relative to Rodrigues' matrix. Compare it with the leading term $\theta^2/\sqrt2$.

\item \textbf{A Generic Matrix Exponential:} Let the $4\times4$ generator $A$ have $A_{12}=-1$, $A_{21}=1$ and all other entries zero. Evaluate \texttt{scipy.linalg.expm}$(tA)$ at $t=\pi/4$ and verify its rotation block. Do not apply a second hat operation to $A$.

\item \textbf{Interpolation Fractions:} Choose two proper rotations and interpolate at $s=0.3$ and $0.7$ on one shortest geodesic. Verify $d(R_0,R(s))=s\,d(R_0,R_1)$. Describe the additional choice needed if their relative angle is $\pi$.

\item \textbf{An Offset-Axis Trajectory:} For constant twist $(1,0,0,0,1,0)$ in radians/s and m/s, compute $T(t)$ at $0$, $\pi/4$ and $\pi/2$ seconds. Show that $p(t)=(0,\sin t,1-\cos t)$ m and identify the zero-pitch rotation axis.
\end{enumerate}
