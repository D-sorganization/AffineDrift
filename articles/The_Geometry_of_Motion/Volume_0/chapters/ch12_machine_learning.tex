...
\begin{algorithm}{REINFORCE}{reinforce}
\begin{algorithmic}
...
class NeuralNetwork:
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        # Layer 1: input -> hidden (He initialization for ReLU layers)
        self.W1 = np.random.randn(hidden_dim, input_dim) * np.sqrt(2.0 / input_dim)
        self.b1 = np.zeros((hidden_dim, 1))

        # Layer 2: hidden -> output (Xavier/Glorot for the output layer)
...
\begin{enumerate}
    ...
    \item \textbf{Gradient Descent}: Simple optimization algorithm that, despite its limitations, works notably well in practice when combined with proper learning rate scheduling (Adam optimizer).
