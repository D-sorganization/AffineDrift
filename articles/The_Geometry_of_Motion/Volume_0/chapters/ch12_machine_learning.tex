...
\subsection{Prediction, Inversion and Policy Are Different Maps}
...
\subsection{Why State Choice Matters for Golf}
...
\begin{tikzpicture}[scale=1.0]
  ...
  % All 15 inter-layer weights are shown; biases are omitted.
  \foreach \i in {1,2,3} {
    \foreach \j in {1,2,3} { \draw[-,thick] (x\i) -- (h\j); }
    \foreach \j in {1,2} { \draw[-,thick] (h\i) -- (y\j); }
  }
  % Labels
  ...
  \node at (2,-1.5) {Compute weight gradients: $\nabla_\mathbf{W} \mathcal{L} = \frac{\partial \mathcal{L}}{\partial \mathbf{W}}$};
\end{tikzpicture}
\caption{A Fully Connected Three-Input, Three-Hidden-Unit, Two-Output Network. All Fifteen Inter-Layer Weights Are Shown; the Five Biases Are Omitted.}
...
\section{Loss Functions and What They Estimate}
...
\subsection{Policy Evaluation Versus Optimality}
...
\subsection{Baselines and REINFORCE}
...
\subsection{Multi-Step Returns and Bias-Variance Tradeoffs}
...
\subsection{Stochastic Bounded Actions}
...
\section{Value-Based Methods and Continuous Actions}
\label{sec:value_based_methods}
\index{value-based methods}\index{DQN}\index{Deep Q-Network}\index{Q-learning}
...
\index{experience replay}\index{target network}
...
\index{DDPG}\index{Deep Deterministic Policy Gradient}
...
\index{TD3}\index{SAC}\index{Soft Actor-Critic}\index{maximum entropy}
...
\index{exploration strategies}\index{exploration-exploitation tradeoff}
...
\index{entropy regularization}\index{curiosity-driven exploration}\index{intrinsic reward}
...
\section{Worked Example: Pendulum Swing-Up Objective}
...
\section{Neural ODEs and the Adjoint Method}
...
\subsection{Hamiltonian Models}
\index{Hamiltonian Neural Network}\index{HNN}
...
\subsection{General Lagrangian Models}
\index{Lagrangian Neural Network}\index{LNN}
...
\subsection{Natural Mechanical Models and DeLaN}
\index{Deep Lagrangian Networks}\index{DeLaN}\index{Cholesky decomposition}
...
\section{Sim-to-Real Transfer and System Identification}
...
\subsection{What Can the Data Identify?}
...
The accompanying example fits the scalar function $y=x^2$ on $[-1,1]$ using a tanh hidden layer and a linear output. Its implementation is shared between the print and web lessons in the repository module \texttt{src.tools.learning\_examples}. This is supervised regression, not reinforcement learning, inverse dynamics or a pendulum controller.
...
\section{Deep Reinforcement Learning for Robot and Golf Control}
...
\subsection{State Representation and Action Parameterization}
...
\subsection{Reward Shaping and Evidence}
...
\href{https://papers.baulab.info/papers/Cybenko-1989.pdf}{Cybenko (1989), Theorem 2} \cite{cybenko1989review}. Uniform approximation on the unit cube; no training guarantee.

\href{https://www.deeplearningbook.org/contents/mlp.html}{Goodfellow, Bengio and Courville (2016), Chapters 6 and 8} \cite{goodfellow2016}. Feedforward models, loss functions, backpropagation and optimization; see the book navigation for Chapter 8.

\href{https://proceedings.neurips.cc/paper/1999/hash/464d828b85b0bed98e80ade0a5c43b0f-Abstract.html}{Sutton and Colleagues (1999), Policy Gradients} \cite{sutton1999review}. State-visitation weighting and compatible function approximation; not a blanket deep actor-critic convergence theorem.

\href{https://arxiv.org/abs/1707.06347}{Schulman and Colleagues (2017), PPO} \cite{schulman2017}. Sections 3 and 5 define the clipped surrogate and rollout/update procedure.

\href{https://www.nature.com/articles/nature14236}{Mnih and Colleagues (2015), DQN}. Replay and target-network method; its game benchmarks are not golf evidence.

\href{https://proceedings.mlr.press/v32/silver14.html}{Silver and Colleagues (2014), Deterministic Policy Gradients}. The deterministic policy-gradient framework and exploration distinction.

\href{https://arxiv.org/abs/1509.02971}{Lillicrap and Colleagues, Continuous Control}. The DDPG actor, critic, replay and target-network implementation.

\href{https://proceedings.mlr.press/v80/fujimoto18a.html}{Fujimoto, Van Hoof and Meger (2018), TD3}. Twin critics, delayed updates and target-policy smoothing; Algorithm 1.

\href{https://proceedings.mlr.press/v80/haarnoja18b.html}{Haarnoja and Colleagues (2018), SAC}. Entropy-regularized objective and approximate soft actor-critic; theoretical assumptions differ from practical approximation.

\href{https://maziarraissi.github.io/PINNs/}{Raissi, Perdikaris and Karniadakis, PINNs} \cite{raissi2019review}. The authors' specified Burgers problem and collocation residual; no general solution-error certificate.

\href{https://arxiv.org/abs/1806.07366}{Chen and Colleagues (2018), Neural ODEs} \cite{chen2018review}. Section 2, numerical considerations and Appendix B explain adjoint calculation and its numerical limits.

\href{https://arxiv.org/abs/1906.01563}{Greydanus, Dzamba and Yosinski (2019), HNNs} \cite{greydanus2019hamiltonian}. Section 2 constructs a learned conserved quantity in canonical coordinates.

\href{https://arxiv.org/abs/2003.04630}{Cranmer and Colleagues (2020), LNNs} \cite{cranmer2020lagrangian}. Euler--Lagrange derivative construction and its distinction from a natural mechanical model.

\href{https://www.ias.informatik.tu-darmstadt.de/uploads/Team/MichaelLutter/lutter_iclr_2019.pdf}{Lutter, Ritter and Peters (2019), DeLaN} \cite{lutter2019review}. Sections 3--4 parameterize inertia and a separate gravity force; distinguish a scalar-potential variant.

\section{Exercises}

\begin{enumerate}
\item \textbf{Representation:} Construct $|x|$ from two ReLU units. Then explain why continuous networks cannot uniformly approximate a jump of nonzero height to arbitrarily small error on a domain containing the jump.
\item \textbf{Backpropagation:} Derive all four parameter gradients for the tanh network. Compare them with centered finite differences at three nonzero inputs. State the loss normalization and array shapes.
\item \textbf{Activation Functions:} Plot sigmoid, tanh, ReLU and GELU and their derivatives. Identify saturation regions and nondifferentiability. Discuss advantages and disadvantages for a specified regression problem without asserting one activation is universally best.
\item \textbf{Gradient Bounds:} Derive the product-of-Jacobians bound for ten sigmoid layers. Evaluate the bound when every weight norm is one and when every weight norm is eight. Explain why neither is a measured parameter gradient.
\item \textbf{Pendulum Task:} Specify an MDP with the downward angle convention, torque bounds, input hold, integrator, observation, horizon and upright reward. Check reward and energy at both resting equilibria and at the numerical state in the text.
\item \textbf{Evaluation and Improvement:} In a 5-by-5 grid, movements are deterministic; attempted off-grid moves leave the state unchanged. The upper-right cell is absorbing with zero future reward; every preceding action earns $-1$, including the action entering the goal. Use $\gamma=0.9$. Compare uniform-random policy evaluation with optimal value iteration from all-zero initial values.
\item \textbf{Policy Gradient:} Derive the finite-horizon score estimator for the stated discounted start-state objective. Show explicitly where the $\gamma^t$ factor and the cancellation of past rewards enter.
\item \textbf{Bootstrap Boundaries:} For rewards $(1,2,3)$, $\gamma=0.9$, critic values $V(s_1)=4$, $V(s_2)=5$, and true termination at $s_3$, compute the one-, two- and three-step targets from $s_0$. Explain what repeated trajectories would be needed to compare their variance.
\item \textbf{Actor-Critic:} Write pseudocode that keeps bootstrap targets and actor advantages detached. Distinguish a genuine terminal transition from a truncated continuing rollout and specify the final observation used for bootstrapping.
\item \textbf{PPO:} Plot the clipped sample objective for $\epsilon=0.2$ and advantages $2$ and $-2$. Give ratios outside the interval that lie on its plateaus. Explain why the plot does not establish a hard probability bound or prevent policy collapse.
\item \textbf{Physics Residual:} Write a trajectory-fitting loss for the forced damped pendulum with known input and initial conditions. Give a residual that would be incorrect if actuation or damping were omitted. Propose held-out residual checks.
\item \textbf{Domain Randomization:} Define a physically consistent uncertainty distribution for the pendulum's mass, length, torque calibration and observation delay. Explain how inertia changes with mass and length, and separate training draws from held-out evaluation cases.
\item \textbf{Identification:} Show that scaling both $g$ and $l$ by the same positive factor leaves free angle trajectories unchanged. Identify an additional measurement or calibrated experiment and explain which ambiguity it removes.
\item \textbf{Adjoint and Conservation:} Verify the scalar Neural ODE derivative in the text analytically and with finite differences. Separately show that explicit Euler applied to $H=(q^2+p^2)/2$ multiplies $H$ by $1+h^2$ per step, despite the exact Hamiltonian flow conserving it.
\item \textbf{Reproducible Learning:} Run the shared regression example with batch sizes that divide the dataset, leave a remainder and exceed its size. Then design, but do not claim to have executed, a multi-seed PPO pendulum comparison with explicit success and failure criteria.
\item \textbf{Golf Interpretation:} A simulated policy uses less commanded torque near impact after a stronger early action. List the retained states and power flows needed to interpret that result. Design a matched counterfactual and identify which conclusions would still require independent human measurements.
\end{enumerate}
