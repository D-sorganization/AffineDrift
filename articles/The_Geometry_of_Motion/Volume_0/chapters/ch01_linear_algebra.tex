...
Given a square matrix $\mathbf{M} \in \Reals^{n \times n}$, a nonzero vector $\mathbf{v} \in \Complex^n$ (or $\Reals^n$ when $\lambda \in \Reals$) is called an \emph{eigenvector} of $\mathbf{M}$ if there exists a scalar $\lambda \in \Complex$ such that:
\begin{equation}
