...
\begin{tikzpicture}[scale=1.2]
  ...
  \draw[->,blue,thick,dashed] (0,0) -- (1.6,2.4) node[anchor=south west] {$\mathbf{A}\mathbf{v}$ (rotated \& stretched)};
  ...
  \draw[->,green,thick] (0,0) -- (0,1.2) node[anchor=west] {$\mathbf{e}_2$};
  \draw[->,green,thick,dashed] (0,0) -- (0,0.6) node[anchor=west] {$\lambda_2 \mathbf{e}_2$};

  \node at (0,-2.2) {Eigenvectors are only scaled, not rotated};
\end{tikzpicture}
\caption{Geometric interpretation of eigenvectors: they are special directions that are only stretched or shrunk by a matrix, never rotated. Dashed arrows show $\mathbf{A}\mathbf{v}$ for various vectors $\mathbf{v}$.}
...
Imagine a spring that is stiff in one direction but compliant in
another---say, a car suspension that resists vertical bumps but
allows lateral flex.  The relationship ``sense displacement in
direction $\mathbf{b}$, produce force along direction $\mathbf{a}$''
is exactly a dyad $\mathbf{a}\,\mathbf{b}^{T}$.

Think of it as a \emph{cause-and-effect pair}: the vector $\mathbf{b}$
is the ``listening direction'' (what the spring feels) and
$\mathbf{a}$ is the ``acting direction'' (how the spring pushes
back). A single dyad can only do one such pairing, which is why
its matrix has rank one---it has only one independent
cause-to-effect channel.
\end{laymansbox}
...
In structural mechanics, the stiffness matrix $\mathbf{K}$ maps a
displacement vector $\mathbf{u}$ to a force vector $\mathbf{f}$:
$\mathbf{f}=\mathbf{K}\,\mathbf{u}$.  For a truss with $N$ members,
each member contributes a rank-1 dyad (stiffness along its axis),
and the global stiffness matrix is the sum:
\[
  \mathbf{K}
  \;=\;
  \sum_{e=1}^{N} k_{e}\,\hat{\mathbf{n}}_{e}\,\hat{\mathbf{n}}_{e}^{T},
\]
a dyadic built from the member directions $\hat{\mathbf{n}}_{e}$
and axial stiffnesses $k_{e}$.
...
\begin{itemize}
  \item \textbf{Screw axes and wrenches (Chapter~5).}
    ...
    object.  The mapping from a twist (generalised velocity) to a wrench
    (generalised force) is a $6\times6$ dyadic---the \emph{spatial
    inertia}.

  \item \textbf{Spatial algebra (Chapter~8).}
    The $6\times6$ spatial inertia matrix is a dyadic on the space of
    twists.  Its coordinate-free formulation uses the same
    $m(\norm{\mathbf{r}}^{2}\,\mathbf{1}-\mathbf{r}\,\mathbf{r}^{T})$
    building block we saw in
    Example~\ref{ex:point_mass_inertia}, extended to six dimensions.
  ...
  \item \textbf{The mass matrix (Chapter~11).}
    ...
    inertias---a sum of dyads, assembled exactly as we sum member
    stiffnesses in a truss.
...
the manipulator equation of motion (Chapter~11) is always symmetric
positive definite.  Therefore the Cholesky decomposition is the
preferred solver for the forward-dynamics equation
$\mathbf{M}\ddot{\mathbf{q}}=\boldsymbol{\tau}-
\mathbf{C}\dot{\mathbf{q}}-\mathbf{g}$~\cite{nocedal2006}.
\end{principle}
