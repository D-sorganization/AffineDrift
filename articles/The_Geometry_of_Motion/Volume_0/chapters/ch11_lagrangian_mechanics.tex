...
\textbf{Hamilton's Principle} states that among all conceivable paths $\mathbf{q}(t)$ connecting two configurations $\mathbf{q}(t_1)$ and $\mathbf{q}(t_2)$ at fixed times, the \emph{actual} physical path satisfies:
...
This yields the \textbf{standard form}:
...
Thus total energy is conserved. This is the mechanical manifestation of Noether's theorem (Section \ref{sec:noether}).
...
\textbf{Emmy Noether's Theorem} (1918) establishes a deep connection between symmetries of the Lagrangian and conservation laws in the equations of motion.
...
An alternative derivation of Lagrangian mechanics begins with \textbf{D'Alembert's Principle}, which unifies constraints and dynamics.
...
The mass matrix $\mathbf{M}(\mathbf{q})$ defines a Riemannian metric on configuration space. The Christoffel symbols $\Gamma^i_{jk}$ of this metric are:
