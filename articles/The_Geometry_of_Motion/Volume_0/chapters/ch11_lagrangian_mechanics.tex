...
\section{What the Lagrangian Formulation Assumes}
\label{sec:lagrangian_scope}
\label{laymans:energy_intuition}
...
\label{eq:lagrangian_body_energy}
\end{equation}
...
\section{Virtual Work and the Role of Constraints}
\label{sec:lagrangian_virtual_work}
...
\label{eq:lagrangian_generalized_force}
\end{equation}
...
\section{Hamilton's Principle and Variation}
\label{sec:lagrangian_action}
...
\label{eq:lagrangian_forced_variation}
\end{equation}
...
For a point-mass pendulum with a massless rigid rod, angle $\theta$ measured counterclockwise from downward vertical, $T=m\ell^2\dot\theta^2/2$ and $V=mg\ell(1-\cos\theta)$. Equation~\ref{eq:el_with_torque} gives $m\ell^2\ddot\theta+mg\ell\sin\theta=u$. Tension does no work in this fixed-length, fixed-pivot model, but its force remains physically necessary. The bob's tangential speed is $\ell|\dot\theta|$, not $\dot\theta$ in metres per second.

\begin{figure}[htbp]
\centering
\includegraphics[width=.85\linewidth]{../figures/lagrangian_pendulum.pdf}
\caption{Point-Mass Pendulum With a Fixed Pivot. Tangential Velocity Depends on Length and Angular Rate; the Equation Also Requires Initial State and Applied Torque.}
\label{fig:lagrangian_pendulum}
\end{figure}

\section{Deriving the Manipulator Equations}
\label{sec:lagrangian_manipulator}
...
\label{eq:lagrangian_momentum_rate}
\end{equation}
...
\label{eq:lagrangian_kinetic_gradient}
\end{equation}
...
\end{aligned}
\label{eq:lagrangian_christoffel_first}
\end{equation}
...
\label{eq:lagrangian_mass_pullback}
\end{equation}
...
\subsection{The Skew Identity and Nonunique Factors}
\label{subsec:lagrangian_coriolis}

For the Christoffel factor in Equation~\ref{eq:lagrangian_christoffel_first}, symmetry of $M$ gives
\begin{equation}
(\dot M-2C)_{ij}
=\sum_k(\partial_iM_{jk}-\partial_jM_{ik})v_k.
\label{eq:lagrangian_skew}
\end{equation}
...
\subsection{Gravity Sign and Energy Units}
\label{subsec:lagrangian_gravity}
...
\section{Energy Balance Before Energy Conservation}
\label{sec:lagrangian_energy}
...
&=v^TQ-\mathcal L_t.
\end{aligned}
\label{eq:lagrangian_energy_balance}
\end{equation}
...
\section{Hamiltonian Mechanics and Phase Space}
\label{sec:lagrangian_hamiltonian}
...
\section{Noether's Theorem and Momentum Balance}
\label{sec:noether}
...
\subsection{Proof for a Configuration Symmetry}
\label{sec:noether_proof}
\index{Noether's theorem!rigorous proof}
\index{diffeomorphism}
\index{infinitesimal generator}
\index{Noether charge}
...
\subsection{Which Quantity Is Actually Conserved?}
\label{subsec:lagrangian_symmetry_examples}
\label{laymans:noether_practice}
...
\section{Constrained Lagrangian Systems}
\label{sec:lagrangian_constraints}
...
\label{eq:lagrangian_constraint_bias}
\end{equation}
...
\label{eq:lagrangian_constraint_solve}
\end{equation}
...
\label{eq:lagrangian_constraint_power}
\end{equation}
...
\section{The Kinetic Metric and Geodesic Motion}
...
\label{eq:lagrangian_christoffel_second}
\end{equation}
...
\section{Worked Example 1: Two Uniform Pendulum Rods}
\label{sec:lagrangian_double_pendulum}

\subsection{Geometry and Distributed Kinetic Energy}
\label{subsec:lagrangian_rods_geometry}
...
\label{eq:lagrangian_rods_velocity}
\end{equation}
Each rod also has center rotational inertia $I_{C_i}=m_i\ell_i^2/12$. Add both terms from Equation~\ref{eq:lagrangian_body_energy}; treating each rod as a point at its center would be a different physical model. With $\Delta=\alpha_1-\alpha_2$ and $k=m_2\ell_1\ell_2/2$,
\begin{equation}
M_\alpha=
\begin{pmatrix}
...
\end{pmatrix}.
\end{aligned}
\label{eq:lagrangian_rods_potential}
\end{equation}
...
\label{eq:lagrangian_rods_motor_power}
\end{equation}
...
\section{Worked Example 2: A Two-Link Robot Arm}
\label{sec:lagrangian_relative_arm}
...
\end{pmatrix}.
\label{eq:lagrangian_arm_jacobian}
\end{equation}
...
\end{aligned}
\label{eq:lagrangian_arm_gravity}
\end{equation}
...
\section{Lagrangian Equations and Recursive Computation}
\label{sec:lagrangian_algorithms}
...
\subsection{Symplectic Euler for a General Hamiltonian}
\label{subsec:lagrangian_symplectic_euler}

One symplectic Euler variant is
\begin{equation}
p_{n+1}=p_n-hH_q(q_n,p_{n+1}),
\label{eq:symp_euler_p}
\end{equation}
\begin{equation}
q_{n+1}=q_n+hH_p(q_n,p_{n+1}).
\label{eq:symp_euler_q}
\end{equation}
...
\label{eq:lagrangian_variable_mass_step}
\end{equation}
...
\subsection{Separable Verlet and Step-Size Restrictions}
\label{subsec:lagrangian_verlet}

For a separable $H(q,p)=K(p)+V(q)$, compose exact kick and drift substeps:
\begin{equation}
p_{n+1/2}=p_n-\tfrac h2\nabla V(q_n),
\label{eq:verlet_half_p}
\end{equation}
\begin{equation}
q_{n+1}=q_n+h\nabla K(p_{n+1/2}),
\label{eq:verlet_q}
...
\label{eq:lagrangian_oscillator_step}
\end{equation}
...
\label{eq:lagrangian_motor_work}
\end{equation} Comparing that analytic work with sampled energy change checks the force-coordinate pairing. For zero torque the same residual becomes energy error. Inspect the maximum error over the interval, not merely the difference between two endpoints; repeat at tighter tolerances before interpreting fine features.
...
\section{Golf and Humanoid Modeling}
\label{sec:lagrangian_golf}
...
\label{sec:lagrangian_reading}

Goldstein, Poole and Safko \cite{Goldstein2002} and Arnold \cite{Arnold1989} treat classical variational and Hamiltonian mechanics. Marsden and Ratiu \cite{marsden1999} develop symmetry, momentum maps and reduction; Murray, Li and Sastry \cite{Murray1994} connect geometric mechanics with robotics.

For accessible primary supplements, Tedrake's multibody notes discuss manipulator equations, force maps and velocity representations \cite{TedrakeMultibodySupplement}; Hairer's numerical-analysis review gives the implicit symplectic Euler formulas and qualified long-time energy reasoning \cite{HairerChallengesSupplement}. Featherstone \cite{Featherstone2008} treats recursive computation, while Siciliano et al.\ and Spong et al.\ develop robot dynamics and control applications \cite{siciliano2009,Spong2006}. The worked formulas, counterexamples and numerical checks in this chapter are derived explicitly so that a citation is not asked to substitute for their verification.

\section{Exercises}
\label{sec:lagrangian_exercises}

\begin{enumerate}
\item\label{ex:lagrangian_1} For $\mathcal S=\int_0^T(q^2+\dot q^2)\,dt$, $T>0$, with $q(0)=0$ and $q(T)=1$, derive $\ddot q-q=0$ and solve for $q(t)$. Compute the second variation and determine whether the stationary path is a strict minimum.

\item\label{ex:lagrangian_2} Derive the point-mass pendulum equation from the stated downward-vertical convention. Add a motor torque $u$ and viscous hinge damping $-b\dot\theta$, with $b\geq0$, and verify $\dot E=u\dot\theta-b\dot\theta^2$.

\item\label{ex:lagrangian_3} Starting from $\mathcal L=m[\dot r^2+r^2(\dot\theta+\Omega)^2]/2$ for a free planar particle in a frame rotating at constant $\Omega$, identify the cyclic momentum. Relate it to inertial angular momentum. Then add a specified table friction torque and derive the new momentum balance.

\item\label{ex:lagrangian_4} Consider a three-link horizontal planar arm with a fixed base, no gravity in its plane and no applied torques. Use relative joint angles and rotational invariance to identify a cyclic coordinate and its conserved momentum. Explain why the other two generalized momenta are not generally conserved. State what changes when gravity acts in the motion plane.

\item\label{ex:lagrangian_5} Prove symmetry from the kinetic Hessian and positive-definiteness from an injective aggregate velocity map with positive body kinetic forms. Give a massless-coordinate or redundant-coordinate example for which nonnegative physical kinetic energy does not imply a positive-definite coordinate matrix.

\item\label{ex:lagrangian_6} Verify $\dot M-2C$, including the time derivative, for both two-rod coordinate descriptions. Construct a nonzero skew addition $K(v)$ in three coordinates with $Kv=0$ to show that the skew-preserving factor is not unique.

\item\label{ex:lagrangian_7} Derive the complete energy of the uniform-rod pendulum and check its balance for arbitrary initial velocities. Explain why release from rest is not required for conservation in the autonomous unforced model. Compare the kinetic quadratic form with independent integration along the rods.

\item\label{ex:lagrangian_8} For $\mathcal L=m\dot x^2/2-kx^2$ with $m,k>0$, compute $p$, $H$ and Hamilton's equations. Identify the oscillator frequency and distinguish the coefficient $k$ in this potential from the conventional spring constant multiplying $x^2/2$.

\item\label{ex:lagrangian_9} A bead of mass $m$ slides frictionlessly on the stationary parabola $y=ax^2$, with downward gravity. Derive the equation using $\delta y=2ax\delta x$ and independently using the reduced Lagrangian. Identify the effective mass, velocity bias and potential gradient.

\item\label{ex:lagrangian_10} A particle moves on $r^Tr=R^2$ in a specified applied force $F$. Set reaction $2\lambda r$, derive $\lambda$ from acceleration consistency, and explain the required initial position/velocity constraints. Verify that the reaction does no power on the stationary sphere.

\item\label{ex:lagrangian_11} Re-derive the relative-coordinate uniform-rod arm's $M,C,G$ directly from center translation and center rotation. Compare with the constant transformation from absolute coordinates. Verify the distal squared-length contribution to the off-diagonal mass entry.

\item\label{ex:lagrangian_12} Put viscous dampers at the physical base and elbow hinges. Their torques are $u_1=-b_1\dot\alpha_1$ and $u_2=-b_2(\dot\alpha_2-\dot\alpha_1)$. Find the generalized damping matrix in absolute coordinates and the energy dissipation rate. Specify when the rate is strictly negative.

\item\label{ex:lagrangian_13} Compute first- and second-kind Christoffel symbols for the polar-plane kinetic metric $M=\operatorname{diag}(1,r^2)$, $r>0$. Derive the free-motion equations and verify that a straight Cartesian path satisfies them. Explain why nonzero connection coefficients do not establish curvature or geodesic deviation.

\item\label{ex:lagrangian_14} For an unconstrained particle in uniform vertical gravity, identify horizontal translation, vertical-axis rotation and time-translation symmetries. Derive the corresponding conserved quantities and the nonzero rate of vertical momentum. State the external-force assumptions.

\item\label{ex:lagrangian_15} Set the point-pendulum potential to zero at the bottom. Find the top's potential barrier and the bottom speed with that energy. For an unforced lossless pendulum, distinguish asymptotic arrival at the unstable top from finite-time crossing. Explain how active control, final velocity and losses change a swing-up requirement.

\item\label{ex:lagrangian_16} Implement the relative-coordinate arm and compare it with the absolute-coordinate example under identical physical torques and initial conditions. Check sampled energy-minus-work residuals at two tolerances. Independently differentiate the variable-mass symplectic map and its old-momentum alternative to test area preservation; do not use energy error alone as a test of symplecticity.
\end{enumerate}
