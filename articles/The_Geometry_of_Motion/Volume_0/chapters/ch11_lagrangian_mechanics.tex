...
\begin{tikzpicture}[scale=1.1]
  ...
  \draw[->] (0,2) arc(-90:-60:1) node[below,xshift=0.3cm] {$\theta$};
...
% ==========================================================================
\section{Derivation of the Standard Form: \texorpdfstring{$\mathbf{M}\ddot{\mathbf{q}} + \mathbf{C}\dot{\mathbf{q}} + \mathbf{G} = \boldsymbol{\tau}$}{M qddot + C qdot + G = tau}}
% ==========================================================================
...
\subsection{The Mass Matrix \texorpdfstring{$\mathbf{M}(\mathbf{q})$}{M(q)}}
...
\subsection{The Coriolis/Centripetal Matrix \texorpdfstring{$\mathbf{C}(\mathbf{q}, \dot{\mathbf{q}})$}{C(q, qdot)}}
...
\textbf{Theorem:} \emph{If $\mathbf{C}$ is constructed from the Christoffel symbols of the second kind} (Section~\ref{sec:christoffel}), then the matrix $\dot{\mathbf{M}} - 2\mathbf{C}$ is \textbf{skew-symmetric}, i.e., $(\dot{\mathbf{M}} - 2\mathbf{C})^T = -(\dot{\mathbf{M}} - 2\mathbf{C})$.

\textbf{The hypothesis is not decorative.} The factorisation of the Coriolis and centrifugal terms into a matrix $\mathbf{C}$ is not unique: any $\mathbf{C}'$ satisfying $\mathbf{C}'\dot{\mathbf{q}} = \mathbf{C}\dot{\mathbf{q}}$ gives the same equations of motion. Only the Christoffel construction makes $\dot{\mathbf{M}} - 2\mathbf{C}$ skew-symmetric as a \emph{matrix identity}; for other valid choices the property generally fails. Passivity- and adaptive-control designs that invoke skew-symmetry therefore require the Christoffel form specifically.

\textbf{Proof:} The kinetic energy is $T = \frac{1}{2}\dot{\mathbf{q}}^T \mathbf{M}\dot{\mathbf{q}}$. Differentiating with respect to time:
...
for all trajectories $\dot{\mathbf{q}}$. This step alone is not enough: $\mathbf{C}$ is itself a function of $\dot{\mathbf{q}}$, so the vanishing of the quadratic form along trajectories does not by itself let us vary $\dot{\mathbf{q}}$ freely and conclude skew-symmetry. It is the Christoffel construction that supplies the missing ingredient, giving $\dot{M}_{kj} - 2C_{kj} = \sum_i \left(\partial M_{ij}/\partial q_k - \partial M_{kj}/\partial q_i\right)\dot{q}_i$, which is manifestly antisymmetric in $k \leftrightarrow j$. With that in hand, the matrix $\dot{\mathbf{M}} - 2\mathbf{C}$ must be skew-symmetric:
\[(\dot{\mathbf{M}} - 2\mathbf{C}) + (\dot{\mathbf{M}} - 2\mathbf{C})^T = \mathbf{0}.\]
This is the celebrated \textbf{skew-symmetry property} of Lagrangian dynamics, which plays a central role in passivity-based and adaptive control design.

\subsection{The Gravity Vector \texorpdfstring{$\mathbf{G}(\mathbf{q})$}{G(q)}}
...
Since the $\delta q_i$ are arbitrary (independent), each coefficient must vanish, recovering the Euler-Lagrange equations. This approach shows directly that Lagrangian mechanics is a direct consequence of Newton's laws when viewed through the lens of generalized coordinates and constraint-respecting displacements.

% ==========================================================================
\section{Christoffel Symbols and the Geometry of \texorpdfstring{$\mathbf{M}$}{M}}
...
Read the entries directly off the quadratic form: the diagonal entries are the coefficients of $\dot{q}_i^2$ inside the bracket, while the off-diagonal entry is \emph{half} the coefficient of the cross term $\dot{q}_1\dot{q}_2$, because that term appears twice in $\dot{\mathbf{q}}^T\mathbf{M}\dot{\mathbf{q}}$.
...
\textbf{Consistency check.} With this $\mathbf{M}$ and this Christoffel $\mathbf{C}$,
...
\begin{enumerate}
    ...
    \item \textbf{Standard Form}: $\mathbf{M}(\mathbf{q})\ddot{\mathbf{q}} + \mathbf{C}(\mathbf{q}, \dot{\mathbf{q}})\dot{\mathbf{q}} + \mathbf{G}(\mathbf{q}) = \boldsymbol{\tau}$ cleanly separates inertia, Coriolis, and gravity effects.
