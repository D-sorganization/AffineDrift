...
Moreover, constraint forces do \emph{no work} in the motion. The tension in a rigid pendulum rod redirects motion but adds no energy. Yet in Newton's framework, we must solve for it anyway. This is unnecessarily cumbersome.
...
The standard proof uses energy conservation more directly. The key fact is:
...
\textbf{Control Design} & Direct (LQR, feedback linearization) & Cumbersome (requires explicit $\mathbf{M}, \mathbf{C}$) \\
