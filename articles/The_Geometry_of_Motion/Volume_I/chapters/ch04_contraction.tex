...
The concept of contraction---trajectories converging robustly---is fundamentally dual to the concept of 
controllability in Agrachev and Sachkov's geometric framework \cite[Ch.~4]{AgrachevSachkov2004}. While 
controllability asks \emph{what is reachable} from a given initial condition via forward-time controls, 
contraction analysis asks \emph{do trajectories converge} and thus provides a certificate of robustness 
and structural stability. Both are geometric properties: controllability relies on Lie bracket accessibility 
in the forward direction, while contraction provides a metric-based certificate of asymptotic behavior.
...
The contraction metric $M(\x)$ acts as a Riemannian metric on the state space, defining distance and 
angle between perturbations in the tangent space. As Agrachev and Sachkov discuss \cite[Ch.~8]{AgrachevSachkov2004}, 
Riemannian geometry is central to sub-Riemannian optimal control problems, where the metric constrains which 
directions are directly accessible via controls. Contraction analysis uses a metric to measure convergence rates
in this geometric sense---a local ruler that changes from point to point, naturally extending linear 
stability theory to nonlinear, curved state spaces.
...
The contraction condition $\mat{A}^\T\mat{M} + \mat{M}\mat{A} \preceq -2\lambda\mat{M}$ is a
\emph{matrix inequality} (i.e., an eigenvalue condition), not an entry-wise condition.
For $\mat{M} = \mat{I}$, we need the eigenvalues of $\mat{A}^\T + \mat{A}$ to satisfy
$\lambda_{\max}(\mat{A}^\T + \mat{A}) \leq -2\lambda$. The eigenvalues of
$\mat{A}^\T + \mat{A} = \begin{bmatrix} -4 & 1 \\ 1 & -6 \end{bmatrix}$
are approximately $-1.79$ and $-3.21$ (computed in the symmetric part analysis above), both negative.
Therefore the identity metric \emph{does} certify contraction, with rate
$\lambda = |\lambda_{\max}(\sym(\mat{A}))| / 2 \approx 1.79 / 2 \approx 0.895$
(since the condition requires $\lambda_{\max} \leq -2\lambda$).
...
Partition $\x = [\x_1; \x_2]$ with dimensions $n_1$ and $n_2$, and write the generalized
Jacobian in the block form
\begin{equation}
\mat{F} = \begin{bmatrix} \mat{F}_{11} & \mat{F}_{12} \\ 0 & \mat{F}_{22}\end{bmatrix},
\label{eq:ch4:cascade-jacobian}
\end{equation}
so that the $\x_2$-dynamics do not depend on $\x_1$ (a \emph{cascade}). Suppose:
\begin{enumerate}
...
\item \textbf{The coupling is bounded}: there is a constant $k$ with
      $\norm{\mat{M}_1^{1/2}\mat{F}_{12}\mat{M}_2^{-1/2}} \leq k$ uniformly.
\end{enumerate}
Then for every $\lambda < \min(\lambda_1,\lambda_2)$ there exists a weight $\theta > 0$,
depending on $k$, such that the full system is contracting with rate $\lambda$ in the
\emph{weighted} block-diagonal metric
\begin{equation}
\mat{M}_\theta = \begin{bmatrix} \mat{M}_1 & 0 \\ 0 & \theta\,\mat{M}_2 \end{bmatrix}.
\label{eq:ch4:block-metric}
\end{equation}
\end{theorem}

\begin{proof}[Proof Sketch]
In the metric $\mat{M}_\theta$ the symmetric part of the transformed Jacobian is block
$\begin{psmallmatrix}-\lambda_1 & \tfrac{1}{2}\theta^{-1/2}k \\ \tfrac{1}{2}\theta^{-1/2}k & -\lambda_2\end{psmallmatrix}$
in the two-block norm. Its largest eigenvalue is negative once
$\theta^{-1}k^2 < 4\lambda_1\lambda_2$, which any sufficiently large $\theta$ achieves.
Weighting the second block down is what absorbs the coupling; this is the standard
small-gain argument, and it is the step that fails if $\theta$ is fixed at $1$.
\end{proof}

\begin{warningbox}
The weight $\theta$ is not cosmetic, and hypothesis~3 is not free. In the
\emph{unweighted} metric ($\theta = 1$) the theorem is false. Take
\begin{equation}
\dot x_1 = -x_1 + K x_2, \qquad \dot x_2 = -x_2 .
\label{eq:ch4:cascade-counterexample}
\end{equation}
Each subsystem contracts at rate $1$ in the identity metric, so hypotheses 1 and 2 hold.
But the symmetric part of the full Jacobian is
$\begin{psmallmatrix}-1 & K/2\\ K/2 & -1\end{psmallmatrix}$, whose eigenvalues are
$-1 \pm K/2$. For $K \geq 2$ the larger one is non-negative and the full system is
\emph{not} contracting in the identity metric --- for any $K$, however large, even though
both subsystems contract at rate $1$.

The system is of course still exponentially stable; a cascade of stable systems always is.
What fails is contraction \emph{in that particular metric}. Contraction is a statement
about a metric, not about a trajectory, and a property that holds blockwise need not
survive assembly unless the blocks are weighted against the coupling.
\end{warningbox}

\begin{example}[Cascade: Velocity Tracking Driving Position Regulation]
Consider
\begin{equation}
\dot{q} = -\alpha q + v, \qquad \dot{v} = -\gamma(v - u(t)),
\label{eq:ch4:mech-example}
\end{equation}
with $\alpha,\gamma > 0$, where $u(t)$ is a time-varying reference.

The Jacobian is $\mat{A} = \begin{psmallmatrix} -\alpha & 1 \\ 0 & -\gamma \end{psmallmatrix}$.
The $v$-subsystem contracts at rate $\gamma$; the $q$-subsystem, driven by $v(t)$, contracts
at rate $\alpha$; the coupling constant is $k = 1$. By the theorem the cascade contracts at
any rate $\lambda < \min(\alpha,\gamma)$ in $\mat{M}_\theta$ with
$\theta > 1/(4\alpha\gamma)$, and all solutions synchronize to the reference motion.

Note what happens if the position feedback is removed ($\alpha = 0$), giving the pure
integrator $\dot q = v$: the Jacobian has an eigenvalue at $0$, the $q$-subsystem is only
marginally stable rather than contracting, and hypothesis~2 fails. A displacement in $q$ is
then never forgotten --- two trajectories with the same velocity history but different
initial positions stay a constant distance apart forever. No metric can certify contraction
for that system, which is why $\alpha > 0$ is required rather than convenient.
\end{example}
...
Contraction metrics provide a natural dual certificate to control distributions and accessibility. 
Agrachev and Sachkov \cite[Part~III]{AgrachevSachkov2004} analyze feedback stabilization and regulation 
through the lens of controlled accessibility: a system is stabilizable if the drift-controlled vector field 
admits appropriate accessibility properties. Control contraction metrics encode this geometric stabilization 
property via a metric that contracts under feedback, providing a constructive and verifiable stability certificate.
...
in detail in Chapter~\ref{ch:optimal-control}. Both frameworks certify stability and convergence,
...
\begin{remark}[Why the Van der Pol oscillator is not globally contracting]
The Van der Pol oscillator possesses a stable limit cycle, toward which all nonzero
initial conditions converge. This means the system is \emph{not} globally contracting:
global contraction would require all trajectories to converge to each other (and hence
to a single trajectory), but the limit cycle is an attractor of dimension one, not zero.
Instead, the system exhibits \emph{partial} contraction---trajectories converge to the
limit cycle (contraction transverse to the cycle) while maintaining a phase offset along it.
A state-dependent metric can certify contraction in the transverse direction, but no
globally valid metric can certify contraction in all directions simultaneously.
\end{remark}
...
It is tempting to now pick numbers --- say $\lambda = 0.3$, $\epsilon = 0.5$, $\gamma
\approx 1.5$ --- and present
\begin{equation}
\mat{M}(x_1) = \begin{bmatrix} 1 & 0 \\ 0 & 1.2(1+1.5x_1^2) \end{bmatrix}
\label{eq:ch4:vdp-metric-solution}
\end{equation}
as a metric certifying global contraction. \emph{No such metric exists}, and this one is
not close. Evaluating the condition it is supposed to satisfy,
\begin{equation}
\mat{A}^\T\mat{M} + \mat{M}\mat{A} + \dot{\mat{M}} + 2\lambda\mat{M} \preceq 0,
\label{eq:ch4:vdp-check}
\end{equation}
on a grid over $[-3,3]^2$ gives a largest eigenvalue that is \emph{positive at every single
sampled point}, reaching $+1.95$ at the origin and growing past $+130$ near the corners. The
condition is violated everywhere, not marginally or in a small region.

That is exactly what the remark above predicts. The Van der Pol oscillator has an attracting
limit cycle, so two trajectories starting at different phases along that cycle stay
separated forever; no metric can make them converge. Searching for a global metric was never
going to succeed, and a numerical solver that appears to return one has either been given a
relaxed constraint set or has converged to an infeasible point.

The honest conclusion is the one the structure of the problem forces: the search must be
restricted to \emph{transverse} contraction --- convergence to the limit cycle rather than
to a single trajectory --- which requires a metric defined on directions transverse to the
flow, and a correspondingly weaker claim.

\begin{intuitionbox}
The ansatz was not a bad idea --- weighting the velocity direction more heavily where the
nonlinearity is strongest is exactly how a state-dependent metric buys contraction that a
constant one cannot. It fails here because the obstruction is topological rather than
quantitative: an attracting limit cycle means neighbouring trajectories converge \emph{to
the cycle} but not \emph{to each other}, and no amount of reweighting changes that.

The lesson generalises. When a metric search will not close, the first question is whether
the system can possibly be contracting --- a limit cycle, multiple equilibria, or any
invariant set of positive dimension rules it out immediately. Only then is it worth blaming
the ansatz or the solver.
\end{intuitionbox}
...
\item \textbf{Limits of numerical metric synthesis}: The Van der Pol oscillator
      (Sec.~\ref{sec:ch4:vdp}) shows how a parameterized metric search is set up --- and
      why it cannot succeed here. An attracting limit cycle rules out global contraction
      outright, so a returned ``solution'' must be checked against the condition it claims
      to satisfy rather than trusted.
