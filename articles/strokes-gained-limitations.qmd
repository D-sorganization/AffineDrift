...
description: "Statistical and methodological limitations of strokes gained analysis for individual golfer assessment, with recommendations for proper interpretation."
author: "Dieter Olson"
date: "2025-11-28"
format:
  html:
...
Strokes gained has transformed golf analytics by decomposing scores into contributions from different parts of the game relative to a benchmark player. However, the same feature that makes strokes gained powerful—the use of a common population baseline—also limits its interpretability for *individual* performance changes and practice priorities.

This article shows, using a formal statistical and mathematical framework, why “if I improve my strokes gained putting by 0.5 per round, I’ll score 0.5 shots better per round” is generally false. Strokes gained is defined with respect to an *average* player’s expected performance surface, not the individual golfer’s. A player’s skill profile may differ from the benchmark: unusually poor from short range, say, or unusually strong with wedges. For that player, the mapping from a mechanical or tactical change—leaving putts closer, for instance—to actual scoring impact can diverge substantially from what the strokes-gained tables suggest.
...
- The difference minus the one stroke you played is how many strokes you gained (positive) or lost (negative) relative to the benchmark.
...
The next section makes that mismatch precise and shows how it distorts the interpretation of strokes-gained improvements for individuals.
...
If a training intervention or strategy change causes the player to leave their first putt, on average, $\Delta x < 0$ meters closer to the hole, then (for small $\Delta x$) the change in the benchmark expectation is approximately
...
Suppose from some short-putt distance $x$ (say 1–3 meters), the reference population has one-putt probability $p_{\text{ref}}(x)$, so that:
...
So the local value (per meter) of getting closer is controlled by how fast make probability increases as you move in.
...
If Golfer A is substantially better from long irons than the reference player, and Golfer B is worse, the actual marginal value of moving both of them 10 yards closer is not the same as the benchmark calculation. The reference curve is computing something like an **average treatment effect** (ATE) of distance across the population; the coach cares about a **conditional individual treatment effect** (CITE) for the specific golfer.
...
A deeper statistical issue, formalized in ergodicity economics by Ole Peters [-@peters2019ergodicity], is the confusion between **ensemble averages** and **time averages**.
...
## 8. Behavioral Limits: Hidden States and Risk

Beyond the statistical aggregation issues, the underlying control theory of strokes gained makes two behavioral assumptions that are frequently violated in competitive golf.

### 8.1 Non-Markovian Hidden States

The definition $J(d, c)$ assumes the state is fully observable from the ball's position. This ignores **hidden internal states** ($\theta_t$) such as psychological pressure, biomechanical fatigue, or "flow" ("the hot hand").

$$
J_{\text{real}}(d, c, \theta) \neq J_{\text{model}}(d, c)
$$

If a player is in a high-anxiety state ($\theta_{\text{stress}}$), their effective putting probability $p(x)$ drops. The standard model, seeing only $d$ and $c$, attributes the resulting missed putt to "poor execution" (negative strokes gained) rather than "state disadvantage." This matters because the remedy for a mechanical error is different from the remedy for a regulatory state failure.

### 8.2 Risk Sensitivity and Utility Functions

The Bellman equation for strokes gained minimizes the expected *number* of strokes:

$$
\min_{\mu} \mathbb{E}[ \text{Score} ]
$$

This assumes **risk neutrality**. However, real players maximize a utility function $U(\text{Score})$ that varies with tournament context:

-   **Risk Averse (Concave $U$):** A player protecting a lead may play away from flags to avoid double-bogeys, voluntarily accepting a lower *expected* score (worse strokes gained) to reduce *variance*.
-   **Risk Seeking (Convex $U$):** A player needing a birdie to make the cut may take aggressive lines that worsen their average score but maximize the probability of a 3.

Strokes gained penalizes both behaviors as "suboptimal," failing to capture the strategic rationality of the player's context.
...
For these uses, the population basis of $J_{\text{ref}}$ is a feature, not a bug.
...
That same machinery, however, is often used to infer the **individual causal impact** of changes in technique, practice, or strategy. Doing so leaves an important assumption unstated: that the individual golfer’s conditional expectation surface $J_i$ looks enough like the benchmark $J_{\text{ref}}$ that the gradients and local changes calculated from $J_{\text{ref}}$ are good proxies for those of $J_i$.
    ...
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          ...
          <h3>The "Good Enough" Heuristic</h3>
        </div>
        <p class="critic-argument">
          While mathematically inexact for individuals, the population baseline is a sufficiently robust heuristic for the vast majority of golfers. If a player loses 2 strokes putting against the field, they are almost certainly a bad putter relative to their other skills, regardless of their specific <i>J<sub>i</sub></i> gradient. The signal-to-noise ratio is high enough to be useful without needing perfect individual calibration.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> We agree that strokes gained is excellent for broad diagnosis (finding weaknesses). The error becomes critical when prescribing specific <em>interventions</em> (treatment). Knowing you are a bad putter is accurate; assuming that a 3-foot improvement in proximity will yield the standard table's stroke value is where the causal logic fails.
        </div>
      </div>
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          <span class="critic-label">Alternative View:</span>
          <h3>Sample Size Reality</h3>
        </div>
        <p class="critic-argument">
          Constructing a true individual value function <i>J<sub>i</sub></i> requires thousands of shots from varying lies and distances to achieve statistical significance. No amateur and few pros have this volume of data. The "flawed" population model <i>J<sub>ref</sub></i> is the only estimator robust enough to use in practice.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> This is a valid practical constraint. However, the solution is not to trust <i>J<sub>ref</sub></i> blindly, but to use Bayesian methods that treat <i>J<sub>ref</sub></i> as a <em>prior</em> and update it with the player's sparse data (shrinkage). This acknowledges the uncertainty rather than presenting the population average as a hard truth.
        </div>
      </div>
       ...
       <div class="critic-item">
        <div class="critic-perspective">
          <span class="critic-label">Alternative View:</span>
          <h3>Stratified Baselines Exist</h3>
        </div>
        <p class="critic-argument">
          Modern commercial tools (like Golfmetrics) allow for handicap-specific baselines (e.g., comparing against an 8-handicap benchmark rather than a Tour Pro). This effectively approximates <i>J<sub>i</sub></i> by grouping players of similar skill levels, mitigating the "average player" mismatch.
        </p>
        <div class="author-response">
          <strong>Our Response:</strong> Stratified baselines are a significant improvement, but they still average over a group. An "8-handicap Scrambler" and an "8-handicap Ball Striker" achieve the same score with vastly different skill profiles and value function gradients. A baseline averaged from both will still misprice shots for either individual.
        </div>
      </div>
...
- Broadie, M. (2014). *Every Shot Counts.* Gotham Books. [@broadie2014every]
- Peters, O. (2019). The ergodicity problem in economics. *Nature Physics*, 15, 1216–1221. [@peters2019ergodicity]
