...
This chapter introduces the language physicists use to describe motion: **coordinates**, **state**, and **state space**. It looks abstract at first, but it's essential. Once you can describe a swing precisely, you can predict what will happen next, diagnose what went wrong, and understand the forces that shaped the motion. Definitions of every term introduced in the book are collected in the [Glossary](glossary.html); the site-wide symbol conventions are maintained in [AffineDrift Notation and Conventions](/pages/notation-conventions.html).
...
Suppose the upper arm has length $L_1$ and the forearm+club has length $L_2$. The shoulder is at the origin, with $x$ horizontal (toward the target) and $y$ vertical (upward), so an arm hanging straight down places the elbow at $y = -L_1$. (Numerical values for $L_1$, $L_2$, masses, and moments of inertia are defined as the canonical parameter set in Chapter 3, Table 3.1; numerical examples in this chapter use symbolic parameters only.)
...
The space of all possible states. For an *unconstrained* $n$-degree-of-freedom system, phase space has $2n$ dimensions: $n$ for position and $n$ for velocity. For a constrained system---one with $k$ holonomic constraint equations of the form $\phi(\bm{q}) = 0$---the independent phase space is reduced to $2(n-k)$ dimensions, because the constraint fixes $k$ of the positions (and, by differentiation, their velocities). Constraint handling is developed in ch07; for most of this chapter we treat the double pendulum as unconstrained.
...
Imagine a plot with $\theta_s$ on the $x$-axis (from $-90°$ to $+150°$, illustrative model range) and $\theta_e$ on the $y$-axis (from $-80°$ to $+80°$, illustrative model range). These ranges are chosen to be representative for this toy model; actual anatomical ranges depend on individual mobility.
...
::: {.callout-tip}
## Constraints as Walls in Configuration Space
...
- **Range limits**: $\theta_s \in [-90°, 150°]$, $\theta_e \in [-80°, 80°]$ (illustrative for this model; realistic anatomical ranges vary by individual).
...
2. **Configuration Space**. Sketch the configuration space of a double pendulum. On the axes, put $\theta_s$ (shoulder, horizontal axis) from $-90°$ to $+150°$ and $\theta_e$ (elbow, vertical axis) from $-80°$ to $+80°$. Mark the regions that are reachable (physically possible) and unreachable. Then sketch a typical golf swing path in this space. (Note: the ranges used here are for the model only—realistic anatomical ranges depend on individual mobility.)

3. **Velocity Calculation**. A golfer's shoulder rotates at $\dot{\theta}_s = 500°/s = 8.7 \text{ rad/s}$ (an illustrative value used as a model input throughout this book). The upper arm has length $L_1 = 0.5$ m. What is the linear speed of the elbow? (Hint: $v = r \omega$.)
