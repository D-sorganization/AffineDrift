...
3. The number of variables $n$ equals the number of **degrees of freedom** (DOF; see [Degrees of Freedom and the Curse of Dimensionality](../../degrees-of-freedom-and-dimensionality.html) for a cross-cutting analysis of dimensionality reduction in human motor control).
...
For the golf swing, the natural choice is **joint angles** (see the [Mathematical Notation Reference](../../../pages/notation.html) for normative conventions across the site).
...
During the downswing, a golfer's shoulder might be rotating at $\dot{\theta}_s = 600 \text{ degrees/second} = 10.5 \text{ rad/s}$ [@Nesbit2005; @Hume2005] (illustrative; typical values vary across skill levels and swing styles).
...
For the double-pendulum model used below:
- Upper arm has length $L_1 = 0.5$ m and mass $M_1 = 2$ kg. Its center of mass is at distance $L_{1,\text{cm}} = 0.25$ m from the shoulder.
...
\bm{r}_{\text{club}} =
\begin{bmatrix}
L_1 \sin(\theta_s) + L_2 \sin(\theta_s + \theta_e) \\
-L_1 \cos(\theta_s) - L_2 \cos(\theta_s + \theta_e)
\end{bmatrix}
$$
...
\bm{r}_{\text{cm},1} =
\begin{bmatrix}
L_{1,\text{cm}} \sin(\theta_s) \\
-L_{1,\text{cm}} \cos(\theta_s)
\end{bmatrix}
$$
...
\bm{r}_{\text{cm},2} =
\begin{bmatrix}
L_1 \sin(\theta_s) + L_{2,\text{cm}} \sin(\theta_s + \theta_e) \\
-L_1 \cos(\theta_s) - L_{2,\text{cm}} \cos(\theta_s + \theta_e)
\end{bmatrix}
$$
...
$$
\dot{\bm{r}}_{\text{elbow}} =
\begin{bmatrix}
L_1 \cos(\theta_s) \dot{\theta}_s \\
L_1 \sin(\theta_s) \dot{\theta}_s
\end{bmatrix}
=
L_1 \dot{\theta}_s
\begin{bmatrix}
...
\dot{\bm{r}}_{\text{club}} =
\begin{bmatrix}
L_1 \cos(\theta_s) \dot{\theta}_s + L_2 \cos(\theta_s + \theta_e) (\dot{\theta}_s + \dot{\theta}_e) \\
L_1 \sin(\theta_s) \dot{\theta}_s + L_2 \sin(\theta_s + \theta_e) (\dot{\theta}_s + \dot{\theta}_e)
\end{bmatrix}
$$
...
- **[Rotation Representations Reference](../../rotation-representations-reference.html)** — SO(3) conventions for three-dimensional swing kinematics
- **[Lagrangian Mechanics Reference](../../lagrangian-reference.html)** — Energy-based equations of motion used from Chapter 3 onward
:::
