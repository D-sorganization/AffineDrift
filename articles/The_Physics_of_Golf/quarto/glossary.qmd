...
**Affine control system**: A system whose dynamics can be written as $\dot{\mathbf{x}} = f(\mathbf{x}) + G(\mathbf{x})\control$, where the control inputs enter linearly. Most mechanical systems, including the golf swing, have affine structure.
...
**Degrees of freedom**: The number of independent ways a system can move. A single rigid body in 3D space has 6 degrees of freedom (3 translational, 3 rotational). A human arm with shoulder, elbow, and wrist joints has about 9 degrees of freedom (a commonly cited approximation; exact counts depend on the joint model and which sub-axes are included).
<!-- TODO: citation needed for the ~9 DOF estimate for the human arm; exact counts vary by biomechanical model ; exact values depends on individual biomechanics and model assumptions -->
