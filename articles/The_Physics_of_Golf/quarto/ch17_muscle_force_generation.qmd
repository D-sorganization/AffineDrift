...
**Type I (slow-twitch, oxidative):** Low contraction speed (~50 ms to peak force), high fatigue resistance, moderate force production. Power primarily from aerobic metabolism. Used for endurance, posture, smooth sustained effort. About 40--50% of most muscles.

**Type IIa (fast-twitch oxidative):** Intermediate speed (~40 ms), high force production, moderate fatigue resistance. Oxidative capacity good. The *most powerful* fiber type. Used for athletic power movements. About 20--30% of muscles.

**Type IIx (fast-twitch glycolytic):** Fastest contraction (~20 ms), highest force production, rapid fatigue. Power from anaerobic glycolysis. Most powerful for brief bursts. About 5--15% of untrained muscles (training can increase this).
...
- $A_{\mathrm{PCSA}}$ = physiological cross-sectional area, $\approx \frac{M \cos\alpha}{\rho \ell^M_{\mathrm{opt}}}$ where $M$ is muscle mass and $\rho \approx 1.06$ g/cm$^3$; the $\cos\alpha$ factor accounts for the pennation angle
- $\sigma_{\max}$ = specific tension (force per unit area), typically 30--60 N/cm$^2$ for healthy muscle
- Maximum isometric force: $F_0 = \sigma_{\max} \times A_{\mathrm{PCSA}}$

For example, published cadaveric and MRI studies place the latissimus dorsi PCSA in the $\approx 25$--$40$ cm$^2$ range for adult males [@Zajac1989; @veeger1997parameters], with a central estimate of $\sim 30$ cm$^2$; that gives a maximum isometric force of $\sim 900$--1400 N at $\sigma_{\max} = 35$ N/cm$^2$. Individual values vary with body size, training history, and measurement method (anatomical cross-section vs. fiber-angle-corrected PCSA), so the chapter's $\sim 30$ cm$^2$ / $\sim 1000$ N figure is a representative midpoint rather than a fixed value.
...
For *eccentric contraction* (lengthening under load, e.g., a muscle applying brakes), Hill's equation is different and shows a counterintuitive result: force *increases* beyond $F_0$. The Thelen [-@thelen2003adjustment] form, which avoids the spurious singularity of the original Hill eccentric expression, is:
$$
f_V^{\mathrm{ecc}}(\tilde{v}) = \frac{(F_{\mathrm{asym}} - 1)\,|\tilde{v}|/V_{\max}}{|\tilde{v}|/V_{\max} + k_{\mathrm{ecc}}} + 1
$$
where $F_{\mathrm{asym}} \approx 1.4$--$1.5$ is the asymptotic eccentric force ceiling and $k_{\mathrm{ecc}} \approx 0.3$--$0.5$. Boundary checks: $f_V^{\mathrm{ecc}}(0) = 1$ (matches isometric) and $f_V^{\mathrm{ecc}} \to F_{\mathrm{asym}}$ as speed increases. This gives $f_V^{\mathrm{ecc}} > 1$ for increasing velocity, meaning eccentric contractions can produce 1.4--1.5 times the isometric force at high lengthening rates.
...
The velocity that matters for Hill's Law is the *muscle fiber* shortening velocity $\dot{\ell}^M$, not the clubhead speed. The chain is:
$$
v_{\mathrm{club}} \xrightarrow{\text{kinematic}} \dot{\theta}_{\mathrm{joint}} \xrightarrow{\times\, r} \dot{\ell}^M \xrightarrow{f_V} F_{\mathrm{muscle}}
$$
Fiber velocity equals joint angular velocity times the muscle's moment arm: $\dot{\ell}^M \approx r \dot{\theta}$. Because moment arms are small (1--3 cm at the wrist, 3--5 cm at the elbow), even very fast joint rotation produces only moderately fast fiber shortening.

**Drift (gravitational + inertial) forces:**

- Inertial (centrifugal, Coriolis) torques scale as $\approx m r^2 \dot{\theta}^2$ --- quadratic in angular velocity
- Gravitational torque $mg\, r \sin(\theta)$ is velocity-independent for a given configuration
- Peak at impact when clubhead speed $v_{\mathrm{club}} \approx 50$ m/s, wrist $\dot{\theta} \approx 70$ rad/s
- Drift torque at wrist: $\sim 50$--$100$ N$\cdot$m

**Muscle force capacity:**

- Scales as Hill's function $f_V(\tilde{v})$ where $\tilde{v} = \dot{\ell}^M / V_{\max}$
- At impact, wrist muscle shortening $\dot{\ell}^M \approx r \dot{\theta} \approx 0.02 \times 70 = 1.4$ m/s
- With $V_{\max} \approx 8$ m/s, normalized velocity $\tilde{v} \approx 0.18$
- From Eq. the hill concentric equation with $k = 0.4$: $f_V \approx 0.5$--$0.55$ --- roughly a 45--50% reduction from isometric
- For upstream joints with longer effective moment arms, $\tilde{v}$ can reach $0.4$--$0.5$, giving $f_V \approx 0.25$--$0.35$
- Peak controllable torque drops from $\sim 20$ N$\cdot$m isometric to $\sim 5$--$10$ N$\cdot$m at impact

**Result:** The drift-to-control ratio at impact is $\approx 5$--$10$:$1$, meaning inertial and gravitational forces dominate. The cause is not that muscles "run off the chart" of Hill's curve --- they sit squarely on the concentric branch --- but that Hill's curve cuts their force capacity roughly in half while the inertial torques simultaneously peak. The dominance is the product of these two effects.
:::
...
- Maximum isometric force: $F_0 \approx 50$ N (small muscle, 5 cm PCSA at 10 N/cm$^2$)
...
- $\tau_{\mathrm{act}} \approx 10$--$30$ ms is the activation time constant for a *mixed-fiber* muscle (rising phase). Fast fatigable (type IIx) fibers sit at the low end---often 5--15 ms in isolated preparations---while slow-twitch (type I) fibers are slower. The 10--30 ms band used here is a whole-muscle compromise for muscles like latissimus dorsi or pectoralis major that contain both fiber types.
- $\tau_{\mathrm{deact}} \approx 40$--$200$ ms is the deactivation time constant (falling phase), also fiber-type dependent.
...
$$
where $a(t)$ evolves according to the activation rise equation and the activation fall equation.
...
$$u_{\mathrm{neural}} \xrightarrow{\text{activation}} a(t) \xrightarrow{\times F_0 f_L f_V} F_{\mathrm{muscle}} \xrightarrow{\times r(\bm{q})\cos\alpha} \tau_{\mathrm{joint}}$$
...
Muscle biology sets the envelope of what is physically possible: maximum forces, activation speeds, and the velocity--force tradeoff. But there is one more force we have not yet accounted for---aerodynamic drag. The next chapter shows why drag is *not* negligible in the golf swing, and why omitting it leads to systematic errors in inverse dynamics calculations.

## Chapter Exercises

- **Force-Length Curve.** A muscle at optimal length ($\tilde{\ell} = 1.0$) produces maximum isometric force $F_0 = 200$ N. Using the active force length equation with $\gamma = 0.5$, compute $f_L(\tilde{\ell})$ at $\tilde{\ell} = 0.8, 1.0, 1.2$. Which configuration is strongest? Which produces zero force (theoretically)?

- **Hill's Equation.** The latissimus dorsi has $V_{\max} = 6$ m/s and $k = 0.4$. At a contraction velocity of $\dot{\ell}^M = 3$ m/s (mid-downswing pace), compute $f_V(\tilde{v})$ using the hill concentric equation. What fraction of maximum force is available?

- **Activation Dynamics.** A muscle with $\tau_{\mathrm{act}} = 25$ ms receives a step neural command $u_{\mathrm{neural}} = 1$ at $t = 0$. Solve the activation rise equation numerically (Euler method, $dt = 5$ ms) for $t = 0$ to $150$ ms. Sketch $a(t)$. At what time does activation reach 95% of steady-state?
