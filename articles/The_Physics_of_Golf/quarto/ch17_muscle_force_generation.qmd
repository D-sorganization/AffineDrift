...
- $A_{\mathrm{PCSA}}$ = physiological cross-sectional area, $A_{\mathrm{PCSA}} = \frac{M \cos\alpha}{\rho \ell^M_{\mathrm{opt}}}$ where $M$ is muscle mass and $\rho \approx 1.06$ g/cm$^3$. The $\cos\alpha$ factor projects force along the tendon axis; $F_0 = \sigma_{\max} A_{\mathrm{PCSA}}$ then gives tendon-direction force [@Zajac1989].
...
For *eccentric contraction* (lengthening under load, e.g., a muscle applying brakes), force *increases* beyond $F_0$. Following Thelen (2003) Eq. 4, the eccentric force-velocity relationship is:
$$
f_V^{\mathrm{ecc}}(\tilde{v}) = \frac{(F_{\mathrm{asym}}-1)(\tilde{v}/V_{\max})}{(\tilde{v}/V_{\max}+k_{\mathrm{ecc}})} + 1
{#eq:hill-eccentric}
$$
where $F_{\mathrm{asym}} \approx 1.4$ is the asymptotic eccentric force (as a fraction of $F_0$) and $k_{\mathrm{ecc}} \approx 0.25$. Note: $\tilde{v} > 0$ for lengthening (eccentric), $\tilde{v} < 0$ for shortening (concentric). Verification: at $\tilde{v}=0$: $f_V=1$ ✓; as $\tilde{v}/V_{\max}\to\infty$: $f_V\to F_{\mathrm{asym}}$ ✓ [@Thelen2003]. This formulation is physiologically realistic: eccentric contractions can produce approximately 1.2--1.5 times the isometric force but plateau at a finite value.
...
**Result:** Many plausible models predict reduced active authority late in the downswing and large inertial terms near impact. A numerical drift-to-control ratio should be reported only with the model, parameters, and sensitivity analysis used to compute it.
:::
...
The control gain $G(\bm{x})$ is not constant. In a muscle-driven model it depends on state $\bm{x}$, including joint configuration, moment arms, activation state, contraction velocity, and tendon compliance. During rapid concentric shortening, the force-velocity factor $f_V(\dot{\ell})$ can reduce effective torque authority. Combined with inertial terms that grow with speed, this can produce a high drift-to-control ratio near impact in some models.
...
- **The drift-to-control ratio can increase near impact.** Inertial terms grow with speed while concentric control authority can fall. Whether impact is effectively controllable depends on the full model.
