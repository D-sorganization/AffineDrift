...
Suppose the moment arm matrix (rows = muscles, columns = joints) is:
$$\bm{R}(\bm{q}) = \begin{pmatrix} 0.05 & 0 \\ 0.02 & 0.04 \end{pmatrix} \, \mathrm{m}.$$

This means:
- Muscle 1 (row 1) has moment arm 0.05 m at the shoulder and 0 m at the elbow (it only acts at the shoulder).
- Muscle 2 (row 2) has moment arm 0.02 m at the shoulder and 0.04 m at the elbow (biarticular---acts at both joints).

Suppose the muscle forces are:
$$\bm{F}^M = \begin{pmatrix} 500 \\ 300 \end{pmatrix} \, \mathrm{N}.$$

The joint torques are:
$$\boldsymbol{\tau} = \bm{R}^T \bm{F}^M = \begin{pmatrix} 0.05 & 0.02 \\ 0 & 0.04 \end{pmatrix} \begin{pmatrix} 500 \\ 300 \end{pmatrix} = \begin{pmatrix} 31 \\ 12 \end{pmatrix} \, \mathrm{N \cdot m}.$$

So $\tau_{\text{shoulder}} = 0.05 \cdot 500 + 0.02 \cdot 300 = 25 + 6 = 31$ N$\cdot$m and $\tau_{\text{elbow}} = 0 \cdot 500 + 0.04 \cdot 300 = 12$ N$\cdot$m.
...
- \textbf{Modulation of joint stiffness:} By varying co-contraction levels, a golfer can stiffen or soften a joint without changing the net torque. This is essential for impact: at ball contact, high stiffness is desirable to resist deformation and maintain club face alignment.

- \textbf{Stability and perturbation rejection:} Redundancy allows the nervous system to activate muscles beyond what is strictly needed, creating a ``stiffness buffer.'' If an unexpected perturbation (gust of wind, uneven ground) occurs, the excess activation can resist the perturbation.

- \textbf{Metabolic efficiency:} The nervous system can choose muscle activation patterns that minimize metabolic cost, not just produce the desired torque. Different combinations of muscles have different metabolic efficiencies.
...
\begin{itemize}
- Early in the backswing: grip force is relatively low.
- At the top of the backswing: grip force increases to resist gravity and prepare for the downswing.
- During the downswing: grip force increases further as the club accelerates.
- At impact: grip force is near its peak, providing stiffness to resist deformation and maintain club face alignment.
