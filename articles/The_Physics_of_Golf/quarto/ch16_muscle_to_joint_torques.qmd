---
pagetitle: "From Muscle Forces to Joint Torques"
description: "Derive signed muscle moment arms, feasible torque sets, biarticular power and stiffness, then connect inverse estimation and hand-club contact to testable golf hypotheses."
css:
...
# From Muscle Forces to Joint Torques {#sec-muscle-to-torque}
[]{#ch-muscle-to-torque}
...
## The Problem: Muscles Pull, Joints Rotate {#the-problem-muscles-pull-joints-rotate}
[]{#sec-muscle-torque-problem}
...
## Moment Arms and the Geometry of Force {#moment-arms-and-the-geometry-of-force}
[]{#sec-moment-arms}
...
::: {.callout-note}
## A Force Needs the Right Direction

[]{#box-moment-arm-plain}
...
### Example: The Biceps at the Elbow {#example-the-biceps-at-the-elbow}
...
### Moment Arm as a Function of Joint Angle {#moment-arm-as-a-function-of-joint-angle}
...
### Configuration-Dependent Jacobian {#configuration-dependent-jacobian}
...
## Redundancy: More Muscles Than Joints {#redundancy-more-muscles-than-joints}
[]{#sec-redundancy}
...
### The Null Space of the Muscle Jacobian {#the-null-space-of-the-muscle-jacobian}
...
::: {.callout-tip}
## Algebraic Freedom and Feasible Tension

[]{#muscle-feasible-family}
...
On narrow screens, scroll the figure or [open it at full size](../figures/muscle_torque_feasibility.svg).

::: {#fig-muscle-jacobian}
::: {.aba-diagram-scroll role="region" aria-label="Constructed torque feasibility and muscle power; scroll horizontally" tabindex="0"}
<img src="../figures/muscle_torque_feasibility.svg" alt="Left: the feasible torque polygon contains 6.5 and 7 newton-metres but excludes simultaneous coordinate maxima 22 and 14. Right: the same muscle delivers 12 watts to coordinate one while absorbing either 12 or 36 watts at coordinate two.">
:::

The Feasible Torque Set and Signed Joint Powers
:::
...
### Why Redundancy Matters for Golf {#why-redundancy-matters-for-golf}
...
### In the Control-Affine Framework {#in-the-control-affine-framework}
...
::: {.callout-note}
## What the Torque Description Leaves Open

[]{#box-redundancy-black-box}
...
## Joint Torques as a Modeling Choice {#why-we-model-with-joint-torques-not-muscle-forces}
...
+\bm Q_{\rm known}+\bm J_c^T\bm\lambda.
$$

Here $\bm h$ includes modeled inertial velocity terms and gravity; $\bm B$ places actuator inputs into generalized coordinates; and contact reactions $\bm\lambda$ must satisfy the declared contact equations and inequalities. In a muscle-driven model, replace the actuator term by $-\bm N^T\bm L^T\bm F$ together with the muscle state equations. A floating base has unactuated directions; it cannot be assigned arbitrary independent joint-torque controls.
...
### Redundancy Avoidance {#redundancy-avoidance}
...
The upper-extremity model of Holzbaur and colleagues illustrates both the value and limits of such models. Its geometry and muscle parameters support specific comparisons, and some parameters were adjusted against joint-moment data. Those fitted comparisons should be distinguished from other validation tasks. Prescribed scapular motion, generic geometry and omitted intrinsic hand muscles limit transfer to golf-grip questions. [@Holzbaur2005]

### Connection to Control {#connection-to-control}
...
::: {.callout-important}
## What a Torque Model Establishes

[]{#prin-why-joint-torques}
...
## Biarticular Muscles: Coupling Between Joints {#biarticular-muscles-coupling-between-joints}
[]{#sec-biarticular}
...
### The Moment Arm Matrix for Biarticular Muscles {#the-moment-arm-matrix-for-biarticular-muscles}
...
### Energy Transfer via Biarticular Muscles {#energy-transfer-via-biarticular-muscles}
...
::: {.callout-note}
## Reading the Direction of Power

[]{#box-biarticular-energy}
...
### Why This Matters for Golf (And Why It Is Hard) {#why-this-matters-for-golf-and-why-its-hard}
...
::: {.callout-note}
## From a Swing Outcome to a Testable Hypothesis

[]{#box-inverse-problem-golf}
...
## Practical Example: The Golf Grip {#practical-example-the-golf-grip}
[]{#sec-grip-forces}

### Anatomy of the Grip {#anatomy-of-the-grip}
...
### Grip Force vs. Grip Torque {#grip-force-vs.-grip-torque}
...
### Moment Arm Changes With Hand Configuration {#moment-arm-changes-with-hand-configuration}
...
### Grip Force Modulation During the Swing {#grip-force-modulation-during-the-swing}
...
[]{#ex-muscle-torque-exercises}

1. Define a signed axial moment arm. Explain when a perpendicular distance is sufficient, and derive the sign of $\bm\tau_M=-\bm L^T\bm F$ from tensile virtual work.

1. A hypothetical actuator transmits 600 N at moment arms 0.05 m and 0.03 m. Compute both torque contributions. Explain why this does not locate a person's maximum-strength posture.

1. For the nonsymmetric two-joint example in this chapter, compute both muscle contributions, the total torque, path rates at speeds $(2,-3)^T$ rad/s and total skeletal power. Explain why transposing the displayed torque map again changes the answer.

1. For the three-muscle feasible-family example, verify the null direction and derive the allowable interval of $t$. Are the independent coordinate maxima simultaneously feasible?

1. Define co-contraction and distinguish it from a null-space force redistribution. Explain how the taut-spring example disproves the claim that positive material stiffness guarantees joint stability.

1. Two synergists have positive moment arms 0.04 m and 0.02 m, each with capacity 1000 N. Characterize all nonnegative force pairs producing 50 Nm. Explain whether this freedom is antagonist co-contraction.

1. State a force-sharing optimization and the assumptions under which its optimum is unique. Explain why that uniqueness does not identify the nervous system's objective or guarantee a dynamically feasible trajectory.

1. Compute the second muscle's joint powers at speeds $(2,-1)^T$ and $(2,-3)^T$ rad/s. Identify the direction of instantaneous transfer in the zero-sum case and explain what remains unknown about tendon and metabolic energy.

1. Design the measurements needed to support a phase-dependent grip-force claim. Distinguish handle pressure, net wrench, ground reaction and surface EMG.

1. Explain why changing from a strong to a weak grip does not specify a unique change in the wrist moment-arm matrix. Identify the anatomical and contact information required for a prediction.

1. For the constraint $\bm q=(s,2s)^T$, compute the reduced torque in the two-joint example. Explain why a torque-input model also needs actuator feasibility and appropriate floating-base/contact equations.


## Worked Answers and Modeling Checks


1. $r_e=\bm e\cdot(\bm\rho\times\bm d)$. Its planar magnitude reduces to the pivot-to-force-line distance. With $\delta W=-\bm F^T\bm L\delta\bm q$, matching the coefficient of independent $\delta\bm q$ gives $\bm\tau_M=-\bm L^T\bm F$. Axis orientation and tension convention must be stated.

1. The contributions are 30 Nm and 18 Nm. The assumed tension is held equal; a person's attainable tension need not be equal at the two postures. No angle, force-length state or measured capacity has been supplied.

1. The contributions are $(25,0)^T$ and $(6,12)^T$ Nm, totaling $(31,12)^T$ Nm. Path rates are $(-0.10,0.08)^T$ m/s and total power is 26 W. Multiplying $\bm r^T\bm F$ would instead give $(25,22)^T$ for these equal-sized matrices and mismatched row meanings; it is not the declared force map.

1. $\bm r(13,-8,12)^T=\bm0$. The force inequalities give $t\in[-25/6,50/3]$ N. Coordinate maxima 22 and 14 Nm cannot coexist: the force choice required for 14 Nm at coordinate 2 leaves at most 14.5 Nm at coordinate 1.

1. Co-contraction concerns opposing actions about a named coordinate; a null direction can instead exchange synergist forces. At the taut-spring equilibrium, the material contribution vanishes because $\ell'=0$ and the geometric contribution is $F\ell''=-20/3$ Nm/rad. The equilibrium is an energy maximum. Even a positive restoring stiffness would not alone establish asymptotic stability without the remaining dynamics.

1. $F_2=2500\,{\rm N}-2F_1$, with $750\,{\rm N}\leq F_1\leq1000\,{\rm N}$ and correspondingly $500\,{\rm N}\leq F_2\leq1000\,{\rm N}$. This is a continuum of feasible synergist sharing, not an antagonist pair. A demand above 60 Nm would be infeasible under these bounds; at 60 Nm both forces must be at capacity.

1. Positive-capacity normalized squared force is strictly convex, so a nonempty convex bounded feasible set has one minimizer. Changing the criterion changes the inference. Independent samples do not enforce activation or tendon dynamics; a neural interpretation requires additional evidence.

1. Powers are $(12,-12)^T$ W and $(12,-36)^T$ W. In the first case, power is absorbed at coordinate 2 and delivered at coordinate 1; in the second, net path power is $-24$ W. Fiber work, tendon storage and metabolic cost remain separate quantities.

1. Use calibrated instrumented-handle or pressure measurements synchronized with kinematics and a defined phase convention. Specify whether the result is contact pressure, integrated normal load or a full wrench, and define the normalization test. Ground forces and EMG are complementary observations, not substitutes for direct handle-force measurement.

1. Measure posture and hand-club placement, define joint and shaft axes, specify muscle paths and a contact model, and propagate their uncertainty. The verbal grip category alone does not supply these quantities or identify a universally advantaged muscle.

1. The reduced torque is $31+2(12)=55$ Nm, with effective moment arms $(0.05,0.10)$ m. This preserves virtual work along the allowed coordinate motion. It does not make the underlying tensions freely controllable or eliminate unactuated base directions, contact feasibility or actuator state dynamics.
