...
This means: **the same muscle force produces different torques at different joint configurations** [@Delp1990; @Delp2007].
...
- $\bm{R}(\bm{q}) \in \mathbb{R}^{n_m \times n_j}$: the muscle-length Jacobian, whose entry $R_{ji}(\bm{q}) = \partial \ell_j / \partial q_i$ is the signed moment arm of muscle $j$ about joint $i$.
...
- Total skeletal muscles: \textasciitilde 630 [@Neumann2017].
...
This is a key mechanism in the kinetic chain: the legs accelerate, the hips follow, and energy transfers from the hip to the torso via biarticular muscles. It then passes from the torso to the shoulders via other biarticular muscles, and so on.
...
In illustrative force-plate and EMG (electromyography) studies, grip force changes dramatically during the swing [@Hume2005]:
