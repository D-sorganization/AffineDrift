...
## Redundancy: More Muscles Than Joints {#sec-redundancy}

The human body has far more muscles than joints. For example:

- Total skeletal muscles: ~630.
- Functional joints (with meaningful DOF): ~200.
- Degrees of freedom for movement: ~244.

For a local region, say the arm, the disparity is even more dramatic:

- Muscles in the arm: ~30 major muscles.
- Functional DOF: ~7--10 (shoulder, elbow, wrist, and a few finger joints).
...
In the control-affine model $\dot{\bm{x}} = f(\bm{x}) + G(\bm{x})\bm{u}$, the control input $\bm{u}$ represents the **net equivalent joint torque**. This is a single number per joint, subsuming all of the redundancy of the underlying muscles.
...
## Why We Model with Joint Torques, Not Muscle Forces {#sec-why-joint-torques}
...
We could equivalently write them in terms of muscle forces $\bm{F}^M$ via the Jacobian relation $\boldsymbol{\tau} = \bm{R}(\bm{q})^T \bm{F}^M$. But there are several reasons why joint torques are preferred:
...
## Biarticular Muscles: Coupling Between Joints {#sec-biarticular}
...
When activated, they produce simultaneous elbow flexion torque and wrist flexion torque, proportional to their respective moment arms at each joint:

$$\tau_{\text{elbow}} = R_{\text{elbow},k} F_k^M, \quad \tau_{\text{wrist}} = R_{\text{wrist},k} F_k^M.$$
...
## The Inverse Problem: From Desired Torques to Muscle Forces {#sec-inverse-problem}
...
$$\min_{\bm{F}^M} \|\bm{F}^M\|^2 \quad \text{subject to} \quad \bm{R}^T \bm{F}^M = \boldsymbol{\tau}^*, \quad \bm{F}^M \geq 0.$$
This minimizes the "effort" in an intuitive sense.

**2. Minimize metabolic cost:**
$$\min_{\bm{F}^M} \sum_i C_i(F_i^M) \quad \text{subject to} \quad \bm{R}^T \bm{F}^M = \boldsymbol{\tau}^*, \quad \bm{F}^M \geq 0,$$
...
**3. Minimize fatigue:**
$$\min_{\bm{F}^M} \sum_i f_i(F_i^M, t) \quad \text{subject to} \quad \bm{R}^T \bm{F}^M = \boldsymbol{\tau}^*, \quad \bm{F}^M \geq 0,$$
...
## Practical Example: The Golf Grip {#sec-grip-forces}
...
At impact, the club experiences a large force from the ball---estimates from ball-impact models and instrumented club studies report roughly 5000--8000 N for a driver impact, depending on swing speed [@penner2003physics; @CochranStobbs1968] (see @sec-ch28_impact_collision for a detailed treatment of impact mechanics). This force tries to deform the shaft and move the club head. To maintain club face alignment and transmit energy efficiently, the grip must be stiff.
...
1. Muscles generate linear forces along their line of action. Joints rotate about axes. The moment arm is the geometric bridge: moment arm = perpendicular distance from line of action to axis. Torque = force × moment arm.

2. The moment arm matrix $\bm{R}(\bm{q})$ maps muscle space to joint space. The muscle Jacobian is $\bm{R}^T$, and the relation is $\boldsymbol{\tau} = \bm{R}^T \bm{F}^M$.
