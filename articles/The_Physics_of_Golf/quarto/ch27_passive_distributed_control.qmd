---
description: "In the motor-control chapters we encountered the bandwidth problem: the brain has limited computational resources, slow feedback..."
---

# Passive and Distributed Control: A Self-Organizing Swing Model {#sec-passive_distributed_control}
...
In the 1980s, the control theorist Neville Hogan proposed a different account: the brain does not control *position* and does not control *force*. Instead, it controls *impedance*.[@Hogan:1984; @Hogan:1985]
...
The damping component of impedance---the $\bm{D}\dot{\bm{q}}$ term---is not merely a passive energy sink. As we explore in @sec-joint_damping_friction, damping at each joint propagates through the mass matrix coupling to affect every other joint in the chain. The brain's choice of co-contraction level at one joint therefore has system-wide consequences for the dynamics of every other joint.
...
Recall from @sec-zero_torque_counterfactual the Zero Torque Counterfactual (ZTCF) decomposition:
...
- Explains $\sim40$--50\% of the actual swing
...
- Explains $\sim70$--80\% of the actual swing (estimated from simulation studies comparing ZTCF trajectories with and without pre-set impedance; experimental validation with EMG-derived stiffness profiles remains an active area of research)
- Realistic because muscles are always at some baseline tone
- Allows small feedforward corrections to fine-tune the motion


The impedance-augmented drift is a much better predictor of the golfer's actual swing because it accounts for the muscle tension that is present.
...
In this illustrative reflex timing budget, this is called the *monosynaptic stretch reflex* because it involves only one synapse in the spinal cord. The latency is remarkably short: 20--30 ms. By contrast, conscious reflexes (routing through the brain) have latencies of 100--300 ms [@Kandel2013; @enoka2002neuromechanics].
...
In @sec-energy_transfer, we saw that power flows from proximal segments (large muscles, high inertia) to distal segments (small muscles, low inertia). The mechanism is the inertial load on the distal joints.
...
This is the opposite of the detailed video analysis approach, which tries to manually adjust every body segment. The Bosch approach trusts the body's self-organizing capacity once the conditions are right [@Bosch:2015].
...
- **@sec-motor_control_brain:** The brain and spinal cord have limited bandwidth. Real-time independent control of 20+ DOF is infeasible within the available time budget.
- **@sec-motor_learning:** The motor system learns and adapts by modifying forward models and internal representations.
- **@sec-motor_learning:** Movement inherently exhibits variability; the motor system manages it through noise shaping and regularization.
- **@sec-passive_distributed_control:** The motor system avoids micromanagement by pre-tuning mechanical properties and distributing control to spinal and local circuits.
...
The traditional control model requires the brain to compute $\sim20,000$ decisions per second. The impedance control model requires the brain to compute $\sim40$ parameters once. Explain how this 500-fold reduction in computational rate is achieved, and why it does not sacrifice precision.
