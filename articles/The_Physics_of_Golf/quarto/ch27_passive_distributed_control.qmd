...
## Passive Control in the ZTCF Framework
{#sec:passive_ztcf}

Recall from Chapter @ch:05_affine_structure the Zero Torque Control Fraction (ZTCF) decomposition:
...
This means the ZTCF---the fraction of the motion explained by gravity, inertia, and Coriolis forces *alone*---increases dramatically when impedance is included. The original drift accounts for maybe 40--50\% of the swing (illustrative; depends on model). The impedance-augmented drift accounts for approximately 70--80\% (illustrative estimate from simulation studies comparing ZTCF trajectories with and without pre-set impedance; experimental validation with EMG-derived stiffness profiles remains an active area of research).
<!-- TODO: citation needed for the 40--[% value] and 70--[% value] ZTCF estimates; these are based on simulation and should be validated against published biomechanical validation ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for the 70--[% value] ZTCF estimate in this callout; consistent with the caveat above ; exact values depends on individual biomechanics and model assumptions -->
...
- Sensory signals travel to the spinal cord (approximately 10--20 ms; depends on individual)
<!-- TODO: citation needed for the 10--[ms value] sensory conduction time to the spinal cord ; exact values depends on individual biomechanics and model assumptions -->
- Spinal circuits activate the motor neurons of the *same muscle*
- Muscle contracts and resists the stretch


This is called the *monosynaptic stretch reflex* because it involves only one synapse in the spinal cord. The latency is remarkably short: approximately 20--30 ms (illustrative; depends on individual). By contrast, conscious reflexes (routing through the brain) have latencies of approximately 100--300 ms (illustrative range; depends on task and individual).
<!-- TODO: citation needed for monosynaptic stretch reflex latency (20--[ms value]) and conscious reflex latency (100--[ms value]) ; exact values depends on individual biomechanics and model assumptions -->
...
In Chapter @ch:energy_transfer, we saw that power flows from proximal segments (large muscles, high inertia) to distal segments (small muscles, low inertia). The mechanism is the inertial load on the distal joints.
...
- **Chapter @ch:motor_control_brain:** The brain and spinal cord have limited bandwidth. Real-time independent control of 20+ DOF is infeasible within the available time budget.
- **Chapter @ch:motor_learning:** The motor system learns and adapts by modifying forward models and internal representations.
- **Chapter @ch:motor_learning:** Movement inherently exhibits variability; the motor system manages it through noise shaping and regularization.
- **Chapter @ch:passive_distributed_control:** The motor system avoids micromanagement by pre-tuning mechanical properties and distributing control to spinal and local circuits.
...
- **Impedance control:** The brain solves this by pre-setting impedance (stiffness and damping) once before the swing, then allowing the passive mechanical system to execute.

- **Pre-tuning reduces computation:** Instead of computing 20,000 control decisions per second, the brain computes $\sim$40 impedance parameters once before the swing.

- **Passive stability:** With the right impedance, the swing is self-correcting. Perturbations are automatically resisted by spring forces and dissipated by damping.

- **Distributed control:** Spinal reflexes, muscle spindles, and local feedback circuits handle moment-to-moment stability. The brain sets the configuration, and the spinal cord executes.

- **Emergent behaviors:** Complex behaviors like the whip effect and distal acceleration emerge automatically from the kinetic chain physics and the impedance profile. They are not explicitly controlled.
