...
description: "In Chapters @sec-motorcontrolbrain--@ch:motorlearning, we encountered the bandwidth problem: the brain has limited computational resources, slow feedback..."
---
...
Recall from Chapter @sec-05_affine_structure the Zero Torque Control Fraction (ZTCF) decomposition:
...
In this illustrative reflex timing budget, this is called the *monosynaptic stretch reflex* because it involves only one synapse in the spinal cord. The latency is remarkably short: 20--30 ms. By contrast, conscious reflexes (routing through the brain) have latencies of 100--300 ms.
...
In Chapter @sec-energy_transfer, we saw that power flows from proximal segments (large muscles, high inertia) to distal segments (small muscles, low inertia). The mechanism is the inertial load on the distal joints.
...
- **Chapter @sec-motor_control_brain:** The brain and spinal cord have limited bandwidth. Real-time independent control of 20+ DOF is infeasible within the available time budget.
- **Chapter @sec-motor_learning:** The motor system learns and adapts by modifying forward models and internal representations.
- **Chapter @sec-motor_learning:** Movement inherently exhibits variability; the motor system manages it through noise shaping and regularization.
- **Chapter @sec-passive_distributed_control:** The motor system avoids micromanagement by pre-tuning mechanical properties and distributing control to spinal and local circuits.
