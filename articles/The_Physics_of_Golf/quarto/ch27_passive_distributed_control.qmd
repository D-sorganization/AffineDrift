---
title: "Passive and Distributed Control: A Self-Organizing Swing Model"
...
where $\bm{\tau}_{\mathrm{ff}}$ is the feedforward torque computed from the internal model, and $\bm{q}_d(t)$, $\dot{\bm{q}}_d(t)$ are the desired trajectory and velocity. The impedance gains $\bm{K}$ and $\bm{D}$ can be set once at the start of a swing; the feedforward torque $\bm{\tau}_{\mathrm{ff}}(t)$ and desired trajectory $\bm{q}_d(t)$ remain time-varying. The computational saving is in eliminating real-time feedback corrections to $\bm{K}$ and $\bm{D}$, not all time-varying signals.
...
In this illustrative reflex timing budget, this is called the *monosynaptic stretch reflex* because it involves only one synapse in the spinal cord. The latency is remarkably short: 20--30 ms (lower limb), 35--50 ms (upper limb) [@Kandel2012]. By contrast, conscious reflexes (routing through the brain) have latencies of 100--300 ms.
...
::: {.callout-note}
## Why Effortless Movement Can Look Stable
...
- **Pre-tuning reduces computation:** The impedance gains $\bm{K}$ and $\bm{D}$ (approximately 40 parameters: 20 stiffness + 20 damping) can be set once before the swing. The feedforward torque $\bm{\tau}_{\mathrm{ff}}(t)$ and desired trajectory $\bm{q}_d(t)$ remain time-varying but are pre-computed, not updated via real-time feedback. This eliminates the need to continuously re-compute feedback corrections to $\bm{K}$ and $\bm{D}$.
...
:::: {.callout-note}
This chapter has a standalone companion article on the main site: **[Passive Distributed Control: Frans Bosch Framework](../../passive-distributed-control.html)**. The article develops the Bosch self-organization framework --- motor attractors, differential learning, elastic energy storage, and fascial tensegrity --- that complements this chapter's impedance-control treatment of the same passive-control principle; the two are best read together.
::::
...
The traditional control model requires the brain to compute $\sim$20,000 decisions per second. The impedance control model requires the brain to compute $\sim$40 impedance gains ($\bm{K}$, $\bm{D}$) once before the swing, while the feedforward torque and desired trajectory remain time-varying but pre-computed. Explain how this reduction in real-time feedback computation is achieved, and why the time-varying feedforward signals do not eliminate the computational advantage.
