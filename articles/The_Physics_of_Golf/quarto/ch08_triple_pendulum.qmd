...
In Chapters @ch:zero_torque_counterfactual and 7, we modeled the arm as a double pendulum: shoulder and elbow. This was useful for illustration, but it's incomplete.
...
The difference: by releasing late, the club inherits the arm's higher velocity, and Coriolis amplifies it further. For illustrative comparison, a release that's approximately 200 ms later in this hypothetical scenario can result in substantially higher club speed; the specific improvement depends on the golfer's kinematics and club parameters.
...
Using a numerical solver with the mass matrix and Coriolis terms from Sections @sec:triple_pendulum_mass_matrix and @sec:triple_pendulum_coriolis, the accelerations are approximately:
...
{Whip Effect}
Simulate the triple pendulum ZTCF (no muscle torques) from the configuration given in Section @sec:triple_pendulum_ztcf.
