---
description: "Each segment has an inertia. The shoulder is hard to move (large , since it includes the torso). The forearm is easier (smaller ). The club is easiest..."
---
...
In @sec-zero_torque_counterfactual and @sec-constraint_forces, we modeled the arm as a double pendulum: shoulder and elbow. This was useful for illustration, but it's incomplete.
...
For a comprehensive counterfactual modeling sweep of distal timing and energy transfer on the companion 2-DOF system, see [Proximal-to-Distal Energy Transfer in the Golf Swing](/articles/proximal-distal-energy-transfer.html).
...
### Understanding the Three Segments {#inplain:triple_pendulum_segments}
In a real golfer:
...
## The Mass Matrix: Understanding Coupling {#sec-triple_pendulum_mass_matrix}
...
M_{13} & M_{23} & M_{33}
\end{bmatrix}
$$ {#eq-triple_mass_matrix_structure}

The diagonal entries are:

$$
\begin{aligned}
M_{11} &= I_1 + m_1 L_{1,\text{cm}}^2 + (m_2 + m_3) L_1^2 + m_2 L_{2,\text{cm}}^2 + m_3 L_2^2 + m_3 L_{3,\text{cm}}^2 \notag\\
       &\quad + 2 m_2 L_1 L_{2,\text{cm}} \cos q_2 + 2 m_3 L_1 L_2 \cos q_2 + 2 m_3 L_1 L_{3,\text{cm}} \cos(q_2 + q_3) + 2 m_3 L_2 L_{3,\text{cm}} \cos q_3 \\
M_{22} &= I_2 + m_2 L_{2,\text{cm}}^2 + m_3 L_2^2 + m_3 L_{3,\text{cm}}^2 + 2 m_3 L_2 L_{3,\text{cm}} \cos q_3 \\
M_{33} &= I_3 + m_3 L_{3,\text{cm}}^2
\end{aligned}
$$
...
$$
\begin{aligned}
M_{12} &= m_2 L_1 L_{2,\text{cm}} \cos q_2 + m_3 L_1 L_2 \cos q_2 + m_3 L_1 L_{3,\text{cm}} \cos(q_2 + q_3) \notag\\
       &\quad + I_2 + m_2 L_{2,\text{cm}}^2 + m_3 L_2^2 + m_3 L_{3,\text{cm}}^2 + 2 m_3 L_2 L_{3,\text{cm}} \cos q_3 \\
M_{13} &= I_3 + m_3 L_{3,\text{cm}}^2 + m_3 L_1 L_{3,\text{cm}} \cos(q_2 + q_3) + m_3 L_2 L_{3,\text{cm}} \cos q_3 \\
M_{23} &= I_3 + m_3 L_{3,\text{cm}}^2 + m_3 L_2 L_{3,\text{cm}} \cos q_3
\end{aligned}
$$
...
### What the Mass Matrix Entries Mean {#inplain:mass_matrix_meaning}
...
Inverting the mass matrix makes the asymmetry explicit:
...
10.0 & -10.0 & 80.0
\end{bmatrix}
$$ {#eq-triple_mass_inverse_numerical}
...
## Coriolis Terms in the Triple Pendulum {#sec-triple_pendulum_coriolis}

### Why Coriolis Gets Complicated With Three Segments {#inplain:triple_coriolis_complexity}
...
### Why Sequential Matters {#inplain:sequential_unlocking_why}
...
A crude analogy: imagine a train with three cars. If all three cars accelerate forward at the same time (equal torque on each), each one reaches a moderate speed. Now let the engine accelerate first and push the second car, which accelerates and pushes the third. The third car ends up faster than any single car accelerated alone.
...
This transfer happens *automatically*, through constraints and inertia, with minimal active muscular effort once the sequence is initiated.
...
### Wrist Cock as Energy Storage {#inplain:wrist_cock_energy}
...
However, the elastic energy in the wrist is *tiny* compared to the kinetic energy of the club at impact. A rough calculation:
- Wrist elastic potential energy: $\sim 5$--$10$ J (depends on flexibility).
- Club kinetic energy at impact: $\sim 200$ J.

So the wrist cock is NOT the primary source of club speed. Instead, it's a *timing mechanism*.

By cocking the wrist and holding it through mid-downswing, the golfer creates a "lag"—a delay in the club's rotation relative to the arm's rotation. This lag stores a relationship, not energy: the club's large moment of inertia keeps it moving slower than the arm. Released at the right moment, it therefore accelerates rapidly.
...
- The club is released when the arm is already moving fast.
- Coriolis torques at the club are now enormous, catapulting the club to $\dot{q}_3 = 60$ rad/s.

In this illustrative scenario, releasing late lets the club inherit the arm's higher velocity, which Coriolis then amplifies. A release 200 ms later can produce 20--30% higher club speed.
...
## Triple Pendulum ZTCF: Passive Dynamics Become Even More Dominant {#sec-triple_pendulum_ztcf}
...
Using a numerical solver with the mass matrix and Coriolis terms from @sec-triple_pendulum_mass_matrix and @sec-triple_pendulum_coriolis, the accelerations are approximately:
...
The club's angular velocity increased by 2 rad/s (from 25 to 27) in just 50 ms, with *zero* muscle torque. This is the whip effect: passive Coriolis acceleration at the club. The specific values depend on the assumed initial conditions; the qualitative result — that passive Coriolis forces accelerate the club without muscular input — is robust [@Nesbit2005].

### The DCR Blows Up Even Larger With Three Segments {#constraint:triple_dcr_blowup}
With a third segment, the Drift-Control Ratio at the club becomes even more extreme. In the final 30 ms before impact:
- Coriolis torque at the club: $\sim 800$--$1000$ Nm.
- Muscular torque available at the club: $\sim 10$--$20$ Nm.
- DCR: on the order of $50:1$ to $100:1$ (model-dependent).

In this regime the club is effectively ballistically committed; attempting to "accelerate" it at impact through muscle torque alone would have negligible effect. Most of the speed comes from the passive dynamics of the prior segments.
...
### Why Distal Segments Move Faster {#inplain:distal_faster_explanation}
...
That is exactly what the downswing shows: the shoulder slows and loses kinetic energy while the club speeds up and gains it. Proximal segments decelerate, distal segments accelerate. It's not magic—it's energy conservation and inertia differences.
...
8. **Control is in the setup and timing:** Once the swing is in motion, there's almost no active control over club speed. Control happens through the setup and through the timing of each release.


:::

**What comes next:** The triple pendulum assumes the body is a serial chain: hips → shoulders → arms → club. But the body is more complex—it forms closed kinematic loops. The pelvis and shoulders are connected through the spine, creating a parallel mechanism. Understanding these parallel structures is the key to explaining phenomena like the X-factor and why some golfers are more effective than others despite similar arm motion.
...
{Whip Effect}
Simulate the triple pendulum ZTCF (no muscle torques) from the configuration given in @sec-triple_pendulum_ztcf.

1. Integrate forward 50 ms and plot all three angles and angular velocities.
2. At what time does the club angular velocity reach its maximum? Does it occur near the end?
