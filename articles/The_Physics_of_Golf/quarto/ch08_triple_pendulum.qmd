...
1. **Timing:** The wrist unlocks last, after the shoulders and hips. This sequential unlocking is the essence of the kinetic chain.
2. **Speed amplification:** The wrist and club can achieve speeds that far exceed what the arm alone could produce.
3. **Stored energy:** During the backswing and early downswing, the wrist is cocked (constrained). This cocking stores elastic potential energy that's released later.
4. **Control:** The wrist is where the golfer has the most precise control, because the club's motion is most sensitive to wrist angles.
...
In a real golfer:
- Segment 1 represents the rotation of the shoulders and torso about the spine. This is the largest, slowest segment.
- Segment 2 represents the rotation of the arm at the elbow. It's smaller and faster than segment 1.
- Segment 3 represents the rotation of the club at the wrist. It's the smallest and fastest.
...
where $\bm{M}(\bm{q})$ is the $3 \times 3$ mass matrix, $\bm{C}(\bm{q}, \dot{\bm{q}})$ is the $3\times 3$ Coriolis/centrifugal matrix (so $\bm{C}\dot{\bm{q}}$ gives the torque vector), $\bm{g}(\bm{q})$ is gravity, and $\bm{u} = (\tau_1, \tau_2, \tau_3)^T$ are the applied torques at the three joints.

## The Mass Matrix: Understanding Coupling

::: {.callout-note}
## The $3 \times 3$ Mass Matrix for a Triple Pendulum
...
$$\begin{aligned}
M_{11} &= I_1 + m_1(L_1/2)^2 + I_2 + m_2(L_1^2 + L_1 L_2 \cos q_2) + I_3 + m_3(L_1^2 + 3L_1 L_2 \cos q_2 + L_2^2)\\
...
$$\begin{aligned}
M_{12} &= I_2 + m_2 L_1 (L_2/2) \cos q_2 + I_3 + m_3(L_2^2 + L_1 L_2 \cos q_2)\\
...
- Segment 1 (arm): $L_1 = 0.4$ m, $m_1 = 2$ kg, $I_1 = 0.05$ kg$\cdot$m$^2$.
- Segment 2 (forearm): $L_2 = 0.3$ m, $m_2 = 0.5$ kg, $I_2 = 0.01$ kg$\cdot$m$^2$.
- Segment 3 (club): $L_3 = 0.5$ m, $m_3 = 0.2$ kg, $I_3 = 0.01$ kg$\cdot$m$^2$.

At a downswing configuration where $q_2 = 90°$ ($\cos q_2 = 0$, so cross-coupling terms vanish), the mass matrix evaluates to:
...
The determinant is approximately $1.8 \times 10^{-4} > 0$, and all leading principal minors are positive ($M_{11} > 0$, $M_{11}M_{22} - M_{12}^2 > 0$), confirming this matrix is symmetric positive definite — as required for any physical mass matrix.
...
To see this more clearly, the approximate inverse is:
...
$$C_{3\cdot} \approx \frac{\partial M_{23}}{\partial q_2} \dot{q}_2 \dot{q}_3 + \text{(cross terms)}$$

The term $\frac{\partial M_{23}}{\partial q_2}$ depends on how the elbow angle affects the coupling between forearm and club. When $q_2$ is near $90°$ (arm extended), this derivative is large. The product $\dot{q}_2 \dot{q}_3 = 20 \times 30 = 600$ rad$^2$/s$^2$ is large.

Even if $\frac{\partial M_{23}}{\partial q_2} \approx 0.01$ (a small number), the Coriolis torque is:

$$C_{3\cdot} \approx 0.01 \times 600 = 6 \text{ N}\cdot\text{m}$$

This is comparable to or exceeding muscular torque at the wrist, and it's purely a consequence of the kinematics, not active effort. The club is yanked forward by the rotating arm, and this yank is encoded in the Coriolis term.
...
3. **Late downswing (0.35--0.45 s):** The wrist unlocks (releases the club). Now the club can rotate independently. But it inherits all the momentum from the arm's rotation. Coriolis torques at the wrist are large, catapulting the club forward.
...
**Wrist cock:** During the backswing and early downswing, the wrist is held in a bent position. This position is maintained by muscular torque, which acts against the spring force of the wrist tendons.
...
However---and this is important---the elastic energy in the wrist is *small* compared to the kinetic energy of the club at impact. Wrist elastic potential energy is small (depending on flexibility and muscular tension), while the club's kinetic energy at impact is far larger.
...
Using our model parameters, the following two scenarios illustrate how release timing affects club speed:

Scenario A: Early wrist release
- At $t = 0.2$ s (early downswing), the wrist is released.
- The arm is rotating at $\dot{q}_1 = 8$ rad/s, $\dot{q}_2 = 10$ rad/s.
- The club, now free, initially has the same angular velocity as the arm constraint would dictate.
- But the club's small inertia means it quickly adopts the arm's velocity, with minimal acceleration.

Scenario B: Late wrist release
- At $t = 0.40$ s (late downswing), the wrist is released.
- The arm is rotating at $\dot{q}_1 = 20$ rad/s, $\dot{q}_2 = 35$ rad/s (much faster).
- The club is released when the arm is already moving very fast.
- Coriolis torques at the club are now larger, catapulting the club to $\dot{q}_3 = 60$ rad/s.
...
## Triple Pendulum ZTCF: Passive Dynamics Become Even More Dominant
...
Using a numerical solver with the mass matrix and Coriolis terms, the accelerations are approximately:
...
$$\begin{aligned}
q_1(0.05) &\approx 50° + 12 \times 0.05 \cdot \tfrac{180°}{\pi} - \tfrac{1}{2} \times 2 \times 0.0025 \cdot \tfrac{180°}{\pi} \approx 50.6°\\
q_2(0.05) &\approx 80° + 20 \times 0.05 \cdot \tfrac{180°}{\pi} + \tfrac{1}{2} \times 15 \times 0.0025 \cdot \tfrac{180°}{\pi} \approx 81.0°\\
q_3(0.05) &\approx 100° + 25 \times 0.05 \cdot \tfrac{180°}{\pi} + \tfrac{1}{2} \times 40 \times 0.0025 \cdot \tfrac{180°}{\pi} \approx 102.9°
\end{aligned}$$
...
The club's angular velocity increased by 2 rad/s (from 25 to 27) in just 50 ms, with *zero* muscle torque. This is the whip effect: passive Coriolis acceleration at the club.

::: {.callout-note}
## The DCR Blows Up Even Larger with Three Segments

With a third segment, the Drift-to-Control Ratio at the club becomes even more extreme in the final milliseconds before impact. The Coriolis torque at the club---driven by the rapidly spinning arm---far exceeds any muscular torque a human wrist can produce. The DCR is large: the system is highly committed to a ballistic trajectory.

The club is so ballistically committed that even attempting to "accelerate" it at impact is futile. All speed comes from the passive dynamics of the prior segments.
:::
...
::: {.callout-important}
## Energy Flows from Proximal to Distal
...
::: {.callout-important}
## Key Takeaways: The Triple Pendulum
...
3. **Coriolis whips the club:** Coriolis torques at the wrist are large---scaling as velocity squared, they can far exceed muscular torque capacity in the late downswing. They're a primary driver of club acceleration.
...
6. **The whip effect is largely passive:** The dramatic club acceleration at the end of the downswing is driven primarily by Coriolis forces in a three-segment system. The DCR becomes very large---the motion is highly ballistic.
...
8. **Control is in the setup and timing:** Once the swing is in motion, active control over club speed is limited. Control happens through initial conditions (setup, backswing) and the precise timing of joint releases.
...
- $I_1 = 0.1$, $m_1 = 3$, $L_1 = 0.4$.
- $I_2 = 0.02$, $m_2 = 1$, $L_2 = 0.3$.
- $I_3 = 0.01$, $m_3 = 0.2$, $L_3 = 0.5$.
...
Simulate the triple pendulum ZTCF (no muscle torques) from the configuration given in the worked example above.
...
**Application: Wrist Release Timing**
