...
where $\bm{M}(\bm{q})$ is the $3 \times 3$ mass matrix, $\bm{C}(\bm{q}, \dot{\bm{q}})$ is the $3\times 3$ Coriolis/centrifugal matrix (so $\bm{C}\dot{\bm{q}}$ is the Coriolis/centrifugal torque vector), $\bm{g}(\bm{q})$ is gravity, and $\bm{u} = (\tau_1, \tau_2, \tau_3)^T$ are the applied torques at the three joints.
