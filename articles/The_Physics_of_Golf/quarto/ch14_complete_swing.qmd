...
In the affine framework, the velocity and acceleration are zero (or small). The drift field $f(\mathbf{x})$ at address points downward (gravity). But muscle torques (control inputs) are active, holding the body in position. The control authority $G(\mathbf{x})$ is fully available.
...
4. **Collision with the ground or obstacles:** If the golfer swings hard and loses control, they might collide with the ground or obstacles. This provides an effective (if painful) deceleration mechanism.
...
From a control perspective, the follow-through is a low-control, high-drift phase, similar to late downswing. The motor cortex is not planning the follow-through; it is letting the body decelerate under muscle damping and gravity.
...
The golfer's muscles are the primary source of energy. When muscles contract, they hydrolyze ATP (adenosine triphosphate) and convert the chemical energy into mechanical work. The power output of muscles can be high during the swing [estimated values; direct measurement of total muscular power during a golf swing is difficult and published values vary widely].
...
5. **In mid-downswing:** The ZTCF is already producing a motion close to the desired motion. The control inputs are small corrections.
...
- **@sec-05_affine_structure** developed the affine structure that underlies all golf physics.
...
- **This chapter** brought it all together, walking through the swing and revealing the dominance of drift in the late downswing.
  ...
  Understanding this clarifies what should be trained: not strength alone, and not speed alone, but the timing and state preparation that determine when drift and control matter.
...
6. **Drift dominance often increases throughout the downswing.** The DCR should be reported with its weighting, torque bound, and model parameters.
...
9. **Control has less late leverage.** In mid-to-late downswing, muscle torques can have greatly diminished path-correction effect while still regulating stiffness and release timing.
10. **Power comes from timed interaction.** The fastest, most powerful swings are those that coordinate muscular work, elastic storage, and drift rather than treating late muscle force as the only source of speed.
11. **The shaft catapult effect is model-dependent but important.** Stored elastic energy in the bent shaft can be released near impact; representative models estimate a modest clubhead-speed contribution [@Nesbit2005; @MacKenzie2009].
12. **The paradox of control:** large late path corrections can reduce power and consistency, while earlier state preparation lets the drift field assist the intended motion.
13. **The lesson for golfers:** practice the early swing, refine setup and sequencing, then use late control as bounded steering rather than wholesale path repair.
...
Describe the kinetic chain sequence (hips, torso, arms, club) using lag and shaft bending. How does this sequence maximize the catapult effect?
