...
This decomposition is the key to understanding the golf swing. It's where drift and control become concrete. The entries for "affine control system", "drift", and "control" in the book [Glossary](glossary.html) summarize this structure in plain language.
...
1. **Unconstrained coordinates.** The generalized coordinates $\bm{q}$ are assumed to be a minimal set (two angles for the double pendulum). Holonomic constraints such as two-hand grip coupling, ground contact, or impact are treated separately in Chapter 7 (Constraint Forces) and Chapter 9 (Parallel Mechanisms).
2. **Fully actuated.** We assume $\bm{G}(\bm{x})$ has rank equal to $\dim(\bm{q})$, so every generalized coordinate has an independent actuator. Under-actuated swings (e.g., the "passive wrist release" discussed later) break this.
3. **Lossless dynamics.** The drift $\bm{f}(\bm{x})$ encodes only conservative passive forces (inertia, Coriolis/centrifugal, gravity). Dissipative forces---aerodynamic drag, joint friction, and soft-tissue damping---are added back as $-D(\bm{q})\dot{\bm{q}}$-style corrections elsewhere.
...
Let's define:
- **Drift dynamics**: $\bm{f}(\bm{x}) = \begin{bmatrix} \dot{\bm{q}} \\ \bm{M}(\bm{q})^{-1} \left[ - \bm{C}(\bm{q}, \dot{\bm{q}}) \dot{\bm{q}} - \bm{g}(\bm{q}) \right] \end{bmatrix}$
- **Control input**: $\bm{u} = \bm{\tau}$ (the applied torques).
- **Control matrix**: $\bm{G}(\bm{x}) = \begin{bmatrix} 0 & 0 \\ 0 & 0 \\ M^{-1}(\bm{q}) \end{bmatrix}$ (all zeros in the top rows, $M^{-1}$ in the bottom).
...
5. **Elite golfers maximize DCR** by understanding and exploiting the drift.
...
8. **Physics is universal**. While individual golfers vary in geometry and strength, the fundamental structure (the equations $\bm{M}, \bm{C}, \bm{g}$) is the same.
