---
pagetitle: "The Affine Structure: Drift and Control"
description: "Derive the affine input structure, coupled response, energy budget and task-specific control limits with reproducible examples and worked answers."
css:
...
## From Equations of Motion to State Space {#from-equations-of-motion-to-state-space}
...
### Solving for Accelerations {#solving-for-accelerations}
Assume $M$ is symmetric positive definite on the independent coordinates. Define $c$ as the complete velocity-dependent inertial bias, and $g=\nabla_q V$ for the gravitational potential. A matrix $C$ may satisfy $c=Cv$, but that factorization is not unique; dropping selected entries without deriving their product changes the dynamics. The standard manipulator formulation and its coordinate limitations are developed in  [@TedrakeMultibodyEnergy].
...
### The Control-Affine Form {#the-control-affine-form}
...
## Explicit Form for the Double Pendulum {#explicit-form-for-the-double-pendulum}
...
$$
[]{#eq-ch05_control_matrix}

$$
u=(u_1,u_2)^T.
$$
[]{#eq-ch05_control_input}
...
## Drift in the Double Pendulum at Rest
...
## Drift as a Vector Field {#drift-as-a-vector-field}
...
On narrow screens, scroll the figure or [open it at full size](../figures/affine_structure_verified.svg).

::: {#fig-ch05_drift_vector_field}
::: {.aba-diagram-scroll role="region" aria-label="Reduced drift and optimal timing examples; scroll horizontally" tabindex="0"}
<img src="../figures/affine_structure_verified.svg" alt="A computed closed orbit follows the directional field of the ideal locked-distal pendulum. Below, minimum-energy input grows late for a decaying scalar plant, stays constant for an integrator and is greatest early for an amplifying plant.">
:::

Declared Models of Drift and Optimal Timing. The Upper Panel Shows Normalized Directions and a Computed Orbit for the Reduced Plant With an Ideal Distal Lock. The Lower Panel Shows Exact Minimum-Energy Inputs for Three Normalized Scalar Plants With the Same Endpoint Task. Neither Panel Contains Measured Golfer Data.
:::

### Flowing Along the Drift {#flowing-along-the-drift}
...
## Control as a Steering Input {#control-as-a-steering-input}
...
### The Control Matrix as Leverage {#the-control-matrix-as-leverage}

::: {.callout-tip}

## Control Leverage at Different Configurations
...
## The Drift-Control Ratio (DCR) {#the-drift-control-ratio-dcr}
...
### Address: A Defined Equilibrium {#address-low-dcr}
...
### Top of Backswing: A Specified State {#top-of-backswing-transitioning-dcr}
...
### Downswing: Velocity Bias and Coupled Response {#downswing-high-dcr}
...
### Impact and Beyond: Specify the Event and System {#impact-and-beyond-extremely-high-dcr}
...
## Optimal Control and the Swing {#optimal-control-and-the-swing}
...
Pontryagin's principle supplies necessary conditions, with the appropriate terminal/transversality conditions; a candidate is not automatically a global optimum. With a free terminal state, $p(T)=\nabla\phi$ in this simple formulation; terminal constraints add multipliers. This Hamiltonian is not generally mechanical energy  [@TedrakeTrajectoryOptimality].

Dynamic programming instead reasons about a value function. Under suitable smoothness its finite-horizon equation is $-\partial_t V=\min_u[L+\nabla_x V^T(f+Gu)]$ with terminal data $V(x,T)=\phi(x)$. Verification needs the stated boundary, admissibility and regularity conditions; nonsmooth values require more general treatment. It is not just another name for solving a single trajectory's two-point boundary-value equations  [@TedrakeDynamicProgramming].
...
## A Solved Comparison of Control Timing
...
## Putting It All Together: The Unified Picture {#putting-it-all-together-the-unified-picture}
...
Continue with [Induced Acceleration](ch30b_induced_acceleration.html), [Triple-Pendulum Dynamics](ch08_triple_pendulum.html), [Brain Control](ch24_motor_control_brain.html), [Lagrangian Reference](../../lagrangian-reference.html), and [Controllability and Drift](../../controllability-drift-ratio.html).

::: {.callout-note}
## On This Site

- **[Controllability and the Drift-Control Ratio](../../controllability-drift-ratio.html)** — Drift–input decomposition and local reachability in golf models
- **[Lagrangian Mechanics Reference](../../lagrangian-reference.html)** — Multibody equations of motion in control-affine form
- **[Superposition in Control-Affine Systems](../../superposition.html)** — Linearized perturbations around a reference swing trajectory
:::

## Chapter Exercises and Worked Answers {#chapter-exercises}

### 1. Compute the Declared Rest State
For the manufactured two-link model at $q=(0,0)$, $v=0$, compute $f$. Does setting torque to zero mean releasing the club?

**Worked answer.** The bias is zero because velocity is zero, and both potential gradients vanish at downward vertical. Thus $a_0=0$ and all four components of $f$ are zero. The ideal hinges and fixed pivot remain. Releasing the club changes contact and requires a different model. Zero net torque also does not specify muscle activation or coactivation.

### 2. Compute a Raised Rest State
Use $q=(150^\circ,-70^\circ)$ and $v=0$. Why must both gravity components enter both accelerations?

**Worked answer.** The defined potential gives $g\simeq(4.76483,1.93219)^T$ N m, and $M\simeq\left[\begin{smallmatrix}0.328445&0.153941\\0.153941&0.130000\end{smallmatrix}\right]$ kg m$^2$. Solving $Ma_0=-g$ gives $a_0\simeq(-16.9466,5.2045)^T$ rad/s$^2$. Inverse-inertia coupling mixes both loads. The first two state-derivative components remain zero at this instant even though velocity begins to change.

### 3. Separate Velocity Bias From Gravity
For $q=(140^\circ,-60^\circ)$ and $v=(5,0)$ rad/s, compare the two acceleration contributions.

**Worked answer.** Although $v_2=0$, $c_2=b\sin(q_2)v_1^2=-1.51554$ N m. The complete contributions are $-M^{-1}c\simeq(-13.6289,28.9563)^T$ and $-M^{-1}g\simeq(-22.1155,13.2066)^T$ rad/s$^2$. Their vector sum is $(-35.7444,42.1629)^T$. Comparing only torque magnitudes or omitting $c_2$ loses the coupled response. This instantaneous decomposition is not an energy-source fraction.

### 4. Interpret a Capacity Ratio
...
**Worked answer.** The ratios are approximately $0$, $0.04158$, $0.02723$ and $0.07705$. They use the largest singular value of $M^{-1}$ multiplied by 30, rather than an assumed realized torque. They are not monotone in the listed angle changes and do not define golf phases. Work requires a realized torque history and the associated joint rates. Reducing the input bound increases a nonzero capacity ratio while reducing available control.

### 5. Determine Optimal Timing From a Defined Task
...
### 6. Compare Larger and Smaller Input Sets
