...
# The Affine Structure: Drift and Control {#sec-05_affine_structure}
...
Recall from @sec-02_language_of_motion that the state is:
...
This is the **control-affine form**, standard in nonlinear control theory [@Isidori1995; @BulloLewis2004]. It's called "affine" (not "linear") because the dependence on control is linear, but the dependence on state is not linear.
...
In robotics, this form is the starting point for designing controllers. In golf, it's the starting point for understanding what you control and what physics does for you.
...
The **Drift-to-Control Ratio (DCR)** is a modeling diagnostic, not a coordinate-free physical constant.
...
DCR is high (drift is doing most of the work).
...
In this illustrative estimate, your muscular contribution is maybe 20 N m (estimates vary widely; exact values depend on golfer strength and technique — see [@Nesbit2005] for inverse-dynamics measurements). DCR is extremely high under this simplified comparison, but the conclusion should be checked against the stated model and torque bounds.
...
Total muscular effort: in this illustrative summary, maybe 60–80 N m generated over the whole swing, distributed in a way that works with physics.

Compare to a novice:

1. At the top, in this illustrative scenario, applies 40 N m to start the downswing.
2. In the same illustrative scenario, applies 40 N m throughout, fighting the increasing drift.
...
In principle, you could pose the golf swing as an optimal control problem [@Penner2001]:
...
The physics—the equations $\bm{M}, \bm{C}, \bm{g}$—is the same for everyone. So while individual golfers vary in strength, flexibility, and body geometry, the fundamental structure of drift and control is universal.
...
8. **Skilled golfers exploit high-DCR phases** by preparing initial conditions and transition timing that let drift assist rather than fight the intended trajectory.
...
11. **The model structure is shared** [@Featherstone2008; @Murray1994]. While individual golfers vary in geometry and strength, smooth multibody models commonly share the manipulator-equation structure $\bm{M}, \bm{C}, \bm{g}$; the numerical predictions remain individual and model-dependent.
