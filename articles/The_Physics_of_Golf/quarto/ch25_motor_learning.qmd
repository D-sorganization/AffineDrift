...
Consider the moment of impact. The clubhead decelerates from $\sim 90\text{ mph}$ to $\sim 60\text{--}65\text{ mph}$ in about 0.5 milliseconds (losing roughly 30--35% of its speed, not all of it---see [Chapter 28](./ch28_impact_collision.qmd#sec-impact_collision)). The collision force is enormous ($\sim 6000\text{--}7000\text{ N}$), and the reaction must be transmitted through the body [@Jorgensen1994; @Penner2003]. That transmission challenge is one reason impact is a useful example of Bernstein's problem: the system must coordinate many joints and tissues quickly, not just one isolated segment.
...
- The shot is executed as a feedforward motor command ([Chapter 24](./ch24_motor_control_brain.qmd#sec-motor_control_brain)).
...
Recall ideomotor theory from [Chapter 24](./ch24_motor_control_brain.qmd#sec-motor_control_brain): actions are represented by their effects, not by motor commands. When you focus on the ball flight (external), you activate the action effect pathway directly. Your motor system automatically retrieves the learned motor program.
