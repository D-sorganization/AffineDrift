...
Consider the moment of impact. The clubhead, which may be traveling at approximately 90 mph or more for advanced golfers, undergoes rapid deceleration during ball contact (contact duration is typically estimated at around 0.5 ms for a driver). This requires an enormous force from the ground (reaction force). This force must be transferred through the body to the club.
<!-- TODO: citation needed for typical clubhead speed and impact contact duration values used in this illustrative example -->
...
- After impact, the ball follows a trajectory. You see where it lands (visual feedback delay that varies with distance and individual; roughly 150 ms or more is a typical estimate, but depends on individual visual processing speed).
<!-- TODO: citation needed for approximate visual feedback latency ([ms value]) in the context of observing ball landing ; exact values depends on individual biomechanics and model assumptions -->
