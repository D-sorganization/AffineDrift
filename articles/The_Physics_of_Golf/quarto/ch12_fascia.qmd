...
# Fascia and Connective Tissue: Separating Myth From Mechanics {#sec-fascia}
...
2. **Biomechanical measurements indicate multi-path force distribution.** In this illustrative summary of controlled stretching experiments, engineers find complex, nonuniform stress distributions. Forces do not concentrate along any one path; they spread widely.
...
The consequence: a simple model that treats each muscle as an independent actuator (each with its own line of pull) overestimates the control authority. The effective control authority is lower because the muscles are mechanically coupled through the fascia. Some of the muscle's force goes to lateral transmission, and the effective torque on the shoulder joint is less than a naive calculation would suggest.
...
In the golf swing, the swing duration is about 0.2 seconds, and the deformation happens in tens of milliseconds. Fascia, being viscous, acts almost like a stiff solid at these timescales. It does not easily absorb or store energy; it just resists the motion.
...
4. **The strain-at-failure numbers show that fascia is highly extensible.** Fascia can stretch 20--40\% before failing, whereas tendon fails at only 5--10\% strain. This extensibility is a feature: it allows large deformations without tearing, distributing stress over a larger volume.
...
In this simplified mechanical picture, the golf swing forces are large. The clubhead experiences high centripetal and tangential accelerations, on the order of hundreds of $g$ at impact (@sec-04_forces_and_torques works through the specific numbers; [@Jorgensen1994] reports similar orders of magnitude). Those forces travel through the arm to the shoulder to the torso, and eventually to the ground. The fascia in the rotator cuff, the arm, and the torso is critical for distributing these forces so that no single tendon or muscle attachment is overloaded.
