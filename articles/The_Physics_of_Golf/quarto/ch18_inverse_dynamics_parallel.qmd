---
pagetitle: Inverse Dynamics and the Parallel Loop Problem
description: "What golf-swing motion, grip sensors, force plates, and EMG can establish about loads, muscle strategy, and forward counterfactuals."
---
...
## Start With the Quantity Being Investigated {#sec-inverse_definition}
...
![From Measurements and Declared Mechanics to Identified Loads and Further Research Questions](../figures/inverse_identification_workflow.png){#fig-inverse_dynamics_flow fig-alt="Kinematic estimates, boundary measurements, and declared mechanics enter balance and identification. Outputs distinguish identified loads and remaining load families from further recruitment and intervention questions. Uncertainty is carried through all stages."}

## Equations, Inputs, and Boundary Conditions {#subsec-canonical_eom}
...
## A Complete Newton–Euler Recursion {#subsec-rnea}
...
[Featherstone spatial-vector implementation](https://royfeatherstone.org/spatial/v2/);
...
## What the Two-Handed Grip Constrains {#sec-loop_closure_formal}
...
## Identification Is a Rank Question {#sec-identifiable}
...
### An Example Where the Sensor Choice Matters
...
## Hand Wrenches and Club Balance {#sec-instrumented_clubs}
...
### What an Instrumented Grip Actually Measures
...
## Sensor Physics With Defined Units {#subsec-strain_gauges}
...
in the [NI strain-measurement documentation](https://www.ni.com/getting-started/set-up-hardware/data-acquisition/i/strain-gages).
...
See the [Kistler force-sensor documentation](https://www.kistler.com/GB/en/cp/piezoelectric-force-sensors-9224a/P0001619)
...
## Optimization Selects a Hypothesis {#subsec-approach_optimization}
...
## Muscles, Activation, and EMG {#subsec-approach_emg}
...
[Farina, Merletti, and Enoka analysis](https://pubmed.ncbi.nlm.nih.gov/25277737/)
...
The [OpenSim activation-dynamics documentation](https://opensimconfluence.atlassian.net/wiki/spaces/OpenSim/pages/53090590/First-Order+Activation+Dynamics)
...
## Ground Loads and Segment-by-Segment Balance {#sec-lower_body_inverse_dynamics}
...
### Yaw Moment Is Not a Vertical Force Offset {#sec-ground_torque_detailed}
...
### Momentum Exchange and Mechanical Power
...
### A Counterexample to the Small-Inertia Argument
...
$$ {#eq-inverse_error_counterexample}
...
## Forward Dynamics and a Declared Counterfactual {#sec-ztcf_forward_dynamics}
...
### Same-State Acceleration and a Forward Branch
...
$$ {#eq-forward_branch_counterexample}
...
## A Golf Study That Can Distinguish Explanations {#sec-synthesis_inverse_dynamics}
...
1. Define the primary output: for example, clubhead velocity and face
...
1. Specify the body graph, joint coordinates, separated grip frames, club
...
1. Calibrate and synchronize kinematics, bilateral ground wrenches, and the
...
1. Solve the declared net-load problem and test segment and whole-system
...
1. Add muscle modeling and EMG only for the recruitment questions they can
...
1. Branch the forward counterfactual from a declared event or state, specify
...
- **[Inverse Dynamics](../../inverse-dynamics.html)** — Site reference for inverse dynamics conventions and limits
- **[The Null Space of the Constraint Jacobian](../../null-space-constraint-jacobian.html)** — Parallel loops and indeterminate reaction loads
:::

## Chapter Exercises


1. Verify the two-coordinate inverse matrix's null vector. Compare ranks
   after measuring the actuator sum, measuring the multiplier, and removing
   the second actuator.
1. Derive wrench transport from invariant power. Reproduce the 46 W example
   and identify the lost lever-arm term when reference points are mixed.
1. Derive grasp-map ranks for two full hand wrenches and for two separated
   point forces. Explain why a transverse opposing force pair creates a couple.
1. Derive the weighted minimum from its Lagrangian and verify $(8,2)$.
   Explain why the selected objective does not establish neural intent.
1. Derive the bridge voltage from its divider voltages and reverse the output
   leads. Convert the piezoelectric charge using each stated capacitance.
1. Reproduce the inverse and forward error calculations for
   $M=\operatorname{diag}(1,\epsilon)$. Verify the straight arm's singular task
   Jacobian and positive-definite inertia.
1. Use the oscillator solution to compare cross-branch and same-state
   acceleration differences at $t=\pi/2$. State what each comparison holds fixed.
1. A paper reports separate left and right spinal muscle moments. Identify
...
1. Design a sensor addition for a remaining golf load-sharing null direction.
