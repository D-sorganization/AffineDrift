...
- **Equations:** $n$ equations of motion (the constrained equations of motion above)
...
Rearranging the constrained equations of motion:
$$
...
The solution to the underdetermined system above is not unique. Instead, the general solution is:
$$
...
The constraint forces $\boldsymbol{\lambda}$ can usually be determined from the constraint equations themselves (given measured $\ddot{\bm{q}}$, the constraint acceleration equation determines $\boldsymbol{\lambda}$). But the applied torques $\bm{u}$ cannot be uniquely determined without additional information or modeling assumptions.
...
**Why?** The coefficient matrix on the left is the KKT matrix above, which is nonsingular for any valid mechanical system. Therefore, the system can be solved uniquely for $\ddot{\bm{q}}$ and $\boldsymbol{\lambda}$ at each instant.
:::
...
Even though the system has parallel loops and redundant actuators, the forward integration is *well-posed*. The KKT matrix is nonsingular at each time step, so the constraint forces $\boldsymbol{\lambda}$ and accelerations $\ddot{\bm{q}}$ are uniquely determined.
