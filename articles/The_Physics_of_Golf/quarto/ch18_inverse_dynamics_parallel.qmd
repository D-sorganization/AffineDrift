---
title: "Inverse Dynamics and the Parallel Loop Problem"
...
Here is the stumbling block: from motion capture technology, we can measure exactly how your skeleton moves. We can film your swing, track every joint angle, compute every velocity and acceleration. The natural question is: *What muscle forces caused this motion?*
...
Yet your muscles caused this motion. Can we compute those muscle forces *backwards* from the motion we measured?

For most of your body, the answer is yes. For the golf swing as a whole—held together by the two-handed grip and the planted feet—the answer is more subtle. Some aspects of muscle recruitment become fundamentally *undetectable* from motion alone. To be precise: the *net* joint torques are always detectable from motion (given accurate kinematics and anthropometrics); what is ambiguous is how that net torque is *distributed* across individual muscles crossing the joint—the classic muscle-redundancy or load-sharing problem [@crowninshield1981physiologically].
...
The mass matrix $\bm{M}(\bm{q}) \in \mathbb{R}^{n \times n}$ is *always* symmetric and positive definite. Therefore:
\begin{equation}
...
This is a fundamental fact of mechanics: you *cannot* have a configuration where the inertia matrix becomes singular. Nature forbids it.
...
This algorithm is used in every motion-capture biomechanics lab in the world to compute joint torques from motion capture. And it works beautifully—*for open chains*.
...
- **Equations:** $n$ equations of motion (Equation \ref{eq:eom_constrained})
...
The constraint forces $\boldsymbol{\lambda}$ are *real forces*. Your left hand genuinely pushes against your right hand. But you cannot see this force directly. It is internal to the system. And when you try to invert the dynamics—to go from measured motion to inferred torques—the constraint forces and applied torques become entangled in a way that makes them impossible to disentangle uniquely.
...
This means **forward dynamics always works**.

**Caveat — near-singular configurations.** The full-rank condition on $\bm{J}_c$ can be lost at kinematic singularities (for example, an elbow or knee driven to full extension, or a wrist aligned with a two-hand grip axis). Near such configurations the KKT matrix becomes ill-conditioned even though it is not formally singular, and direct inversion amplifies numerical noise. In practice the augmented system is regularized by adding $\lambda I$ to the constraint block (Tikhonov regularization) or by truncating the smallest singular values of $\bm{J}_c$ (SVD truncation). Both preserve the solution near the singularity at the cost of introducing a small bias.
:::
...
**Why?** The coefficient matrix on the left is the KKT matrix (Equation \ref{eq:KKT_matrix}), which is nonsingular for any valid mechanical system. Therefore, the system can be solved uniquely for $\ddot{\bm{q}}$ and $\boldsymbol{\lambda}$ at each instant.
:::
...
Despite the ambiguity, inverse dynamics is not useless. Many aspects of the motion *are* uniquely determined by kinematics. It is important to know exactly which aspects are observable and which are invisible.
...
::: {.callout-note}
## Identifiable vs. Unidentifiable Components

From motion capture data of a system with loop constraints, the following quantities are **uniquely determined**:

- **Net generalized force on the system:**
\begin{equation}
...
- **Constraint forces $\boldsymbol{\lambda}$:**
...
- **Total net moment about the center of mass:**
For multi-body systems, the total kinetic moment can be uniquely computed from kinematics.

The following are **NOT uniquely determined**:

- **Individual muscle torques in redundant systems:** If two muscles can produce the same net effect, their individual contributions cannot be separated from kinematics alone.

- **Load sharing between limbs:** In the two-handed grip example, the split between left-hand and right-hand torques is unobservable.

- **Internal forces in closed kinematic loops:** For example, the shear force at an intervertebral disc connecting two vertebrae in a twisted spine cannot be determined from kinematics alone.
...
This approach yields a unique solution—but it is a **modeling choice**, not a physical fact. The assumption that muscles minimize "effort" (measured as norm of torques) is a modeling assumption. It may or may not be accurate for a real golfer.

Different cost functions yield *different* solutions to the same load-sharing problem: sum of squared activations ($\sum a_i^2$) favors distributing load widely across synergists; sum of cubed muscle stresses ($\sum \sigma_i^3$, [@crowninshield1981physiologically]) produces recruitment orderings consistent with the size principle; metabolic-cost objectives further penalize slow-fatiguing muscles. These choices can change predicted individual-muscle forces by 30--50\% while producing net joint torques that match measurement to within a few percent. The "correct" cost function depends on how the nervous system actually controls motion, which remains an open neuroscience question.
...
Record electromyographic (EMG) signals from major muscles during the swing. These signals (motor unit action potentials) are correlated with muscle activation. Use these measurements as *constraints* on the inverse dynamics problem:
...
## Implications for the Zero-Torque Club-First Framework

::: {.callout-note}
## Why Forward Simulation is More Robust

The ZTCF (Zero-Torque Club-First) framework asks a different question than traditional inverse dynamics. Rather than asking *What muscle torques caused this motion?*, it asks *What motion occurs if the muscles apply zero torque to the club?*
...
The ZTCF trajectory is a **forward integration problem**. Given initial conditions $\bm{q}(0), \dot{\bm{q}}(0)$ and the constraint equations $\bm{\Phi}(\bm{q}) = \bm{0}$, the motion $\bm{q}(t)$ is **uniquely determined** by integrating the constrained EOM forward.

Even though the system has parallel loops and redundant actuators, the forward integration is *well-posed*. The KKT matrix (Equation \ref{eq:KKT_matrix}) is nonsingular at each time step, so the constraint forces $\boldsymbol{\lambda}$ and accelerations $\ddot{\bm{q}}$ are uniquely determined.

The ZTCF trajectory is the unique solution to the zero-control problem. There is no ambiguity in what the motion is.

**This is the fundamental advantage of the ZTCF approach.** It does not suffer from the underdetermined inverse problem. It is robust even in systems with complex parallel loops.
...
### Trust Completely: Open Serial Chains
...
- **Mathematically unique:** One solution, no ambiguity.
- **Numerically stable:** Small errors in kinematics produce small errors in inferred torques.
- **Physically meaningful:** The computed torques correspond directly to muscle activity.
...
- **The fundamental asymmetry:** Forward dynamics is always well-determined (given forces, find motion). Inverse dynamics is often underdetermined (given motion, find forces) when the system has loops and redundant actuators.
...
- **What remains observable:** Net forces, constraint forces, aggregate power, and center-of-mass motion are uniquely determined from kinematics. Individual muscle contributions in redundant systems are not.

- **The ZTCF advantage:** By asking "what motion occurs with zero applied torque?" instead of "what torques caused this motion?", the ZTCF poses a forward problem, which is always well-determined. The trajectory is unique and robust to modeling assumptions about muscle recruitment.

- **Practical implication:** The ZTCF baseline trajectory can be computed with high confidence. Deviations from the ZTCF trajectory reveal the net effect of applied muscle torques (the "control signal"). This is more robust than trying to infer individual muscle contributions directly.

**In Plain Language:** Knowing how a golfer swings does not uniquely specify what every muscle was doing. But knowing the trajectory if no muscles acted (the ZTCF) is unique and can serve as a reference baseline. The difference between the actual swing and the ZTCF baseline reveals the net muscle control, without requiring that control to be fully decomposed into individual muscles.
...
- Explain why constraint forces are usually *uniquely* determined from kinematics, even though applied torques are not.
- What equation determines the constraint forces? (Hint: it comes from the constraint equations themselves.)
- Why are constraint forces independent of the null space ambiguity in applied torques?
