...
Even for an open-chain limb, inverse dynamics identifies net joint loads under a specified model; individual muscle forces require additional assumptions because muscles are redundant [@Winter2009; @Robertson2004; @Zajac1989]. For the golf swing, loop constraints and grip/ground contact add another layer of non-identifiability. This chapter explains what remains measurable, what requires instrumentation or optimization assumptions, and how the ZTCF framework reframes part of the question as a forward simulation.
...
This means there is an 8-dimensional family of joint configurations that all result in the same club position and orientation. Some examples of these null space motions:
...
For example, a golfer might grip the club tightly (large internal grip force) while maintaining identical club trajectory compared to a golfer with light grip. The kinematics are the same, but the internal stresses differ. The tight-grip golfer may fatigue forearm muscles and risk injury from the excess internal force.
...
The Coriolis force is also *perpendicular* to the velocity that creates it. This perpendicularity is a kinematic fact, not dependent on forces.
...
The optimization approach has become standard in biomechanics. Different objective functions (minimize muscle activation, minimize metabolic cost, minimize joint torques, etc.) yield different load distributions, all of which are mechanically consistent with the measured motion.
:::
...
Despite the ambiguity, inverse dynamics is not useless. Some quantities are directly measured, some are model-conditioned estimates, and some remain unidentifiable without additional data. Knowing which category each claim occupies is essential.
...
- **Net generalized load on the system, once the model and external loads are specified:**
$$
\bm{M}(\bm{q})\,\ddot{\bm{q}} + \bm{C}(\bm{q},\dot{\bm{q}})\,\dot{\bm{q}} + \bm{g}(\bm{q})
$$

This is computed from kinematics plus model parameters and is independent of which muscles did the work.

- **Constraint compatibility and modeled multipliers:**
From the constraint equations:
$$
\bm{J}_c\,\ddot{\bm{q}} + \dot{\bm{J}}_c\,\dot{\bm{q}} = \bm{0}
$$

The equations identify admissible accelerations. The multipliers $\boldsymbol{\lambda}$ require the dynamic model and any measured or assumed applied loads; they are not observed directly from motion capture.
...
The following evidence ladder should be checked before stronger biomechanical language is used:
...
The following are **not uniquely determined from motion capture alone**:
...
The same inverse dynamics framework applies to the lower body, with one advantage: force plate data provides a known boundary condition.
...
Take the ground torque as $M_z = 80$ N·m (an illustrative value chosen from the scale of reported force-plate and inverse-dynamics golf-swing measurements, not a universal benchmark) [@Nesbit2005]. Applied at the feet, roughly 0.2 m from the pelvis, it gives a pelvis torque of approximately $\tau_{\text{pelvis}} = M_z / I_{\text{hip}} \times$ (accounting for moment arms and mass distribution).
...
Consider a simple two-link arm (shoulder and elbow). When the arm is fully extended, the Jacobian becomes singular—the wrist can no longer move in all directions. The mass matrix at this configuration has a high condition number.
