...
A critical insight from [Chapter 9](./ch09_parallel_mechanisms.qmd#sec-parallel_mechanisms): drift terms can be computed from kinematics once the model, segment inertias, and coordinate convention are specified. They are not affected by muscle load-sharing ambiguity, but they are still model-conditioned.
...
This total drift term has no muscle load-sharing ambiguity, but it depends on the selected model, parameters, and filtering. The ZTCF framework ([Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual)) leverages this distinction without claiming that drift estimates are independent of modeling choices.
:::
...
The ZTCF framework ([Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual)) asks: "What motion does the declared effective plant produce from this state when applied generalized control is zero?" It does not specify muscle activation.
