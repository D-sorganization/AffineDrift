...
- **Equations:** $n$ equations of motion (Equation @eq:eom_constrained)
...
Rearrange Equation @eq:eom_constrained:
$$
...
The solution to Equation @eq:inverse_underdetermined is not unique. Instead, the general solution is:
$$
...
The constraint forces $\boldsymbol{\lambda}$ can usually be determined from the constraint equations themselves (given measured $\ddot{\bm{q}}$, Equation @eq:constraint_acceleration determines $\boldsymbol{\lambda}$). But the applied torques $\bm{u}$ cannot be uniquely determined without additional information or modeling assumptions.
...
**Why?** The coefficient matrix on the left is the KKT matrix (Equation @eq:KKT_matrix), which is nonsingular for any valid mechanical system. Therefore, the system can be solved uniquely for $\ddot{\bm{q}}$ and $\boldsymbol{\lambda}$ at each instant.
:::
...
Even though the system has parallel loops and redundant actuators, the forward integration is *well-posed*. The KKT matrix (Equation @eq:KKT_matrix) is nonsingular at each time step, so the constraint forces $\boldsymbol{\lambda}$ and accelerations $\ddot{\bm{q}}$ are uniquely determined.
