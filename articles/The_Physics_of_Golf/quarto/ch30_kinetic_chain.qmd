...
where $\tau_i^{(m)}$ is the muscle torque applied at that joint and $\tau_i^{(c)}$ is the constraint torque (the reaction torque from the distal segment). Using the equations of motion from Chapter @sec-04_forces_and_torques, the constraint torque at joint $i$ arises from the accelerations of the distal chain:
...
Biomechanical data from high-speed video and marker tracking suggest approximate sequencing patterns such as the following (from [@Cheetham2001, Hume2005]):
...
At first glance, it seems that the wrist should be a major power source: the wrist muscles are substantial, and a violent wrist snap seems to intuitively contribute to club speed. However, Chapter @sec-muscle-force-generation showed that skeletal muscles operate under a fundamental constraint: they are weak at high speeds (Hill's force-velocity relation). By the time the club reaches near-impact speeds (say, 80 mph at the hands, which is $\sim 1200^{\circ}$/s in rotational terms), the muscle cannot exert much force.
...
One of the most profound insights from the ZTCF formalism (Chapter @sec-zero_torque_counterfactual) is that the proximal-to-distal sequencing of the kinetic chain does *not* need to be programmed into the motor control system. Instead, it *emerges naturally* from the structure of the equations of motion when the body is modeled as a coupled kinetic chain with realistic mass, geometry, and stiffness parameters.
...
The Drift-Correction-Response (DCR, introduced in Chapter @sec-zero_torque_counterfactual) describes how the golfer corrects deviations from the desired trajectory. During the kinetic chain sequence, the DCR is especially important because small timing errors can propagate down the chain.
