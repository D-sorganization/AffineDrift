...
Instead of the brain commanding: "Move the shoulder to 90 degrees at 3 degrees per millisecond," it commands: "Set shoulder stiffness to 2.3 N$\cdot$m per degree." (These specific values are illustrative; actual impedance values vary by individual, task, and muscle co-contraction level; approximately 1--5 N$\cdot$m per degree is a typical estimated range <!-- TODO: add citation for illustrative shoulder stiffness values in impedance control, e.g., motor control or rehabilitation engineering literature -->).
...
- These pre-activations happen approximately 50--100 ms before the event (illustrative; depends on task and individual; typical pre-activation windows vary roughly 30--150 ms in the motor control literature <!-- TODO: add citation for the anticipatory pre-activation window estimate, e.g., EMG studies of feed-forward motor control -->), anticipating it using the forward model.
