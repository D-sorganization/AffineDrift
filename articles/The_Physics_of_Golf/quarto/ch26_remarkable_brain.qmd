...
- **Control space size:** If you could independently vary muscle activations at each time point, the total number of possible swings would be roughly $(7^{200})^{30}$. Since $7^{6000} = 10^{6000 \\log_{10} 7} \\approx 10^{5070}$, this thought experiment produces an astronomically large space (well above $10^{5000}$ distinct swings).

- **Computational challenge:** To find the optimal swing by exhaustive search would require evaluating an astronomically large number of candidates. Even if each evaluation took a nanosecond, this would take far longer than the age of the universe.
...
These constraints reduce the effective dimensionality from the astronomically large naive space to something manageable: perhaps a few dozen parameters to tune, though the exact number is heuristic rather than literal.
...
This is not a limitation of the brain; it is a fundamental constraint of the system. Even with very fast neural processing, real-time feedback control of the downswing would be limited by sensory and actuator delays. The practical solution is to rely heavily on feedforward structure, while still allowing fast reflexive and stabilizing feedback to contribute where it can.
...
- **Uses minimal neural power: cortical computation uses ~20 W; spinal control ~10 W; muscular actuation during a swing ~200–500 W. The brain's computational efficiency is remarkable precisely because the expensive part is outsourced to biochemical muscle.**
