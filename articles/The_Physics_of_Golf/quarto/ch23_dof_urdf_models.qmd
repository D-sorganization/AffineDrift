...
# Degrees of Freedom and Robot Models of the Human Body {#sec-23_dof_urdf_models}
...
Suppose you have a model of the human body made from the joints we discussed in @sec-anatomy_joint_modeling. You specify:
...
**Loop constraints:** The four bars form a closed loop. This loop closure imposes constraints. The end of bar 1 must meet the start of bar 2, and the end of bar 3 must meet the start of bar 4. These are 6 constraints per loop (3 position + 3 orientation), but in a planar 4-bar mechanism only some of these constraints are active.
...
## Worked Example --- The Golf Swing Model {#sec-golf_swing_dof}
{#sec-golf_swing_dof}
...
This creates a **loop closure constraint**: the club's position and orientation relative to the lead hand must be consistent with its position and orientation relative to the trail hand.
...
This is why motion capture shows that no two golfers swing identically [@Glazier2011; @Myers2008]. Glazier (2011) reviews movement variability in the golf swing and argues it is partly functional rather than simply error, and Myers (2008) reports the spread across 100 golfers.
...
Biomechanical optimization of the golf swing must account for this redundancy: each club position admits not a unique optimal arm configuration but a manifold of near-optimal ones.
:::
...
The golf swing is the same. A golfer has approximately 25--30 degrees of freedom [@Delp2007; @Rajagopal2016]. That is a commonly cited illustrative estimate; exact counts vary across biomechanical models and depend on how joints are defined, typically ranging from 20 to 35 DOF depending on model granularity.
...
Two golfers with different body proportions, flexibility, and strength navigate *different* solution manifolds. Their manifolds are different subsets of joint space, because their anthropometry differs.
...
So how does the brain choose? The answer is not that it picks one solution at random. Rather, physics and physiology constrain the solution dramatically:
...
Recall from @sec-05_affine_structure and 6 that the system evolves under drift and control:
...
To populate the URDF with realistic inertia values, we use anthropometric data. The most widely used data comes from de Leva (1996) [@DeLeva1996], who measured body segment masses and moments of inertia on cadavers and living subjects.
...
**Note:** This is a *simplified* excerpt. A complete URDF would include all 20 joints: 3 per arm shoulder, 2 per elbow, 2 per wrist, 3 for lumbar, and 3 for thorax. Each axis would require careful definition.
...
Here's the power of these tools. In @sec-parallel_mechanisms, we derived the equations of motion:
...
**More sensitive:** The early downswing (first 0.1 s) is *very sensitive* to initial angular velocity. If the pelvis rotation rate is 1{}^\circ off, the club path changes significantly.
:::
...
**Key finding:** The ZTCF (Zero Torque Counterfactual, from @sec-soft_tissue_pliable) is moderately sensitive to anthropometric parameters but *extremely sensitive* to initial conditions and joint torques.
...
### Looking Ahead
We now have a complete musculoskeletal model: joints with defined degrees of freedom, kinematic chains described in URDF, and dynamics engines to simulate them. But a model without a controller is just a ragdoll. Who---or what---controls the swing? Part VIII turns to the remarkable organ that orchestrates all of this: the human brain. @sec-motor_control_brain examines how the brain solves the control problem under severe constraints of delay, noise, and bandwidth.

## Chapter Exercises

- **DOF Counting:** For the golf swing model described in @sec-golf_swing_dof, compute the DOF using the Grübler formula:
