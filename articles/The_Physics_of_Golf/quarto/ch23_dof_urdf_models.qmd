---
pagetitle: "Degrees of Freedom and Robot Models of the Human Body"
description: "Admissible motion, club-task redundancy, torque and contact dynamics, physically consistent robot models, and identifiable golf experiments."
...
### The Formula and Its Rank Assumption
...
[]{#thm-grubler}
This is the spatial Grübler--Kutzbach count. The independence assumption is essential. A negative result signals an inappropriate or dependent-constraint count; it is not a physical negative mobility. Special geometric arrangements and closed loops require a rank calculation. The [Modern Robotics mobility lesson](https://modernrobotics.northwestern.edu/nu-gm-book-resource/2-2-degrees-of-freedom-of-a-robot/) illustrates precisely this limitation.
...
### Bodies, Coordinates and Reference Frames
...
### Worked Example 1: A Serial Robot Arm
...
### Worked Example 2: A Closed Loop
...
![Upper-Body Tree and Two-Hand Closure. Numbers Indicate Compound-Joint Freedoms; the Dashed Connection Closes the Second Grip.](../figures/upper_body_tree.svg){#fig-urdf_tree fig-alt="A fixed pelvis connects through lumbar and thorax joints to two arms. Each arm has shoulder, elbow and forearm, and wrist freedoms. One club attaches rigidly to the lead hand; a dashed trail-hand connection closes the loop."}

[]{#simplified-upper-body-model}

### A Reconciled Upper-Body Count
...
### The Grip Constraint and the Club Body

[]{#princ-prin:dof_practice}
...
### Adding Legs and Ground Contact
...
## The Human Arm --- A Seven-DOF Redundant Manipulator
...
### DOF Count and the Selected Task
...
[]{#laymans-box:ch23_redundancy_intro}
...
### The Task Jacobian on Admissible Velocities
...
[]{#princ-prin:ch23_motor_equivalence}
...
The existence of this redundancy is a mathematical property of the chosen map. Showing that golfers organize variability along it requires data. @Myers2008 measured torso/pelvis kinematics and ball velocity in 100 recreational golfers; those associations do not establish equality of club trajectories or identify null-space torques. @Glazier2011 discuss competing interpretations and methods for movement variability. Neither source supplies a universal count of freedoms "used" by novices and experts.

### Bernstein's Degrees of Freedom Problem

[]{#laymans-box:ch23_freezing_freeing}


[]{#def-ch23_bernstein_problem}
...
### Self-Organization, Compensation and Evidence
...
## The Seven-DOF Arm --- Redundancy in Miniature
...
### A Concrete Elbow-Circle Calculation
...
### From Joint Velocities to Applied Torques
...
### The Acceleration-Level Task Map
...
### A Dynamically Consistent Projection
...
### Contact, Underactuation and Internal Grip Forces
...
Inputs that leave all admissible accelerations unchanged may nevertheless change constraint reactions. With two hands, opposing forces can cancel in the net club wrench while changing the loads at each grip. This force redundancy is not the same as an elbow-swivel velocity. The [multicontact analysis of Sentis, Park and Khatib](https://cs.stanford.edu/groups/manips/publications/pdfs/Sentis_2010_TRO.pdf) connects support constraints, task control and internal forces under explicit actuation/contact assumptions. It does not provide evidence for a particular golfer's neural strategy.
...
[]{#null-space-optimization-in-the-ztcf-family}

### Null-Space Optimization and Its Limits

[]{#ex-ch23_elbow_swivel_null_space}
...
### Basic Structure and Frames

URDF describes a tree of links connected by joints. Every non-root link has one parent joint; the root has none. Each joint specifies a parent, child, origin transform and joint type. A conventional revolute joint axis is expressed in its joint frame. The zero-position transform and rotation sequence are part of the definition: changing them while retaining the same angle values changes the physical pose. Consult the [URDF model API](https://docs.ros.org/kinetic/api/urdf/html/index.html) for the underlying tree and joint representation, and the selected importer's documentation for supported features.
...
The downloadable [teaching_arm.urdf](../models/teaching_arm.urdf) and the generator `TwoLinkArm.urdf()` use the same model. A content test checks that they agree. This example is a verifiable building block; the open two-link tree does not implement a golfer's two-hand closure, foot contacts or muscle physiology.

[]{#urdf-joint-types}

### Link Inertia and Joint Semantics
...
### Compound Joints and Closed Loops
...
## Building a Full-Body Golf Model
...
### A Construction Sequence That Preserves the Physics
...
### Scaling Mass, Length and Inertia
...
### What the Software Supplies

**Drake** provides multibody modeling within a systems and optimization framework. Its [Parser](https://drake.mit.edu/pydrake/pydrake.multibody.parsing.html) adds model instances to a plant; the returned model-instance identifiers are not the plant itself. A usable setup creates the plant, parses into it, specifies world attachment and actuation as needed, finalizes the plant, creates a context and supplies state/input before simulation. This distinction matters when translating a short API sketch into an executable experiment.

**MuJoCo** compiles URDF or its native MJCF into a model and advances a separate data/state object. MJCF exposes richer engine-specific constraints, tendons and actuators. Its [modeling documentation](https://mujoco.readthedocs.io/en/3.4.0/modeling.html) allows multiple primitive joints on one physical body, avoiding dummy bodies for such compound representations. An RL algorithm remains a separate controller/training procedure; a physics engine does not itself establish policy learning or biological validity.

**Pinocchio** supplies rigid-body algorithms including inverse dynamics, generalized inertia, kinematics and Jacobians, with URDF parsing, as described in its [official feature documentation](https://stack-of-tasks.github.io/pinocchio/). A dynamics library, numerical integrator, contact solver, derivative evaluator and trajectory optimizer are distinct components even when a larger package integrates them. Select and test the required combination rather than ranking software by an unsupported steps-per-second number.

### An Executed Import and Dynamics Check
...
[]{#example:four_bar_loop}

[]{#many-ways-to-reach-the-cup}

### Independent Mechanics Behind the Check
...
### Fixed-Control and Feedback Sensitivities
...
### A Bounded Numerical Interpretation
...
### Sensitivity to Initial Conditions and Robustness
...
[]{#princ-prin:sensitivity_practical}
...
## Primary Sources and Reading Guidance


1. [Lynch and Park, Mobility](https://modernrobotics.northwestern.edu/nu-gm-book-resource/2-2-degrees-of-freedom-of-a-robot/), and [Singularities](https://modernrobotics.northwestern.edu/nu-gm-book-resource/5-3-singularities/): independent constraints, task Jacobians and rank; these are mechanisms lessons, not human motor-control measurements.
1. [Khatib, Dynamic Consistency](https://khatib.stanford.edu/publications/pdfs/Khatib_1990_2.pdf): operational-space inertia and torque projection. Follow the dimensions of every matrix when reading the scanned equations.
1. [Sentis, Park and Khatib (2010)](https://cs.stanford.edu/groups/manips/publications/pdfs/Sentis_2010_TRO.pdf): multicontact and internal-force control, under the paper's contact and actuation assumptions.
1. [de Leva (1996)](https://doi.org/10.1016/0021-9290(95)00178-6): adjusted segment definitions, Table 4 and reference endpoints. Do not combine whole-trunk and trunk-subsegment fractions.
1. [Holzbaur, Murray and Delp (2005)](https://pubmed.ncbi.nlm.nih.gov/16078622/): an explicit arm and hand model; compare retained joints and muscles before transferring its parameters.
1. [Myers and Colleagues (2008)](https://pubmed.ncbi.nlm.nih.gov/17852693/) and [Glazier (2011)](https://pubmed.ncbi.nlm.nih.gov/21699094/): torso/pelvis associations and the interpretation of movement variability, respectively; neither identifies the torque-null controller of a golfer.
1. [MuJoCo 3.4 Python Bindings](https://mujoco.readthedocs.io/en/3.4.0/python.html), the linked modeling manual, URDF API, Drake Parser and Pinocchio feature documentation: verify importer semantics and runtime version independently of scientific validation.


::: {.callout-note}
## On This Site

- **[Model Ladder](../../../models/model-ladder.html)** — Progressive model complexity for human and equipment systems
- **[Degrees of Freedom and Dimensionality](../../degrees-of-freedom-and-dimensionality.html)** — State dimension, observability, and control authority
:::

## Chapter Exercises

[]{#exercises-ex:ch23_exercises}
