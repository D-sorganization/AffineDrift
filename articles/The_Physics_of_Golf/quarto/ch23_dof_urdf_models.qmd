...
Consider a classic planar 4-bar linkage: 4 rigid links connected by 4 revolute joints in a closed loop (ground → link 1 → link 2 → link 3 → ground). All motion is in the plane.

Apply the **planar Grübler formula** directly:
$$
\mathrm{DOF}_{\text{planar}} = 3(N-1) - \sum_{i=1}^{J} c_i^{\text{planar}}
$$

where each planar revolute joint imposes $c^{\text{planar}} = 2$ constraints (it removes 2 translational DOF while leaving the 1 rotational DOF free).

**Count:**
- $N = 4$ bodies (ground + 3 moving links)
- $J = 4$ revolute joints, each with $c^{\text{planar}} = 2$

$$\mathrm{DOF} = 3(4-1) - 4 \times 2 = 9 - 8 = 1$$

**Result:** A 4-bar linkage has **1 DOF**. Specify one joint angle and the entire mechanism is determined. $\checkmark$
:::
...
<robot name="golfer">
  ...
  <!-- Lumbar spine: 3-DOF implemented as three sequential revolutes.           -->
  <!-- URDF has no native type="spherical"; three 1-DOF revolutes are used     -->
  <!-- with massless intermediate links to approximate a spherical joint.       -->
  <link name="lumbar_flex_intermediate">
    <!-- massless intermediate link -->
    <inertial>
      <mass value="0.001"/>
      <inertia ixx="0.000001" ixy="0" ixz="0" iyy="0.000001" iyz="0" izz="0.000001"/>
    </inertial>
  </link>
  <link name="lumbar_bend_intermediate">
    <!-- massless intermediate link -->
    <inertial>
      <mass value="0.001"/>
      <inertia ixx="0.000001" ixy="0" ixz="0" iyy="0.000001" iyz="0" izz="0.000001"/>
  ...
  <!-- Lumbar flexion/extension about y-axis -->
  <joint name="lumbar_flexion" type="revolute">
    <parent link="pelvis"/>
    <child link="lumbar_flex_intermediate"/>
    <origin xyz="0 0 0" rpy="0 0 0"/>
    <axis xyz="0 1 0"/>
    <limit lower="-0.7" upper="0.7" effort="200" velocity="2.0"/>
  </joint>

  <!-- Lumbar lateral bending about x-axis -->
  <joint name="lumbar_lateral_bend" type="revolute">
    <parent link="lumbar_flex_intermediate"/>
    <child link="lumbar_bend_intermediate"/>
    <origin xyz="0 0 0" rpy="0 0 0"/>
    <axis xyz="1 0 0"/>
    <limit lower="-0.35" upper="0.35" effort="150" velocity="2.0"/>
  </joint>

  <!-- Lumbar axial rotation about z-axis -->
  <joint name="lumbar_axial_rotation" type="revolute">
    <parent link="lumbar_bend_intermediate"/>
    <child link="lumbar"/>
    <origin xyz="0 0 0" rpy="0 0 0"/>
    <axis xyz="0 0 1"/>
...
**Note on URDF spherical joints:** URDF does not natively support `type="spherical"`. The standard workaround is three sequential single-axis revolute joints connected by massless intermediate links, as shown above for the lumbar spine. This correctly models 3-DOF behavior; the massless intermediate links have negligible effect on dynamics but allow each rotational axis to be independently specified. Engines such as Drake and MuJoCo provide their own spherical-joint extensions that circumvent this limitation.
