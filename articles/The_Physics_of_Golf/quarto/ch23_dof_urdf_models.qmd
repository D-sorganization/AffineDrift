...
This is consistent with motion capture observations suggesting that golfers tend to show individual variation in arm configuration even when producing similar club trajectories. The task (club trajectory) is constrained, but the arm configuration is not. Different golfers choose different null-space motions (illustrative; depends on individual motor strategies and training history).
<!-- TODO: add citation for motion capture evidence of golfer arm-configuration variability, e.g., 3D motion analysis studies -->
...
The golf swing is the same. A golfer has approximately 25--30 degrees of freedom (a commonly cited illustrative estimate; exact counts vary across biomechanical models and depend on how joints are defined; typically ranges from 20 to 35 DOF depending on model granularity).
<!-- TODO: add citation for the 25--30 DOF estimate for the golf swing, e.g., musculoskeletal modelling literature -->
