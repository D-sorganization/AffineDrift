...
Suppose you have a model of the human body made from the joints we discussed in the previous chapter on anatomy and joint modeling. You specify:
...
## Counting Degrees of Freedom --- The Robotics Way {#sec-counting-dof}
...
Each revolute in a plane allows 1 rotation, imposes 2 constraints (prevents 2 translations).
...
## Worked Example --- The Golf Swing Model {#sec-golf-swing-dof}
...
1. **Base:** Pelvis (fixed to ground). We'll define the pelvis in the global frame. The pelvis orientation is fixed (for now), or we can allow small pelvis rotations in the $z$-axis (azimuth, yaw).
...
4. **Trail side (right arm for right-handed golfer):**
...
The open-chain DOF is simply the sum of all joint DOFs: $\mathrm{DOF}_{\text{open}} = 3 + 3 + 3 + 2 + 2 + 3 + 2 + 2 = 20$.
...
## The Human Arm --- A 7-DOF Redundant Manipulator {#sec-arm-redundancy}
...
## Kinematic Redundancy and the Manifold of Solutions {#sec-manifold-solutions}
...
$$\text{Redundancy} = n - 6 = 25 - 6 = 19 \text{ DOF}$$
...
$$\mathcal{N}(\bm{J}) = \{\dot{\bm{q}} \in \mathbb{R}^n : \bm{J}\dot{\bm{q}} = \bm{0}\}$$
...
$$\mathcal{M}(\bm{x}^*) = \{\bm{q} \in \mathbb{R}^n : h(\bm{q}) = \bm{x}^*\}$$
...
## The 7-DOF Arm --- Redundancy in Miniature {#sec-arm-redundancy-detailed}
...
## Redundancy and Null-Space Control {#sec-null-space-control}

::: {.callout-note}
## Null-Space Optimization in the ZTCF
Recall from earlier chapters on affine structure and control that the system evolves under drift and control:
...
## URDF --- The Unified Robot Description Format {#sec-urdf}
...
## Building a Full-Body Golf URDF {#sec-golf-urdf}
...
## From URDF to Simulation --- Drake, MuJoCo, and Pinocchio {#sec-simulation-engines}
...
Here's the power of these tools. In an earlier chapter on parallel mechanisms, we derived the equations of motion:
...
## Sensitivity Analysis --- What Happens When Parameters Are Wrong? {#sec-sensitivity}
...
**Key finding:** The ZTCF (zero-torque constraint force analysis from the earlier chapter on soft tissue) is moderately sensitive to anthropometric parameters but *extremely sensitive* to initial conditions and joint torques.
...
## Building Intuition: From URDF to Golf Physics {#sec-intuition}
...
- **Sensitivity:** Initial conditions matter more than anthropometry for forward dynamics; both matter for inverse dynamics. High-quality motion capture is essential either way.
...
**Looking Ahead:** We now have a complete musculoskeletal model: joints with defined degrees of freedom, kinematic chains described in URDF, and dynamics engines to simulate them. But a model without a controller is just a ragdoll. Who---or what---controls the swing? The next part turns to the remarkable organ that orchestrates all of this: the human brain. The following chapter examines how the brain solves the control problem under severe constraints of delay, noise, and bandwidth.

## Chapter Exercises

- **DOF Counting:** For the golf swing model described in @sec-golf-swing-dof, compute the DOF using the Grübler formula:
   1. Count the bodies $N$ (include ground)
   2. Count the joints $J$ and their DOFs $f_i$
   3. Compute $\mathrm{DOF} = 6(N-1) - \sum(6-f_i)$
   4. If the model includes a two-handed grip on the club, subtract the loop closure constraints

- **Null-Space Motion:** For a 7-DOF arm (shoulder 3, elbow 1, pronation 1, wrist 2):
   1. Describe the null-space motion (what can the arm do without moving the hand?)
   2. If the task is to keep the hand at a fixed position, how many possible elbow positions exist?
   3. If you add the constraint that the elbow must stay at a specific height (z-coordinate), how many DOF remain in the null space?

- **URDF Construction:** Write a minimal URDF for a golfer's spine and trail arm (3-link: thorax, upper arm, forearm). Include:
   1. Three links with realistic inertias (use de Leva data for 70 kg athlete)
   2. Two 3-DOF spherical joints (thorax rotation, shoulder) implemented as three sequential revolutes (ZYX order)
   3. Masses and inertia tensors
