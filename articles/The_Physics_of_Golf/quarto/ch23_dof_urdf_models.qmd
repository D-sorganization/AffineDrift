...
This is consistent with motion capture observations suggesting that golfers tend to show individual variation in arm configuration even when producing similar club trajectories [@kwon2012kinematic; @nesbit2005three]. The task (club trajectory) is constrained, but the arm configuration is not. Different golfers choose different null-space motions.
...
The golf swing is the same. A golfer has approximately 25--30 degrees of freedom (a commonly cited estimate; exact counts vary across biomechanical models and depend on how joints are defined) [@bernstein1967coordination; @winter2009biomechanics]. But the task --- place the club face square to the target with maximum speed at impact --- requires control of only 6 variables: the position (3) and orientation (3) of the club head.
