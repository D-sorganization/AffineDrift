...
description: "The Grübler-Kutzbach formula (also called the Chebychev-Grübler-Kutzbach formula) counts the total degrees of freedom of a kinematic system."
---
...
Suppose you have a model of the human body made from the joints we discussed in Chapter @sec-anatomy_joint_modeling. You specify:
...
**Bodies:** Ground (if we consider the base fixed) + 3 moving links = 4 bodies. Count the ground as the reference and use $N = 4$ rigid bodies total.

A 4-bar mechanism has 4 links forming a loop. One link is the ground frame (fixed), so $N = 4$ (ground + 3 moving links).
...
Negative DOF? This is wrong. The open-chain formula gives a negative answer when there is a loop, so loop closure constraints must be included.

**Loop constraints:** The four bars form a closed loop. This loop closure imposes a constraint: the end of bar 1 and the start of bar 2 must meet (position match), AND the end of bar 3 and the start of bar 4 must meet. These are 6 constraints per loop (3 position + 3 orientation), but in a planar 4-bar mechanism only some of these constraints are active.

The correct formula for a mechanism with loops is:
...
For a planar mechanism, the Grübler formula is:

$$\mathrm{DOF}_{\text{planar}} = 3(N-1) - \sum c_i^{\text{planar}}$$

For a planar 4-bar: $N = 4$, and each revolute imposes $c^{\text{planar}} = 1$ constraint.
$$\mathrm{DOF} = 3(3) - 4(1) = 9 - 4 = 5$$

This result is still incorrect. A 4-bar linkage should have 1 DOF: one input angle determines the remaining configuration.

The issue is the counting convention. In a planar mechanism:
...
Each revolute in a plane allows 1 rotation and imposes 2 constraints, preventing the two in-plane translations.
...
For a 4-bar: $N = 4$, $J = 4$ revolutes, and $\sum c = 4 \times 2 = 8$.

$$\mathrm{DOF} = 3(4-1) - 8 = 9 - 8 = 1$$

A 4-bar linkage has 1 DOF. $\checkmark$
:::
...
For this open chain, use the simpler tree formula:
...
This is consistent with motion capture observations suggesting that golfers tend to show individual variation in arm configuration even when producing similar club trajectories [@Nesbit2005; @Horan2010]. The task (club trajectory) is constrained, but the arm configuration is not. Different golfers choose different null-space motions (illustrative; depends on individual motor strategies and training history).
...
The golf swing is the same. A golfer has approximately 25--30 degrees of freedom [@Delp2007; @Rajagopal2016] (a commonly cited illustrative estimate; exact counts vary across biomechanical models and depend on how joints are defined; typically ranges from 20 to 35 DOF depending on model granularity).
...
Recall from Chapters @sec-05_affine_structure and 6 that the system evolves under drift and control:
...
Here's the power of these tools. In Chapter @sec-parallel_mechanisms, we derived the equations of motion:
...
**Key finding:** The ZTCF (zero-torque constraint force analysis from Chapter @sec-soft_tissue_pliable) is moderately sensitive to anthropometric parameters but *extremely sensitive* to initial conditions and joint torques.
...
### Looking Ahead
We now have a complete musculoskeletal model: joints with defined degrees of freedom, kinematic chains described in URDF, and dynamics engines to simulate them. But a model without a controller is just a ragdoll. Who---or what---controls the swing? Part VIII turns to the remarkable organ that orchestrates all of this: the human brain. Chapter @sec-motor_control_brain examines how the brain solves the control problem under severe constraints of delay, noise, and bandwidth.
