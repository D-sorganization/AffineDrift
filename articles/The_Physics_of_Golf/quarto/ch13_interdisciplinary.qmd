...
The affine framework provides a bridge between control theory and biomechanics. Both disciplines can now speak the same language: the language of drift $f(\state)$ and control $G(\state)\bm{u}$.
...
Carbon fiber composites are strong, stiff, and light. This is why they are used in golf shafts. A carbon shaft can be engineered to have specific stiffness properties: the bending stiffness (described by $EI$ in the chapter on flexible shaft) can be tuned by varying the fiber orientation and the resin matrix.
...
A COR of 1.0 means no energy loss in the normal (compression) direction; energy can still be lost through oblique impact spin and tangential deformation even when COR = 1. A COR less than 1.0 means additional energy is lost in the normal direction. Modern golf ball and clubface technology has COR values in the range of roughly 0.8 or above [@penner2003physics], meaning most of the normal-direction impact energy is transferred to the ball. The USGA regulates driver face compliance through the Characteristic Time (CT) test (limit: 239 μs), which corresponds to an effective COR of approximately 0.82 measured at reference speed.
...
This is where ball fitting meets swing physics. As a general principle, a slower golfer benefits from a softer ball with higher COR, because the softer ball deforms more, allowing the golfer to transfer more energy. A faster golfer can use a harder ball with lower COR, gaining better control and tighter dispersion [@penner2003physics].

**Example: Ball Fitting for Swing Speed**

For this illustrative example, consider two golfers with different swing speeds. A slower golfer uses a soft ball (low compression, relatively high COR [@penner2003physics]). The impact dynamics transfer the energy efficiently, and the ball launches at a relatively high angle with low spin (due to the soft compression).

A faster golfer uses a harder ball (high compression, somewhat lower COR). The impact dynamics are harsher, but the higher clubhead speed means the ball still leaves the face at high speed. The harder ball produces a lower launch angle and higher spin, which can be used for control.

If the slower golfer tried the harder ball, they would lose distance (lower COR means less energy transfer). If the faster golfer tried the softer ball, they would gain too much carry distance and lose the control they get from the higher compression ball.
...
But there is an inverse problem: given the observed ball trajectory, what can we infer about the club and ball at impact? This is where data science and physics-informed parameter estimation come in.
...
This physics-informed approach constrains the solution space using the known structure of the drift field, in contrast to purely empirical regression methods that lack physical interpretability.
...
For this illustrative example, consider a golfer who hits a driver from a tee and a launch monitor records:
- Ball speed below the golfer's target.
- Spin rate above the optimal range for the given launch angle.
- Launch angle near optimal.
- Club path slightly open of target.
- Club face angle slightly open of target.

### The Physics

From the ball speed and the COR of the ball and clubface combination [@penner2003physics], we can estimate the clubhead speed just before impact. From the club path and face angle, we can infer the club's orientation and velocity in 3D.
...
The launch monitor displays a carry distance short of the golfer's target. The model can identify the root cause:
1. **Ball speed is low.** The ball speed is below the target for this golfer, suggesting the clubhead speed was low or the club made off-center contact.
2. **Spin rate is high.** The spin rate is adding excess backspin, reducing carry distance. The extra spin is likely because the club face was slightly closed relative to the club path, imparting extra spin.
3. **Launch angle is near optimal.** The launch angle is close to the optimal range for this golfer's swing speed.
...
5. **Data science and physics-informed parameter estimation allow us to learn personalized drift fields from launch monitor data.**
...
10. **Technology enables insight:** launch monitors, motion capture, and statistical inference make the invisible physics visible, allowing golfers and coaches to learn from data rather than just intuition.
