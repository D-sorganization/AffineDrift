---
description: "Control theory originated in engineering: how do you design a system that reliably achieves a goal, even when it is subject to disturbances and..."
---
...
Biomechanics has historically treated the swing as a list of muscle activations: "the infraspinatus fires, then the pectoralis major, then the forearm extensors," and so on. This is descriptive but not explanatory. Why does the infraspinatus fire at that moment? How does its activation contribute to the clubhead trajectory?
...
**Example: The Infraspinatus Activation**

The infraspinatus (one of the rotator cuff muscles responsible for external rotation) activates during the transition and early downswing to control the shoulder's rotational state. (Note: the supraspinatus primarily *abducts* the shoulder; external rotation is performed by the infraspinatus and teres minor.) Traditionally, biomechanists say "the infraspinatus activates to control external rotation."

From a control theory perspective: the early downswing is a phase of high control authority and low drift (many muscle groups are active, and gravity has not yet accelerated the clubhead much). The infraspinatus activation is part of the overall control strategy to set up a drift field that will naturally lead to the desired clubhead trajectory by mid-downswing. Once the swing is moving fast (mid to late downswing), the infraspinatus activation ceases or decreases; the drift dominates, and control authority is low.

This perspective explains why the infraspinatus is most active early in the swing, not late---it is setting up the drift, not fighting it.

The affine framework provides a bridge between control theory and biomechanics. Both disciplines can now speak the same language: the language of drift $f(\mathbf{x})$ and control $G(\mathbf{x})\control$.
...
The human nervous system seems to plan in task space (where the hand should be) and then translate into joint space (what angles are needed). But for repetitive, well-practiced motions like the golf swing, the nervous system may learn a direct joint-space trajectory that it can execute quickly.
...
This is consistent with the affine framework. In that picture, the motor cortex specifies the desired hand trajectory (or perhaps the desired phase of the swing---early, mid, late downswing). The cerebellum then translates this high-level instruction into joint trajectories and muscle activation patterns.
...
The control authority is highest early in the swing (when velocities are low and the nervous system can respond quickly to errors) and lowest late in the swing (when velocities are high and feedback delays make meaningful adjustment impractical). This is consistent with what the affine framework predicts: drift dominates late in the swing, when control authority is low.
...
These materials have mechanical properties that significantly affect the swing and the outcome.

### Shaft Materials and Design

Carbon fiber composites are strong, stiff, and light. This is why they are used in golf shafts. A carbon shaft can be engineered to have specific stiffness properties: the bending stiffness (described by $EI$ in @sec-flexible_shaft) can be tuned by varying the fiber orientation and the resin matrix.

A shaft with fibers oriented primarily in the lengthwise direction (along the shaft) is stiff in bending and torsion. A shaft with fibers oriented at an angle (spirally wrapped) is more compliant and can absorb more deformation. A shaft with a variable fiber orientation (stiffer near the grip, more flexible near the tip) produces a specific bending profile.
...
The compression affects how much energy is transferred to the ball and how much is lost to heat and deformation. Two related quantities are important:

The **coefficient of restitution** (COR) is the ratio of the *relative separation speed* to the *relative approach speed* of the clubface and ball:

$$
\mathrm{COR} = \frac{v_{\text{separation}}}{v_{\text{approach}}}
$$

A COR of 1.0 would mean a perfectly elastic collision (no energy loss). Modern golf ball and clubface technology has COR values of about 0.83 (the USGA limit). The fraction of kinetic energy transferred depends on both the COR and the mass ratio of club to ball, not on COR alone.

The **smash factor** is the ratio of ball speed to clubhead speed:

$$
\text{Smash Factor} = \frac{v_{\text{ball}}}{v_{\text{club}}}
$$

Typical smash factors for drivers are reported to be approximately 1.45--1.50; this varies with strike quality and equipment [@Broadie2014; @trackman_launch]. These two quantities are related but distinct: COR describes the collision efficiency, while smash factor describes the speed amplification (which depends on the mass ratio as well as COR).

The COR depends on both the ball and the clubface:
- Softer balls (lower compression) tend to have higher COR at lower swing speeds.
- Harder balls (higher compression) tend to have lower COR at lower swing speeds but may perform better at higher speeds.
- The relationship between clubhead speed and effective COR is complex and depends on ball construction.

This is where ball fitting meets swing physics. In this illustrative example, a slower golfer (say, 80 mph swing speed) benefits from a softer ball with higher COR, because the softer ball deforms more, allowing the golfer to transfer more energy. A faster golfer (say, 100 mph) can use a harder ball with lower COR, gaining better control and tighter dispersion. The specific thresholds vary by manufacturer and golfer; the general principle---that ball compression should be matched to swing speed---is widely discussed in equipment fitting literature [@Penner2001].
...
Alice uses a soft ball (low compression, COR $\approx 0.82$). In this illustrative estimate, the impact dynamics transfer the energy efficiently, and the ball launches at a relatively high angle with low spin (due to the soft compression).

Bob uses a harder ball (high compression, COR $\approx 0.78$). In this illustrative estimate, the impact dynamics are harsher, but Bob's higher clubhead speed means the ball still leaves the face at high speed. The harder ball produces a lower launch angle and higher spin, which Bob can use for control. This pattern---lower-compression balls exhibiting higher COR than higher-compression balls when struck at comparable speeds---is consistent with empirical measurements [@Jorgensen1994].
...
The model that predicts ball trajectory from swing parameters is a solution to the equations of motion for the ball under gravity, air drag, and spin-induced lift (Magnus effect). This is a well-understood physics problem.
...
Consider the rotator cuff, which must stabilize the shoulder joint against the large forces generated during the swing. The affine framework predicts that if the golfer relies primarily on arm and shoulder muscles (high local control inputs), the rotator cuff bears a larger share of the load. If the golfer sequences the motion to use the legs and torso (distributing loads across the whole body), the model predicts lower rotator cuff loading. While this reasoning is biomechanically sound, the direct causal relationship between swing technique and rotator cuff injury rates has not been established through controlled clinical trials.
...
Sports medicine could potentially use this framework to inform injury prevention programs. The goal would be to train the golfer to use technique (good sequencing, high drift) rather than pure force (high control inputs). The framework predicts this would reduce the load on vulnerable structures, though prospective studies would be needed to validate the clinical effectiveness of such an approach.
...
This exercise is valuable, but the framework suggests it may be incomplete. It strengthens the rotator cuff muscles in isolation, improving the control authority $G$ at the shoulder. The affine decomposition raises the possibility that overload injuries may sometimes arise not from weakness alone, but from how forces are distributed across the kinematic chain---though this hypothesis would need clinical validation.
...
3. **Late downswing:** Drift dominates. The golfer is effectively on autopilot. Muscle activation is at a peak, but the nerves are executing a pre-planned sequence. Control inputs will have little effect on the clubhead trajectory.
...
3. **Is actionable.** A cue must describe something the golfer can do. "Swing faster" is not actionable (how do you swing faster?). "Rotate your hips more aggressively" is more actionable (the golfer knows to use their leg and hip muscles more).
...
The problem may instead lie in the grip, with the hands too far inside at the top of the backswing. The coach could then cue "move your hands more toward the ball at address" or "feel like your hands are in front of your torso at the top." This changes the initial condition, which in turn changes the entire drift field for that swing.
...
7. **Sports medicine:** The framework suggests that injuries may arise when the forces required to execute the swing exceed the tissues' capacity. By optimizing the ZTCF (through technique) or through equipment changes, the model predicts reduced loading on vulnerable structures.
...
Using the smash factor (ball speed / clubhead speed $\approx$ 1.49 for a well-struck driver), the clubhead must have been traveling at about $145 / 1.49 \approx 97$ mph just before impact. From the club path and face angle, we can infer the club's orientation and velocity in 3D.
...
6. **Sports medicine may benefit from force analysis:** the framework predicts that good technique distributes load across the body, which may reduce injury risk.
...
In the context of injury prevention, use the affine framework to argue why optimizing the ZTCF (through technique) might be more effective long-term than strengthening muscles alone. What evidence would be needed to validate this hypothesis?
