...
The affine framework provides a bridge between control theory and biomechanics. Both disciplines can now speak the same language: the language of drift $f(\mathbf{x})$ and control $G(\mathbf{x})\bm{u}$.
...
1. **Poor technique:** High control inputs in the arm and shoulder, swinging primarily with arm muscles. This means high $\bm{u}$ in the shoulder joints. The control authority $G$ in the shoulder is high, so the required torques are large. The resulting forces in the rotator cuff tendons are large.
2. **Good technique:** High control inputs in the legs and torso, swinging with the whole body. This means high $\bm{u}$ in the hip and spine joints, but lower $\bm{u}$ in the shoulder. The drift field carries much of the motion automatically. The forces in the rotator cuff are lower.
