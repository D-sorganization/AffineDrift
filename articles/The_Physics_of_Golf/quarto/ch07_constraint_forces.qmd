...
You learned in [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual) that the downswing is passive—gravity and Coriolis drive acceleration. But [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual) left a critical question unanswered:
...
- **Grip constraint:** Your hand is fixed on the grip, so the grip point cannot move relative to your palm: $\bm{\Phi}_{\text{grip}}(\bm{q}) = \bm{x}_{\text{grip}} - \text{const} = \mathbf{0}$, which is the kind of wrench-bearing static coupling formal screw theory was built to describe [@crane2022screw_theory].
...
Return to the double pendulum from [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual), but now add a constraint: the wrist is locked so that $q_1 + q_2 = \text{const} = 115°$. This forces the club to move relative to the arm in a coordinated way.
...
At the configuration from [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual) ($q_1 = 45°$, $\dot{q}_1 = 8$ rad/s, $q_2 = 70°$, $\dot{q}_2 = 12$ rad/s), the unconstrained accelerations (from the ZTCF family) are:
...
Solving (using values from [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual)):
...
**What comes next:** Understanding constraints, we now ask: what if there are *multiple* constraints forming a closed loop? What if the body is not just a serial chain (hips → shoulders → arms → club) but a parallel mechanism with closed kinematic chains? That's the subject of [Chapter 9](./ch09_parallel_mechanisms.qmd#sec-parallel_mechanisms).

But first, [Chapter 8](./ch08_triple_pendulum.qmd#sec-triple_pendulum) extends the double pendulum to a triple pendulum, adding the wrists, and shows how the constraint-driven energy transfer becomes even more dramatic with an additional segment.

## Chapter Exercises: Constraint Forces

{Conceptual}
Explain in plain language: Why can a grip force of 100 N (an illustrative value; actual grip forces vary with swing speed and technique; approximately 50--150 N is a typical estimated range [@Hume2005]) do zero net work on the system, yet transfer energy from the arm to the club?
