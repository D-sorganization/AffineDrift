...
- **Inequality (unilateral) constraints** such as ground contact ($z_{\text{feet}} \geq 0$) or one-way grip coupling: active only when engaged, with a sign-definite multiplier. These are the subject of Chapter~\ref{ch:09_parallel_mechanisms} (closed kinematic chains) and Chapter~\ref{ch:15_underactuation_release} (contact breaking / under-actuation).
...
3. **Club-ball impact.** During the $\sim 0.5$ ms of ball-club contact the ball's position is constrained to the face: $\bm{x}_{\text{ball}} = \bm{x}_{\text{face-contact}}(\bm{q})$. This is a transient holonomic constraint; see Chapter~\ref{ch:28_impact_collision} for the impulse-exchange treatment that uses the multiplier formula above evaluated at the instantaneous contact configuration.
4. **Rigid club shaft.** The idealization "shaft length is constant" is the holonomic constraint $\|\bm{x}_{\text{tip}} - \bm{x}_{\text{grip}}\| - L_{\text{shaft}} = 0$. Deviations (shaft droop, bend) are modeled as compliant rather than rigid in Chapter~\ref{ch:12_compliant_shaft}.
...
A professional golfer's grip force during the downswing—measured via instrumented clubs [@cochran1968search; @penner2003physics]—can be substantial, ranging from tens to over a hundred Newtons. This force is *not* a muscular torque directly. Instead, it's the reaction force that the grip constraint exerts on the club.
...
During the downswing, this ground constraint produces enormous vertical forces. Studies using force plates show that peak ground reaction forces during the swing can exceed body weight by a significant margin [@penner2003physics]. These forces are constraint forces—they arise from the geometry of contact with the ground.
