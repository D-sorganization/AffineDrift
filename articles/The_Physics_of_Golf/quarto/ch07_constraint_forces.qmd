---
description: "Now here's the key insight: to enforce a constraint, the joint must push. If your arm wants to fly away from your shoulder due to centrifugal effects, the..."
---


# Constraint Forces: The Hidden Engines of the Swing {#sec-constraint_forces}

::: {.callout-tip}
## Why Constraint Forces Matter Most

You learned in @sec-zero_torque_counterfactual that the modeled downswing is largely passive—gravity and Coriolis drive much of the acceleration. But @sec-zero_torque_counterfactual left a critical question unanswered:
...
The answer is constraint forces. They are the *hidden engines* of the kinetic chain, exactly the kind of noncontributing forces analyzed in Kane-style constrained dynamics [@roithmayr2016kanes_method].
...
- **Forces that are enormous:** The constraint forces at your wrist during a swing can exceed $500$ N—far more than your muscles produce [@Nesbit2005].
...
- **Grip constraint:** Your hand is fixed on the grip, so the grip point cannot move relative to your palm: $\bm{\Phi}_{\text{grip}}(\bm{q}) = \bm{x}_{\text{grip}} - \text{const} = \mathbf{0}$. Screw theory was built to describe exactly this kind of wrench-bearing static coupling [@crane2022screw_theory].
...
And here's the paradox: this push does zero net work. Why? Because the push acts along the direction of the constraint. The forearm can't move away from the shoulder; the constraint prevents it. A force along that forbidden direction never moves the endpoint, so it does zero net work.
...
For a double pendulum (shoulder and elbow), suppose the wrist constraint enforces that the endpoint of the arm is at a fixed angle relative to the global frame. This is a simplification: imagine the wrist locked at a specific orientation.
...
## Solving for the Constraint Forces {#sec-solving_constraint_forces}
...
For ideal, scleronomic (time-independent) constraints $\bm{\Phi}(\bm{q}) = \mathbf{0}$, the power delivered by a constraint force is:

$$
P_{\text{constraint}} = \bm{\lambda}^T \bm{J}_c(\bm{q}) \dot{\bm{q}}
$$

Since $\bm{J}_c(\bm{q}) \dot{\bm{q}} = \frac{d\bm{\Phi}}{dt}$ and $\bm{\Phi}(\bm{q}(t)) = \mathbf{0}$ for all $t$ (with no explicit time dependence $\frac{\partial \bm{\Phi}}{\partial t} = \mathbf{0}$), we have:
...
At any instant, for a partitioned system $\bm{q} = \begin{bmatrix} \bm{q}_1 \\ \bm{q}_2 \end{bmatrix}$ with mass matrix $\bm{M}(\bm{q}) = \begin{bmatrix} \bm{M}_{11} & \bm{M}_{12} \\ \bm{M}_{21} & \bm{M}_{22} \end{bmatrix}$, the total kinetic energy is:

$$
T = T_{\text{arm}} + T_{\text{club}} + T_{\text{cross}} = \frac{1}{2}\dot{\bm{q}}_1^T \bm{M}_{11}(\bm{q}) \dot{\bm{q}}_1 + \frac{1}{2}\dot{\bm{q}}_2^T \bm{M}_{22}(\bm{q}) \dot{\bm{q}}_2 + \dot{\bm{q}}_1^T \bm{M}_{12}(\bm{q}) \dot{\bm{q}}_2
$$

where $T_{\text{cross}} = \dot{\bm{q}}_1^T \bm{M}_{12}(\bm{q}) \dot{\bm{q}}_2$ accounts for cross-coupling kinetic energy between the segments due to the off-diagonal mass matrix block $\bm{M}_{12}$.
...
## Constraint Force in a Double Pendulum Wrist Hinge {#sec-double_pendulum_constraint_force}

**Example: Wrist Constraint Force in the Double Pendulum**

Return to the double pendulum from @sec-zero_torque_counterfactual, but now add a constraint: the wrist is locked so that $q_1 + q_2 = \text{const} = 115°$. This forces the club to move relative to the arm in a coordinated way.
...
At the configuration from @sec-zero_torque_counterfactual ($q_1 = 45°$, $\dot{q}_1 = 8$ rad/s, $q_2 = 70°$, $\dot{q}_2 = 12$ rad/s), the unconstrained accelerations (from the ZTCF) are:
...
Using the procedure from @sec-solving_constraint_forces, we solve for $\lambda$. The constraint acceleration condition is:
...
The constraint force acts equally on both joints, with generalized-force components of $\lambda$ at each joint.
...
Solving (using values from @sec-zero_torque_counterfactual):
...
The constraint force magnitude is $\lambda \approx 0.265$ Nm. This is the "effort" required to maintain the constraint. It appears equally at both joints.
...
Those raw numbers are not the full power balance. The zero-power result above applies to the coupled generalized constraint force over the whole system, while the segment-level bookkeeping depends on how the constraint is projected into each coordinate. The safe conclusion is:
...
5. By the time the chain reaches the club, the constraint force is enormous (hundreds of Newtons), far exceeding what any muscle can produce.
...
However, the golfer *can* modulate the grip force by changing muscle tension or by allowing some slip. A gentle grip (low force) allows some flexibility; a firm grip (high force) enforces the constraint tightly. This modulation is a control knob.

**Insight:** The large grip forces in a golf swing (often 100+ N at late downswing) are not evidence of muscular effort. They're evidence of massive inertial forces being channeled through the constraint. Muscles hold the constraint in place; they do not drive the acceleration.
...
**What comes next:** Understanding constraints, we now ask: what if there are *multiple* constraints forming a closed loop? What if the body is not just a serial chain (hips → shoulders → arms → club) but a parallel mechanism with closed kinematic chains? That's the subject of @sec-parallel_mechanisms.

But first, @sec-triple_pendulum extends the double pendulum to a triple pendulum, adding the wrists, and shows how the constraint-driven energy transfer becomes even more dramatic with an additional segment.

## Chapter Exercises: Constraint Forces

{Conceptual}
Take a grip force of 100 N. This is an illustrative value: actual grip forces vary with swing speed and technique, and approximately 50--150 N is a typical estimated range [@Hume2005]. Explain in plain language: why can such a force do zero net work on the system, yet transfer energy from the arm to the club?

(Hint: Think about the direction of the grip force and the direction of motion. Are they parallel or perpendicular?)

{Geometric}
For the wrist constraint in the double pendulum (@sec-double_pendulum_constraint_force), sketch the configuration of the arm and club when $q_1 + q_2 = 115°$. Identify the null space (allowed motion) and the direction of the constraint force.
...
For the double pendulum with constraint (@sec-double_pendulum_constraint_force):
