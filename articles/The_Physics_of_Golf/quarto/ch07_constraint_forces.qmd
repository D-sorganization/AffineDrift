...
then multiply the EOM on the left by $\bm{J}_c\bm{M}^{-1}$:

$$
\bm{J}_c\ddot{\bm{q}} = \bm{J}_c\bm{M}^{-1}\bigl[\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g}\bigr] + \bm{J}_c\bm{M}^{-1}\bm{J}_c^{\top}\bm{\lambda}.
$$

Setting this equal to $-\dot{\bm{J}}_c\dot{\bm{q}}$ (from the twice-differentiated constraint) and solving:
...
To find $\bm{\lambda}$ in practice, we use the constraint that $\bm{\Phi}(\bm{q}) = \mathbf{0}$ must be maintained at all times. Taking the time derivative twice:

$$
\bm{J}_c(\bm{q}) \ddot{\bm{q}} = -\dot{\bm{J}}_c(\bm{q}, \dot{\bm{q}}) \dot{\bm{q}}
$$

Substituting the constrained EOM and solving for $\bm{\lambda}$:

$$
\bm{J}_c \bm{M}^{-1} \bm{J}_c^T \bm{\lambda} = -\bm{J}_c \bm{M}^{-1} (\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g}) - \dot{\bm{J}}_c \dot{\bm{q}}
$$
