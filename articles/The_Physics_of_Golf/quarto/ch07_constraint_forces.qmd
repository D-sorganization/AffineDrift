...
- **Forces that are enormous:** The constraint forces at your wrist during a swing can far exceed what your muscles produce---computations with realistic swing models routinely yield wrist constraint forces in the hundreds of Newtons.
...
::: {.callout-warning}
## Scope: Holonomic Equality Constraints

This chapter treats **holonomic equality constraints** only---algebraic constraints of the form $\bm{\Phi}(\bm{q}) = \mathbf{0}$ that restrict the configuration at every instant in time. Two important generalizations are deferred:

- **Inequality (unilateral) constraints** such as ground contact ($z_{\text{feet}} \geq 0$) or one-way grip coupling: active only when engaged, with a sign-definite multiplier. These are the subject of Chapter 9 (closed kinematic chains) and a later chapter on contact breaking / under-actuation.
- **Non-holonomic constraints** (velocity-level constraints that cannot be integrated to configuration form, e.g., rolling contact): they require Pfaffian-form treatment with different multiplier bookkeeping. No important non-holonomic constraints arise in the golf swing at the level of modeling we use here; they are flagged for completeness.

Within the holonomic-equality world, the Lagrange-multiplier formulation below is exact, not an approximation.
...
$$\bm{\Phi}(q_1, q_2) = q_1 + q_2 - \theta_{\text{fixed}} = 0$$
...
$$\bm{M}(\bm{q})\ddot{\bm{q}} + \bm{C}(\bm{q}, \dot{\bm{q}})\dot{\bm{q}} + \bm{g}(\bm{q}) = \bm{B}(\bm{q})\bm{u} + \bm{J}_c(\bm{q})^T \bm{\lambda}$$

where:
- $\bm{M}(\bm{q})$ is the mass matrix.
- $\bm{C}(\bm{q}, \dot{\bm{q}})$ is the Coriolis/centrifugal matrix (so $\bm{C}\dot{\bm{q}}$ gives the Coriolis/centrifugal torque vector).
...
1. **Gravity and Coriolis** ($\bm{C}\dot{\bm{q}}, \bm{g}$): passive forces we've already discussed.
...
### Derivation from the Constrained Lagrangian

Before specifying the computational procedure, it is worth deriving the constraint-force term $\bm{J}_c^{\top}\bm{\lambda}$ and the multiplier formula from first principles. The derivation uses the method of Lagrange multipliers applied to the classical Lagrangian $L = T - V$.

**Step 1.** Form the **augmented Lagrangian** by adding each constraint weighted by an unknown multiplier $\lambda_i$:

$$\tilde{L}(\bm{q}, \dot{\bm{q}}, \bm{\lambda}) = L(\bm{q}, \dot{\bm{q}}) + \bm{\lambda}^{\top} \bm{\Phi}(\bm{q})$$

The multipliers $\bm{\lambda} \in \mathbb{R}^{n_c}$ are treated as auxiliary generalized coordinates.

**Step 2.** Apply the Euler--Lagrange equations to $\tilde{L}$. For the original coordinates $\bm{q}$,

$$\frac{d}{dt}\frac{\partial \tilde{L}}{\partial \dot{\bm{q}}} - \frac{\partial \tilde{L}}{\partial \bm{q}} = \mathbf{0} \quad\Longrightarrow\quad \frac{d}{dt}\frac{\partial L}{\partial \dot{\bm{q}}} - \frac{\partial L}{\partial \bm{q}} = \underbrace{\left(\frac{\partial \bm{\Phi}}{\partial \bm{q}}\right)^{\top}}_{\bm{J}_c^{\top}} \bm{\lambda}$$

The left-hand side, after including the applied generalized forces $\bm{B}\bm{u}$, is exactly the manipulator equation left-hand side, so we recover

$$\bm{M}(\bm{q})\ddot{\bm{q}} + \bm{C}(\bm{q},\dot{\bm{q}})\dot{\bm{q}} + \bm{g}(\bm{q}) = \bm{B}(\bm{q})\bm{u} + \bm{J}_c(\bm{q})^{\top}\bm{\lambda}$$

For the auxiliary coordinates $\bm{\lambda}$, the Euler--Lagrange equation reproduces the constraint itself:

$$\frac{\partial \tilde{L}}{\partial \bm{\lambda}} = \bm{\Phi}(\bm{q}) = \mathbf{0}$$

**Step 3.** To solve for $\bm{\lambda}$, differentiate the constraint twice in time,

$$\ddot{\bm{\Phi}} = \bm{J}_c(\bm{q})\ddot{\bm{q}} + \dot{\bm{J}}_c(\bm{q},\dot{\bm{q}})\dot{\bm{q}} = \mathbf{0}$$

then substitute from the EOM $\ddot{\bm{q}} = \bm{M}^{-1}[\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g} + \bm{J}_c^{\top}\bm{\lambda}]$:

$$\bm{J}_c\bm{M}^{-1}\bigl[\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g}\bigr] + \bm{J}_c\bm{M}^{-1}\bm{J}_c^{\top}\bm{\lambda} + \dot{\bm{J}}_c\dot{\bm{q}} = \mathbf{0}$$

Solving for $\bm{\lambda}$:

$$\boxed{\bm{\lambda} = \bigl[\bm{J}_c \bm{M}^{-1} \bm{J}_c^{\top}\bigr]^{-1} \Bigl(-\bm{J}_c\bm{M}^{-1}[\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g}] - \dot{\bm{J}}_c\dot{\bm{q}}\Bigr)}$$

The matrix $\bm{J}_c \bm{M}^{-1} \bm{J}_c^{\top}$, sometimes called the **operational-space inertia** or **Delassus matrix**, is a kinetic-energy-weighted Gram matrix of the constraint directions. It is $n_c \times n_c$, symmetric, and (generically) positive definite.

::: {.callout-note}
## Why the Derivation Clarifies the "Mystery" Term

The procedural derivation in the previous edition of this chapter quoted the formula $\bm{\lambda} = [\bm{J}_c\bm{M}^{-1}\bm{J}_c^{\top}]^{-1}(\cdots)$ without showing where it comes from. The augmented-Lagrangian derivation makes two things clear:

1. $\bm{J}_c^{\top}\bm{\lambda}$ is not a *new* physical force---it is the generalized-force expression of the constraint in the coordinates of $\bm{q}$, arising from the variational principle once we insist on $\bm{\Phi} = \mathbf{0}$.
2. The multiplier $\bm{\lambda}$ has dimensions determined by $\bm{\Phi}$ (force if $\bm{\Phi}$ is a position constraint, torque if $\bm{\Phi}$ is an angle constraint), and its physical meaning is "how much constraint force (or torque) per unit constraint violation is required to maintain the constraint."

:::

### Singular Configurations

The Lagrange-multiplier formula requires inverting $\bm{J}_c \bm{M}^{-1} \bm{J}_c^{\top}$. This matrix is positive definite *if and only if* $\bm{J}_c$ has full row rank ($\text{rank}(\bm{J}_c) = n_c$). When this fails, the configuration is called **constraint-singular**:

- **Redundant constraints** ($n_c$ constraint equations but only $n_c - 1$ independent directions) collapse a row of $\bm{J}_c$ and send the multiplier to $+\infty$ or leave it undetermined.
- **Geometric singularities** arise when two constraint surfaces become tangent---for example, the wrist constraint and the ground-contact constraint lined up along the same Cartesian axis at an awkward posture.

In practice one regularizes with $\bm{J}_c\bm{M}^{-1}\bm{J}_c^{\top} + \varepsilon \bm{I}$ for small $\varepsilon > 0$ (Tikhonov) or falls back to an SVD-based pseudoinverse truncating the smallest singular values. Both restore numerical well-posedness at the cost of a small constraint-violation rate; in golf-swing simulations singular configurations are rare but the ground-contact + two-hand-grip combination near the top of the backswing is a realistic risk point.

### Golf-Specific Constraints

The formalism above applies to four concrete constraints relevant to the golf swing, all holonomic and all equality when engaged:

1. **Two-hand grip coupling.** Both hands are on the grip; the trailing and lead hands must translate and rotate together. In a two-arm model this is a multi-dimensional position-and-orientation constraint reducing the relative wrist DOFs between the two arms to zero. Typical Lagrange-multiplier magnitudes at impact are in the $10^2$ N range [@Cochran1968].
2. **Ground contact (both feet planted).** $z_{\text{L-foot}} = 0$ and $z_{\text{R-foot}} = 0$ during the address-through-downswing phase. These become inequality constraints the instant a foot lifts (heel-lift at the top of the backswing, lead-heel drop during transition); in the phases where both feet are planted they are exact holonomic equality constraints. Peak vertical ground-reaction forces exceed body weight [@penner2003physics].
3. **Club-ball impact.** During the $\sim 0.5$ ms of ball-club contact the ball's position is constrained to the face: $\bm{x}_{\text{ball}} = \bm{x}_{\text{face-contact}}(\bm{q})$. This is a transient holonomic constraint; see Chapter 28 for the impulse-exchange treatment that uses the multiplier formula above evaluated at the instantaneous contact configuration.
4. **Rigid club shaft.** The idealization "shaft length is constant" is the holonomic constraint $\|\bm{x}_{\text{tip}} - \bm{x}_{\text{grip}}\| - L_{\text{shaft}} = 0$. Deviations (shaft droop, bend) are modeled as compliant rather than rigid in Chapter 12.

Each of these produces a $\bm{\lambda}$ that is physically meaningful and measurable: grip force, ground-reaction force, ball contact impulse, and shaft tension. The Lagrange-multiplier framework unifies their treatment.

::: {.callout-note}
## Computing Lagrange Multipliers: Summary

From the derivation above, the formula for the constraint multipliers is:

$$\bm{\lambda} = \bigl[\bm{J}_c \bm{M}^{-1} \bm{J}_c^{\top}\bigr]^{-1} \Bigl(-\bm{J}_c\bm{M}^{-1}[\bm{B}\bm{u} - \bm{C}\dot{\bm{q}} - \bm{g}] - \dot{\bm{J}}_c\dot{\bm{q}}\Bigr)$$

The correct derivation comes from differentiating $\bm{\Phi}(\bm{q}) = \mathbf{0}$ twice to get $\bm{J}_c\ddot{\bm{q}} + \dot{\bm{J}}_c\dot{\bm{q}} = \mathbf{0}$, then substituting the EOM for $\ddot{\bm{q}}$. There is no $\ddot{\bm{q}}_{\text{unconstrained}}$ term—that would be a double-counting error.
:::
...
::: {.callout-note}
## Null Space and Range of $\bm{J}_c$
...
Using the Lagrange-multiplier procedure, we solve for $\lambda$. The constraint acceleration condition is:
...
$$
\begin{aligned}
\bm{M}_{11} \ddot{q}_1 + \bm{M}_{12} \ddot{q}_2 + C_1 + g_1 &= \lambda\\
\bm{M}_{21} \ddot{q}_1 + \bm{M}_{22} \ddot{q}_2 + C_2 + g_2 &= \lambda
\end{aligned}
$$
...
With the constraint in place, the shoulder decelerates more slowly ($-2.44$ rad/s$^2$ vs unconstrained $-5.56$), and the elbow decelerates to $+2.44$ rad/s$^2$ (vs unconstrained $+21.9$).

The constraint force magnitude is $\lambda \approx 0.265$ Nm. This is the "effort" required to maintain the constraint. It acts equally at both joints ($\bm{J}_c^T \lambda = \begin{bmatrix} 0.265 \\ 0.265 \end{bmatrix}$ Nm), since the constraint Jacobian is $[1\ 1]$.

Now let's verify the zero-net-power condition. The power delivered by the constraint force to the system is:

$$
P_{\text{constraint}} = \bm{\lambda}^T \bm{J}_c \dot{\bm{q}} = \lambda (\dot{q}_1 + \dot{q}_2) = \lambda \frac{d}{dt}(q_1 + q_2)
$$

Since the constraint enforces $q_1 + q_2 = \text{const}$, we have $\dot{q}_1 + \dot{q}_2 = 0$, so $P_{\text{constraint}} = 0$. The constraint does zero net work on the system as a whole, as expected.

**Key Finding:** The constraint force is small ($\sim 0.3$ Nm) but its *direction* is crucial: it acts to slow the shoulder and slow the club's unconstrained extension. Over the course of the downswing, integrated over time, these forces accumulate to produce the dramatic energy transfer from arm to club.
...
For a torque-type constraint (angle constraint), the Lagrange multiplier $\lambda$ has units of Nm. For a position-type constraint (Cartesian endpoint position), $\lambda$ has units of N, and is directly the joint reaction force.
...
5. By the time the club is fully released, the constraint forces can be enormous (hundreds of Newtons), far exceeding what any muscle directly applies to the club.
...
And this is why active muscular control is secondary in the late downswing: muscles set up the initial conditions and make fine adjustments. Constraints do the heavy lifting.
...
A professional golfer's grip force during the downswing---measured via instrumented clubs [@Cochran1968; @penner2003physics]---can be substantial, ranging from tens to over a hundred Newtons. This force is *not* a muscular torque directly. Instead, it's the reaction force that the grip constraint exerts on the club.
...
**Insight:** The large grip forces in a golf swing are not evidence of muscular effort. They're evidence of massive inertial forces being channeled through the constraint. Muscles are simply holding the constraint in place, not driving the acceleration.
...
During the downswing, this ground constraint produces large vertical forces. Studies using force plates show that peak ground reaction forces during the swing can exceed body weight by a significant margin [@penner2003physics]. These forces are constraint forces---they arise from the geometry of contact with the ground.
...
5. **Double pendulum example:** The wrist constraint couples shoulder and elbow, producing a constraint force ($\sim 0.3$ Nm for this simplified example) that redistributes energy from arm to club.
6. **Grip and ground forces:** Grip forces and ground reaction forces are constraint forces, not muscular efforts. Muscles hold the constraints, but physics enforces them.
...
**What comes next:** Understanding constraints, we now ask: what if there are *multiple* constraints forming a closed loop? What if the body is not just a serial chain (hips → shoulders → arms → club) but a parallel mechanism with closed kinematic chains? That's the subject of Chapter 9.
...
For the wrist constraint in the double pendulum ($q_1 + q_2 = 115°$), sketch the configuration of the arm and club. Identify the null space (allowed motion) and the direction of the constraint force.
...
For the double pendulum with constraint (the worked example above):
