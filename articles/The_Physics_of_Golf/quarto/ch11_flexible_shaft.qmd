...
$$
\dot{\mathbf{x}} = f(\mathbf{x}) + G(\mathbf{x}) \bm{u}
$$
...
A slower golfer (say, 70 mph swing speed, as an illustrative example; typical amateur swing speeds vary widely, roughly 60--90 mph depending on skill level [@Broadie2014; @trackman_launch]) has less centrifugal stiffening throughout the swing. The shaft remains more flexible. This means:
...
- Natural frequency: $\omega_n = 15$ rad/s, so $c_s = 2 \times 0.08 \times 15 \times m_{\text{eff}} \approx 0.3$ N$\cdot$s/m.
