...
For most of this book, we have treated the golf club as if it were a rigid stick. This simplification has been valuable---it let us understand the fundamental forces and constraints that shape the swing. But a real golf shaft is not rigid. It bends, twists, and vibrates. These are not negligible effects. Shaft flexibility contributes to energy storage and release during the swing, and ignoring it leads to inaccurate predictions of clubhead orientation, trajectory, and spin at impact.

The shaft is not a passive passenger in the swing; it is an energy storage and release mechanism. Shaft flexibility contributes a modest but measurable boost to clubhead speed at impact---estimated at 1--5% of total clubhead kinetic energy [@nesbit2005three]---through the so-called "catapult effect."
...
At the moment of impact in a typical swing, the clubhead experiences centripetal acceleration whose magnitude depends on the angular velocity and the effective radius. As an illustrative order-of-magnitude estimate, the centripetal acceleration at the clubhead can reach hundreds to over a thousand $\mathrm{m/s^2}$, while the shaft grip end accelerates at a substantially lower rate. This differential force bends the shaft.

For a modern driver shaft (approximately 44 inches long, $50 \, \mathrm{g}$ mass), this differential loading produces measurable deflection at the clubhead. The magnitude depends on the shaft's flexural rigidity ($EI$) and the loading profile; shorter, stiffer shafts exhibit smaller deflection but the effect is never eliminated entirely.
...
In practical terms: a stiffer shaft (larger $EI$) requires a larger force to achieve the same deflection. A flexible shaft (smaller $EI$) stores less elastic energy during the swing and releases it later (closer to impact).
...
For a typical golf shaft, the first-mode natural frequency is in the range of roughly 4--6 Hz ($\omega_n \approx 25$--$40$ rad/s) for a driver held at the grip, though the effective frequency during the swing is modulated by centrifugal stiffening (see below). The swing duration is about 0.2 seconds, so the shaft goes through roughly one bending cycle during the entire downswing.
...
$$
F_{\text{damping}} = -c_s \dot{\eta}
$$

where $c_s = 2\zeta\omega_n m_{\text{eff}}$ is an effective damping coefficient.
...
- A damping force $-c_s\dot{\eta}$ that opposes the shaft's deformation rate.
...
$$
K_{s,\text{eff}} = K_{s,0} + \Delta K_s^{\text{cf}}(\omega)
$$

where $K_{s,0}$ is the passive bending stiffness and $\Delta K_s^{\text{cf}}$ is the additional stiffness due to centrifugal effects.

Early in the downswing, $\omega$ is small, so the effective stiffness is close to $K_{s,0}$. The shaft bends easily. Later in the downswing, $\omega$ grows, the centrifugal stiffening kicks in, and the shaft becomes harder to bend. This is another reason the shaft might snap back: it is not just that the driving force $f_{\text{modal}}$ changes sign, but also that the shaft becomes stiffer.
...
$$
E_{\text{shaft}} = \frac{1}{2} K_s \eta^2 \quad \text{(elastic potential energy at moment of snapback)}
$$

The magnitude of this energy depends on how much the shaft is bent ($\eta$) and how stiff it is ($\shaftstiff$). A very flexible shaft bends a lot (large $\eta$) but stores less energy per unit deflection (low $\shaftstiff$). A very stiff shaft bends less but packs more energy into that small deformation.
...
$$
W_{\text{muscles}} = \Delta KE_{\text{clubhead}} + E_{\text{stored in shaft}} + E_{\text{dissipated by damping}}
$$
...
- Effective tip mass (clubhead dominant): $m_{\text{eff}} \approx 0.2$ kg.
- First-mode natural frequency: $\omega_n \approx 30$ rad/s ($\approx 5$ Hz), so shaft stiffness $K_s = \omega_n^2 m_{\text{eff}} \approx 180$ N/m and damping $c_s = 2\zeta\omega_n m_{\text{eff}} \approx 1.0$ N$\cdot$s/m.

Suppose the hand (grip) starts at $y = 0$ and is prescribed to move with:

$$
y_{\text{grip}}(t) = 0.5 \sin\!\left(\frac{\pi t}{0.2}\right), \quad t \in [0,\, 0.2]\text{ s}
$$
...
The magnitude of the effect depends on the hand acceleration profile and the shaft parameters. For typical golfer numbers, the extra clubhead displacement due to shaft flex is 5--15 cm (2--6 inches) at the moment of maximum deflection. The energy contribution from shaft flex at impact is estimated at 1--5% of total clubhead kinetic energy [@nesbit2005three], meaning the catapult effect is real but modest in magnitude.

::: {.callout-note}
## Figure: Rigid vs. Flexible Shaft ZTCF (Qualitative)

**Rigid shaft** (smooth): clubhead position follows the grip trajectory
nearly sinusoidally — rising to 0.50 m at $t = 0.10$ s, then falling back
to 0 m at $t = 0.20$ s.

**Flexible shaft** (oscillatory): the same grip motion now produces
oscillations near the peak. The clubhead overshoots to 0.52 m at
$t = 0.08$ s, dips to 0.48 m at $t = 0.10$ s, and recovers to 0.51 m at
$t = 0.12$ s before returning to 0 m by $t = 0.20$ s. This extra
oscillation is the catapult effect: elastic energy stored in the bent shaft
is released near the peak, briefly accelerating the clubhead beyond the
rigid-shaft trajectory.

The flexible shaft does **not** produce a lower ZTCF trajectory; it produces
a *different* one whose oscillations encode the shaft dynamics.
:::

If your swing muscles and gravity are tuned to work with a particular drift field, using a shaft with different stiffness changes that drift field, and you must adjust your muscle activation (control inputs) to compensate.
...
5. **The catapult effect is real but modest.** Elastic energy stored in the bent shaft is released near impact, contributing approximately 1--5\% of total clubhead kinetic energy at impact [@nesbit2005three]. The shaft straightens during the critical 10--20 ms before impact, adding a measurable boost to clubhead speed.
...
A golf shaft has a first bending mode with natural frequency $\omega_n = 30$ rad/s ($\approx 5$ Hz, realistic for a driver shaft held at the grip) and damping ratio $\zeta = 0.05$. If the shaft is initially bent to $\eta_0 = 0.03$ m (3 cm) and released from rest, how long does it take for the deflection to decay to $e^{-1} \approx 37\%$ of its initial value?
...
In the worked example above, suppose the hand acceleration profile is changed so that the hand decelerates earlier (starting at $t=0.08$ s instead of $t=0.1$ s). How would you expect the shaft deformation to change? Would the catapult effect occur earlier or later?
