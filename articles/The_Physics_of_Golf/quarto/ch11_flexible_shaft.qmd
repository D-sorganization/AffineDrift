...
For a modern driver shaft (about 44 inches long, $50 \, \mathrm{g}$ mass), this can produce measurable deflection at the clubhead [citation needed for specific deflection magnitudes]. In shorter, stiffer shafts, this is reduced but never eliminated.
...
$$
\mathbf{x} =  \mathbf{q} \\ \dot{\mathbf{q}} \\ \eta \\ \dot{\eta}
$$

where $\eta = \eta$ and $\dot{\eta} = \dot{\eta}$. If we keep multiple modes, $\eta$ and $\dot{\eta}$ are vectors.
...
For a typical golf shaft, the first bending mode natural frequency is in the range of 3--6 Hz (approximately 19--38 radians per second) [@nesbit2005work; @mackenzie2009parametric]. The swing duration is about 0.2 seconds, so the shaft goes through approximately one bending cycle during the entire downswing.
...
3. The sixth component is $\ddot{\eta}$, which is driven by:
   ...
   \ddot{\eta} = -2\zeta\omega_n\dot{\eta} - \omega_n^2\eta + f_{\text{modal}}(q, q-dot)
...
$$
k_{\text{shaft}}_{\text{eff}} = k_{\text{shaft}}_0 + \Deltak_{\text{shaft}}_{\text{centrifugal}}(\omega)
$$

where $k_{\text{shaft}}_0$ is the passive bending stiffness and $\Deltak_{\text{shaft}}_{\text{centrifugal}}$ is the additional stiffness due to centrifugal effects.
...
The magnitude of this energy depends on how much the shaft is bent ($\eta$) and how stiff it is ($k_{\text{shaft}}$). A very flexible shaft bends a lot ($high$ $\eta$) but stores less energy per unit deflection (low $k_{\text{shaft}}$). A very stiff shaft bends less but packs more energy into that small deformation.
...
1. The shaft can absorb and hold more energy per unit bend (because $k_{\text{shaft}}$ is lower).
...
5. **The catapult effect is real.** Elastic energy stored in the bent shaft is released near impact, providing an estimated 1--5\% extra clubhead speed (depending on swing speed and shaft fit) [@nesbit2005work; @mackenzie2009parametric].
...
In the worked example (Section @sec:ch11:worked), suppose the hand acceleration profile is changed so that the hand decelerates earlier (starting at $t=0.08$ s instead of $t=0.1$ s). How would you expect the shaft deformation to change? Would the catapult effect occur earlier or later?
