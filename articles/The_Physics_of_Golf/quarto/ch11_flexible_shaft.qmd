---
description: "When we modeled the golf swing as a double or triple pendulum, we implicitly assumed that the club shaft is infinitely stiff. This is like modeling a..."
---
...
Modal decomposition assumes each mode shape $\phi_j(z)$ satisfies the eigenproblem implied by the dynamic beam equation. Projecting onto these modes produces the coupled ODEs for the modal amplitudes $\eta_j(t)$.
...
In practical terms, a stiffer shaft (higher $EI$) requires more force to achieve the same
deflection. A flexible shaft (lower $EI$) stores less elastic energy during the swing and
releases it later, closer to impact.
...
$$
\mathbf{x} = \begin{bmatrix} \mathbf{q} \\ \dot{\mathbf{q}} \\ \eta \\ \dot{\eta} \end{bmatrix}
$$
...
For a typical driver shaft, the first bending mode natural frequency is about 3--5 Hz ($\omega_n \approx 20$--$30$ rad/s), corresponding to a shaft "frequency" rating of roughly 240--280 CPM (cycles per minute) [@Nesbit2005; @MacKenzie2009]. The downswing lasts about 0.2--0.25 seconds, so at $\sim 4$ Hz the shaft passes through roughly **one** bending cycle (load, then recover) during the downswing — the basis of the familiar "kick" at impact.
...
The timing of the snapback depends on the shaft's stiffness and damping. A stiff shaft snaps back early and weakly. A flexible shaft snaps back late and more powerfully, but it might still be bending as the ball is struck, which is bad for accuracy.
...
$$
F_{\text{elastic}} = -k_{\text{shaft}} \eta
$$
...
- An elastic restoring force $-k_{\text{shaft}}\eta$ that tries to straighten the shaft.
...
As the shaft rotates faster and faster during the downswing, the centrifugal force on the shaft itself increases. This centrifugal force acts like an additional stiffness, making the shaft harder to bend.
...
$$
k_{\text{shaft,eff}} = k_{\text{shaft,0}} + \Delta k_{\text{shaft,centrifugal}}(\omega)
$$
...
We can now explain the catapult effect precisely.
...
Then, near impact, the hands slow sharply (they are being decelerated by the ground reaction forces through the legs and torso). Suddenly, the driving force $f_{\text{modal}}$ becomes large and positive. At the same time, the centrifugal stiffening reaches its maximum. The shaft, which has been bent for 100+ milliseconds of the downswing, suddenly has every incentive to snap back: the inertial driving force is trying to straighten it, and the increased stiffness makes the elastic restoring force huge.

The result: the shaft unbends rapidly. As it does, it does work on the clubhead, accelerating it further. This is the catapult effect.

$$
\text{Energy released by shaft} = \frac{1}{2}k\eta^2 \quad \text{(at the moment before snapback)}
$$

The magnitude of this energy depends on both how much the shaft is bent ($\eta$) and how stiff it is ($k_{\text{shaft}}$). A flexible shaft bends a lot (high $\eta$) but stores less energy per unit deflection (low $k_{\text{shaft}}$). A stiff shaft bends less, but the same bend stores more energy.
...
**Golfer A:** Very stiff shaft ($k_{\text{shaft}} = 10{,}000$ N/m, $\zeta = 0.1$) [illustrative values].
...
**Golfer B:** More flexible shaft ($k_{\text{shaft}} = 6{,}000$ N/m, $\zeta = 0.1$) [illustrative values].
...
Shaft fitting adjusts $k_{\text{shaft}}$, $\zeta$, the flex profile, and the total shaft mass to produce a drift field whose ZTCF closely matches the golfer's target trajectory for their measured swing speed and swing geometry.
...
For a flexible shaft, the term $E_{\text{stored in shaft}}$ is significant. In a fast swing, the shaft absorbs and releases energy within milliseconds of mid-downswing. In a slow swing, the shaft might remain bent for most of the downswing, absorbing a larger fraction of the muscle work temporarily.

This storage mechanism lets the muscles do work earlier in the downswing, when the torque levers are shorter and the muscles are fresh. The energy is released later, near impact, when the lever arm is longest and the clubhead can best use it.

::: {.callout-tip}
## Why Flexible Shafts Help Slower Golfers More

A slower golfer (say, 70 mph swing speed, below tour-professional levels [@Jorgensen1994], as an illustrative example; typical amateur swing speeds vary widely, roughly 60--90 mph depending on skill level [@Broadie2014; @trackman_launch]) has less centrifugal stiffening throughout the swing. The shaft remains more flexible. This means:
1. The shaft can absorb and hold more total energy, even though the energy per unit bend is lower when $k_{\text{shaft}}$ is lower.
...
A flexible shaft lets a slower golfer borrow energy from the shaft dynamics, partially compensating for lower muscle power.
...
## Worked Example: Comparing Rigid and Flexible Shaft ZTCF {#sec-ch11_worked}
...
- Shaft stiffness: $k_{\text{shaft}} = 7000$ N/m [illustrative value for a mid-flex shaft].
...
The control authority $G(\mathbf{x})$ (the range of accelerations the muscles can produce at the clubhead) is not enhanced by shaft flexibility. Instead, shaft flexibility changes $f(\mathbf{x})$, the drift field itself. A more flexible shaft produces a different $f$, which means a different ZTCF.
...
5. **The catapult effect is real.** Elastic energy stored in the bent shaft is released near impact, providing an estimated 1--5\% extra clubhead speed (depending on swing speed and shaft fit) [@Nesbit2005; @MacKenzie2009].
...
Explain in plain language why a stiffer shaft stores less elastic energy for a given deflection than a more flexible shaft. What is the resulting trade-off between timing and control?
...
A golfer notices that with a stiffer shaft, they hit the ball farther, but with less dispersion. With a more flexible shaft, the ball goes slightly farther on a perfect swing but much farther when they swing a bit faster. Explain these observations using the drift field and the catapult effect.

In the worked example (@sec-ch11_worked), suppose the hand acceleration profile is changed so that the hand decelerates earlier (starting at $t=0.08$ s instead of $t=0.1$ s). How would you expect the shaft deformation to change? Would the catapult effect occur earlier or later?
