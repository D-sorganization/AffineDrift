---
description: "1. Gravitational potential energy: When you raise your arms during the backswing, you increase potential energy. This energy is highest at the top of the..."
---
...
We've examined forces (@sec-zero_torque_counterfactual), constraint forces (@sec-constraint_forces), multiple segments (@sec-triple_pendulum), and parallel mechanisms (@sec-parallel_mechanisms). Now we ask: **where does the energy go?**
...
This flow is the essence of the kinetic chain. Understanding it requires thinking about power, energy partition, and where energy is stored and released.
...
That holds only if the velocities are related by the constraint. Before the constraint is enforced, $v_a \neq v_c$, and the constraint force accelerates the club and decelerates the arm, transferring energy between them.
...
Track the kinetic energy of each segment during the downswing. The specific times and velocities used below are illustrative estimates derived from a simplified triple-pendulum model [@Sprigings2000; @Nesbit2005]; actual golfer data will vary and depends on individual swing mechanics. Assume a triple pendulum (torso, arm, club) with the following initial energies:
...
5. **Phase 5: Impact (t = 0.42 s, 420 ms)**
   ...
   Total energy has decreased to 245.25 J (from peak of 281.25 J) because the late downswing includes negative work from eccentric muscle braking, with smaller losses to air resistance and internal dissipation.
...
6. By the time the club reaches impact, it has received energy (and thus momentum) from all prior segments, concentrated into its small inertia, resulting in high speed.
...
This is the magic of the kinetic chain: energy is conservatively transferred (no energy creation), but speed is amplified because the inertias differ. The amplification happens automatically via constraint forces—no muscle effort is required at the distal end to achieve high speed. High speed emerges from the system's geometry and the energy of prior segments.
...
When the wrist is cocked (large lag), the arm and club are not moving together. The wrist constraint is tight (muscular tension). When the wrist is cocked and rotating during the downswing, the constraint force stores energy in the relative motion. The arm's rotation pulls the club along, while the constraint force keeps the two from moving identically (the wrist is not fully released).
...
The energy perspective confirms this: by holding the wrist cocked, you're maintaining the constraint that prevents the club from accelerating at its full rate. When you finally release, the constraint force is removed, and the club's acceleration can increase sharply. The release delivers to the club the energy stored in the constraint state: potential energy in the stretched wrist tendons and kinetic energy in the coupled arm-club system.
...
This is why timing is everything in golf: the total energy input is roughly constant (same muscular effort), but the final club speed depends critically on when the wrist releases. An 80 ms later release can double the club speed, because of the arm's energy state at the moment of release.
...
\bm{C} &=
\begin{bmatrix}
-m_2 L_1 L_2 \sin(q_2) \dot{q}_1 \dot{q}_2 \\
0
\end{bmatrix} \\
\bm{g} &=
\begin{bmatrix}
m_1 g (L_1/2) \sin(q_1) + m_2 g L_1 \sin(q_1) \\
m_2 g (L_2/2) \sin(q_1 + q_2)
\end{bmatrix}
\end{aligned}
$$
...
**Drift (Passive):** Once the swing is initiated, gravity, Coriolis, and constraint forces handle energy redistribution. The system is close to ballistic. Energy flows from proximal to distal segments automatically.
...
**What comes next:** This concludes our journey through the physics of the golf swing. We've examined forces (@sec-zero_torque_counterfactual), their transmission through constraints (@sec-constraint_forces), multi-segment systems (@sec-triple_pendulum), parallel body structure (@sec-parallel_mechanisms), and energy flow (@sec-energy_transfer).
...
Simulate or model a double pendulum swing (or use the parameters from @sec-triple_pendulum).
