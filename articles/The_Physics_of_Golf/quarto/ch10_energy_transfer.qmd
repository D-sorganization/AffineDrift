...
We've examined forces (the chapter on zero torque counterfactual), constraint forces (the chapter on constraint forces), multiple segments (the chapter on triple pendulum), and parallel mechanisms (the chapter on parallel mechanisms). Now we ask: **where does the energy go?**
...
$$
\frac{dE}{dt} = \frac{dT}{dt} + \frac{dV}{dt} = \bm{\tau}^T \dot{\bm{q}} + P_{\text{non-conservative}}
$$

where $\bm{\tau}^T \dot{\bm{q}}$ is the power delivered by all torques (muscles, gravity, constraints).
...
$$
P_{\text{constraint}} = P_{\text{constraint, seg 1}} + P_{\text{constraint, seg 2}} + \cdots = 0
$$
...
$$
\begin{aligned}
P_{\text{constraint, arm}} &= \lambda_{\text{wrist}} \cdot v_{\text{arm}} & &\text{(work done on arm)}\\
P_{\text{constraint, club}} &= (-\lambda_{\text{wrist}}) \cdot v_{\text{club}} & &\text{(reaction on club)}
\end{aligned}
$$
...
The **summation of speed principle** is often stated as: ``Each segment reaches peak speed sequentially—the hips fastest, then the shoulders, then the arms, then the wrist. Each segment's peak speed is higher than the previous one.'' This pattern is well documented in golf biomechanics [@Cheetham2001; @Hume2005].
...
**Example: Numerical Summation of Speed (Illustrative Bound)**

For this illustrative example, assume all segments receive the same amount of energy ($100$ J each) at their moment of peak speed, and use representative model inertias. This is an idealized bound — real swings have sequential momentum transfer, not equal energy allocation — but the speed amplification is genuine.

$$
\begin{aligned}
\text{Hips:} &\quad I_h = 5 \text{ kg·m}^2,\quad \omega_h = \sqrt{2 \times 100 / 5} = 6.3 \text{ rad/s}\\
\text{Torso:} &\quad I_s = 2 \text{ kg·m}^2,\quad \omega_s = \sqrt{2 \times 100 / 2} = 10.0 \text{ rad/s}\\
\text{Arm:} &\quad I_a = 0.5 \text{ kg·m}^2,\quad \omega_a = \sqrt{2 \times 100 / 0.5} = 20.0 \text{ rad/s}\\
\text{Club:} &\quad I_c = 0.1 \text{ kg·m}^2,\quad \omega_c = \sqrt{2 \times 100 / 0.1} = 44.7 \text{ rad/s}
\end{aligned}
$$
...
::: {.callout-warning}
## Qualifier: Equal-Energy Is an Illustrative Upper Bound, Not the Actual Mechanism

The equal-energy assumption ($100$ J per segment) produces the $\omega \propto 1/\sqrt{I}$ speed
scaling, which is a useful illustrative upper bound. It is not a description of how the kinetic
chain actually operates. The actual mechanism is sequential momentum transfer: proximal segments
decelerate while distal segments accelerate, mediated by joint torques and constraint forces —
not equal energy distribution [@Sprigings2002]. The factor-of-7 speed ratio is an idealized
ceiling; measured values depend on individual timing, inertia parameters, and the degree of
proximal deceleration at the moment of distal segment peak speed.
:::
...
The third term is the coupling term — it is nonzero only if both segments are rotating.
...
$$
\bm{C} = \begin{bmatrix} -m_2 L_1 L_2 \sin(q_2) \dot{q}_1 \dot{q}_2 \\ 0 \end{bmatrix}, \quad
\bm{g} = \begin{bmatrix} m_1 g \tfrac{L_1}{2} \sin(q_1) + m_2 g L_1 \sin(q_1) \\ m_2 g \tfrac{L_2}{2} \sin(q_1 + q_2) \end{bmatrix}
$$

The Coriolis term contributes:

$$
P_{\text{Cor}} = m_2 L_1 L_2 \sin(q_2) \dot{q}_1^2 \dot{q}_2
$$
...
$$P_{\text{Cor}} = 0.2 \times 0.4 \times 0.3 \times \sin(90°) \times 10^2 \times 5 = 12 \text{ W}$$
...
$$P_{\text{Cor}} = 0.2 \times 0.4 \times 0.3 \times 1 \times 400 \times 40 = 384 \text{ W}$$
...
**What comes next:** This concludes our journey through the physics of the golf swing. We've examined forces (the chapter on zero torque counterfactual), their transmission through constraints (the chapter on constraint forces), multi-segment systems (the chapter on triple pendulum), parallel body structure (the chapter on parallel mechanisms), and energy flow (the chapter on energy transfer).
...
**Exercise 1 (Conceptual: Energy Flow)**
...
**Exercise 2 (Energy Partition Tracking)**
Simulate or model a double pendulum swing (or use the parameters from the chapter on triple pendulum).
...
**Exercise 5 (Efficiency Calculation)**
...
**Exercise 6 (Application: Lag and Distance)**
