...
You've likely heard that drag is "negligible" in golf. That claim deserves scrutiny. At clubhead speeds of 80--120 mph, the club is moving fast enough that air resistance can be a meaningful correction during the late downswing. When you compute inverse dynamics—trying to infer the joint torques from measured motion—ignoring drag may introduce systematic errors; the size of the bias depends on swing speed, geometry, and the model assumptions rather than on any single universal percentage [@jorgensen1993physics; @penner2003physics]. For a researcher or coach trying to understand the *true* control applied by muscles, that bias can matter. Understanding drag improves both the accuracy of swing analysis and the fairness of athlete assessment.
...
This seems small (3 N for a 50 m/s clubhead), but over the course of a downswing lasting approximately 0.25 seconds in representative swing analyses [@nesbit2005three; @hume2005role] with the club accelerating and decelerating, the impulse (integrated force) can be significant. Moreover, drag acts throughout the downswing, not just at impact.
...
- $\dot{\theta}_{\text{shoulder}} = 25$ rad/s (impact angular velocity, approximately 1430 deg/s, consistent with published data [@zheng2008golf])
- $\rho = 1.225$ kg/m$^3$
...
So the shaft alone produces a drag torque of about 0.7 N·m at the shoulder in this model. For reference, inverse-dynamics studies of the golf swing place shoulder moments at impact in the tens of N·m [@nesbit2005three; @nesbit2014work], so shaft drag accounts for only a small fraction of the total resistive effect in this estimate. While small for the shaft alone, the combined drag from the shaft and clubhead together is more significant (see below).
...
The moving mass of the golfer's arms and torso also experiences drag. The arms move at speeds up to 10--15 m/s in representative golf swing analyses [@nesbit2005three; @hume2005role] and much more slowly than the club, while the frontal area is roughly 0.05 m$^2$ (shoulders and torso width, an illustrative estimate). The drag force on the arms is:
$$
