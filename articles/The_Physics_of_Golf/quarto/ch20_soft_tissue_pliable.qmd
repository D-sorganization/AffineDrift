...
- **Total: approximately 13--18 kg out of the torso's total mass of roughly 25--35 kg [@DeLeva1996; @Winter2009] (illustrative; varies with body size and composition)**
...
When $I(t)$ is time-varying, a new term appears: $\dot{I}(t) \dot{\theta}$. The muscles must account for this variation; the swing dynamics become more complex.

::: {.callout-warning}
## The $\dot{I}\dot{\theta}$ Term Is NOT Damping

Note: The $\dot{I}\dot{\theta}$ term is NOT damping --- it is a conservative contribution arising from angular momentum change as limb configuration changes. It does not dissipate energy. Derived from $d(I\dot{\theta})/dt = I\ddot{\theta} + \dot{I}\dot{\theta} = \tau$.
...
**A pressurized abdomen at 100 mmHg generates an upward force of roughly 530 newtons on the diaphragm (for this representative area estimate).** IAP generates a spinal extension moment $M_{ext} = F_{diaphragm} \cdot r_{diaphragm} \approx 40\text{-}50$ N·m via the diaphragm's moment arm, offsetting 5-40% of lumbar compressive load [@McGill2007]. Comparing raw IAP force to extensor force magnitude is not mechanically meaningful --- the extension moment is the relevant quantity.
