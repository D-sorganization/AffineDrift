...
Consider the upper arm during the downswing. The humerus (upper arm bone) accelerates very rapidly during the swing; the exact peak depends on golfer size, sequencing, and swing speed, so the 500 m/s$^2$ figure here is only an order-of-magnitude example [@nesbit2005three; @hume2005role]. The soft tissue hanging on the arm---the muscles, the fat, the skin---has inertial mass. This soft tissue experiences an inertial force that tries to keep it moving in its old direction. The tissue oscillates relative to the bone, creating a secondary motion superimposed on the primary swing motion.

Similarly, the torso during a full downswing rotation is not a solid block. It contains a substantial amount of soft tissue and viscera whose mass is not rigidly locked to the skeleton; the exact partition varies with body size and composition [@winter2009biomechanics]. During the violent rotation of a golf downswing (torso angular velocity reaching 600${}^\circ$/s), this soft mass does not rotate in perfect lockstep with the skeletal torso. It shifts, it redistributes, it creates time-varying inertial properties that the rigid body model assumes away.
...
- Rigid core: the skeletal portion plus the more tightly coupled deep tissue
- Soft shell: the more compliant tissue layers surrounding the bone
- Estimated stiffness and damping: both are strongly person-dependent and should be treated as model parameters, not universal constants

During the downswing, which lasts only a fraction of a second in representative golf swing analyses [@nesbit2005three; @hume2005role], the arm accelerates rapidly enough to excite oscillations in the soft tissue. Typical wobble amplitudes are on the order of centimeters during the swing and post-swing phases. The wobble mass will oscillate for several cycles after the swing ends, dissipating energy through damping.
...
Now consider a much larger effect: the torso itself. The human torso is not a solid block of muscle and bone. It is a container of skeleton, muscle, connective tissue, blood, and viscera, and the exact partition of mass is individual-specific. The important point for this chapter is not the exact kilogram count but the fact that a large fraction of torso mass is soft, mobile tissue rather than rigid bone [@winter2009biomechanics]. These tissues are not rigidly attached to the skeleton, so they can move, shift, and redistribute during motion.
