...
In @sec-parallel_mechanisms, we established that the golf swing is a serial-to-parallel hybrid mechanism: serial segments connected by parallel constraints (the closed kinematic chains formed by the pelvis-torso loop, shoulder girdle closure, and ground contact loop). Those constraints generate forces---the loop Jacobian couples joint motions, the ground reaction forces close the chain, and the X-factor represents an over-constrained constraint on rotation.
