...
**Important in golf:** In three-dimensional kinematic analyses of the downswing, substantial motion occurs at the pelvis and hips. While pelvic rotation toward the target often exceeds $40\text{--}50^\circ$ by impact [@Hume2005], the actual anatomical lead-hip internal rotation (femur relative to pelvis) depends heavily on individual foot flare, pelvic tilt, and stance mechanics. Peak anatomical hip internal rotation during the downswing typically reaches $25\text{--}35^\circ$ in measured cohorts [@McTeigue1994; @Hume2005], staying within typical active ROM limits ($\approx 40^\circ$) for golfers without impingement. Notably, studies focusing on torso-pelvis separation (the X-factor) and wrist velocity profiles [@Cheetham2001] measure segment and thorax rotations rather than isolated femoral-acetabular joint angles. Conflating global pelvic rotation or torso turn with joint-level hip internal rotation can severely overestimate joint demands.
...
**Note on ROM values:** The range-of-motion values cited throughout this chapter (knee flexion ~140°, hip flexion ~120°, etc.) are representative values from standard anatomy and biomechanics references [@Neumann2017; @Nordin2012]. Individual variation is substantial; values for a specific subject should be obtained from clinical measurement or motion capture data.
