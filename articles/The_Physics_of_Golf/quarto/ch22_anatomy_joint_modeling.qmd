...
## Joint Types in Robotics --- A Taxonomy {#sec-joint-types}
...
**Constraint equation:** If the axis is $\bm{u}$ in frame A, and bodies are at relative orientation $\bm{R}$, the constraints on relative position and non-axis rotations prevent unwanted motion (the position of the two attachment points must be coincident, and the rotation must be about $\bm{u}$ only).
...
**Constraint equation:** If the axis is $\bm{u}$, then all motion perpendicular to $\bm{u}$ and all rotations are constrained to zero; only translation along $\bm{u}$ is free.
...
**Constraint equation:** If the two axes are $\bm{u}_1$ and $\bm{u}_2$ (perpendicular), the joint constrains position coincidence and rotation about the cross-product axis $\bm{u}_1 \times \bm{u}_2$; only the two rotations about $\bm{u}_1$ and $\bm{u}_2$ are free.
...
Using Grübler: $N = 5$ links (including ground), $J = 3$ revolute joints. For a **tree** structure (no loops), the correct formula is simply:
$$\text{DOF} = \sum_{i=1}^{J} f_i = 1 + 1 + 1 = 3$$

This matches intuition: a 3-link robot with 3 revolutes has 3 DOF. The full Grübler formula $6(N-1) - \sum(6-f_i)$ also gives 3 for this case (it is only when loops are present that careful application of loop-closure constraints is needed to get the right answer). The full Grübler formula applies when there are **loops**.
:::

## The Knee --- A Revolute (Mostly) {#sec-knee-joint}
...
## The Hip --- A Ball-and-Socket (Almost) {#sec-hip-joint}
...
1. **Shift of center of rotation:** The femoral head is not perfectly spherical (it's distorted by muscle attachment sites and loading). The center of rotation shifts slightly with joint angle---on the order of $\approx 5$ mm, which is small but measurable.

2. **Femoroacetabular impingement (FAI):** The femur and pelvis have bony anatomy that limits motion in certain directions. When the hip is flexed and internally rotated, the femoral neck can contact the acetabular rim. This contact limits further motion and can cause pain and cartilage damage over time. Two separate prevalence numbers are often conflated: cam- or pincer-type bony morphology appears on hip imaging in a very large fraction of asymptomatic athletes (commonly cited ranges run from 20% to 70%+ depending on the cohort), whereas *clinically significant* FAI syndrome---morphology plus symptoms and positive impingement tests---is substantially less common. Professional golfers with symptomatic FAI often modify their swing to avoid impingement.
...
## The Shoulder Complex --- A Gimbal (But Much More) {#sec-shoulder-complex}
...
The scapula does NOT stay fixed. There is a coupling between humeral motion and scapular motion called the **scapulohumeral rhythm**. The classic rule of thumb, stated as a *population-averaged* ratio, is:
...
## The Wrist --- A Universal Joint (Approximately) {#sec-wrist-joint}
...
## The Elbow --- A Revolute (Very Good Approximation) {#sec-elbow-joint}
...
## The Ankle/Foot --- The Ground Connection {#sec-ankle-foot}
...
## Summary Table and Modeling Guidelines {#sec-summary-table}
...
## Injury Biomechanics of the Golf Swing {#sec-ch22-injury}
