---
pagetitle: "The Zero-Torque Counterfactual"
description: "Derive input-removal interventions, coupled rigid-body mechanics, velocity resets and the limits of control-authority inference."
...
## Formal Definition of the ZTCF {#sec-ztcf_formal_definition}

[]{#formal-definition-of-the-ztcf}
[]{#the-zero-torque-counterfactual}
[]{#inplain:ztcf_meaning}
...
Four constructions answer different questions. A **pointwise ZTCF sample** evaluates $f(x(t),t)$ at one reference state. A **stitched pointwise ZTCF trace** collects those samples along the reference; it is not an integrated path. A **forward ZTCF trajectory** solves the zeroed-input initial-value problem. A **branched ZTCF trajectory** starts that solve at a reference state and compares it with a separately specified reference branch. The full-state vector $f$ includes velocity and internal-state derivatives; only its mechanical acceleration block has acceleration units.

Zeroing every declared input gives

$$
\dot x^0=f(x^0,t),\qquad x^0(t_0)=x_0.
$$ {#eq-ztcf_acceleration}
...
`affinedrift.ztcf-intervention/v1`. The published normative schema and
...
[Normative Schema](https://affinedrift.com/data/ztcf/ztcf_intervention_v1.schema.json) and
[Rigid Three-Link Golden Fixture](https://affinedrift.com/data/ztcf/planar_golf_forward_fixture_v2.json).
...
### Constraints and Physical Outputs
...
### Force Bookkeeping and Inverse Dynamics
...
## Computing the ZTCF: Algorithm and Simulation {#sec-ztcf_algorithm}
...
## The Zero-Velocity Counterfactual {#sec-zvcf}

[]{#the-zero-velocity-counterfactual-zvcf}
[]{#the-zero-velocity-counterfactual}
[]{#inplain:zvcf_vs_ztcf}

The **instantaneous Zero-Velocity Counterfactual (ZVCF)** evaluates acceleration at the same configuration with specified velocities reset and declared input zero. For the unconstrained convention above, holding $z,t,p$ fixed gives
...
## Two Uniform Rods: A Mechanics Check {#sec-double_pendulum_ztcf}
...
$$
 With $T=\tfrac12v^TMv$, the Euler--Lagrange equations give $M a+h_v+h_g=u$, where
...
### Magnitude Is Not Direction or Reachability {#sec-swing_ballistic_commitment}
...
## Why ZTCF Cannot Be Measured Directly {#sec-ztcf_model_dependent}

[]{#why-ztcf-cannot-be-measured-directly}
[]{#ztcf-is-model-dependent}
[]{#inplain:ztcf_model_error}
...
## Summary and Preview {#sec-ztcf_summary}
...
## Chapter Exercises {#ex:chapter_6_exercises}

[]{#chapter-exercises-the-zero-torque-counterfactual}

1. Starting from the two COM positions above, recover all entries of $M$ using the displayed sum of translational and rotational kinetic energies. Explain why a COM inertia and a pivot inertia cannot be interchanged without a parallel-axis correction.
1. Derive $h_v$ from the Euler--Lagrange equations and $h_g$ from $\nabla V$. Verify $v^Th_v=\tfrac12v^T\dot Mv$, and recover both components of the stated reference input. Explain why zero applied input does not imply constant velocity.
1. For a uniform rod with mass $2$ kg and length $0.7$ m, let $\theta$ measure displacement from vertically downward. At $\theta=45^\circ$, the pivot inertia is $mL^2/3=0.326667$ kg·m$^2$ and gravity acceleration is $-3g\sin\theta/(2L)=-14.864395$ rad/s$^2$. Verify these values. An illustrative $+10$ N·m input adds $30.612245$ rad/s$^2$; it is not a measured muscle capacity.
1. Replay the registered three-link Python golden fixture without changing its parameters or source authority. Report its terminal state and units, then assess step refinement separately. Explain why matching the fixture cannot establish a second engine, flexible-shaft validity or an individual golfer's muscle state.
1. Define a fixed-time and an impact-event branch comparison. State what happens if a branch never reaches impact. For the double-integrator example, derive the displacement bound and explain why adding a final-velocity constraint can change the attainable endpoint.

## On This Site

- [Canonical ZTCF Reference](../../zero-torque-counterfactual.html) defines the four constructions and their interpretation.
- [Induced Acceleration Analysis](ch30b_induced_acceleration.html) develops constrained pointwise attribution.
- [Constraint Forces](ch07_constraint_forces.html) explains reaction recovery and feasibility.
- [Drift-Control Ratio](../../controllability-drift-ratio.html) separates magnitude comparisons from reachable outcomes.
