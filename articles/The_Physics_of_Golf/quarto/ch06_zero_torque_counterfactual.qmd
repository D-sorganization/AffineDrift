...
## The Drift-to-Control Ratio (DCR) and Late-Downswing Authority

::: {.callout-note}
## Drift-to-Control Ratio (DCR)

At any time during the swing, the **Drift-to-Control Ratio (DCR)** compares modeled drift against bounded modeled control under an explicit norm or weighting. In the raw textbook framing, it is a ratio of passive to active torques; in the more general control-authority framing below, the same idea is expressed as drift magnitude relative to bounded control magnitude. Consistent with Chapter @sec-05_affine_structure, use:
...
The Drift-to-Control Ratio is a *model-dependent control-authority diagnostic*. It asks: **How large is the modeled drift field compared with the control acceleration available under the stated torque bound and weighting?**
...
For this illustrative example, assume the club head reaches $100$ mph (44.7 m/s) at impact, with an angular velocity of $50$ rad/s about the shoulder. Under the chosen weighting and torque bound, the DCR at this moment may be high, for example on the order of 10--20:1.
...
But the timing window is small: the contact lasts only $0.5$ ms in this illustrative timing example. A correction that is delayed until impact cannot materially reshape the incoming state; effective steering must occur earlier than the collision interval.
...
4. **DCR growth:** As velocity increases, several passive terms grow with speed, while bounded muscular torques do not necessarily scale with the same law. The Drift-to-Control Ratio can become large in late downswing, but its magnitude depends on the weighting, torque bound, and model parameters.
...
{Analysis}
The Drift-to-Control Ratio (DCR) compares modeled drift with bounded modeled control under a stated weighting and torque bound. Choose plausible early- and late-downswing values for $\bm{W}$ and $u_{\max}$, declare the model parameters that matter most, then estimate how the DCR changes under at least two sensitivity perturbations.
