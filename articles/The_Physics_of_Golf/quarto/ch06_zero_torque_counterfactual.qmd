...
The COM positions are $c_1=r_1(\cos q_1,\sin q_1)^T$ and $c_2=L_1(\cos q_1,\sin q_1)^T+r_2(\cos(q_1+q_2),\sin(q_1+q_2))^T$. Summing translational COM and rotational kinetic energies gives
...
The potential energy is $V=g[(m_1r_1+m_2L_1)\sin q_1+m_2r_2\sin(q_1+q_2)]$. With $T=\tfrac12v^TMv$, the Euler--Lagrange equations give $M a+h_v+h_g=u$, where
...
At $q=(45,70)$ degrees and $v=(8,12)$ rad/s, $\cos(q_1+q_2)=\cos115^\circ=-0.422618$. All computations use unrounded values; displayed matrices are rounded:
...
1. Starting from the two COM positions above, recover all entries of $M$ using $T=\tfrac12\sum_i m_i\|\dot c_i\|^2+\tfrac12J_1v_1^2+\tfrac12J_2(v_1+v_2)^2$. Explain why a COM inertia and a pivot inertia cannot be interchanged without a parallel-axis correction.
