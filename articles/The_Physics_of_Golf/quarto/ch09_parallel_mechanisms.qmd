...
In Chapters @sec-zero_torque_counterfactual--8, we modeled the golf swing as a *serial* kinematic chain: hips → shoulders → arms → club. This is pedagogically useful, but it's *biomechanically wrong*.
...
The X-factor is the shoulder rotation relative to the pelvis:

$$\text{X-factor} = \theta_{\text{shoulder}} - \theta_{\text{pelvis}} = \Delta\theta_{\text{spine}}(\bm{q}_{\text{spine}})$$

The rigid coupling is: shoulder rotation equals pelvis rotation plus the spine's relative
rotation:

$$\theta_{\text{shoulder}} = \theta_{\text{pelvis}} + \Delta\theta_{\text{spine}}(\bm{q}_{\text{spine}})$$

::: {.callout-warning}
## Correction: Absolute-Angle Sum Is Not a Valid Closure Equation

Writing $\theta_p + \theta_s + \theta_{sh} = 0$ (absolute angles summing to zero) is incorrect:
if the pelvis is at 60° and the shoulders are at 20° (both in the lab frame), their sum is
80°, not zero. A numerical example would give $60° + 40° + 20° = 120° \neq 0$, which shows the
inconsistency. The correct constraint uses *relative* angles. The X-factor is a
**difference** (shoulder minus pelvis), not a sum.
:::

Or in general vector form:

$$\bm{\Phi}_{\text{spine}}(\theta_{\text{pelvis}}, \Delta\theta_{\text{spine}}, \theta_{\text{shoulder}}) = \theta_{\text{shoulder}} - \theta_{\text{pelvis}} - \Delta\theta_{\text{spine}} = 0$$

This is a holonomic constraint, just like the joint constraints from Chapter @sec-constraint_forces.
...
It has the same interpretation as the constraint Jacobian from Chapter @sec-constraint_forces: its null space defines allowed motions, and its range defines constraint force directions.
...
**Example: Grübler Applied to the Golfer**

To apply Grübler's formula correctly, every joint must be paired with the bodies it connects.
A minimal spine-plus-arms planar model requires:

- **Bodies ($N$):** pelvis (1), lumbar segment (2), thorax (3), shoulder girdle (4), left upper
  arm (5), left forearm (6), right upper arm (7), right forearm (8). Total: $N = 8$.
- **Joints ($J$):** pelvis–lumbar (1), lumbar–thorax (2), thorax–shoulder girdle (3), left
  shoulder (4), left elbow (5), right shoulder (6), right elbow (7). Total: $J = 7$.
- **Ground contact:** $C_{\text{passive}} = 3$ (two feet fix the pelvis in-plane: 2 translation
  + 1 rotation).

Mobility (planar approximation, 2D):

$$M = 3(8-1) - 2 \times 7 + 3 = 21 - 14 + 3 = 10$$

::: {.callout-warning}
## Note: Body Count N=3 Underestimates the Skeletal Model

An earlier version of this text used $N=3$ (pelvis, torso, shoulders), $J=11$, and obtained
$M = 3(3-1) - 2(11) - 3 = -19$. This is incorrect because it mixes an arm-joint count ($J=11$
includes shoulder + elbow joints for both arms) with a body count that omits the arm segments
those joints connect. A minimal 5-joint spine alone requires 6 rigid bodies; including the
pelvis, thorax, shoulder girdle, and upper arms gives $N \geq 13$.

Using a consistent model with $N=13$, $J=11$, $C_{\text{passive}}=3$:
$$M = 3(13-1) - 2(11) - 3 = 36 - 22 - 3 = 11 \text{ DOF (upper body, planar)}$$

The key result is unchanged: when the grip constraint closes the two-arm loop, effective DOF
drops sharply and the system becomes over-constrained — resolved by soft-tissue elasticity.
:::

Ten degrees of freedom is consistent with a body that can move. The formula gives a *positive*
mobility because bodies and joints are enumerated consistently. When the two hands grip the
club (adding loop-closure equations coupling the two arms), the effective DOF is reduced and
the analysis reveals over-constraint — which is physically resolved by soft-tissue elasticity.

**Insight:** The critical requirement is consistent enumeration: every joint connects two bodies,
so $J$ joints imply at least $J+1$ bodies in a serial sub-chain. Mixing a joint count that
includes arm joints with a body count that omits arm segments gives an incorrect (unphysically
negative) result. With a properly enumerated model, Grübler confirms that the gripped-club loop
creates over-constraint, resolved only because muscles and tendons are elastic.
...
Every loop closure constraint produces constraint forces, just like the joint constraints from Chapter @sec-constraint_forces. But because a body has multiple loops, there can be multiple constraint forces acting simultaneously.
...
The correct loop closure uses relative angles. With pelvis at $\theta_p = 60°$ and shoulders at
$\theta_{sh} = 20°$, the X-factor is $\theta_p - \theta_{sh} = 40°$, and the spine must absorb
exactly this difference: $\Delta\theta_{\text{spine}} = 40°$. The constraint is:

$$\bm{\Phi}_{\text{spine}} = \theta_{sh} - \theta_p - \Delta\theta_{\text{spine}} = 20° - 60° + 40° = 0 \checkmark$$

::: {.callout-warning}
## Corrected Form

An earlier version of this text wrote
$\bm{\Phi}_{\text{spine}} = \theta_p + \theta_s + \theta_{sh} = 60° + 40° + 20° = 120° \neq 0$,
which is inconsistent: absolute-angle sums do not form a valid loop closure equation. The
correct form uses the relative-angle difference as shown above.
:::
...
[@Vaughan1981; @Vaughan1983] were among the first to recognize that the golf swing is a *closed kinematic chain* and to formalize the mathematics of loop constraints in biomechanics. His work addressed the fundamental problem: when both hands grip a club and both feet touch the ground, how can inverse dynamics resolve the ambiguity in force distribution?
...
[@Stewart1965] published the foundational paper on the Stewart platform, a hexapod parallel manipulator with 6 legs connecting a base to a moving platform. The Stewart platform has become ubiquitous in precision engineering, flight simulators, and machine tools.
...
[@Merlet2006] provides a comprehensive treatise on parallel robots that systematized the theory of parallel mechanisms, including:
...
[@Hunt1978] provided foundational work on the geometry and algebraic foundations of kinematic mechanisms, including parallel systems. Hunt's screw theory approach is particularly relevant to understanding constraint forces.

**Gosselin & Angeles' Singularity Analysis (1990)**

[@GosselinAngeles1990] developed comprehensive methods for analyzing singularities in parallel mechanisms. Their classification of singularities into workspace boundaries, leg singularities, and architecture singularities has become standard in robotics.
