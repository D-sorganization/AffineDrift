...
::: {.callout-note}
## Intuitive Difference: Serial vs. Parallel
...
The closure constraint states: if the pelvis rotation (in the ground frame) is $\theta_p$ and the shoulders rotation is $\theta_{sh}$, then the spine must absorb the difference. Defining $\Delta\theta_{\text{spine}}$ as the rotation of the shoulders *relative to* the pelvis via the spine, the compatibility requirement is:

$$
\theta_{sh} = \theta_p + \Delta\theta_{\text{spine}} \quad \Longleftrightarrow \quad \bm{\Phi}_{\text{spine}} = \theta_{sh} - \theta_p - \Delta\theta_{\text{spine}} = 0
$$

The **X-factor** is $\Delta\theta_{\text{spine}} = \theta_p - \theta_{sh}$: the amount by which the hips lead the shoulders. For example, if $\theta_p = 60°$ (hips open) and $\theta_{sh} = 20°$ (shoulders partially open), then $\Delta\theta_{\text{spine}} = 40°$---the spine has absorbed 40° of differential rotation.

Or in general vector form:

$$
\bm{\Phi}_{\text{spine}}(\theta_p, \Delta\theta_{\text{spine}}, \theta_{sh}) = \mathbf{0}
$$

This is a holonomic constraint, just like the joint constraints from the chapter on constraint forces.
...
It has the same interpretation as the constraint Jacobian from the chapter on constraint forces: its null space defines allowed motions, and its range defines constraint force directions.
...
$$
\bm{x}_L(\bm{q}_L) + \bm{x}_R(\bm{q}_R) = 2 \bm{x}_{\text{grip}} + \bm{\epsilon}
$$
...
\dfrac{\partial x_L}{\partial q_1^L} & \dfrac{\partial x_L}{\partial q_2^L} &
\dfrac{\partial x_R}{\partial q_1^R} & \dfrac{\partial x_R}{\partial q_2^R}
\end{bmatrix}
$$
...
::: {.callout-note}
## Grübler's Criterion for Planar Mechanisms
...
::: {.callout-note}
## What Grübler's Formula Tells You
...
$$
M = 6(2-1) - 6 \times 3 = 6 - 18 = -12
$$

This gives $M = -12$, which is nonsensical for a device that clearly moves. The issue is that the 6 legs are *redundantly constrained*—the structure is over-constrained but still rigid. The Grübler formula in its basic form does not account for redundant constraints; a corrected version is:

$$
M = 6(N-1) - \sum_i f_i + \lambda
$$

where $\lambda \geq 0$ is the number of redundant constraints. In the Stewart platform, the 6 legs provide 18 constraints for 6 DOF of a free body — exactly 12 redundant constraints — giving $M = 6 - 18 + 12 = 0$ (the platform is fully driven by 6 actuators with no passive DOF). The redundancy is intentional: the 6 legs over-constrain the moving platform, which allows high stiffness and precision.

:::

**Example: Grübler Applied to the Golfer**

To apply Grübler's formula, every joint must be paired with the bodies it connects. Use a minimal spine-plus-arms planar model:

- **Bodies ($N$):** pelvis (1), lumbar segment (2), thorax (3), shoulders platform (4), left upper arm (5), left forearm (6), right upper arm (7), right forearm (8). Total: $N = 8$.
- **Joints ($J$):** pelvis–lumbar (1), lumbar–thorax (2), thorax–shoulders (3), left shoulder (4), left elbow (5), right shoulder (6), right elbow (7). Total: $J = 7$.
- **Ground contact:** $C_{\text{passive}} = 3$ (two feet fix the pelvis in-plane: 2 translation + 1 rotation constraints).
...
Ten degrees of freedom is consistent with a body that can move. The formula gives a *positive* mobility because bodies and joints are enumerated consistently. When the two hands grip the club (adding loop-closure equations coupling the two arms), the effective DOF is reduced and the analysis reveals over-constraint—which is physically resolved by soft-tissue elasticity.

**Insight:** The critical requirement is consistent enumeration: every joint connects two bodies, so $J$ joints imply at least $J+1$ bodies in a serial sub-chain. Mixing a joint count that includes arm joints with a body count that omits arm segments gives an incorrect (unphysically negative) result. With a properly enumerated model, Grübler confirms that the gripped-club loop creates over-constraint, resolved only because muscles and tendons are elastic.
...
Every loop closure constraint produces constraint forces, just like the joint constraints from the chapter on constraint forces. But because a body has multiple loops, there can be multiple constraint forces acting simultaneously.
...
Care is needed with sign conventions: if all rotations are measured from a common (ground) reference frame, the loop constraint is not a simple sum but a compatibility relation. Defining $\Delta\theta_{\text{spine}}$ as the rotation of the shoulders *relative to* the pelvis via the spine, the constraint reads

$$
\bm{\Phi}_{\text{spine}} = \theta_{sh} - \theta_p - \Delta\theta_{\text{spine}} = 0
$$

That is, the shoulder rotation in the ground frame equals the pelvis rotation plus the spine twist. The X-factor is then simply $\Delta\theta_{\text{spine}} = \theta_p - \theta_{sh}$ (negated by convention if hips lead shoulders).
...
1. **Vertical force:** The golfer's weight is $90 \times 9.81 = 883$ N. As the golfer rotates and transfers weight forward, the ground reaction force must increase above body weight to prevent downward acceleration. This is a constraint force—the ground must push up harder to prevent the golfer from accelerating downward.
2. **Horizontal force:** As the upper body rotates and accelerates forward, the golfer's momentum would carry them forward. The friction between feet and ground provides a horizontal constraint force to prevent sliding.
3. **Torque:** The horizontal force is applied off-center (at the feet), creating a torque about the center of mass. This torque is a constraint effect: the ground is preventing the golfer from rotating forward uncontrollably.
...
$$
\Phi_{\text{loop}}(\theta_p, \theta_s, \theta_a) = 0
$$

This constraint encodes the requirement that the shoulder position (given pelvis rotation $\theta_p$, spine twist $\theta_s$, and arm angle $\theta_a$) is consistent with the ground-frame shoulder location. This is an algebraic equation relating the three rotations. The constraint force arises when this equation is enforced dynamically.
...
$$
\tau_{\text{constraint, spine}} = I_{\text{spine}} \ddot{\theta}_s + C_s \dot{\theta}_s + g_s
$$
...
::: {.callout-important}
## Key Takeaways: Parallel Mechanisms and Loop Constraints
...
7. **Constraint forces are enormous:** Loop constraints produce large forces at the spine, grip, and other joints—far exceeding what muscles alone produce. These are not muscular forces—they're automatic consequences of the constraint geometry.
...
Compute the mobility using Grübler's formula (planar approximation). Is it positive, negative, or zero? What does this imply?

**Exercise 4 (Loop Jacobian for Grip Constraint)**
...
**Exercise 5 (Constraint Forces in the Spine)**
...
**Exercise 6 (Application: Footing and Ground Reaction)**
