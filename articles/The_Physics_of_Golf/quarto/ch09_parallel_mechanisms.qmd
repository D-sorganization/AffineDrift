...
This is a holonomic constraint, just like the joint constraints from [Chapter 7](./ch07_constraint_forces.qmd#sec-constraint_forces).
...
It has the same interpretation as the constraint Jacobian from [Chapter 7](./ch07_constraint_forces.qmd#sec-constraint_forces): its null space defines allowed motions, and its range defines constraint force directions.
...
Every loop closure constraint produces constraint forces, just like the joint constraints from [Chapter 7](./ch07_constraint_forces.qmd#sec-constraint_forces). But because a body has multiple loops, there can be multiple constraint forces acting simultaneously.
...
[Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual) introduced drift forces (gravity, Coriolis, centrifugal) and control forces (muscle-applied torques). In a serial chain, these forces and the applied torques solve the dynamics uniquely. In a parallel mechanism, the story is more subtle.
...
A key insight from [Chapter 6](./ch06_zero_torque_counterfactual.qmd#sec-zero_torque_counterfactual) was that *drift forces dominate active muscle forces* during the downswing. This is especially true in a parallel system with significant inertia and angular velocities.
...
::: {.callout-note}
## Ground Reaction Force {#definition:grf}

The **ground reaction force** (GRF) is the force exerted by the ground on the golfer's feet to maintain the contact constraint. It has three orthogonal components:
- $F_z$ (vertical): supports the golfer's body weight and accelerates the center of mass vertically.
- $F_x$ (anterior-posterior): drives weight shift and linear acceleration forward/backward.
- $F_y$ (medial-lateral): stabilizes the pelvis and drives rotational acceleration.
:::
...
::: {.callout-note}
## Center of Pressure {#definition:cop}
...
## Synthesis: The Golfer's Mechanical Reality {#sec-synthesis_golfer}
...
::: {.callout-tip}
## The Golfer as a Parallel Mechanism {#context:golfer_parallel_mechanism}
