...
This chapter develops the math and physics of parallel mechanisms, then applies them to the golfer's body structure. "Parallel mechanism" carries the same sense here as in the robotics literature on closed-chain manipulators and over-constrained mechanisms [@briot2015parallel_robots].
...
**Insight:** A negative Gruebler mobility tells us that the human skeleton, taken as rigid, is over-constrained—it can't move. The only reason it *does* move is because soft tissues (ligaments, muscle tendons) have elasticity. This elasticity is crucial for defining effective DOF.
...
The grip constraint makes the forces generated by the two hands ambiguous. Robotics studies that ambiguity extensively under the name "parallel mechanism force distribution."
...
In practice, the golfer controls this load sharing through grip pressure, hand position variations, and wrist cocking — all of which affect the effective moment arms and force distribution. This is why two golfers with identical kinematics may differ widely in grip pressure and hand muscle activation.
...
A golfer's balance and footing matter because the ground constraint carries the swing's energy transfer.
...
A critical insight: *drift forces are computed from position, velocity, and acceleration alone and are not constrained by the loop closure equations*. Three consequences follow:
...
This is why inverse dynamics can compute *joint-level* torques — the sum of all muscles at a joint — without ambiguity. Individual muscle forces in a parallel system need additional information: EMG, force plates, and similar measurements.
...
**What comes next:** We have now seen how forces are produced (ZTCF), transmitted (constraint forces), and routed through a parallel mechanism (loop constraints). The final step is to follow the energy through the system. How does the kinetic energy partition between the segments? Where is energy stored and released? When is energy lost? These questions are answered in @sec-energy_transfer.
...
Merlet's work is essential reading for anyone serious about parallel mechanism theory in biomechanics.
...
The golfer's body is neither purely serial nor purely parallel — it is a hybrid. The lower body (torso + pelvis + ground) forms a parallel mechanism with the ground, while the upper body (shoulders + arms) forms a serial kinematic chain branching from the shoulders.
...
This is an implicit equation for the club position as a function of arm angles. Forward recursion cannot solve it. An iterative method such as Newton-Raphson, or a constraint-satisfaction method, is required.
...
This singularity is mechanically useful: it "stiffens" the arm at the moment of force application, allowing maximum acceleration to be transmitted to the ball.
...
The preceding sections treated GRF conceptually. This section shows how to measure and analyze lower body mechanics from force plate data.
...
A force plate reports the ground reaction torque directly as $M_z$. It can also be inferred from the offset of the resultant GRF vector relative to the body's center of rotation.

### Sequential Computation From Distal to Proximal {#sec-sequential_inverse_dynamics}

With force plate data, lower body inverse dynamics can be computed sequentially:
...
Force plate data quantifies this ground-up flow. The ground reaction torque often contributes more to pelvis acceleration than the hip muscles themselves do — another example of drift dominance.
