---
description: "Two rigid rods: - Link 1 (upper arm): length , mass , moment of inertia about the elbow . - Link 2 (forearm + club): length , mass , moment of inertia..."
---
...
### Numerical Parameters (Realistic Golf) {#sec-ch03_numerical}
...
The Lagrangian formalism (which we'll use next) exploits a variational principle: nature minimizes the action, $\int (T - V) dt$. By tracking $T$, we can derive the forces without writing down force balances explicitly.
...
From $T$ (@eq-ch03_T_rearranged), the terms involving $\dot{\theta}_1$ are:
...
@eq-ch03_eq1_raw and @eq-ch03_eq2_raw are the equations of motion for the double pendulum. They're complicated, with many terms. Let's parse them:
...
Let's plug in numbers and see what the forces are.
...
$$
\begin{aligned}
M_{11} &\approx 0.077 + 1.5 \cdot 0.35^2 + 1.5 \cdot 0.5^2 + 0.4 + 2 \cdot 1.5 \cdot 0.35 \cdot 0.5 \cos(-5°) \\
&\approx 0.077 + 0.184 + 0.375 + 0.4 + 0.523 \\
&\approx 1.56 \text{ kg m}^2
\end{aligned}
$$
...
$$
\begin{aligned}
g_1 &= (2.5 \cdot 0.175 + 1.5 \cdot 0.35) \cdot 9.81 \cdot \sin(0°) + 1.5 \cdot 9.81 \cdot 0.5 \cdot \sin(-5°) \\
&\approx 0 - 0.64 \text{ N m} \\
&\approx -0.64 \text{ N m}
\end{aligned}
$$
...
&= 1.5 \cdot 0.35 \cdot 0.5 \cdot 10.47^2 \cdot \sin(-5°) \\
&\approx 1.5 \cdot 0.35 \cdot 0.5 \cdot 109.7 \cdot (-0.087) \\
&\approx -2.5 \text{ N m}
\end{aligned}
$$
...
At impact:
- The total mass/inertia of the arm+club is about 1.56 kg m$^2$.
- Gravity is pulling with a torque of 0.64 N m.
- The centrifugal term (from the high shoulder rotation) is creating a torque of 2.5 N m trying to straighten the elbow.

For comparison, a human can generate roughly 30–50 N m of torque at the shoulder [@Nesbit2005; @Gatt1998] (illustrative; values vary with body size, training, and joint angle). So gravity and centrifugal effects are much smaller than the muscular torque budget in this illustrative example. The muscles do the rest.
...
In the absence of control ($\bm{\tau} = 0$), the double pendulum exhibits chaos only in certain regimes. But once you add muscular control ($\bm{\tau} \neq 0$), the picture becomes complex. Small errors in muscle activation can compound.
...
2. **Potential Energy Change**. At address, both arms are down ($\theta_1 = 0°, \theta_2 = 0°$). At the top of the backswing, $\theta_1 = 150°, \theta_2 = -70°$. Using the parameters in @sec-ch03_numerical, compute the change in potential energy. How much gravitational work is available during the downswing?
