...
This chapter reframes the nervous system as an engineering control system and asks: what kind of controller is the brain? How does it manage to work within such severe constraints? The answer is that the brain does not attempt to regulate every detail of the trajectory. Instead, it *leverages* the drift field---the passive dynamics we've been studying---to carry most of the motion, committing active control primarily to shaping initial conditions and to targeted corrections. The language of "exploits" here is metaphorical; the underlying mechanism is that efficient feedforward plans allocate scarce control bandwidth to where it is most needed, while the passive dynamics do the rest. The brain is not fighting physics; it is working *with* physics as its primary actuator.
...
Recall from earlier chapters on the zero-torque counterfactual and drift-dominated control that the golf swing is a control-affine system:

$$\dot{\bm{x}} = f(\bm{x}) + G(\bm{x})\bm{u}(t)$$ {#eq-control-affine}
...
- **Noisy sensors:** Proprioceptive acuity varies by joint and is coarser at proximal joints than distal ones [@kandel2013principles]. Vision has high acuity but slow feedback. The nervous system must work with uncertain, delayed measurements.
...
- **Bounded control:** Maximum torques are limited. The proximal joints (shoulder, elbow) produce far more torque than the distal joints (wrist, fingers). The brain cannot exceed these physical limits.

:::

::: {.callout-tip}
## Feedback Latency Budget --- Four Distinct Loops

The nervous system does not have *one* feedback delay --- it has several loops operating in parallel at very different latencies. Lumping them together misrepresents what corrective feedback is and is not available during a 250--300 ms downswing.

| Loop | Typical round-trip latency | Signal carried | Role in the swing |
|---|---|---|---|
| Spinal stretch reflex (monosynaptic) | 20--40 ms | Muscle spindle stretch $\to$ $\alpha$-motoneuron | Stabilizes against sudden perturbations; operates *without* cortical involvement |
| Long-loop (transcortical) proprioceptive | 50--100 ms | Spindle/Golgi $\to$ cortex $\to$ cord | Task-dependent intramovement corrections |
| Vestibulo-ocular / vestibulospinal | 10--20 ms (VOR), 50--100 ms (postural) | Semicircular canals, otoliths | Balance, gaze stabilization during trunk rotation |
| Cerebellar internal-model update | $\sim$100--200 ms | Climbing-fiber error $\to$ Purkinje LTD/LTP | Trial-to-trial calibration of the forward model [@wolpert1998internal; @shadmehr2010error] |
| Visual (conscious feedback) | 100--200 ms (detection) to $\sim$300--400 ms (corrective command reaches muscle) | Retinal image $\to$ visual cortex $\to$ motor cortex | Too slow to correct the downswing itself; primary role is *trial-to-trial* learning and pre-swing planning |

Add the downstream motor-side delays --- $\sim$10--20 ms corticospinal transmission and $\sim$40--60 ms electromechanical activation --- and the picture becomes:

- **Spinal and vestibular loops** *can* fire inside a 300 ms downswing and shape it. They handle local perturbation rejection, not semantic errors like clubface angle.
- **Long-loop proprioceptive corrections** can land once, perhaps twice, during the downswing; they are the most plausible substrate for any genuine mid-swing adjustment.
- **Visual feedback** for a 300 ms downswing is effectively post-hoc: any correction triggered by what the eye detects during the downswing arrives at the muscle *after* impact. Its role is learning the next swing, not fixing this one [@wolpert2000computational; @scott2004optimal; @todorov2004optimality].
- **Cerebellar updates** are not in-swing corrections at all: they update the forward/inverse models over trials, using sensed outcomes as error signals [@shadmehrmussaivaldi2012biological; @shadmehr2010error].

So the correct slogan is not "feedback is unavailable during the downswing" but rather: *fast subcortical loops are available and actively used; cortical/visual loops are not.* The downswing is predominantly a pre-programmed ballistic action whose robustness is provided by the fast reflex and vestibular loops, not by conscious, vision-mediated correction.
:::
...
The controller measures the current state $\bm{x}(t)$, compares it to the desired state $\bm{x}_d(t)$, computes an error $\bm{e}(t) = \bm{x}_d(t) - \bm{x}(t)$, and adjusts the control input based on this error. In the simplest linear case,
$$
\bm{u}(t) = \bm{K}\bm{e}(t) = \bm{K}[\bm{x}_d(t) - \bm{x}(t)]
$$
where $\bm{K}$ is a constant gain matrix. This is only a first-order approximation: it assumes constant gains, linear dynamics, instantaneous measurement, and no delay. Biological feedback is none of those things --- gains are state- and task-dependent, the dynamics are nonlinear, and every loop has latency (see the latency table above). More accurate biological models use *optimal feedback control* with stochastic, time-varying, task-conditioned gains [@todorov2004optimality, scott2004optimal, shadmehrmussaivaldi2012biological]. We use the linear form here to fix ideas, not as a realistic model of the nervous system.
...
Now here is the critical insight: **the downswing trajectory is dominated by feedforward control**, but "feedforward" here means *cortical/visual* feedforward, not absence of all feedback. Fast subcortical loops (spinal, vestibular, long-loop proprioceptive) run continuously through the downswing and provide the robustness that makes a ballistic plan actually work in a noisy world.

Why is cortical/visual feedback ruled out for within-swing trajectory correction? The timescales don't work out. Suppose a visual feedback signal detected an error at time $t=100$ ms (early in the downswing). This signal takes 100--200 ms to reach cortex and be parsed into a motor correction; the corrective command then takes another 50--100 ms to reach the muscle. The correction reaches the muscles at $t \approx 250$--400 ms. But the swing is over by $t \approx 300$ ms. The correction arrives at or after impact.
...
Proprioceptive feedback is faster (30--50 ms) but still problematic. A proprioceptive error signal arrives in time for maybe one correction cycle. The elite golfer does not rely on this. Instead, the swing is executed as a *ballistic* movement---once it starts, it unfolds according to a pre-planned motor program, with only minimal real-time adjustments.

This realization has direct implications. It means the brain does its real work *before* the swing starts: encoding the initial conditions, setting muscle stiffness, and preparing the motor program. The execution phase is largely deterministic.
...
- **Size:** The cerebellum is only ~10% of the brain's volume, but contains ~70% of all neurons (approximately 69 billion granule cells---making it the most neuron-dense structure in the brain. It also contains roughly 15 million Purkinje cells, the large output neurons that project to the deep cerebellar nuclei).

- **Computational density:** This makes the cerebellum the most densely packed neural tissue in the brain---a parallel processor of extraordinary power.
...
$$\Delta w_{ij} = -\eta \cdot e(t) \cdot x_i(t) \cdot y_j(t)$$ {#eq-cerebellar-learning}
...
Signal transmission in these fibers is fast---roughly 70--120 m/s in myelinated fibers [@kandel2013principles]. This means a motor command takes on the order of 30--50 ms to reach the upper limb muscles.
...
- **The golf swing transitions from low force to high force.** During setup and the backswing, forces are moderate and the system is highly controllable. As the transition approaches, forces increase. By the downswing, the system is in a high-force regime where ballistic execution is the only option.
...
This is a *mismatch between the controller bandwidth and the plant dynamics*---a classic problem in control engineering, which we encountered as the Disturbance Rejection vs. Control (DCR) problem in an earlier chapter on zero-torque counterfactual analysis.
...
- In the earlier chapter on zero-torque counterfactual analysis, we introduced the Disturbance Rejection vs. Control (DCR) problem: high control of small disturbances, or low control of large changes. How does the bandwidth limitation of the brain (10--40 Hz) relate to the plant bandwidth (50--200 Hz) in the context of DCR?
...
- In the section on motor unit recruitment above, we discussed Henneman's size principle: motor units are recruited in order of size. Why is this principle important for control? What would happen if large motor units were recruited first?
