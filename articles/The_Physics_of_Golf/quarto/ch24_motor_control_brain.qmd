...
In this illustrative delay budget, the downswing takes approximately 250--300 ms. Consider the delay budget:

- Visual processing: $\sim100$ ms from retina to visual cortex
- Proprioceptive processing: $\sim50$ ms from muscle spindles to cortex
- Motor planning and command generation: $\sim50$--100 ms
- Neural transmission to muscles: $\sim10$--20 ms
- Electromechanical delay (muscle activation): $\sim40$--60 ms

Total round-trip delay: approximately 250--330 ms. In this simplified timing picture, this exceeds the entire downswing duration. The implication is stark: **meaningful corrective feedback is largely unavailable during the downswing.** The downswing is predominantly a pre-programmed ballistic action. Most "corrections" you feel yourself making were likely planned before the downswing began, triggered by predicted (not actual) sensory feedback. Some limited proprioceptive correction may occur in the earliest phases of the downswing, but the scope for mid-course adjustment is severely constrained.
...
Proprioceptive feedback is faster (30--50 ms) [@Kandel2013; @enoka2002neuromechanics] but still problematic in this illustrative timing budget. A proprioceptive error signal arrives in time for maybe one correction cycle. The elite golfer likely does not rely heavily on this. Instead, the swing is executed predominantly as a *ballistic* movement---once it starts, it unfolds largely according to a pre-planned motor program, with only minimal real-time adjustments.

This suggests that the brain does its most important work *before* the swing starts: encoding the initial conditions, setting muscle stiffness, and preparing the motor program. The execution phase is largely pre-programmed, though not perfectly deterministic.
...
where $w_{ij}$ is the synaptic weight between parallel fiber $i$ and Purkinje cell $j$, and $e(t)$ is the climbing fiber error signal. The remaining terms $x_i(t)$ and $y_j(t)$ are the firing rates of parallel fiber $i$ and of the Purkinje cell. The negative sign indicates that errors drive depression of relevant synapses. Over time, this makes the forward model more accurate.

This is a simplified form of the Marr-Albus-Ito hypothesis. More precisely, the climbing fiber carries an error signal, and parallel fiber synapses that are co-active with climbing fiber input undergo LTD. The learning rule above captures the essential structure: weights decrease when error and pre-synaptic activity coincide [@wolpert1998internal; @flanagan2003control; @kawato1999internal].
...
The forward model supports *predictive control*; the inverse model supports *planning*. Together they make it possible to compute motor commands in advance that reliably produce desired outcomes.
...
The ideomotor principle also connects to the drift field. In the ZTCF framework, the brain doesn't need to control every detail of the trajectory. Instead, it specifies the initial conditions and lets physics (the drift field) carry the system to the outcome. In ideomotor terms, the brain specifies the desired sensory consequence (impact with the ball, ball trajectory), and the motor system retrieves the commands that, given physics, produce that consequence [@prinz2002perception; @hommel2009theory; @wolfensteller2016action].


## The Nervous System Architecture: Hardware Specifications


The nervous system is not a single, monolithic controller. It is a hierarchy of controllers, each operating at a different timescale and with different capabilities.

### The Motor Cortex

The primary motor cortex (M1) lies in the frontal lobe, just in front of the central sulcus. It contains roughly 100,000 corticospinal neurons per hemisphere, each sending axons down through the spinal cord to synapse onto motor neurons.
...
A light flickering at 60 Hz appears steady to your eye, even though it is off half the time. This is because 60 Hz exceeds your conscious bandwidth. Each on--off cycle is too fast to perceive as distinct.
...
This is barely enough for one feedback correction cycle. Worse, sensory feedback for the first window doesn't arrive until after the second window has ended.
...
- **Energy consumption:** The brain uses roughly 20 watts of power at rest, about 20% of the body's total metabolic rate. This seems like a lot, but it is efficient for the amount of computation being performed.
...
The brain does something different: it *learns*, over thousands of trials, a motor program that reliably produces the desired outcome. Then, at execution time, it *recalls* and *executes* the learned program. This is much cheaper computationally.
