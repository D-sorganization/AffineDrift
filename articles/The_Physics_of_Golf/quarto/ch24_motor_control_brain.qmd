...
Under the timing budgets above, visual feedback cannot close the loop within a single downswing. However, proprioceptive feedback arrives earlier (~30–50 ms) and can support some early-downswing correction. What is often perceived as within-swing feedback is in large part a learning signal from the completed swing [@Kandel2012]. The golf swing is predominantly planned and executed as a feedforward command.
:::
...
- **Per-neuron computation:** With  100 billion neurons, this is about $10^5$ operations per neuron per second, or roughly 100 floating-point operations per action potential. For context, a 3 GHz CPU core with SIMD performs ~$10^{10}$ FLOP/s (floating-point operations per second). The brain's estimated ~$10^{15}$ FLOP/s equivalent corresponds to $10^4$–$10^5$ CPU cores — a meaningful but digestible comparison. The brain compensates with massive parallelism and extremely low power per operation.
