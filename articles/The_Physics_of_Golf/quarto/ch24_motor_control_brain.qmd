---
pagetitle: "Motor Control I: The Brain as Controller"
description: "Connect golf mechanics to delayed estimation, feasible actuation, prediction, feedback and learning, with worked examples and bounded neuroscience evidence."
css:
...
[]{#sec-motor_control_brain}

# Motor Control I: The Brain as Controller
[]{#ch-motor_control_brain}
...
## The Control Problem: What the Brain Must Solve {#the-control-problem-what-the-brain-must-solve}
[]{#sec-ch24_control_problem}
[]{#constr-box:ch24_constraints}
...
## Feedback and Feedforward in One Policy
...
::: {.callout-note}
## Prepared Commands and Feedback Policies
...
[]{#subsec-ch24_ff_vs_fb}
[]{#myth-box:ch24_feedback_fantasy}
...
Short-latency upper-limb stretch responses are often discussed in approximate onset bands of 20--45 ms, long-latency responses in 50--100 ms, and voluntary responses at greater than 100 ms. These are exemplar EMG-response bands from specific experiments, not universal golf constants or complete times to change club motion  [@kurtzer2014longlatency; @pruszynski2012longlatency]. Parallel pathways do not form a single serial sum.
...
::: {.callout-note}
## Inverse Control With a Feasibility Residual
...
## A Rectangular Input Cannot Set Every Derivative
...
[]{#subsec-ch24_why_models_matter}

[]{#the-cerebellum-as-the-seat-of-internal-models}

### The Cerebellum and the Limits of a Neural Interpretation
[]{#subsec-ch24_cerebellum}
[]{#princ-box:ch24_cerebellum_arch}
...
## Predictive Processing and Distinct Kinds of Error
[]{#sec-ch24_predictive_processing}
...
::: {.callout-note}
## Prediction, Task and Reward Errors

[]{#ch24_predictive_processing}
...
This identity requires suitable support and finite expectations. Its nonnegative KL term makes it an upper bound on negative log evidence. It is an information quantity, not tissue energy measured in joules. Active-inference models must additionally specify actions, beliefs, preferred outcomes and their optimization; simply imagining a target is not a mathematical definition of active inference  [@Friston2010ActionModel].
...
## Ideomotor Theory and Task-Oriented Instruction
[]{#sec-ch24_ideomotor}
[]{#thm-ch24_ideomotor}
Ideomotor accounts propose that learned links between actions and their sensory effects contribute to action selection  [@hommel2009theory]. This motivates a hypothesis about recalling an action through a desired effect. It is not a theorem that all movement is represented only as effects, that mechanical instruction confuses a motor system, or that thinking about a target selects a uniquely correct muscle sequence.

::: {.callout-important}
## Effects Do Not Determine One Motor Solution
...
In an engineering formulation, a task objective can penalize strike error and effort without requiring every joint to follow a prescribed path. Stochastic optimal feedback control provides one formal account of allowing variation in directions that do not interfere with the goal  [@Todorov2002]. That prediction depends on the cost, dynamics and uncertainty model. It does not imply that any joint can be ignored throughout a swing or that all variation is beneficial.
...
## Distributed Neural Contributions and the Mechanical System
[]{#sec-ch24_architecture}
[]{#sec-ch24_hardware}
...
On narrow screens, scroll the figure or [open it at full size](../figures/brain_control_verified.svg).

::: {#fig-ch24_hierarchy}
::: {.aba-diagram-scroll role="region" aria-label="Functional control roles and activation impulse; scroll horizontally" tabindex="0"}
<img src="../figures/brain_control_verified.svg" alt="Prepared policies, commands, body mechanics, delayed sensory observations and prediction are connected in a functional loop. A second panel shows smaller accumulated impulse for slower activation at the same time remaining after delay.">
:::

Functional Roles in Control and a Manufactured Activation Example. Preparation, Prediction and Feedback Interact; the Diagram Does Not Assign Each Role to One Brain Region. The Curves Show Normalized Impulse After a Declared Delay, Not Measured Golf Responses.
:::

### The Motor Cortex {#the-motor-cortex}
[]{#subsec-ch24_motor_cortex}
[]{#princ-box:ch24_pop_coding}
Motor-cortical activity participates in movement preparation, execution and feedback. A preferred-direction population vector is a useful decoding model under specified conditions, not a complete law that every movement is the vector sum of neuronal votes. Churchland and colleagues reported structured population dynamics during monkey reaching  [@Churchland2012Dynamics]. This motivates alternatives to a fixed movement-parameter code; it does not establish a unique neural representation for golf or imply that losing neurons has negligible consequences.

::: {.callout-note}
## Descending Pathways and Feedback
...
Pruszynski and colleagues used mechanical perturbations in monkeys and stimulation in humans to investigate rapid multijoint feedback. Their findings support an M1 contribution to integrating shoulder and elbow information in responses around 50 ms in those experiments  [@Pruszynski2011Feedback]. This is evidence that some rapid responses reflect coupled limb mechanics. It does not measure correction of clubface orientation during a golf downswing.

[]{#the-spinal-cord-local-control-centers}

### The Spinal Cord: Local Regulation Within a Larger Network
[]{#subsec-ch24_spinal_cord}

::: {.callout-note}
## Stretch Responses and Prepared Gain
...
::: {.callout-note}
## Central Pattern Generators and Scope
...
### The Basal Ganglia, Reward and Credit Assignment
[]{#subsec-ch24_basal_ganglia}
The basal ganglia participate in interacting circuits involved in action and learning; reducing them to an isolated routine selector leaves out that interaction. Dopaminergic activity has been related to reward prediction errors, not simply whether an outcome is objectively good  [@schultz1997dopamine].

::: {.callout-note}
## A Motor Program as a Modeling Abstraction
...
::: {.callout-tip}
## Reward Does Not Identify the Effective Part of a Routine
...
[]{#sec-ch24_recruitment}

### Motor Unit Recruitment and Force Development
[]{#subsec-ch24_recruitment}
[]{#princ-box:ch24_size_principle}
...
[]{#bandwidth-limitations-the-brains-processing-speed}

## Delay, Bandwidth and Time Remaining Before Impact
[]{#sec-ch24_bandwidth}
[]{#constr-box:ch24_bandwidth_problem}
...
::: {.callout-note}
## Delay Adds Phase to a Defined Loop
...
::: {.callout-tip}
## An Actuator Can Begin Responding Without Completing a Correction
...
[]{#the-brains-processing-power-energy-and-computation}

## Variability, Energy and Computation
[]{#sec-ch24_power}
...
::: {.callout-important}
## Separate Mechanical Work, Metabolic Cost and Computation

[]{#box:ch24_power_budget}
...
::: {.callout-tip}
## Online Control and Learning Across Trials

[]{#ch24_online_vs_offline}
...
Continue with [Muscle Force Generation](ch17_muscle_force_generation.html), [Triple-Pendulum Dynamics](ch08_triple_pendulum.html), [Passive Distributed Control](ch27_passive_distributed_control.html), and [Controllability and Drift](../../controllability-drift-ratio.html).
...
- **[Ideomotor Theory and the Predictive Brain](../../ideomotor-theory-and-predictive-brain.html)** — Delayed estimation, prediction, and feedback in motor control
- **[Nonlinear Control Insights](../../nonlinear-control-insights.html)** — Control-affine brains, limits, and distributed feedback
- **[Controllability and the Drift-Control Ratio](../../controllability-drift-ratio.html)** — Relating neural delays to finite-horizon correction
:::

## Chapter Exercises and Worked Answers
### 1. Correct the Forward Prediction
...
**Worked answer.** The prediction is $(1.02,2.96)^T$. It adds a change to the existing state. Equating the next state directly to $(2,-4)^T$ is dimensionally wrong. Smoothness gives local error of order $\Delta t^2$; this estimate alone does not verify the model or numerical stability over a whole swing.

### 2. Diagnose an Infeasible Inverse
...
**Worked answer.** Minimizing $1+(u-3)^2+u^2$ gives unconstrained $u=1.5$, or bounded $u=1$ with $|u|\leq1$. The desired-minus-achieved residual is $(0,1,2,-1)^T$. It exposes both the kinematic mismatch and conflicting acceleration requests. Over time, acceleration affects position; finite-time reachability must be analyzed through the dynamics, not from the zero position block alone.

### 3. Compare Delay With Activation
...
**Worked answer.** The remaining time is 40 ms. The normalized impulse is $0.04-0.05(1-e^{-0.8})\simeq0.012466$ s. Multiplying by a constant 10 N m scale gives approximately 0.12466 N m s. Computing a club velocity additionally requires the mechanical response map. The example does not erase pre-existing activation or responses in other pathways.

### 4. Interpret a Frequency Comparison
...
**Worked answer.** The observed consequence can have a small sensory prediction error, a substantial task error and a positive reward prediction error. Their dimensions and reference values differ. One scalar reward does not specify which muscle, state estimate or pre-shot action should change.

### 7. Test the Free-Energy Identity
...
**Worked answer.** The evidence is 0.30 and the posterior belief is $(0.4,0.6)$. At that belief the KL term vanishes and $\mathcal F=-\log0.30\simeq1.204$ with natural logarithms. A different belief adds a nonnegative KL term. This is an information-theoretic identity, not a mechanical or metabolic energy budget and not proof of a neural implementation.

### 8. Evaluate a Recruitment Claim
...
**Worked answer.** Recruitment order does not prescribe a universal serial waiting time. Recruitment speed, discharge behavior, prior activation and contractile transmission jointly affect force. Measure motor-unit or appropriately interpreted EMG activity and force with synchronized timing. Evidence from an explosive isometric leg task cannot simply be assigned to a wrist during a swing.

### 9. Design a Controller Comparison
