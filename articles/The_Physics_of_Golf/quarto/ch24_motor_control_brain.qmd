...
<!-- TODO: citation needed for proprioceptive acuity values (1--2 degrees wrist, 3--5 degrees shoulder) ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for the 5--[% value] muscle force variability figure ; exact values depends on individual biomechanics and model assumptions -->

- **Bounded control:** Maximum torques are limited. Illustrative ranges: the shoulder can produce roughly 100--200 N$\cdot$m of torque, the elbow 100--150 N$\cdot$m, the wrist 30--50 N$\cdot$m (depends on individual strength and joint angle). The brain cannot exceed these.
<!-- TODO: citation needed for the joint torque capacity estimates (shoulder [Nm range], elbow [Nm range], wrist [Nm range]) ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for each of the neural latency components above; these are approximate values consistent with motor neuroscience literature but should be supported by specific references ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for visual processing latency (80--[ms value] to visual cortex) and motor command generation latency (50--[ms value]) values ; exact values depends on individual biomechanics and model assumptions -->
...
Proprioceptive feedback is faster (illustrative: 30--50 ms; depends on individual) but still problematic. A proprioceptive error signal arrives in time for maybe one correction cycle. The elite golfer likely does not rely heavily on this. Instead, the swing is executed predominantly as a *ballistic* movement---once it starts, it unfolds largely according to a pre-planned motor program, with only minimal real-time adjustments.
<!-- TODO: citation needed for proprioceptive feedback latency (30--[ms value]) in the context of the golf swing ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for the [ms value] visual processing latency estimate used as an illustrative value here -->
