...
- **Cerebellar updates** are not in-swing corrections at all: they update the forward/inverse models over trials, using sensed outcomes as error signals [@shadmehrmussaivaldi2012biological].
...
Modern neuroscience has converged on a powerful unifying idea: the brain is fundamentally a *prediction machine*. This framework, developed by Karl Friston and others, is called the *free energy principle* and the *predictive processing* framework \cite{friston2010action, clark2013whatever}.
...
This is a *mismatch between the controller bandwidth and the plant dynamics*---a classic problem in control engineering, which we encountered as the Disturbance Rejection vs. Control (DCR) problem in Chapter \ref{ch:zero_torque_counterfactual}.
...
- In Chapter \ref{ch:zero_torque_counterfactual}, we introduced the Disturbance Rejection vs. Control (DCR) problem: high control of small disturbances, or low control of large changes. How does the bandwidth limitation of the brain (10--40 Hz) relate to the plant bandwidth (50--200 Hz) in the context of DCR?
...
- In Section \ref{subsec:ch24_recruitment}, we discussed Henneman's size principle: motor units are recruited in order of size. Why is this principle important for control? What would happen if large motor units were recruited first?
