...
In the manipulator equation, every torque comes from one of five sources in the conservative + actuator budget:
...
A *sixth* category---**dissipative forces** (joint friction, soft-tissue damping, hysteresis, aerodynamic drag)---is genuinely present in the real swing but is deferred: drag is the subject of ch19, and viscoelastic/damping models of soft tissue and joints are covered in ch12 and ch29. For the classical-mechanics derivations in Vol I we omit dissipative forces from the five-source accounting and add them back as $-D(\bm{q})\dot{\bm{q}}$-style corrections in the later chapters. Readers should treat the five sources here as the *lossless* core of the dynamics and the sixth as the physical correction applied on top of it.
...
Recall from @sec-ch03-numerical that the effective moment of inertia at the shoulder is about $M_{11} \approx 2.22 \text{ kg m}^2$.
...
This is the baseline. Just to accelerate the arm at this rate requires 38.7 N m. If there's also gravity pulling (a few N m depending on configuration), the total required muscular torque is correspondingly larger.
...
For a point rotating at angular velocity $\omega$ at distance $r$ from the axis, the centripetal acceleration directed toward the center is:

$$a_c = \omega^2 r$$

In the rotating-frame perspective this appears as an outward "centrifugal" acceleration of the same magnitude. It arises from circular motion, not from a separate force-generating agent.
...
The centripetal acceleration at the elbow is:

$$
a_c = \dot{\theta}_1^2 \cdot L_1 = 10.47^2 \times 0.35 = 38.3 \text{ m/s}^2 \approx 3.9\,g
$$

The full expression for the centrifugal torque on the elbow involves the mass distribution and geometry of the forearm-club system, which we develop rigorously in @sec-affine-structure. The key observation is that centrifugal force acts to straighten the arm during the downswing, contributing to the passive dynamics that accelerate the clubhead without additional muscular effort.
...
The maximum depends on the muscle's strength and leverage. Net joint torques are the *resultant* of many muscles pulling in parallel:

- **Shoulder**: the rotator cuff (supraspinatus, infraspinatus, teres minor, subscapularis) stabilizes the joint, while deltoids, pectoralis major, and latissimus dorsi generate the gross torque. Peak resultant torque in elite golfers is on the order of $\tau_{\max}^{\text{sh}} \sim 40\text{--}60$ N m [@Nesbit2005].
- **Elbow**: biceps brachii and brachialis flex; triceps extend. Peak resultant torque is on the order of $\tau_{\max}^{\text{el}} \sim 20\text{--}40$ N m, smaller than the shoulder because of the shorter moment arms.
- **Wrist (radiocarpal)**: wrist flexors (flexor carpi radialis/ulnaris) and extensors (extensor carpi radialis longus/brevis, extensor carpi ulnaris) together drive the release. Peak resultant torque is much smaller, $\tau_{\max}^{\text{wr}} \sim 10\text{--}20$ N m, but acts at high angular velocity.
- **Forearm pronation/supination**: pronator teres and pronator quadratus vs. supinator and biceps brachii govern face-angle control through the forearm-rotation DOF (see ch31); peak torque $\sim 5\text{--}15$ N m.

These are *resultant* torques; individual muscle forces are under-determined (the redundancy problem discussed in ch18). What matters for the manipulator equation is the resultant torque at each joint, not the individual muscle forces.
...
With the elbow also rotating and the arm accelerating, total accelerations are higher still. Published measurements of clubhead acceleration during the golf swing report values on the order of 100$g$ or more [@Cochran1968; @Penner2003], though exact figures depend on the golfer and measurement methodology.
...
The hand is holding the club. The club moves in a curved path, so the hand must supply the centripetal force:

$$
F_c = M_{\text{club}} \omega^2 r = 0.310 \text{ kg} \times 10.47^2 \times 1.35 \approx 45.7 \text{ N}
$$

The hand also accelerates the club tangentially through the shoulder's angular acceleration. The total grip force is the vector sum of centripetal and tangential components; for a typical impact it's on the order of 40–80 N.
...
7. **At impact, large accelerations require careful force balance**. No single source (gravity, muscle, velocity-dependence) does it alone. All work together.
...
3. **Coriolis Coupling**. Suppose the shoulder rotates at constant rate $\dot{\theta}_1 = 8 \text{ rad/s}$. The elbow suddenly starts rotating at $\dot{\theta}_2 = 5 \text{ rad/s}$. Using the Coriolis term from @sec-ch03-numerical, estimate the torque this creates at the shoulder.
4. **Gravity Torque**. At the top of the backswing, $\theta_1 = 150°, \theta_2 = -70°$. Compute the gravitational torque at the shoulder (use parameters from @sec-ch03-numerical). Is gravity trying to rotate the shoulder forward (into the downswing) or backward?
...
6. **Grip Force**. The club (mass 0.310 kg) is 1.35 m from the shoulder and the shoulder rotates at $\dot{\theta}_1 = 10.47$ rad/s. What is the centripetal grip force on the club?
7. **Work Done by Gravity**. From address ($\theta_1 = 0°$) to the top ($\theta_1 = 150°$), how much does the potential energy increase? (Use parameters from @sec-ch03-numerical.) From the top to impact ($\theta_1 = 0°$), how much does gravity do in work?
