...
Recall from Chapter @ch:03_double_pendulum that the effective moment of inertia at the shoulder is about $M_{11} \approx 2.22 \text{ kg m}^2$.
...
This is the baseline using the model parameters from Chapter @ch:03_double_pendulum. Just to accelerate the arm at this rate requires 38.7 N m. If there's also gravity pulling (10.6 N m backward), then you need to generate at least $38.7 + 10.6 = 49.3$ N m of muscular torque to achieve this acceleration in this model (illustrative; depends on individual body parameters <!-- TODO: add citation for representative shoulder angular acceleration values during golf downswing -->).
...
3. **Coriolis Coupling**. Suppose the shoulder rotates at constant rate $\dot{\theta}_1 = 8 \text{ rad/s}$. The elbow suddenly starts rotating at $\dot{\theta}_2 = 5 \text{ rad/s}$. Using the Coriolis term from Chapter @ch:03_double_pendulum, estimate the torque this creates at the shoulder.
4. **Gravity Torque**. At the top of the backswing, $\theta_1 = 150°, \theta_2 = -70°$. Compute the gravitational torque at the shoulder (use parameters from Chapter @ch:03_double_pendulum). Is gravity trying to rotate the shoulder forward (into the downswing) or backward?
...
7. **Work Done by Gravity**. From address ($\theta_1 = 0°$) to the top ($\theta_1 = 150°$), how much does the potential energy increase? (Use parameters from Chapter @ch:03_double_pendulum.) From the top to impact ($\theta_1 = 0°$), how much does gravity do in work?
