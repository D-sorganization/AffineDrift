...
description: "1. Inertial forces (from ): forces that resist acceleration. To speed up your arm, you must overcome its inertia. 2. Velocity-dependent forces (from ):..."
---
...
Recall from Chapter @sec-03_double_pendulum that the effective moment of inertia at the shoulder is about $M_{11} \approx 2.22 \text{ kg m}^2$.
...
This is the baseline using the model parameters from Chapter @sec-03_double_pendulum. Just to accelerate the arm at this rate requires 38.7 N m. If there's also gravity pulling (10.6 N m backward), then you need to generate at least $38.7 + 10.6 = 49.3$ N m of muscular torque to achieve this acceleration in this model (illustrative; depends on individual body parameters <!-- TODO: add citation for representative shoulder angular acceleration values during golf downswing -->).
...
The full expression involves the mass distribution and geometry of the forearm-club system, which we develop rigorously in @sec-05_affine_structure. The key observation is that centrifugal force acts to straighten the arm during the downswing, contributing to the passive dynamics that accelerate the clubhead without additional muscular effort.
...
The maximum depends on the muscle's strength and leverage. For the shoulder, elite golfers can generate roughly 50 N m in both directions, and for the elbow perhaps 30 N m (illustrative estimates; actual values vary with body size, training, and joint angle <!-- TODO: add citation for peak shoulder and elbow torque capacity in elite golfers, e.g., upper-extremity biomechanics literature -->). These are illustrative estimates; actual values vary with body size, training, and joint angle.
...
This matches observation: impact accelerations are typically 15–20 $g$.

### Where Do These Accelerations Come From?

At impact:
- **Inertial**: to achieve these accelerations, the arm must overcome its own inertia. $\tau_{\text{inertia}} = M_{11} \ddot{\theta}_1 \approx 40 \text{ N m}$.
- **Gravity**: at impact ($\theta_1 \approx 0°$), gravity is nearly balanced. Gravity's contribution is small, perhaps 5–10 N m in this model, and mostly neutral (neither helping nor hurting).
- **Coriolis/Centrifugal**: these are large. At 100 mph, $\dot{\theta}_1$ and $\dot{\theta}_2$ are both large. The velocity-dependent forces are on the order of 10–20 N m in this simplified model, and they're helping (pushing the arm in the direction it's accelerating) (illustrative; depends on model parameters <!-- TODO: add citation for measured velocity-dependent torques during golf downswing -->).
- **Muscular**: the total muscular torque must be roughly $40 + 10 = 50$ N m (inertia plus gravity, minus the velocity-dependent forces that are helping) in this illustrative model. Humans can generate torques of this magnitude (illustrative estimate; varies with individual and conditioning <!-- TODO: add citation for human shoulder torque capacity, e.g., strength and conditioning literature -->).
...
3. **Coriolis Coupling**. Suppose the shoulder rotates at constant rate $\dot{\theta}_1 = 8 \text{ rad/s}$. The elbow suddenly starts rotating at $\dot{\theta}_2 = 5 \text{ rad/s}$. Using the Coriolis term from Chapter @sec-03_double_pendulum, estimate the torque this creates at the shoulder.
4. **Gravity Torque**. At the top of the backswing, $\theta_1 = 150°, \theta_2 = -70°$. Compute the gravitational torque at the shoulder (use parameters from Chapter @sec-03_double_pendulum). Is gravity trying to rotate the shoulder forward (into the downswing) or backward?
...
7. **Work Done by Gravity**. From address ($\theta_1 = 0°$) to the top ($\theta_1 = 150°$), how much does the potential energy increase? (Use parameters from Chapter @sec-03_double_pendulum.) From the top to impact ($\theta_1 = 0°$), how much does gravity do in work?
