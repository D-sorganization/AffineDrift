...
This is the baseline using the model parameters from Chapter @ch:03_double_pendulum. Just to accelerate the arm at this rate requires 38.7 N m. If there's also gravity pulling (10.6 N m backward), then you need to generate at least $38.7 + 10.6 = 49.3$ N m of muscular torque to achieve this acceleration in this model. Using 1000 deg/s$^2$ here is a pedagogical benchmark; inverse-dynamics golf models report joint accelerations and torques in this general order of magnitude [@nesbit2005three; @nesbit2014work].
...
The maximum depends on the muscle's strength and leverage. For the shoulder, golf inverse-dynamics studies report joint moments in the tens of N m, and 50 N m is a reasonable illustrative upper-end benchmark for elite swings; elbow moments are typically smaller [@nesbit2005three; @nesbit2014work]. These are illustrative estimates; actual values vary with body size, training, and joint angle.
...
- **Coriolis/Centrifugal**: these are large. At 100 mph, $\dot{\theta}_1$ and $\dot{\theta}_2$ are both large. The velocity-dependent forces are on the order of 10--20 N m in this simplified model, and they are helping (pushing the arm in the direction it's accelerating) [@koike2019dynamic; @nesbit2005three].
- **Muscular**: the total muscular torque must be roughly $40 + 10 = 50$ N m (inertia plus gravity, minus the velocity-dependent forces that are helping) in this illustrative model. Golf inverse-dynamics studies place shoulder moments in this range, so the number is plausible as a peak benchmark rather than a universal constant [@nesbit2005three; @nesbit2014work].
