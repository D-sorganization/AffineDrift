...
If you can attribute each force to its source, you gain enormous insight. You understand what you're controlling. You see which forces help you and which fight you. You can diagnose problems.
...
Recall from @sec-03_double_pendulum that the effective moment of inertia at the shoulder is about $M_{11} \approx 2.22 \text{ kg m}^2$.
...
This is the baseline using the model parameters from @sec-03_double_pendulum. Just to accelerate the arm at this rate requires 38.7 N m. Gravity also pulls, with 10.6 N m acting backward. Adding the two, you need at least $38.7 + 10.6 = 49.3$ N m of muscular torque to achieve this acceleration in this model [@Nesbit2005] (illustrative; depends on individual body parameters).
...
At high speeds, the muscular forces flip sign. Instead of pulling the arm, they're *resisting* the arm's acceleration. The arm is accelerating so fast from gravity and velocity-dependent forces that muscles must apply a braking torque to slow it down to the right speed for impact.
...
Later in the downswing, gravity fights you (pulling the arm down when you want to pull it forward). But by that point, the arm has so much kinetic energy that gravity's resistance is small.
...
The maximum depends on the muscle's strength and leverage. For the shoulder, elite golfers can generate roughly 50 N m in both directions, and for the elbow perhaps 30 N m [@Gatt1998; @Nesbit2005] (illustrative estimates; actual values vary with body size, training, and joint angle).
...
Finally, there are forces that appear automatically to enforce mechanical constraints. These don't show up explicitly in the manipulator equation (as written in generalized coordinates), but they're present as reaction forces at hinges and joints.
...
Let's do a complete force attribution at impact. At impact, the clubhead is moving at roughly 100 mph = 44.7 m/s — a value in the range for skilled golfers [@Jorgensen1994]. The clubhead acceleration is enormous.
...
- **Coriolis/Centrifugal**: these are large. At 100 mph, $\dot{\theta}_1$ and $\dot{\theta}_2$ are both large. The velocity-dependent forces are significant in this simplified model, and they're helping (pushing the arm in the direction it's accelerating) [@MacKenzie2009] (illustrative; depends on model parameters).
- **Muscular**: the total muscular torque must be roughly $40 + 10 = 50$ N m (inertia plus gravity, minus the velocity-dependent forces that are helping) in this illustrative model. Humans can generate torques of this magnitude [@Gatt1998; @Nesbit2005] (illustrative estimate; varies with individual and conditioning).
...
3. **Coriolis Coupling**. Suppose the shoulder rotates at constant rate $\dot{\theta}_1 = 8 \text{ rad/s}$. The elbow suddenly starts rotating at $\dot{\theta}_2 = 5 \text{ rad/s}$. Using the Coriolis term from @sec-03_double_pendulum, estimate the torque this creates at the shoulder.
4. **Gravity Torque**. At the top of the backswing, $\theta_1 = 150°, \theta_2 = -70°$. Compute the gravitational torque at the shoulder (use parameters from @sec-03_double_pendulum). Is gravity trying to rotate the shoulder forward (into the downswing) or backward?
...
7. **Work Done by Gravity**. From address ($\theta_1 = 0°$) to the top ($\theta_1 = 150°$), how much does the potential energy increase? (Use parameters from @sec-03_double_pendulum.) From the top to impact ($\theta_1 = 0°$), how much does gravity do in work?
