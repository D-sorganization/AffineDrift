...
Recall from [Chapter 17](./ch17_muscle_force_generation.qmd#sec-muscle-force-generation) that the Hill muscle model [@Hill1938] consists of an active contractile element (CE), a parallel elastic element (PE), and a series elastic element (SE). Crucially, the Hill model also includes a *dashpot* — a viscous element — in parallel with the contractile element. This dashpot represents the internal friction of muscle fiber sliding.
...
Recall from [Chapter 9](./ch09_parallel_mechanisms.qmd#sec-parallel_mechanisms) that the unforced kinematic chain is governed by:
$$
...
In the control-affine setting, recall that the ZTCF family is:
$$
...
This connects to impedance control [@hogan1985impedance] from [Chapter 27](./ch27_passive_distributed_control.qmd#sec-passive_distributed_control). Recall that you can write:
$$
...
During the final 100 ms of the downswing, club speed is changing at a rate of approximately 50 m/s$^2$ [@Nesbit2005] (illustrative: local acceleration; this is a typical-order estimate from simulation; depends on swing speed and phase; roughly 30--80 m/s$^2$ is a plausible estimated range). At these speeds, a damping torque of even 1 Nm becomes significant.
...
Recall from [Chapter 11](./ch11_flexible_shaft.qmd#sec-flexible_shaft) that the shaft is a distributed elastic beam. In addition to stiffness, shafts have material damping — the internal friction of the composite material itself.
