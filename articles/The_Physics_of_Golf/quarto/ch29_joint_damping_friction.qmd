...
<!-- TODO: citation needed for the peak shoulder rotation rate ([deg/s value]) and wrist rotation rate ([deg/s value]) figures; these should be validated against motion capture data ; exact values depends on individual biomechanics and model assumptions -->
...
<!-- TODO: citation needed for the energy dissipation breakdown table; these estimates should be supported by published biomechanical validation sources ; exact values depends on individual biomechanics and model assumptions -->
...
During the final 100 ms of the downswing, club speed is changing at a rate of $\sim 50$ m/s$^2$ (illustrative: local acceleration; this is a typical-order estimate from simulation; depends on swing speed and phase). At these speeds, a damping torque of even 1 Nm becomes significant.
<!-- TODO: citation needed for the [acceleration value] club acceleration rate in the final [ms value]; this should be validated against high-speed motion capture data ; exact values depends on individual biomechanics and model assumptions -->
...
For a driver swing (club speed 80--90 mph, illustrative range for intermediate to advanced golfers; depends on individual), a 2 mph change is about 5% of club speed. This is not negligible, but it's not dominant either. Most of the sensitivity in club speed comes from muscular timing and force magnitude, not from damping.
<!-- TODO: citation needed for representative driver swing speed range (80--[mph value]) and the claim that damping sensitivity is [% value] of club speed ; exact values depends on individual biomechanics and model assumptions -->
...
- **If modeling the full swing (address to follow-through)**: Include a constant damping matrix with values from the table in Section @sec:sources_damping. Use linear viscous damping; the added complexity of nonlinear friction is not justified by the accuracy gain for most purposes.
