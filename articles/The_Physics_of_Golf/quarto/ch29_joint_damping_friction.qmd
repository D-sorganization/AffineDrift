...
Recall from the chapter on muscle force generation that the Hill muscle model [@Hill1938] consists of an active contractile element (CE), a parallel elastic element (PE), and a series elastic element (SE). Crucially, the Hill model also includes a *dashpot* — a viscous element — in parallel with the contractile element. This dashpot represents the internal friction of muscle fiber sliding.
...
Recall from earlier chapters on parallel mechanisms that the unforced kinematic chain is governed by:
$$
...
A joint can be classified by its *damping ratio*. For a **translational** system ($m\ddot{x} + c\dot{x} + kx = 0$):
$$
\zeta = \frac{c}{2\sqrt{km}}
$$
where $c$ is the damping coefficient, $k$ the stiffness, and $m$ the effective mass. For a **rotational** system ($I\ddot{\theta} + c\dot{\theta} + k\theta = 0$) — the relevant case for joints — the moment of inertia $I$ replaces mass:
$$
\zeta_{\text{rot}} = \frac{c}{2\sqrt{kI}}
$$
Throughout this chapter, joint dynamics are rotational, so $I$ is the appropriate inertial quantity.
...
This connects to impedance control [@hogan1985impedance] from the chapter on passive distributed control. Recall that you can write:
$$
...
A typical golfer converts a significant fraction of muscular energy into useful clubhead kinetic energy, but a substantial portion is dissipated across multiple pathways. The qualitative ordering of dissipation sources---from largest to smallest---is reasonably consistent across biomechanical studies [@Winter2009], even though precise numerical budgets have not been established for the golf swing specifically.

### Sources of Dissipation

The primary dissipation pathways, in qualitative order of magnitude, are:

1. **Muscle internal friction and metabolic cost**: The largest single category; muscles do not convert chemical energy to mechanical work with perfect efficiency.
2. **Tendon hysteresis**: Tendons store and release elastic energy, but with some loss per cycle.
3. **Joint damping**: Viscous resistance at all joints in the kinematic chain.
4. **Swing-to-impact transition inefficiency**: Energy in arm and trunk motion that is not fully transferred to the club.
5. **Body segment soft tissue**: Muscle, fat, and skin oscillation.
6. **Ground deformation and contact friction**: Typically small.
7. **Aerodynamic drag on arms and torso**: Small compared to mechanical losses.
8. **Grip and hand interface friction**: Small.
...
- Joint damping is a significant but not dominant loss pathway. Changes in damping coefficient produce correspondingly modest changes in club speed.

- Tendon hysteresis is one of the larger dissipative sources. Yet tendons also store elastic energy. The net effect of tendon mechanics is beneficial: they act as elastic accumulators, releasing energy at optimal moments.

- Body segment soft tissue wobble dissipates energy through oscillation of muscle, fat, and skin. The magnitude varies substantially with body composition.
...
- **If modeling the full swing (address to follow-through)**: Include a constant damping matrix with values from the joint damping table earlier in this chapter. Use linear viscous damping; the added complexity of nonlinear friction is not justified by the accuracy gain for most purposes.
...
Recall from the chapter on the flexible shaft that the shaft is a distributed elastic beam. In addition to stiffness, shafts have material damping — the internal friction of the composite material itself.
...
$$
where $\zeta$ is the damping ratio (valid for light damping, $\zeta \ll 1$). High Q (high stiffness, low damping) means oscillations persist. Low Q means oscillations decay quickly.
...
- Graphite (carbon fiber) shaft: $Q \approx 100--300$ (lower damping, oscillations decay more slowly).
...
*Note: The numerical parameters in these exercises are illustrative values chosen to make the calculations tractable. They are not empirical measurements of specific golfers or joints.*
...
- **Damping Ratio and Step Response.** A wrist joint has moment of inertia $I = 0.05$ kg$\cdot$m$^2$, stiffness $k = 5$ Nm/rad, and damping $c = 0.2$ Nm$\cdot$s/rad. Compute the natural frequency $\omega_n = \sqrt{k/I}$, the critical damping $c_c = 2\sqrt{kI}$, and the damping ratio $\zeta = c/c_c$. Is the joint underdamped, critically damped, or overdamped? Estimate the settling time (time to reach 2% of steady-state value) using $t_s \approx 4 / (\zeta \omega_n)$.
