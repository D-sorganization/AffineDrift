...
Recall from Chapter @sec-muscle-force-generation that the Hill muscle model [@Hill1938] consists of an active contractile element (CE), a parallel elastic element (PE), and a series elastic element (SE). Crucially, the Hill model also includes a *dashpot* — a viscous element — in parallel with the contractile element. This dashpot represents the internal friction of muscle fiber sliding.
...
Recall from Chapter @sec-parallel_mechanisms that the unforced kinematic chain is governed by:
$$
...
This connects to impedance control [@hogan1985impedance] from Chapter @sec-passive_distributed_control. Recall that you can write:
$$
...
Recall from Chapter @sec-flexible_shaft that the shaft is a distributed elastic beam. In addition to stiffness, shafts have material damping — the internal friction of the composite material itself.
