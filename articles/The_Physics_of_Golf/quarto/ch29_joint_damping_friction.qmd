...
Recall from Chapter~\ref{ch:muscle-force-generation} that the Hill muscle model \citep{Hill1938} consists of an active contractile element (CE), a parallel elastic element (PE), and a series elastic element (SE). Crucially, the Hill model also includes a *dashpot* — a viscous element — in parallel with the contractile element. This dashpot represents the internal friction of muscle fiber sliding.
...
- **If modeling the full swing (address to follow-through)**: Include a constant damping matrix with values from the table in Section~\ref{sec:sources_damping}. Use linear viscous damping; the added complexity of nonlinear friction is not justified by the accuracy gain for most purposes.
