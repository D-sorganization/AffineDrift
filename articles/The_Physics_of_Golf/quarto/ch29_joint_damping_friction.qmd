...
Why? Because the proximal joint has higher angular velocity (relative to body-fixed rates) and moves a larger inertia. The power dissipated by damping is $P_d = c \omega^2$. In golf swing motion-capture studies, the shoulder can rotate much faster than the wrist, so even with the same damping coefficient the shoulder dissipates far more power [@kwon2012kinematic; @hellstrom2009kinematic].
...
Note: These are rough estimates based on a model bookkeeping exercise rather than a direct measurement. The exact breakdown varies widely with individual swing mechanics, body composition, and equipment.
...
During the final 100 ms of the downswing, club speed is changing rapidly, so even a damping torque of 1 N$\cdot$m becomes significant. This is a model-scale estimate meant to show sensitivity, not a universal biomechanical constant [@nesbit2005three; @hume2005role].
...
For a driver swing (club speed 80--90 mph, a representative range for intermediate to advanced golfers [@hume2005role]), a few-mph change would be noticeable, but it would still be smaller than the effects of poor timing or force application. Most of the sensitivity in club speed comes from muscular timing and force magnitude, not from damping.
