...
\begin{description}
    ...
    \item[$\state^{\mathrm{ZTCF}}(t)$] Zero-Torque Counterfactual trajectory (evolution under drift only, $\control = \bm{0}$).
    \item[$\state^{\mathrm{ZVCF}}(t)$] Zero-Velocity Counterfactual state (system state if $\dot{\bm{q}} = \bm{0}$).
    \item[$\mathrm{DCR}(t)$] Drift-to-Control Ratio: $\norm{f(\state(t))}/\norm{G(\state(t))\control(t)}$.
\end{description}
