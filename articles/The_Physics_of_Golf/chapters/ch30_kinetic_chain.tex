\chapter{The Kinetic Chain: Motion, Work, and Control in the Golf Swing}
\label{ch:kinetic_chain}
\index{kinetic chain}\index{proximal-to-distal sequencing}\index{energy flow!kinetic chain}
...
\section{Define the Observation Before Explaining It}
\subsection{Segments, Joints, and Comparable Rates}
...
\subsection{A Sequence Is a Measurement Result}
...
\subsection{Transfer, Generation, Storage, and Dissipation}
...
\subsection{Whole-System Equations Do Not Identify Individual Muscles}
...
\section{What Coupled Oscillators Actually Predict}
\subsection{A Solvable Two-Rotor Counterexample}
...
\subsection{Normal Modes Belong to the Coupled System}
...
\section{X-Factor: Relative Motion Before Biological Interpretation}
\subsection{Use a Declared Angular Convention}
...
\subsection{What the Classic Study Measured}
Cheetham and colleagues compared 10 highly skilled and 9 less-skilled right-handed golfers using 5-irons and electromagnetic tracking. Their reported 30 Hz sampling and pelvis/T3 rotation measurements concern the transition region. They found a greater early-downswing X-factor increase in the higher-skill group, but no statistically significant between-group difference at the top of the backswing \citep{Cheetham2001}. This group comparison does not establish that increasing one player's separation will increase distance.
...
\section{Wrist Motion and Work at the Grip}
\subsection{Lag Is Geometry, Not a Muscle State}
...
\subsection{Holding, Releasing, and Feasibility}
...
\section{Ground Reaction: Momentum Channel and Contact Constraint}
\subsection{Compute the Full Moment Vector}
...
\end{equation} If that net yaw moment stays constant for $0.05$ s, its yaw angular impulse is $3.5$ N m s. These are declared illustrative inputs; the calculation is not a prediction of hip angular speed.

\subsection{Whole-Body Angular Momentum Is Not Pelvic Rotation}
...
\subsection{Center of Pressure and Ground Work}
...
\section{Close the Energy Budget Before Naming Efficiency}
\subsection{Choose the System and the Time Interval}
...
\subsection{Signed Work, Positive Work, and Metabolism}
...
\section{How Drift and Control Connect to the Chain}
\subsection{Declare What Zero Input Removes}
...
\section{Test the Explanation}
\subsection{Link Each Claim to an Observable}
...
\subsection{Test Interventions Against Competing Explanations}
...
\section{What the Reader Can Use in Practice}
...
\section{Reproduce the Mechanical Checks}
...
\section{Exercises}
\begin{enumerate}
\item \textbf{Sequence Counterexample.} Starting from the two-rotor equations, derive the mean and relative solutions and reconstruct both angular velocities. On $[0,1]$ s, locate rotor 1's maximum velocity and rotor 2's first acceleration maximum. Explain why their order refutes a universal proximal-speed/distal-acceleration rule without refuting every possible sequencing mechanism. Verify the input-work identity independently.

\item \textbf{X-Factor and Storage.} In the declared backswing-positive convention, let thorax and pelvis angles at the reference event be $90$ and $45$ degrees. At a later event let them be $88$ and $38$ degrees. Compute separation at each event and its change. For an assumed torsional spring with rest separation $30$ degrees and stiffness $100$ Nm/rad, compute the storage change in joules. State why this result is not a measurement of human trunk energy and why both turn rates becoming zero would not remove preload.

\item \textbf{A Moving-Grip Release.} Derive the club equation by eliminating grip force from Newton--Euler balances. Reproduce the three acceleration cases and check energy power. Then prescribe a smooth grip trajectory and an arm angle history of your choosing, state them explicitly, and compute the holding couple. Explain the additional endpoint, feasibility, and input-work checks needed before comparing two release times. Zero grip couple alone is not a complete boundary condition.

\item \textbf{Ground Wrench and Reference Point.} Reproduce all three moment components in the ground example. Change the origin by a known displacement and transform the moment consistently. Integrate the original yaw moment over the stated interval. Identify the additional information needed to infer pelvic acceleration from whole-system angular impulse, and explain why center of pressure alone cannot supply it.
