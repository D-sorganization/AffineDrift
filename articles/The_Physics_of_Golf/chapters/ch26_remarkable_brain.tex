\chapter[Motor Control III: The Computational Brain]{Motor Control III:\\The Computational Brain}
...
\begin{principle}{Count Schedules, Dimensions and Feasible Motions Separately}{box:ch26_numbers}
...
\begin{constraintbox}{Separate Delay, Activation and Remaining Time}{box:ch26_time_constraint}
...
Primary experiments show why the distinction matters. Pruszynski and colleagues found rapid integration of shoulder and elbow information in arm perturbation tasks, with responses on a timescale of about 50ms \citep{Pruszynski2011Feedback}. Saunders and Knill found online corrections to virtual-hand perturbations during reaching. Their mean detected latency was 163ms; their analysis estimated a positive detection bias of about 25ms \citep{Saunders2003Feedback}. These are different tasks, signals and measurements. Neither result supplies a universal downswing latency, but both contradict the premise that fast movement categorically excludes feedback.

\section[How Prediction, Feedback and Mechanics Cooperate]{How Prediction, Feedback\\and Mechanics Cooperate}
\label{sec:ch26_strategies}

\subsection{A Worked Remaining-Time Calculation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/brain_correction_window.pdf}
\caption{Remaining Time Changes the Same Command's Effect. The Declared Rotor Model Compares Instantaneous Torque With Activation Filtering After the Same 60ms Delay. These Are Illustrative Responses, Not Measured Golf Data.}
\label{fig:brain_correction_window}
\end{figure}
...
\subsection{Predictive Control and Anticipatory Activation}
\label{subsec:ch26_predictive}

\begin{principle}{Prediction Uses Information Already Available}{box:ch26_anticipatory}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/brain_control_loop.pdf}
\caption{Preparation, Mechanics, Estimation, Feedback and Learning Interact. Within-Trial Responses and Changes Across Trials Must Be Distinguished Experimentally.}
\label{fig:brain_control_loop}
\end{figure}

\subsection{Drift Is a Model Term, Not a Free Actuator}
...
\begin{example}{What This Means for Grip and Balance}{ch26_impedance_golf}
...
Kutch and Valero-Cuevas used cadaveric measurements and computational models to show that biomechanics and task selection can produce low-dimensional patterns without a controller that groups muscles \citep{Kutch2012Synergies}. This is a counterexample to a particular inference, not a disproof of all neural coordination. It motivates testing the mechanical alternatives alongside the neural hypothesis.
...
\begin{example}{A Compact Basis Can Remove the Needed Action}{ch26_synergies_example}
...
\subsection{What Adaptation Experiments Can Establish}
\label{subsec:ch26_brain_learns_physics}

\begin{example}{Separate Task Error From Prediction Error}{ch26_physics_evidence}
Mazzoni and Krakauer introduced a visuomotor rotation and supplied an explicit aiming strategy that initially corrected it. Continued implicit adaptation subsequently interfered with that strategy \citep{Mazzoni2006Strategy}. This demonstrates that explicit and implicit processes can coexist and conflict in that task. It does not show that all rapid learning is unconscious or that successful golf requires a uniquely identified internal physics model.
\end{example}
...
\begin{principle}{Specify a Comparable Benchmark}{box:ch26_brain_ai_learning}
...
There is no generic rule that reinforcement learning uses joint angles only, lacks internal models, or learns only from a final scalar reward. Dreamer, published in 2020, learned a world model from images and trained behavior using imagined latent trajectories \citep{dreamer2020scalable}. This is a concrete counterexample to that characterization, not evidence that it reproduces human golf learning or a claim about the best current algorithm.

\subsection{What Human Performance Contributes to the Comparison}
\label{subsec:ch26_brain_better}

\begin{principle}{Measure Adaptation and Transfer}{box:ch26_brain_excel}
...
\subsection{What Computation Does and Does Not Guarantee}
\label{subsec:ch26_ai_better}

\begin{principle}{Deterministic Evaluation Is Not a Deterministic Shot}{box:ch26_ai_excel}
...
\begin{principle}{Organize Decisions by Their Role}{box:ch26_hierarchical_brain}
...
\begin{example}{Use Prediction Where It Is Reliable}{ch26_model_based_rl}
...
\begin{principle}{Mechanics Redistributes and Stores Energy}{box:ch26_embodiment}
...
\begin{example}{Learn From Consequences, Then Test Transfer}{ch26_self_supervised}
...
\begin{principle}{Measure the Chain From Action to Outcome}{box:ch26_golf_model_system}
...
\begin{example}{A Discriminating Golf Experiment}{ch26_golf_research}
...
\subsection{Open Questions That Golf Helps Address}
\label{subsec:ch26_open_questions}

\begin{principle}{Ask Questions That Could Change the Model}{box:ch26_open_questions}
...
\section{Why Skilled Performance Sometimes Fails}
\label{sec:ch26_failures}

\subsection{The Yips: Describe the Phenotype Before the Mechanism}
...
\begin{principle}{Evidence Does Not Support a Single Corrupted Program}{box:ch26_yips_theories}
Adler and colleagues studied 50 golfers using movement recordings and surface EMG. Findings depended on whether grouping used self-report or video-defined involuntary movement; the latter groups differed in pronation-supination movement, while the co-contraction comparison was a statistical trend \citep{Adler2011Yips}. This supports careful phenotype measurement, not a universal diagnosis or a demonstrated corrupted cerebellar trajectory.
\end{principle}
...
\begin{example}{A Miss Is Not Its Own Explanation}{ch26_choking_golf}
...
\begin{principle}{Test an Intervention Against the Intended Outcome}{box:ch26_avoid_choking}
...
\section{What the Combined Account Establishes}
\label{sec:ch26_conclusion}

\begin{principle}{The Computation Is Distributed Across Time and Mechanisms}{box:ch26_extraordinary}
...
\label{ex:ch26_exercises}
\label{exercises:ex:ch26_exercises}

\begin{enumerate}[leftmargin=*,itemsep=8pt]
\item Compute the schedule count for 200 channels, seven levels and 30 intervals. Repeat for five channels. Explain why neither result is a dimension or a count of successful swings.
\item For $\bm a=\bm W\bm c$, derive the coefficient constraints induced by $0\leq\bm a\leq1$. Use $\tau=a_1-a_2$ to explain why the basis $(1,1)^\top$ loses torque authority despite reducing the command dimension.
\item Derive the constant-input endpoint sensitivity for $\dot x=c+ax+u$. Why does a large nominal drift not prove a small endpoint response? Identify the additional quantities needed for a golf delivery calculation.
\item Integrate the delayed actuator/rotor model. Reproduce the four perturbation-time examples and derive the $h^3/(6T_a)$ short-time limit. State precisely when this model predicts zero response before impact.
\item Derive the spring-damper storage balance for fixed and moving references. Calculate the energy added when stiffness increases from 10 to 30N\,m/rad at a clamped 0.1rad error.
\item Explain how task error can vanish while sensory prediction error remains. Propose measurements that distinguish explicit aiming, implicit adaptation and a change in mechanical impedance after a club perturbation.
\item Compare model-based rollout, a cached policy and a learned value function. Which observations would be needed before attributing one of these computational descriptions to a neural implementation?
\item Propose a hierarchy for a humanoid golf controller. Describe information flowing upward as well as downward, and specify how the controller handles an infeasible delivery request.
\item A delayed feedback model produces jerky putting. Give two competing explanations and design a measurement that could distinguish them. Explain why simulation alone does not diagnose the yips.
\item Contrast explicit-monitoring and distraction accounts of performance under pressure. State a result that would count against each account in a specified task.
\item Design an attention experiment with novice and experienced putters. Include randomization, a pressure manipulation, a manipulation check, a performance outcome, and retention or transfer testing.
\item Derive the magnitude and phase of the delay-plus-activation transfer function at 1, 3 and 5Hz. Explain why neither neural firing rate nor shaft resonance alone specifies closed-loop bandwidth or stability.
\item Specify a human/robot golf benchmark with at least five matched conditions. Include prior experience, sensing, actuation, delivery uncertainty and energy boundaries. Distinguish processor energy from actuator work.
\item Design one experiment that separates prepared-state sensitivity, immediate mechanical response, delayed feedback and adaptation across trials. State which parts require additional measurements and where the inference could remain ambiguous.
\end{enumerate}

\subsection{Worked Checks and Answer Guidance}
