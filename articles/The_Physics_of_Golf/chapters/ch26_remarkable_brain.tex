\chapter{Motor Control III: Biological Constraints and Human Performance}
\label{ch:remarkable_brain}

\begin{laymansbox}{What This Chapter Is About}{box:ch26_intro}
The previous two chapters described how the nervous system controls the golf swing and how it learns. The present chapter takes a step back and asks about the scale of the problem the brain is solving --- not to celebrate the brain, but to understand which constraints matter for an engineering model of human swing control.

When a golfer swings a club, the central nervous system is simultaneously responsible for:
\begin{itemize}
\item coordinating on the order of a few hundred muscles acting across roughly twenty skeletal joints;
\item issuing motor commands through a corticospinal pathway whose transmission delay is a few tens of milliseconds;
\item waiting for proprioceptive feedback on a similar timescale and visual feedback on a somewhat longer one;
\item commanding actuators (skeletal muscle) whose force response rises over a similar several-tens-of-milliseconds envelope;
\item doing this in a downswing whose entire duration is only a few hundred milliseconds.
\end{itemize}
Each of these timescales is drawn from the neurophysiology and motor-control literature \cite{Kandel2013, shadmehr2005computational, shadmehr2010computational}.

No currently deployed robotic platform reproduces the combination of speed, precision, adaptability, and energy efficiency that a skilled human achieves in this task; whether any future one will is a research question, not a closed matter. Here we ask the narrower engineering question: given the timing and signal constraints listed above, what class of controller is compatible with observed human performance? We also ask the inverse question --- why does this controller occasionally fail in dramatic ways (the yips, choking under pressure) --- because those failures tell us as much about the control architecture as the successes do.
\end{laymansbox}
...
\item \textbf{Time points during the swing:} At a coarse temporal resolution (e.g., 10 ms per time step, rough order of magnitude for neural control), a 300 ms downswing has roughly 30 time points. At each time point, you could independently command each muscle.
...
\item Given the 300~ms downswing duration and $\sim$100~ms sensorimotor loop delay \cite{scott2004optimal}, the brain can process at most 1--2 feedback correction cycles during the swing. This is barely enough for any feedback-based control.
...
\item These pre-activations happen well before the event, anticipating it using the forward model.
...
\textbf{Experiment 2: Perturbed feedback.} Researchers study learning in a 2D reaching task where the visual feedback is systematically rotated \citep{wolpert2011motor}. For example, when you try to move your hand right, the feedback shows it moving at a rotated angle.
...
\section{What Makes Biological Motor Learning Distinctive}
\label{sec:ch26_brain_vs_ai}

\index{biological motor learning}
\index{cerebellum}
\index{internal models}
\index{motor control}

The brain's approach to learning the golf swing differs in several concrete ways from classical engineered control systems. The differences are worth articulating because they suggest which engineering ideas are likely to transfer to the biological case and which are not.

\subsection{Distinctive Features of Biological Motor Learning}
\label{subsec:ch26_brain_vs_ai_learning}

\begin{principle}{Biological vs. Engineered Learning Mechanisms}{box:ch26_brain_ai_learning}
...
The brain is \textit{sample-efficient}. It learns from roughly 10,000 repetitions. Engineered learning algorithms often require $10^6$ to $10^8$ simulated episodes. Why?
...
\item \textbf{Built-in structure:} The brain's neural circuits are not random. The cerebellum has a stereotyped architecture well suited to learning dynamics. This prior structure accelerates learning.

\item \textbf{Transfer learning:} The brain leverages prior knowledge from other movements. A tennis player learning golf already has some internal models for rotational motion.
...
\subsection{Strengths of Biological Motor Learning}
\label{subsec:ch26_brain_better}

\begin{principle}{Dimensions Where Biological Learning Excels}{box:ch26_brain_excel}
\begin{itemize}
\item \textbf{Transfer learning:} Humans transfer skills across very different contexts (tennis to golf, squash to badminton).

\item \textbf{Robustness:} Humans handle perturbations gracefully. If the wind blows during the swing, the golfer adjusts. If fatigued, the golfer adapts.

\item \textbf{Compositionality:} Humans combine learned skills in novel ways. A golfer who has played tennis and baseball might discover a unique swing combining elements of both.

\item \textbf{One-shot learning:} Humans can sometimes learn from a single demonstration.

\item \textbf{Physical understanding:} Humans develop intuitive physics through play and exploration. A heavier club naturally prompts a slower swing.
\end{itemize}
\end{principle}

\subsection{Constraints of Biological Motor Learning}
\label{subsec:ch26_ai_better}

\begin{principle}{Dimensions Where Biological Learning is Constrained}{box:ch26_ai_excel}
\begin{itemize}
\item \textbf{Consistency:} Biological motor control is subject to noise and variability. The brain sometimes produces the yips.

\item \textbf{Speed of deliberation:} The brain processes at roughly 10--40 Hz for conscious motor planning, limiting the rate of explicit search.

\item \textbf{Global optimization:} The brain typically settles into local optima that are good enough rather than performing exhaustive search.

\item \textbf{Scalability of practice:} The brain's learning is limited by the physical time humans can spend practicing.

\item \textbf{Precision:} Biological motor commands have irreducible noise and delays.
\end{itemize}
\end{principle}

\cite{lake2017building}

\section{Principles of Biological Motor Intelligence}
\label{sec:ch26_ai_from_brain}

\index{embodied cognition}
\index{motor intelligence}
\index{biological control}
...
\subsection{Internal Forward Models}
...
\begin{example}{The Forward Model in the Golf Swing}{ex:ch26_forward_model_golf}
...
The brain learns from \textit{prediction errors}. At every moment during the swing, the brain predicts what proprioceptive signals it expects, compares to actual signals, and learns from the mismatch. The agent generates its own learning signal by predicting its own sensory feedback---no external teacher is required.

\begin{example}{The Novice Golfer's First Practice Session}{ex:ch26_prediction_error_golf}
...
\item \textbf{How do experts avoid choking under pressure?} Under pressure, performance sometimes degrades (choking). But expert golfers are more resistant to choking. Why? Is it deeper motor memory? Better emotion regulation? Automatic execution that bypasses conscious interference?
...
\section{Why the Brain Sometimes Fails: The Remarkable Failures}
...
\begin{principle}{The Extraordinary Computation}{box:ch26_extraordinary}
\begin{itemize}
...
\item \textbf{Maintains consistency: elite golfers produce clubhead speeds within 1--3\% of their average, swing after swing.}
...
\item The brain is sample-efficient, building accurate models from 10,000--100,000 repetitions, while engineered learning methods often require millions. The brain achieves this through built-in structure, hierarchical control, and rich sensory feedback.

\item Biological motor control embodies several general principles of intelligent control: hierarchical decomposition, model-based planning, embodied exploitation of physics, and prediction-driven learning.
...
\item Compare model-based and model-free learning. Which approach does the brain use, and what is the evidence?

\item Describe three principles of biological motor control that illuminate how complex movement is organized. Explain each with reference to the golf swing.
