...
Recall from Chapter 25 that the Hill muscle model \citep{Hill1938} consists of an active contractile element (CE), a parallel elastic element (PE), and a series elastic element (SE). Crucially, the Hill model also includes a \emph{dashpot} — a viscous element — in parallel with the contractile element. This dashpot represents the internal friction of muscle fiber sliding.
...
Recall from Chapter 12 that the unforced kinematic chain is governed by:
\[
...
This connects to impedance control \citep{hogan1985impedance} from Chapter 27. Recall that you can write:
\[
...
\begin{exercises}{Chapter 29 Exercises}{ex:ch29}
