...
During the final 100 ms of the downswing, club speed is changing at a rate of $\sim 50$ m/s$^2$ (local acceleration) \citep{Nesbit2005}. At these speeds, a damping torque of even 1 Nm becomes significant.
...
In the model developed in this chapter, this derivative is approximately $-0.02$ m/s per (Nm$\cdot$s/rad) of damping. It is a model output, not an empirical measurement: it follows from the damping coefficients and segment inertias assumed earlier, and a different damping model would give a different slope. The consequences below inherit that status. Thus:
\begin{itemize}
