...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw[thick] (1.5, -0.2) -- (1.6, 0.2) -- (1.7, -0.2) -- (1.8, 0.2) -- (1.9, -0.2) -- (2.0, 0);
  \node at (1.75, 0.6) {$k$ (stiffness)};

  % Dashpot (damper) element
  \draw[thick, rounded corners=0.1] (2.2, -0.2) rectangle (2.8, 0.2);
  \draw[thick] (2.5, -0.15) -- (2.5, 0.15);
  \node at (2.5, 0.6) {$c$ (damping)};

  % Connection
  \draw[thick] (2.8, 0) -- (3.2, 0);

  % Mass element
  \draw[thick, fill=lightgray] (3.2, -0.35) rectangle (4.0, 0.35);
  \node at (3.6, 0) {$m$};

  % Velocity vector
  \draw[thick, ->, color=blue] (4.0, 0) -- (5.0, 0);
  \node[color=blue] at (4.5, 0.4) {$\dot{x}$};

  % Below: forces
  \node at (1.0, -1.0) {Spring force: $F_s = -kx$};
  \node at (2.5, -1.0) {Damping force: $F_d = -c\dot{x}$};
  \node at (1.75, -1.5) {Equation: $m\ddot{x} + c\dot{x} + kx = 0$};
\end{tikzpicture}
\caption{Linear spring-dashpot (mass-spring-damper) model of a joint with stiffness $k$, damping coefficient $c$, and inertial mass $m$. The damping force opposes velocity; the spring force opposes displacement.}
...
% TODO(citation): "shoulder rotates at ~7000°/s and wrist at ~3000°/s" are approximate values from biomechanical studies of the golf swing; should cite motion capture data (e.g., Nesbit 2005) to support these peak angular velocity estimates.
Why? Because the proximal joint has higher angular velocity (relative to body-fixed rates) and moves a larger inertia. The power dissipated by damping is $P_d = c \omega^2$. In a maximally extended arm, the shoulder rotates at $\sim 7000^\circ$/s (122 rad/s), whereas the wrist rotates at $\sim 3000^\circ$/s relative to the body. Even if both have the same damping coefficient $c$, the shoulder dissipates $\sim 17 \times$ more power.
...
\subsection{Grip Force and Hand-Club Interface Damping}
...
The relationship can be characterized as: higher grip force \(\to\) higher interface damping \(\to\) reduced shaft oscillation amplitude but increased energy dissipation at the grip. Conversely, lower grip force \(\to\) lower interface damping \(\to\) greater shaft oscillation potential but reduced energy loss at the grip.
...
\begin{tabular}{lrr}
\hline
\textbf{Dissipation Source} & \textbf{Energy (J)} & \textbf{\% of Total} \\
\hline
...
\hline
\textbf{Total dissipation} & \textbf{145--245} & \textbf{48--65\%} \\
\hline
\end{tabular}
\end{center}

\textbf{Important:} These values are \textbf{illustrative estimates} constructed from published biomechanical energy analyses \citep{Nesbit2005} and general musculoskeletal dissipation data, not from direct measurement of all dissipation channels in a single study. The exact breakdown varies widely with individual swing mechanics, body composition, and equipment. The table is intended to convey relative magnitudes and should not be cited as definitive measured values.
...
Key observations:
\begin{itemize}
\item Joint damping accounts for 13--15\% loss. This is significant and not negligible. A $\pm 50\%$ change in damping would shift distance by roughly 6--8\%, all else being equal.

\item Tendon hysteresis is one of the largest dissipative sources. Yet tendons also store elastic energy. The net effect of tendon mechanics is beneficial: they act as elastic accumulators, releasing energy at optimal moments.

\item Body segment soft tissue wobble dissipates 5--8\%. This is the "shake" of muscle, fat, and skin. Stronger individuals with less soft tissue wobble (i.e., low body fat) dissipate less here.
...
% TODO(citation): "club speed changing at ~50 m/s² during the final 100 ms" is consistent with motion capture studies but should be sourced (e.g., Nesbit 2005 for clubhead acceleration profiles).
During the final 100 ms of the downswing, club speed is changing at a rate of $\sim 50$ m/s$^2$ (local acceleration). At these speeds, a damping torque of even 1 Nm becomes significant.
...
% TODO(citation): "this derivative is approximately $-0.02$ m/s per (Nm·s/rad) of damping" is an estimate from sensitivity analysis; the precise value depends on swing model parameters and should be validated against simulation or experimental data.
Empirically, this derivative is approximately $-0.02$ m/s per (Nm$\cdot$s/rad) of damping. Thus:
\begin{itemize}
\item $50\%$ increase in damping ($+5$ Nm$\cdot$s/rad): $\Delta v \approx -0.1$ m/s, or $\sim 2$ mph loss.
\item $50\%$ decrease in damping ($-5$ Nm$\cdot$s/rad): $\Delta v \approx +0.1$ m/s, or $\sim 2$ mph gain.
\end{itemize}
...
\subsection{Material Damping and Shaft Behavior}
