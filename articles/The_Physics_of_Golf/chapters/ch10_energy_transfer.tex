\chapter[Energy Transfer: How Power Flows Through the Kinetic Chain]{Energy Transfer: How Power Flows\\Through the Kinetic Chain}
...
\section{Energy Fundamentals: Define the System}
...
\subsection{What Is Stored, and What Is Merely a Configuration?}
...
\section{Power and the Whole-System Work Balance}
...
are also developed in Tedrake's multibody notes \citep{TedrakeMultibodyEnergy}.
...
$E(t)-E(t_0)=W_{\mathrm{in}}-\int D\,dt$.
...
\subsection{Signed Work, Positive Work and Metabolic Cost}

For separately modeled actuator powers $P_a$, distinguish
$W_{\mathrm{net}}=\sum_a\int P_a\,dt$ from
$W_+=\sum_a\int\max(P_a,0)\,dt$. The latter is positive work; it does not cancel
...
\section{Constraint Power: Zero for Which Boundary?}
\label{sec:constraint_power_zero}

An ideal reaction to a position constraint $\Phi(q,t)=0$ has generalized force
$Q_c=J^T\lambda$, where $J=\partial\Phi/\partial q$. Its actual power is
\begin{equation}
...
\subsection{Why Energy Can Cross an Ideal Joint}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/energy_interface_ledger.pdf}
\caption{Both Sides of the Internal Boundary Must Balance. Hinge-Force Power Cancels Between Bodies, While Motor Power Depends on Their Relative Angular Velocity.}
...
\subsection{Reporting Point, Observer and Ground Contact}

A wrench's power is $\mathbf F\cdot\mathbf v_O+\mathbf n_O\cdot\boldsymbol\omega$
...
\section[Energy Partition During a Declared Passive Motion]{Energy Partition During a\\Declared Passive Motion}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/energy_passive_transfer.pdf}
\caption{An Explicit Passive Mechanism Redistributes Energy in Both Directions. The Outer Rod Gains Energy, Loses Energy and Later Gains It Again; Total Energy Remains Constant.}
\label{fig:energy_passive_transfer}
\end{figure}
...
\section{Sequential Speed Peaks and Their Interpretation}
...
\section[Storage, Active Work and a Balanced Swing Ledger]{Storage, Active Work and a\\Balanced Swing Ledger}
...
U_e &: (0,\ 10,\ 2,\ 0,\ 0).
\end{aligned}
\label{eq:energy_storage_ledger}
\end{equation}
...
\subsection{Impact Is a New Boundary and Time Interval}
...
\label{eq:energy_collision_speeds}
\end{equation}
...
With reduced mass $\mu=m_hm_b/(m_h+m_b)$, the loss is
$\mu(1-e^2)u_h^2/2$. The ratio of ball energy to pre-impact head energy is
$m_hm_b(1+e)^2/(m_h+m_b)^2$, approximately 49.21 percent here. This is an impact
...
\section{Lag, Elasticity and Release Timing}
...
$\partial U_e/\partial t=\dot k(q_2-q_{20})^2/2-k(q_2-q_{20})\dot q_{20}$.
...
\subsection{A Fair Release-Timing Comparison}
...
\section{Two-Link Dynamics and Physical Energy Transfer}
...
$q_1=\theta_1$ and $q_2=\theta_2-\theta_1$, where the absolute angles $\theta_i$
are measured from downward. Let $u=\dot q_1$, $w=\dot q_2$ and
$a=I_1+m_1c_1^2+m_2L_1^2$, $b=I_2+m_2c_2^2$, $d=m_2L_1c_2$.
...
$(I_1+m_1c_1^2)u^2/2$; the rest belongs to the translating and rotating outer
...
$\tau=(\tau_1,\tau_2)^T$ give $M\ddot q+h+G=\tau$, where
\begin{equation}
M=\begin{bmatrix}
...
\label{eq:energy_two_link_dynamics}
\end{equation}
...
m_2c_2g\sin(q_1+q_2)
\end{bmatrix}.
\label{eq:energy_two_link_gravity}
\end{equation}
...
The shared \texttt{rod\_energy\_ledger} routine computes center energies and
their derivatives from \texttt{RodPair} dynamics. Its state uses absolute
...
\section[Evidence, Measurement and an Investigable Golf-Swing Argument]{Evidence, Measurement and an\\Investigable Golf-Swing Argument}
...
\citep{Nesbit2005b}. It supports examining coordinated body and club work within
...
Use the inverse-dynamics and constraint-force chapters (Chapters~\ref{ch:inverse_dynamics_parallel} and~\ref{ch:constraint_forces}) to construct physically
compatible forces. Close the whole-system ledger and both sides of each
important interface before interpreting a sequence of peaks. The complete-swing
and synthesis chapters (Chapters~\ref{ch:complete_swing} and~\ref{ch:interdisciplinary}) connect these local mechanisms to finite-time delivery
...
\label{eq:energy_power_uncertainty}
\end{equation}
...
\section{Chapter Exercises: Energy Transfer}

\begin{enumerate}
\item Derive the mechanical-energy balance from the manipulator equation.
...
\item At an arm--club hinge, force power into the club is 12 W. The base motor
...
\item Check the five-state storage ledger between the second and third states.
...
\item For the stated isolated impact, derive the speeds, energy-transfer ratio
...
\item Derive $M$, $h$ and $G$ for the two-link relative chart. For the aligned
...
losing energy.
\item Design a comparison of two release strategies that could distinguish
...
\begin{enumerate}
\item Differentiate $K=\dot q^TM\dot q/2$, substitute the equation of motion,
and add $\dot V=\dot q^T\nabla V$. The terms
$\dot q^T\dot M\dot q/2$ and $\dot q^TC\dot q$ cancel by the compatible
...
\item Club moment power is $3(5)=15$ W, so its energy rate is $12+15=27$ W.
...
\item Stored energy rises from 25 to 87 J, a 62 J increase. Active work over
...
zero elastic energy.
\item Solve $m_hu_h=m_hv_h+m_bv_b$ and $v_b-v_h=e u_h$. The stated speeds
...
\item Differentiate the kinetic-energy quadratic twice with respect to rates
...
$\left[\begin{smallmatrix}4/3&1/2\\1/2&1/3\end{smallmatrix}\right]$,
with zero instantaneous velocity bias and gravity. Solving against $(-1,0)^T$
gives $(-12/7,18/7)^T$. Inner energy rate is
$(1/3)(2)(-12/7)=-8/7$ W; total motor power is $-2$ W, so outer energy rate
...
\item Use identical positions, velocities and internal elastic states, with
