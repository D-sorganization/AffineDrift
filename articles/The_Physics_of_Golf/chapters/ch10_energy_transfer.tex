...
\item $P_{\text{Coriolis}} = -\bm{C}^T \dot{\bm{q}}$: power from Coriolis/centrifugal effects.
...
Track the qualitative evolution of kinetic energy across five phases of the downswing for a triple pendulum (torso, arm, club). The specific numbers below are computed from our triple pendulum model with the illustrative parameters from Chapter~\ref{ch:triple_pendulum}—they are order-of-magnitude illustrations, not measurements.

\begin{enumerate}
\item \textbf{Phase 1: Top of backswing.} The swing has just reversed. Angular velocities are near zero; kinetic energy is minimal. Potential energy (elevated arms, stretched muscles) is at its maximum.

\textbf{Energy partition: nearly 0\% kinetic, essentially all potential.}

\item \textbf{Phase 2: Early downswing.} Hip acceleration has begun. The torso is rotating fast; the arms lag behind. Most kinetic energy is in torso rotation; the arm and club carry little.

\textbf{Energy partition: torso-dominated; club fraction is negligible.}

\item \textbf{Phase 3: Mid-downswing.} The torso approaches peak speed; the arm begins to accelerate. Potential energy has nearly all converted to kinetic. Torso still holds the majority, but the arm is catching up.

\textbf{Energy partition: still torso-dominated, but arm fraction is rising.}

\item \textbf{Phase 4: Late downswing.} The torso reaches maximum speed and begins to decelerate; the arm and club are accelerating rapidly via constraint forces and Coriolis. The club fraction is growing quickly.

\textbf{Energy partition: torso still largest share, but club fraction is now significant.}

\item \textbf{Phase 5: Impact.} The torso and arm are both decelerating; the club is at maximum speed. The club now carries the largest share of kinetic energy.

\textbf{Energy partition: club carries the majority of remaining kinetic energy.}
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Gravitational potential energy:} Raising the arms stores energy $mgh$. At the top of the backswing, the elevated arms carry meaningful gravitational potential energy relative to impact position.

\item \textbf{Elastic potential energy in muscles and tendons:} The coiled position of the backswing stretches muscles and stores elastic energy. The magnitude depends on individual flexibility and muscular tension.

\item \textbf{Rotational kinetic energy:} As the hips, shoulders, and arms accelerate during the downswing, they store kinetic energy. This is the primary energy source during the downswing.
...
\begin{enumerate}
\item \textbf{Energy input:} Muscles do work during the backswing (lifting arms, creating tension) and early downswing (accelerating hips). The two sources are muscular work and gravitational potential energy recovered as the arms fall.

\item \textbf{Energy peak:} At late downswing, kinetic energy is at its maximum. This is the sum of muscular work plus gravitational energy from the falling arms.

\item \textbf{Constraint force redistribution:} The peak kinetic energy is redistributed from hips and torso (which decelerate) to the arms and club (which accelerate) via constraint forces. No external energy is added; this is pure redistribution.

\item \textbf{Energy to the ball:} At impact, the club transfers a fraction of its kinetic energy to the ball. The rest is dissipated as sound, heat, and club deformation.

\item \textbf{Efficiency:} A meaningful fraction of muscular input energy reaches the ball, with losses to air resistance, joint friction, and incomplete energy capture at impact.
\end{enumerate}
\end{example}
...
\begin{itemize}
\item The arm is rotating at a moderate speed when the wrist releases.
\item The club, now free, inherits a moderate arm velocity.
\item After release, Coriolis accelerates the club, but the arm is already slowing. The final club speed is limited.
\end{itemize}

Scenario B: Late release (wrist released late in the downswing)
\begin{itemize}
\item The arm is rotating much faster when the wrist releases.
\item The club inherits a much higher arm velocity.
\item After release, Coriolis accelerates the club further to a substantially higher final speed.
\end{itemize}

\textbf{Comparison:} Late release results in significantly more club kinetic energy, even though no additional muscular effort was applied. The difference is purely due to \emph{timing} of the constraint release.
...
\item \textbf{Energy partition:} Energy starts concentrated in the torso (hips and shoulders) and gradually flows to the arm and club. By impact, the club carries the majority of kinetic energy.
...
\item \textbf{Lag timing is critical:} Releasing the wrist at the right moment substantially changes the final club speed even with identical muscular effort. Timing determines when the club inherits the arm's energy.

\item \textbf{Efficiency:} A meaningful fraction of muscular input energy reaches the ball as kinetic energy. Losses come from air resistance, inefficient constraint transfers, and incomplete energy capture at impact.

\item \textbf{The role of muscles:} Muscles provide the initial energy input (early downswing) and set up the constraint configuration (lag, X-factor). But the final club speed emerges primarily from passive constraint-based energy redistribution and Coriolis effects.
\end{enumerate}
\end{principle}
