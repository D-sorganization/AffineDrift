...
\begin{definition}{Kinetic and Potential Energy in a Multi-Segment System}{energy_components}
...
\item $P_{\text{input}} = \bm{u}^T \dot{\bm{q}}$: power from muscular torques. This term can be positive or negative; eccentric muscle action and braking phases absorb mechanical energy rather than add it.

\item $P_{\text{constraint}} = \bm{\lambda}^T \bm{J}_c \dot{\bm{q}} = 0$: power from ideal bilateral constraint forces in the declared generalized-coordinate model. This statement assumes the constraint is holonomic, time-independent, and lossless; compliant grips, slip, impact, and damping belong in separate force or loss terms.
\end{itemize}
\end{theorem}
...
\begin{example}{Energy Partition at Five Downswing Phases}{energy_partition_five_phases}
...
\textbf{Energy partition:} Most energy is in torso rotation. 	te{Nesbit2005, MacKenzie2009}
...
\begin{tikzpicture}[scale=1.0, every node/.style={align=center}]
  ...
  \draw[fill=green!50, draw=green!50!black, thick] (9, 0.5) rectangle (10.5, 2.5) node[midway] {\large Club\\at impact};
...
\begin{example}{Numerical Summation of Speed}{summation_of_speed_numbers}
...
\begin{definition}{Energy Storage Mechanisms}{energy_storage}
...
\begin{example}{Energy Budget for a Typical Swing}{energy_budget}
...
\item \textbf{Energy peak:} At late downswing, kinetic energy reaches $\sim 200$ J. This is the sum of muscular work ($\approx 100$ J) plus energy from gravity as the arms fall through the downswing ($\sim 100$ J).
...
\item \textbf{Energy to the ball:} At impact the clubhead carries on the order of $200$ J. A regulation $45.93$-gram ball ($0.04593$ kg) initially at rest leaves at $\sim 150$ mph ($\approx 67$ m/s), so it gains $\tfrac{1}{2}(0.04593)(67)^2 \approx 100$ J of kinetic energy. The clubhead does \emph{not} stop --- it retains roughly half its speed --- so the remaining energy stays as residual clubhead kinetic energy, with only a small fraction lost to deformation and heat.

\item \textbf{Efficiency:} Of the clubhead's pre-impact kinetic energy, the ball carries away roughly 50\% (ball KE divided by clubhead KE). The rest remains as residual clubhead KE, plus small losses to:
\begin{itemize}
...
\begin{example}{Lag Release Timing and Energy Transfer}{lag_energy_transfer}
...
This is why timing is everything in golf: the total energy input is roughly constant (same muscular effort), but the final club speed depends critically on when the wrist releases. An 80 ms later release can double the club speed due to the energy state of the arm at the moment of release.
\end{example}
...
\begin{example}{Energy Partition in the Double Pendulum}{double_pend_energy_complete}
...
\item \textbf{Constraint forces redistribute energy in the ideal model:} Ideal bilateral constraints do zero net work on the total constrained system, but they redistribute energy between segments. Real grips, joints, damping, and impact can add losses or external work that must be modeled separately.
