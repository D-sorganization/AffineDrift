\chapter[Passive and Distributed Control]{Passive and Distributed Control:\\Mechanics, Feedback and Stability}
...
\section{Time Scales and the Distribution of Control}
...
\begin{principle}{Separate Preparation From Execution}{lab:myth_central}
...
In a primary study of humans and rhesus monkeys, Pruszynski and colleagues found evidence for multi-joint feedback processing through primary motor cortex at roughly 50 ms after a mechanical perturbation \citep{Pruszynski2011Feedback}. This establishes that fast feedback need not be exclusively spinal. It does not identify the controller of a golf swing or imply that a response triggered by ball contact can alter the preceding impact state.

\section{Passive Mechanical Systems: Springs and Dampers}
\label{sec:passive_mechanics}

\begin{definition}{Mechanical Passivity}{lab:def_passive_control}
...
\begin{laymansbox}{What Mechanical Preparation Can Do}{lab:hardware_stability}
...
\section{Impedance as Part of Motion Control}
\label{sec:impedance_control}

Hogan's impedance-control framework treats motion and mechanical interaction together. His 1985 manipulation paper explicitly includes both physical hardware and a real-time controller; it does not establish that the cortex controls impedance exclusively \citep{hogan1985impedance}. Earlier work examined coactivation of antagonist muscles as a way of modulating mechanical impedance \citep{Hogan:1984}.

\begin{definition}{Specify the Mechanical Port}{lab:def_impedance}
...
\begin{principle}{A Testable Impedance Hypothesis}{lab:princ_impedance}
...
\section{Preparation, Reference Motion and Power}
\label{sec:pretuned}
...
\begin{example}{What Parameter Counts Establish}{lab:constraint_pretuning}
...
\section{Passive Forces in the Forward Counterfactual}
...
\begin{definition}{Declare What Is Held and What Is Zeroed}{lab:drift_impedance}
...
\section{Muscle Tone and Distributed Feedback}
...
An instructed-delay human experiment reported goal-dependent changes in spindle afferent activity and stretch responses during preparation \citep{Papaioannou2021Preparation}. This supports preparatory sensory tuning. It does not directly measure gamma motor-neuron firing or establish a fixed gamma schedule for golf. Avoid a one-to-one rule equating greater gamma drive with a known joint stiffness.

\begin{example}{A Restoring Sign Does Not Guarantee Passivity}{lab:spinal_controller}
...
\section{Coordination in the Coupled Chain}
\label{sec:self_organization}

\subsection{Interpreting Proximal and Distal Energy Flow}
...
\subsection{The Whip Effect and Distal Acceleration}
...
\begin{principle}{Test the Mechanism Behind the Sequence}{lab:princ_emergent}
...
\subsection{Impedance and Impact Transmission}
...
\section{Stability Analysis: What the Model Establishes}
\label{sec:stability}

\subsection{Equilibrium Storage and Lyapunov Stability}
...
\]
The kinetic derivative's $\dot M$ term cancels the corresponding Coriolis contribution; it must not simply be discarded as a small force. With no external torque and positive-definite damping, $\dot V\leq0$. This is \emph{negative semidefinite}, because it is zero whenever velocity is zero, including some displaced states.

\begin{principle}{A Bounded Equilibrium Result}{lab:thm_passive_stability}
...
\subsection{Tracking and Finite-Time Recovery}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/passive_damping_regimes.pdf}
\caption{Manufactured Recovery With Equal Inertia and Stiffness: Only the Underdamped Case Spirals. All Curves Start at 0.1 rad With Zero Velocity; Orange Arrows Indicate Time Direction Over a Two-Second Interval.}
\label{fig:passive-damping-regimes}
\end{figure}
...
\begin{laymansbox}{What Apparent Ease Can Tell Us}{lab:effortless_swing}
...
\section{Learning and Identifying Impedance}
\label{sec:training}
...
\begin{principle}{Mechanical Conditions and Learning}{lab:principle_conditions}
...
\section{Practical Interpretation of Measurements}
\label{sec:practical}

\subsection{Grip Pressure and Joint Response}
...
\subsection{Posture and the Mechanical Map}
...
\subsection{What a Waggle Can Reveal}
\label{subsec:waggle}
...
\subsection{Combining Sensation and External Feedback}
...
\section{A Connected Model of Mechanics and Control}
\label{sec:unified}
...
\begin{exercises}{Chapter Exercises and Worked Answers}{label:ex_ch27}

\textbf{1. Phase-Dependent Hypotheses}
...
\textbf{Answer.} The spring and damper each contribute $-5$ N m, so total torque is $-10$ N m and power on the coordinate is $-10$ W. Spring storage increases at 5 W and the damper dissipates 5 W. When the same displaced spring moves toward its reference, spring power can be positive; viscous dissipation remains nonnegative. Resisting displacement and resisting velocity are different statements.

\textbf{7. Differentiate the Correct Storage}
...
\textbf{Answer.} For fixed reference, differentiation of $I\dot q^2/2+K(q-q_0)^2/2$ and substitution cancel the spring work, leaving $-D\dot q^2$. For varying reference and stiffness the derivative becomes
\[
...
\textbf{Answer.} The cases are underdamped, critically damped and overdamped. Only the first has complex poles and spiralling phase trajectories. The critical solution is $e(t)=0.1(1+10t)\exp(-10t)$, so $e(0.1)=0.2/\mathrm e\approx0.07358$. The overdamped system's slow pole is approximately $-2.0871$ s$^{-1}$, slower than the critical decay scale. Increasing damping indefinitely does not guarantee faster recovery.

\textbf{9. An Underdetermined Impact Estimate}
...
\textbf{Answer.} One cannot identify a safe range. The scale is 400 N m/rad under the usual dimensionless-radian convention; choosing $K=I/T^2$ merely imposes $\sqrt{K/I}\,T=1$. It supplies no loading, allowable excursion, damping, contact mechanics or tissue outcome. The interval is not automatically a forcing period, and resonance is not automatically instability. No injury or grip recommendation follows.

\textbf{10. A Waggle Association}
\label{ex:waggle_experiment}
...
\textbf{Answer.} It can describe a within-golfer association under the recorded conditions. Separate sessions used to formulate the hypothesis from those used to test it; record task, equipment and relevant setup changes. Fatigue, expectation and preparation may affect both sensation and outcome. No mechanical stiffness is identified without a known perturbation and force/motion response, and association alone does not show that changing the sensation causes improved delivery.

\textbf{11. What Identifies the Response?}
\label{ex:grip_pressure}
