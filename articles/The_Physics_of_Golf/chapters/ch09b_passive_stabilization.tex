...
\begin{introduction}
  ...
  The key insight from affine control theory is that \emph{pre-tensioning modifies the drift field $\drift$, not the control input $\inputmap$}. Thus, passive stabilization is about building a drift field that is already close to the desired trajectory. This is the foundation of Bosch's principle: \emph{Coordinate the assembly lines, not the muscles.}
\end{introduction}
...
\begin{equation}
  \bm{F}_c = \loopjac^T(\configvec) \boldsymbol{\lambda},
\end{equation}
where $\loopjac(\configvec) \in \Reals^{m \times n}$ is the loop Jacobian, $\boldsymbol{\lambda} \in \Reals^m$ is the vector of constraint multipliers, and $\bm{F}_c$ is the resulting generalized constraint force.

The equations of motion become:
\begin{equation}
  \massmat \configddot + \cormat \configdot + \gravvec = \boldsymbol{\tau} + \bm{F}_c.
...
\begin{principle}{Co-Contraction Modulates Impedance}{ch9b_cocontraction_impedance}
...
The mechanical response of muscle to a sudden length change occurs on a timescale of 10--50\,ms, which is faster than neural feedback can react (typical reflex latency is 50--100\,ms). This \emph{preflex} or zero-delay response arises from the passive elastic properties of muscle and tendon.
...
\begin{constraintbox}{Fascial Networks as Passive Stabilization}{ch9b_fascial_networks}
...
\begin{equation}
  \configdot = \driftfield(\configvec) + \sum_{i=1}^{m} u_i \inputfield_i(\configvec),
...
When we pre-tension the system (increase baseline muscle activation or adjust the reference length of antagonist muscles), we change $\driftfield(\configvec)$. Specifically:
\begin{equation}
  \driftfield(\configvec) = \driftfield_0(\configvec) + \Delta \driftfield_{\text{pretension}}(\configvec),
\end{equation}
where $\Delta \driftfield_{\text{pretension}}$ represents the change in the natural trajectory caused by the pre-tension. If the pre-tension is well chosen, the new drift field is already oriented toward the desired motion, reducing the need for control inputs.

\begin{driftcontrol}{Pre-Tensioning Optimizes the Drift Field}{ch9b_pretensioning}
...
\begin{theorem}{Attractor-Driven Stabilization}{ch9b_attractor_stabilization}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=1.0]
    ...
    % Stiffness curve: high at address, decreasing through the swing as
    % pre-tension transfers to elastic elements, rising again at impact --
    % matching the caption below. (Previously drawn as the opposite shape,
    % a mid-swing peak; the caption is the one consistent with the
    % chapter's own account of alpha*(t), see #3707.)
    \draw[thick, blue] (0, 4) -- (2.5, 3.5) -- (5, 2) -- (7.5, 1) -- (10, 4);
...
The uncontrolled manifold (UCM) hypothesis, developed by \cite{Scholz1999}, provides an empirical way to test the attractor-fluctuation framework. The hypothesis states that variability in movement is not uniformly distributed across all degrees of freedom. Instead, variance is \emph{concentrated in task-irrelevant directions} (the uncontrolled manifold) and minimized in task-relevant directions.
...
\begin{mythreality}{``The Body Doesn't Compute Trajectories---It Creates Energy Landscapes''}{ch9b_body_computes}
...
\begin{equation}
  \configdot = \driftfield(\configvec, \alpha) + \sum_{i=1}^{m} u_i(t) \inputfield_i(\configvec),
...
The drift field $\driftfield(\configvec, \alpha)$ includes the effects of gravity, Coriolis forces, centrifugal forces, and the passive properties of muscles and tendons:
\begin{equation}
  \driftfield(\configvec, \alpha) = -\massmat^{-1} \left[ \cormat \configdot + \gravvec - \boldsymbol{\tau}_{\text{passive}}(\configvec, \alpha) \right].
\end{equation}
...
\begin{equation}
  \text{ZTCF: } \quad \configdot = \driftfield(\configvec, \alpha) \quad (\text{with } u_i = 0).
\end{equation}
...
Recall from Chapter~\ref{ch:zero_torque_counterfactual}, Definition~\ref{def:dcr}, the
Drift--Control Ratio (DCR):
\begin{equation}
  \text{DCR} = \frac{\|\driftfield(\configvec)\|}{\left\|\sum_i u_{\max,i}\,\inputfield_i(\configvec)\right\|}.
  \label{eq:ch9b:dcr}
\end{equation}
...
$\|\drift\|^2 / (\|\drift\|^2 + \sum_i \|u_i \inputfield_i\|^2)$, which is bounded in
$[0,1]$ by construction. That form is a perfectly reasonable quantity --- it is the fraction
of squared dynamics attributable to drift --- but it is \emph{not} the DCR, and mixing the
...
See \texttt{NOTATION.md}, which reserves the bare acronym DCR site-wide for the ratio.
\end{warningbox}
...
\begin{driftcontrol}{Passive Stabilization Through Favorable Drift}{ch9b_favorable_drift}
...
\begin{constraintbox}{The Optimal Stiffness is Task and Context Dependent}{ch9b_optimal_stiffness}
...
\begin{algorithm}{Computing Optimal Joint Stiffness Distribution}{stiffness_optimization}
  \textbf{Input:}
  \begin{itemize}
    \item Desired trajectory $\configvec_d(t)$ for a golf swing phase.
    \item Model of joint dynamics, including mass matrix $\massmat$, Coriolis matrix $\cormat$, and gravity $\gravvec$.
  ...
  \begin{enumerate}
    ...
    \begin{equation}
      \configdot = \driftfield(\configvec, \alpha(t)) + \inputfield(\configvec) u(t).
    \end{equation}
  ...
  \textbf{Notes:}
  \begin{itemize}
    \item The key is that increasing $\alpha$ changes the drift field $\driftfield(\configvec, \alpha)$. A favorable drift field reduces the need for control inputs, thus reducing $C_{\text{control}}$. However, the co-contraction itself has a cost $w(\alpha)$. The algorithm finds the balance.
...
\begin{equation}
  \configvec_{k+1} = \configvec_k + \Delta t \left[ \driftfield(\configvec_k, \alpha_k) + \inputfield(\configvec_k) u_k \right]
\end{equation}
...
\begin{exercises}{Passive Stabilization in Parallel Loops}{ch9b_passive_stabilization}

\medskip
\noindent\textbf{Exercise 1: Conceptual: Attractor Stability}\par
...
\medskip
\noindent\textbf{Exercise 2: Mathematical: Impedance Control}\par
...
\medskip
\noindent\textbf{Exercise 3: Conceptual: Drift Field and ZTCF}\par
...
\medskip
\noindent\textbf{Exercise 4: Computational: Optimize Stiffness During Downswing}\par
...
\medskip
\noindent\textbf{Exercise 5: Experimental Design: Measuring Stiffness}\par
...
\medskip
\noindent\textbf{Exercise 6: Theoretical: UCM Decomposition}\par
