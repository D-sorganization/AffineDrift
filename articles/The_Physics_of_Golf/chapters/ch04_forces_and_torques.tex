...
\node (inertial) [draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, fill=lightblue!30] at (0, 1.5) {\textbf{Inertial}\\$\bm{M}(\bm{q}) \ddot{\bm{q}}$};

\node (velocity) [draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, fill=lightgreen!30] at (0, 0.3) {\textbf{Velocity-Dependent}\\$\bm{C}(\bm{q}, \dot{\bm{q}}) \dot{\bm{q}}$};

\node (gravity) [draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, fill=lightyellow!30] at (0, -0.9) {\textbf{Gravitational}\\$\bm{g}(\bm{q})$};

\node (muscular) [draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, fill=lightcoral!30] at (0, -2.1) {\textbf{Muscular (Control)}\\$\bm{\tau}$};

\node (constraint) [draw, rectangle, minimum width=2.5cm, minimum height=0.8cm, fill=lightgray!30] at (0, -3.3) {\textbf{Constraint}\\(implicit in $\ddot{\bm{q}}$)};
...
\begin{itemize}
\item \textbf{Inertial term} $\bm{M} \ddot{\bm{q}}$: The mass matrix scaled by acceleration. At impact, the clubhead acceleration is $\approx 180 \text{ m/s}^2 = 18.3 g$. Given moment of inertia $M_{11} \approx 2.22 \text{ kg m}^2$, the inertial term is on the order of $\ddot{\theta}_1 \approx 40 \text{ rad/s}^2$.
...
\textbf{Numerical illustration:} If $\ddot{\theta}_1 = 40 \text{ rad/s}^2$, $M_{11} = 2.22 \text{ kg m}^2$, $\bm{C} \dot{\bm{q}} \approx 15 \text{ N m}$, and $\bm{g} \approx 5 \text{ N m}$, then:
\begin{equation}
\tau_1 = M_{11} \ddot{\theta}_1 + C \dot{\bm{q}} + g = 2.22 \times 40 + 15 + 5 = 88.8 + 15 + 5 \approx 109 \text{ N m}
\end{equation}
