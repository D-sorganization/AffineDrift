...
When the system moves fast, velocity-dependent forces emerge that are negligible at low speeds. These forces---Coriolis and centrifugal---arise from the geometry of rotational motion and play a dominant role in the golf swing.
...
Consider a driver swing where the club (mass $m = 0.3$~kg, effective length $r = 1.5$~m from shoulder) is moving at $\omega = 30$~rad/s (about 110~mph clubhead speed) just before impact.

The centrifugal force pulling the club outward is:
$$F_{\text{centrifugal}} = m \omega^2 r = 0.3 \times 30^2 \times 1.5 = 405 \text{ N} \approx 91 \text{ lbs}$$

That is 91 pounds of outward pull---and it is entirely a \textbf{drift} force. Your muscles did not create this force directly; it arose from the geometry and velocity of the motion. This force straightens the wrist cock, extends the arms, and loads the shaft. It is one of the largest forces in the entire golf swing, and it is free.

This is the centrifugal loading experienced by the club. The golfer feels this as a pull on the hands---specifically, it is the constraint force at the grip that must provide the centripetal acceleration to keep the club in its curved path. The 405~N is not a separate force acting on the golfer; it is the reaction to the constraint force the golfer's hands apply to the club.
\end{example}
...
\begin{align}
\tau_c &= I_{\text{forearm}} \cdot a_c = ?
\end{align}

Actually, this is getting complicated. The point is: the centrifugal force is trying to straighten your arm. This is why the club feels ``light'' during the downswing if you're rotating fast—centrifugal force is doing work for you.
...
\begin{laymansbox}{Why High Speeds Help}{box:ch04_speed_helps}
Here's a rough accounting of forces at different swing speeds:

\textbf{Address (no motion)}:
\begin{itemize}
\item Gravity: $\approx 10$ N m (pulling arm down)
\item Coriolis/Centrifugal: 0 (no velocity)
\item Muscular force needed: $\approx 10$ N m (just to hold the arm up)
\end{itemize}

\textbf{Top of backswing} (arm up, velocities near zero):
\begin{itemize}
\item Gravity: $\approx 30$ N m (pulling arm down strongly)
\item Coriolis/Centrifugal: 0 (velocities still small)
\item Muscular force needed: $\approx 30$ N m (to hold the arm in this extended position)
\end{itemize}

\textbf{Transition} ($\dot{\theta}_1$ ramping up):
\begin{itemize}
\item Gravity: $\approx 20$ N m (downward)
\item Coriolis/Centrifugal: $\approx 5$ N m (upward, helping)
\item Muscular force needed: $\approx 15$ N m net (gravity minus helpful velocity-dependent forces)
\end{itemize}

\textbf{Downswing} ($\dot{\theta}_1$ large, $\dot{\theta}_2$ ramping up):
\begin{itemize}
\item Gravity: $\approx 10$ N m (variable, becoming helpful)
\item Coriolis/Centrifugal: $\approx 10$ N m (strong, helping)
\item Muscular force needed: $\approx -5$ N m (gravity and velocity-dependent forces are doing most of the work!)
\end{itemize}

At high speeds, the muscular forces actually flip sign. Instead of pulling the arm, they're \emph{resisting} the arm's acceleration. The arm is accelerating so fast from gravity and velocity-dependent forces that muscles must apply a braking torque to slow it down to the right speed for impact.
...
Wait—this seems to contradict the observation that gravity helps. The resolution: gravity \emph{does} do positive work during the downswing (arm goes from up at the top to down during transition), but \emph{all that work is already done by the time you reach impact}. So the conversion of potential energy to kinetic energy happens early in the downswing.
...
\begin{itemize}
\item $\tau_i \geq -\tau_{\text{max}, i}$ (muscles can pull in one direction up to a max).
\item $\tau_i \leq +\tau_{\text{max}, i}$ (muscles can pull in the opposite direction up to a max).
\end{itemize}
...
\subsection*{Muscular Forces Are the Only Thing You Control}

This is the crucial point: \textbf{you don't directly control the forces from gravity, inertia, or velocity-dependence. You control only $\bm{\tau}$.}
...
\section{Putting It Together: Force Attribution at Impact}

Using our illustrative double-pendulum parameters from Chapter~\ref{ch:03_double_pendulum}, let's sketch a force attribution at impact. At impact the clubhead is moving fast and the accelerations are large.

\subsection*{The Clubhead Acceleration}

The clubhead is at distance $r = L_1 + L_2 = 1.35$ m from the shoulder. Using the illustrative shoulder rotation speed $\omega = 10.47 \text{ rad/s}$ from Chapter~\ref{ch:03_double_pendulum}, the centripetal acceleration (toward the shoulder) is:
\begin{equation}
a_c = \omega^2 r = 10.47^2 \times 1.35 = 147.6 \text{ m/s}^2 \approx 15\,g
\label{eq:ch04_centripetal_impact}
\end{equation}

With the elbow also rotating and the arm accelerating, total accelerations are higher still. Published measurements of clubhead acceleration during the golf swing report values on the order of $100\,g$ or more \cite{cochran1968search, penner2003physics}, though exact figures depend on the golfer and measurement methodology.
...
\begin{itemize}
\item \textbf{Inertial}: the arm must overcome its own inertia to sustain the acceleration.

\item \textbf{Gravity}: near impact ($\theta_1 \approx 0°$), gravity's lever arm about the shoulder is small, so its contribution is modest.

\item \textbf{Coriolis/Centrifugal}: at high angular velocities, the velocity-dependent forces become large and can assist the acceleration.

\item \textbf{Muscular}: the net muscular torque is whatever is needed to close the gap between the inertial demand and the passive contributions.
\end{itemize}
...
\label{eq:ch04_grip_force_impact}
\end{equation}
...
Here's the key insight: \textbf{you don't decide the grip force. Physics decides it.} You can only control the muscular torques at the shoulder and elbow. Given those torques and the gravitational/inertial forces, the dynamics evolve and determine what the grip force must be.
...
\section{Qualitative Summary: How Forces Shift Throughout the Swing}

The relative importance of each force source changes dramatically across swing phases:

\begin{itemize}
\item \textbf{Address/Backswing}: Velocities are low, so Coriolis and centrifugal forces are negligible. Gravity is the dominant passive force. Muscles must work against gravity to elevate the club and wind the backswing.

\item \textbf{Top of backswing}: The system is momentarily near rest. Gravity creates a large restoring torque. Muscles hold the position.

\item \textbf{Transition}: The golfer initiates the downswing. Inertial demands grow rapidly. Gravity begins to assist as the club descends.

\item \textbf{Downswing}: Velocities increase sharply. Velocity-dependent forces (Coriolis, centrifugal) grow quadratically with speed and become the largest passive contributors. Muscles may actually need to \emph{brake} in certain joints rather than accelerate, because passive forces are doing most of the work.

\item \textbf{Impact}: Accelerations are enormous. Passive forces dominate. The muscular contribution is relatively small compared to the total forces in play.
\end{itemize}

The exact magnitudes at each phase depend on the model parameters and the golfer's technique. What matters qualitatively is the \emph{shift}: early in the swing, muscles do most of the work; late in the swing, passive dynamics dominate.

\section{The Fundamental Insight}

Here's what all of this means:

\textbf{The golf swing is not primarily a muscular feat. It's a physics problem that muscles solve elegantly.}
...
Elite golfers have trained their nervous systems to solve this control problem. Novices have not. They often try to \emph{force} the club to move the way they want, fighting the passive physics. This is why they get tired and inconsistent.
...
\item \textbf{At impact, 180+ $g$ accelerations require careful force balance}. No single source (gravity, muscle, velocity-dependence) does it alone. All work together.

\item \textbf{Elite golfers ride the passive dynamics}. They use small, efficient muscular torques to steer the system toward impact.
\end{enumerate}
\end{principle}

\subsection*{Looking Ahead}
We have catalogued five sources of force in the golf swing: inertial, velocity-dependent, gravitational, muscular, and constraint. But which forces dominate at each phase of the swing? Chapter~\ref{ch:05_affine_structure} introduces the control-affine framework that separates all passive forces (the \emph{drift}) from muscular forces (the \emph{control}), revealing a central truth: by impact, passive forces overwhelm muscular control by a factor of 100 to 1.

\begin{exercises}{Chapter Exercises}{ex:ch04}
\begin{enumerate}

\item \textbf{Inertial Torque}. The shoulder has moment of inertia $M_{11} = 2.22$ kg m$^2$. What muscular torque is needed just to accelerate the shoulder at $\ddot{\theta}_1 = 500 \text{ deg/s}^2 = 8.7 \text{ rad/s}^2$, ignoring all other forces?
...
\item \textbf{Force Balance at Impact}. At impact, the total shoulder acceleration is $\ddot{\theta}_1 = 200 \text{ rad/s}^2$. The gravity torque is about 5 N m (small). The Coriolis/centrifugal torques total about 20 N m. Using $M_{11} = 2.22$ kg m$^2$, how much muscular torque is needed?
