...
This chapter builds on the two-link model of Chapter~\ref{ch:03_double_pendulum}. It explains how to recover physical loads, distinguish them from coordinate-dependent terms, and map the resulting motion to an endpoint task. A large force need not supply large power, a negative torque need not remove energy, and a net joint moment does not identify individual muscle forces. Keeping those distinctions explicit makes the connection to golf more informative.
\end{laymansbox}

\section{Five Parts of a Mechanical Accounting}
...
\begin{itemize}
\item \textbf{Acceleration terms} \(\bm M\ddot{\bm q}\) come from differentiating momentum. They are not an additional external agency pushing the body.
\item \textbf{Velocity-bias terms} \(\bm c\) arise from the same kinetic-energy model in moving generalized coordinates. Their decomposition into individual \(C_{ij}\) entries is not unique.
\item \textbf{Gravity} has physical generalized moment \(-\bm g\). The positive gradient appears on the left because it has been moved there.
\item \textbf{Applied generalized moments} \(\bm\tau\) represent the declared actuators and other loads included in that input convention. In a muscle-driven model, their production has additional geometry and internal dynamics.
\item \textbf{Constraint reactions} act at physical interfaces. Ideal hinge reactions eliminated by compatible coordinates still exist, but must not be added a second time to the same reduced equation.
\end{itemize}

These are not five independent physical force sources to add without regard to signs and body boundaries. The kinetic terms together play the role of \(m\bm a\). A more general model can include elastic and dissipative loads, external wrenches, and explicit contact reactions on the right. The standard manipulator formulation provides this organization; the actual terms depend on the declared model. \citep{TedrakeMultibodyEnergy}

\section{Inertial Terms and Coupled Acceleration}
...
\begin{example}{Specified Acceleration and Required Moments}{ch04_inertia_shoulder}
Use Chapter 3's manufactured parameters, not a measured golfer: masses \((2.5,0.4)\) kg, proximal length 0.35 m, distal endpoint length 1 m, COM distances \((0.175,0.5)\) m, centroidal inertias \((0.025,0.03)\) kg m\(^2\), and \(g=9.81\) m/s\(^2\).

At \(\bm q=(0,-5^\circ)\), with both rates zero, prescribe \(\ddot{\bm q}=(1000\pi/180,0)\) rad/s\(^2\). Full inverse dynamics gives
\[
...
\subsection*{Why Mass Distribution Matters}
...
\section{Velocity-Dependent Terms: Coriolis and Centrifugal}

\subsection*{Centrifugal Force and Inertial Acceleration}
...
\begin{definition}{Circular-Motion Acceleration}{ch04_centrifugal}
...
\begin{example}{A Declared Circular Particle}{ch04_centrifugal_numbers}
For a separate 0.2 kg particle, \(r=1.35\) m and \(\omega=30\) rad/s imply speed 40.5 m/s and inward acceleration 1215 m/s\(^2\). Its required inward resultant is 243 N. This is an ideal circular-particle calculation. It is not a whole-club grip measurement, and it does not establish shaft stress or muscle activity. Gravity and other applied loads must be included when finding a particular support force.
\end{example}

\begin{example}{Curvature in the Two-Link Model}{ch04_centrifugal_golf}
...
\subsection*{Coriolis Force and the Moving Frame}
...
\begin{definition}{Generalized Velocity Bias}{ch04_coriolis}
The two-link expression uses \(b=m_2L_1r_2=0.07\) kg m\(^2\):
\[
\bm c=b\sin q_2
...
\begin{example}{The Sign of a Bias Is Not Assistance}{ch04_coriolis_golf}
...
\subsection*{What Quadratic Scaling Establishes}
...
\begin{laymansbox}{Speed, Power and the Chosen Task}{box:ch04_speed_helps}
...
\section{Gravitational Forces and Potential}

In a uniform field, each body experiences \(m\bm g_0\), where \(\bm g_0=(0,-9.81)\) m/s\(^2\) in this inertial plane. Its magnitude does not depend on joint configuration; its generalized moments do. With COM distances \(r_1,r_2\),
\begin{equation}
...
\subsection*{Gravity Work and the Complete Cycle}
...
\section{Muscular Forces and the Input Model}
...
\subsection*{From Excitation to Realized Moment}
...
\section{Constraint Forces and Contact Feasibility}

\begin{definition}{Constraint Reactions}{ch04_constraint_forces}
...
\subsection*{Why Some Reactions Disappear From Reduced Equations}
...
\section{Putting It Together: Attribution Before Contact}

Use \(\bm q=(0,-5^\circ)\), \(\dot{\bm q}=(10,9)\) rad/s and prescribed \(\ddot{\bm q}=(20,-30)\) rad/s\(^2\). This is a manufactured state before contact, not a measured impact. It fixes all quantities needed for an instantaneous inverse-dynamics calculation:
\[
\begin{aligned}
...
\]
The four vectors, in horizontal/vertical m/s\(^2\), are approximately
\[
\begin{aligned}
...
\subsection*{Recovering the Distal Body's Interface Load}
...
Choi and Park's instrumented-grip study combines measured internal grip loading with spatial upper-limb inverse dynamics, illustrating why separate hand loads need information beyond a net club wrench. Its nine professional participants and modified 551 g club with foam balls limit direct transfer to ordinary driver swings. It does not establish a universal squeeze prescription. \citep{Choi2020GripKinetics}

OpenSim likewise distinguishes net generalized forces from the structural reaction loads produced after actuator forces are specified. Reports must identify the receiving body, frame and included actuators. Neither output alone is tissue stress or proof of muscle inactivity. \citep{OpenSimJointReactions2019}
\end{laymansbox}

\section{Wrenches: Forces, Moments and Power}

\begin{definition}{A Wrench With a Declared Reference}{ch04_wrench}
...
\]
These transformations preserve power. Rotating axes rotates both three-vectors consistently. Other texts use moment-first ordering; the matching twist must change too. A resultant wrench describes the net rigid-body load, not its detailed contact-pressure distribution. \citep{LynchParkWrenches}
\end{definition}
...
\section{Numerical Summary: Compare Declared States}
...
\subsection*{4. Gravity and Coordinate Signs}
...
\subsection*{5. Complete Inverse Dynamics}
...
\subsection*{6. COM Force, Moment and Contact}
...
\subsection*{7. Gravity Over a Closed Configuration Cycle}
