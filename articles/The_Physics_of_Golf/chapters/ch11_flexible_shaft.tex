...
\begin{tikzpicture}[scale=1.2]
  ...
  \draw[thick, color=red] (0,0) .. controls (1.5,-0.3) and (3.5,-0.8) .. (5,-1);
  \node at (2.5,-1.5) {\small Bending mode 1 (lowest frequency)};
  \node at (2.5,-2) {\small Elastic deformation};
  \fill[black] (0,0) circle (0.15);
  \node[above] at (0,0) {\small Grip};
  \fill[black] (5,-1) circle (0.15);
  \node[above right] at (5,-1) {\small Clubhead};
\end{tikzpicture}
\caption{Shaft Bending Modes: Fundamental Bending Creates Elastic Energy Storage.}
...
where:
\begin{itemize}
\item $w(z,t)$ is the vertical deflection of the shaft at position $z$ and time $t$.
...
\item $A$ is the shaft cross-sectional area.
\item $q(z,t)$ is the distributed load on the shaft (in our case, inertial forces from acceleration).
...
For a typical driver shaft, the first bending mode natural frequency is about 3--5 Hz ($\omega_n \approx 20$--$30$ rad/s), corresponding to a shaft ``frequency'' rating of roughly 240--280 CPM (cycles per minute). The downswing lasts about 0.2--0.25 seconds, so at $\sim 4$ Hz the shaft passes through roughly \emph{one} bending cycle (load, then recover) during the downswing --- the basis of the familiar ``kick'' at impact.
...
\begin{driftcontrol}{Shaft Flexibility in Drift vs.\ Control}{ch11:shaft_drift_control}
...
The consequences are far-reaching: a more flexible tip means more bend and more stored energy, but the bend is concentrated near the clubhead, which affects the timing of energy release and the orientation of the clubhead at impact. A stiffer tip means less stored energy but potentially better control of the clubhead orientation.
...
\begin{itemize}
\item \textbf{Higher stiffness:} The shaft generally bends less and the $\ztcf$ approaches the corresponding rigid-shaft trajectory. The sign and magnitude of the delivery-speed change depend on excitation timing, damping, mass distribution, and initial state.

\item \textbf{Lower stiffness:} Larger deformation may store more elastic energy, but its release can occur before or after the delivery window. A deterministic $\ztcf$ has no statistical variance by itself; dispersion requires a declared distribution of initial states, parameters, or controls.

\item \textbf{Task-matched stiffness:} A candidate shaft is supported only when calibrated simulations and held-out measurements show improved task metrics while respecting orientation, load, and robustness constraints.
\end{itemize}
...
\begin{principle}{Shaft Fitting as Drift Design}{ch11:shaft_fitting}
...
\begin{laymansbox}{Why Flexible Shafts Help Slower Golfers More}{ch11:slow_golfer}
A slower golfer (for illustration, consider a driver swing speed around 70 mph, which is below tour-professional levels \citep{Jorgensen1994}) has less centrifugal stiffening throughout the swing. The shaft remains more flexible. This means:
\begin{enumerate}
...
Shaft flexibility changes $f(\state)$ because elastic and damping effects belong to the declared effective plant. The generalized-coordinate input map $G(\state)$ may remain unchanged in a particular actuation model, but task-space authority can still change through configuration, Jacobians, constraints, and admissible-input bounds. A different shaft therefore implies a different drift field and forward $\ztcf$; it does not justify a universal claim about control capacity.

\begin{driftcontrol}{Shaft Flexibility: Drift Only}{ch11:drift_only}
