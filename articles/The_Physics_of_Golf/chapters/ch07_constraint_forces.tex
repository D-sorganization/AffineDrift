...
\item \textbf{Forces that are enormous:} Constraint forces at joints can reach hundreds of Newtons during dynamic movement, substantially exceeding individual muscle forces \citep{Nesbit2005}.
...
\begin{example}{Wrist Constraint Jacobian}{wrist_constraint_jacobian}
...
\begin{definition}{Null Space and Range of $\bm{J}_c$}{jacobian_spaces}
...
\begin{example}{Null Space for the Wrist Hinge}{wrist_null_space}
...
Under ideal scleronomic conditions, constraint forces do \emph{zero} net power to the system.
\end{theorem}
...
\begin{example}{Wrist Constraint Force in the Double Pendulum}{double_pend_constraint_force}
...
At the configuration from Chapter~\ref{ch:zero_torque_counterfactual} ($q_1 = 45°$, $\dot{q}_1 = 8$ rad/s, $q_2 = 70°$, $\dot{q}_2 = 12$ rad/s), the unconstrained accelerations (from the ZTCF family) are:
\begin{align}
...
Note: these individual terms do not sum to zero. This is because the constraint velocity condition $\dot{q}_1 + \dot{q}_2 = 0$ must hold when the constraint is active. The initial velocities ($\dot{q}_1 = 8$, $\dot{q}_2 = 12$) do not satisfy this condition, indicating that the constraint was imposed on a state that was not on the constraint manifold. When the constraint is properly enforced, $P_{\text{constraint}} = \lambda(\dot{q}_1 + \dot{q}_2) = 0$, confirming zero net work.

\textbf{Key Finding:} The constraint force is small ($\sim 0.3$ Nm) but its \emph{role} is crucial: it couples the two joints, redistributing energy between them while doing zero net work on the system. Over the course of the downswing, these coupling forces accumulate to produce the energy transfer from arm to club.
\end{example}
...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw[thick, blue] (0,0) -- (2,0) node[midway, above] {Segment 1 (arm)};
  \draw[thick, red] (2,0) -- (3.5,-1.5) node[midway, right] {Segment 2 (club)};

  % Draw hinge joint
  \draw[fill=black] (2,0) circle (0.1);

  % Draw constraint force vectors at the joint
  % Force on segment 1 (pointing left and down)
  \draw[->, line width=2pt, green!60!black] (2,0) -- (1.3,-0.5) node[left] {$\bm{F}_c$ (on arm)};
  % Force on segment 2 (pointing right and up)
  \draw[->, line width=2pt, green!60!black] (2,0) -- (2.7,0.5) node[right] {$\bm{F}_c$ (on club)};

  % Draw velocity vectors to show motion
  \draw[->, line width=1.5pt, blue] (1,0) -- (0.5,-0.5) node[left, blue] {$\dot{\bm{q}}_1$};
  \draw[->, line width=1.5pt, red] (2.8,-0.75) -- (2.3,-1.3) node[below right, red] {$\dot{\bm{q}}_2$};

  % Add labels
  \node at (0,-0.5) {Shoulder};
  \node at (3.7,-2) {Wrist (hinge)};

  % Illustrate constraint: annotation
  \node[above right=2cm and 1cm] {\small Constraint: $\dot{\bm{q}}_1 \parallel \dot{\bm{q}}_2$};
  \node[above right=2.8cm and 1cm] {\small Constraint forces do zero net work};
  \node[above right=3.6cm and 1cm] {\small but redirect energy between segments};

\end{tikzpicture}
\caption{Constraint Forces at a Hinge Joint. The constraint force $\bm{F}_c$ acts at the joint, perpendicular to the allowed motion direction. On segment 1 (arm), the force opposes motion; on segment 2 (club), it drives motion. These action-reaction pair forces satisfy zero net power: $\bm{F}_c \cdot \dot{\bm{q}}_1 + (-\bm{F}_c) \cdot \dot{\bm{q}}_2 = 0$, yet they enable energy transfer from segment 1 to segment 2.}
...
\textbf{Insight:} The large grip forces in a golf swing are not primarily evidence of active muscular contraction driving the motion. Rather, they reflect the inertial forces that must be channeled through the grip constraint to maintain the coupling between hand and club. The magnitude of these forces is determined by the system's kinematics and inertias, not by muscle activation level.
\end{example}
...
\item \textbf{Grip and ground forces:} Grip forces ($50$--$150$ N) and ground reaction forces ($1300$--$1800$ N) are constraint forces, not muscular efforts. Muscles hold the constraints, but physics enforces them.
...
Explain in plain language: Why can a grip force (for example, 100 N---a representative value used here for illustration; actual grip forces depend on swing speed and grip technique) do zero net work on the system, yet transfer energy from the arm to the club?
