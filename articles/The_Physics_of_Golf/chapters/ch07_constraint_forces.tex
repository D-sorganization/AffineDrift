...
\textbf{What comes next:} Understanding constraints, we now ask: what if there are \emph{multiple} constraints forming a closed loop? What if the body is not just a serial chain (hips → shoulders → arms → club) but a parallel mechanism with closed kinematic chains? That's the subject of Chapter 9.
