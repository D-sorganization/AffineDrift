...
The golf swing connects multiple scientific disciplines through shared mathematical structures. Figure~\ref{fig:ch13_interdisciplinary_map} illustrates how these fields intersect at the core dynamics $\dot{\state} = f(\state) + G(\state)\control$.

\begin{figure}[h]
\centering
\begin{tikzpicture}[scale=1.0, every node/.style={align=center}]
  % Central equation
  \node[draw, rectangle, minimum width=4cm, minimum height=0.8cm, font=\small] (core) at (0,0) {$\dot{\state} = f(\state) + G(\state)\control$};

  % Discipline nodes
  \node[draw, circle, minimum width=1.2cm] (control) at (-4, 2) {Control\\Theory};
  \node[draw, circle, minimum width=1.2cm] (biomech) at (-1.5, 2.5) {Biomechanics};
  \node[draw, circle, minimum width=1.2cm] (robotics) at (1.5, 2.5) {Robotics};
  \node[draw, circle, minimum width=1.2cm] (neuro) at (4, 2) {Neuroscience};
  \node[draw, circle, minimum width=1.2cm] (materials) at (-2.5, -2) {Material\\Science};
  \node[draw, circle, minimum width=1.2cm] (data) at (2.5, -2) {Data\\Science};
  ...
  % Key contributions
  \node[font=\small, right=0.3cm of control] {Drift/Control\\Decomposition};
  \node[font=\small, right=0.3cm of biomech] {Force\\Generation};
  \node[font=\small, left=0.3cm of robotics] {Hierarchical\\Structure};
  \node[font=\small, left=0.3cm of neuro] {Motor\\Commands};
  \node[font=\small, right=0.3cm of materials] {Elastic\\Properties};
  \node[font=\small, left=0.3cm of data] {Parameter\\Estimation};
\end{tikzpicture}
\caption{The Affine Control Framework Provides a Common Language for Multiple Scientific Disciplines, Each Contributing Essential Insights to Understanding the Golf Swing.}
...
\begin{example}{The Supraspinatus Activation}{ch13:supraspinatus}
...
The affine framework establishes mathematical correspondence between control theory and biomechanics through the shared structure: $\dot{\state} = f(\state) + G(\state)\control$.

For control theorists, this structure clarifies the biological instantiation: the drift $f(\state)$ comprises gravity, Coriolis forces, elastic forces in the shaft, and passive damping in connective tissue. The control field $G(\state)$ represents muscle torques distributed across multiple joints.
...
\begin{laymansbox}{Robots as Mathematical Models}{ch13:robot_lesson}
...
The affine framework provides a model for separating state-dependent plant dynamics from declared applied control. It does not establish what the nervous system learns. At each state, $G(\state)$ maps the declared control coordinates, while $f(\state)$ specifies the autonomous vector field of the effective plant.
...
The material science of ball design thus determines the energy transfer coefficient $\mathrm{COR}$, which is one parameter in the impact model that relates clubhead velocity at impact to ball velocity after impact.
\end{example}
...
\begin{example}{Rotator Cuff Training for Golfers}{ch13:rotator_cuff_training}
...
\begin{example}{Coaching Cue Design}{ch13:cue_design}
...
% ─────────────────────────────────────────────────────────────────
\section{Synthesis: The ZTCF Family Framework Unifies These Perspectives}

\index{ZTCF!unification}

How do all these disciplines fit together? Through the zero-torque counterfactual.

Recall that a forward ZTCF trajectory integrates the effective plant after setting the declared applied generalized-control channel to zero. It depends on $f(\state)$, the branch state, parameters, contact mode, and retained constraints; it does not specify muscle activation.
...
\textbf{Internal forces.} Muscle forces map to joint torques through configuration-dependent moment arms (Chapter~\ref{ch:muscle-to-torque}), with each muscle's force limited by biology---the force--length and force--velocity relationships of Hill-type muscle models (Chapter~\ref{ch:muscle-force-generation}). Recovering individual muscle forces from motion can be nonunique even in an open limb; closed contacts add load-sharing questions whose identifiability depends on the dynamic and measurement maps (Chapter~\ref{ch:inverse_dynamics_parallel}).
...
\textbf{The controller.} The brain operates under severe constraints: 150~ms sensory delays, 40--60~ms electromechanical delays, and noise at every stage (Chapter~\ref{ch:motor_control_brain}). Motor learning is the process of building internal models of the drift field and discovering impedance profiles that make the swing self-correcting (Chapter~\ref{ch:motor_learning}). The brain's computational capacity is substantial---managing 200+ muscles in real time---yet fragile, as the yips and choking under pressure demonstrate (Chapter~\ref{ch:remarkable_brain}). Perhaps most importantly, the brain does not solve the control problem alone: passive mechanics, muscle pre-tension, and spinal reflexes provide distributed control that reduces the brain's computational burden (Chapter~\ref{ch:passive_distributed_control}).
...
The clubhead must have been traveling at about $145 / 1.48 \approx 98$ mph just before impact,
dividing by the smash factor rather than by the coefficient of restitution --- ball speed
\emph{exceeds} clubhead speed for a driver, so the two are not interchangeable and the ratio
does not run the other way (Chapter~\ref{ch:impact_collision_collision}). From the club path and face angle, we can infer the club's orientation and velocity in 3D.
...
% ─────────────────────────────────────────────────────────────────
\begin{principle}{Key Takeaways}{ch13:takeaways}
\begin{enumerate}
...
% ─────────────────────────────────────────────────────────────────
\begin{exercises}{Chapter Exercises}{ch13}
