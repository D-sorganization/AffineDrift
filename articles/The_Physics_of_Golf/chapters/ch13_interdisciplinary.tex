...
\epigraph{Golf is a microcosm where physics, engineering, neuroscience, and biomechanics all collide. Understanding the collision is understanding the game.}{}
...
These materials have mechanical properties that substantially affect the swing and the outcome.
...
A COR of 1.0 would mean perfectly elastic collision (no energy loss). A COR less than 1.0 means some energy is lost. Modern golf ball and clubface technology has COR values in the range of roughly 0.8 or above \cite{penner2003physics}, meaning most of the impact energy is transferred to the ball.
...
But there is an inverse problem: given the observed ball trajectory, what can we infer about the club and ball at impact? This is where physics-informed parameter estimation and statistical inference become essential.
...
\begin{itemize}
\item Ball speed below the golfer's target.
\item Spin rate above the optimal range for the given launch angle.
\item Launch angle near optimal.
\item Club path slightly open of target.
\item Club face angle slightly open of target.
\end{itemize}

\subsection*{The Physics}

From the ball speed and the COR of the ball and clubface combination \cite{penner2003physics}, we can estimate the clubhead speed just before impact. From the club path and face angle, we can infer the club's orientation and velocity in 3D.
...
\begin{enumerate}
\item \textbf{Ball speed is low.} The ball speed is below the target for this golfer, suggesting the clubhead speed was low or the club made off-center contact.

\item \textbf{Spin rate is high.} The spin rate is adding excess backspin, reducing carry distance. The extra spin is likely because the club face was slightly closed relative to the club path, imparting extra spin.

\item \textbf{Launch angle is near optimal.} The launch angle is close to the optimal range for this golfer's swing speed.
\end{enumerate}
...
\item \textbf{Statistical inference and parameter estimation allow us to learn personalized drift fields from launch monitor data.}
...
\item \textbf{Technology enables insight:} launch monitors, motion capture, and computational modeling make the invisible physics visible, allowing golfers and coaches to learn from data rather than just intuition.
\end{enumerate}
\end{principle}
