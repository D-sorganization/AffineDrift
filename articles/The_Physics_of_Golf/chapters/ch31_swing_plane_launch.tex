...
\index{attack angle!typical values}
\index{attack angle}
\index{address plane}
\index{ZTCF family!launch conditions}
\index{Magnus force}
\index{Hogan, Ben!plane of glass}
\index{D-plane}

\begin{laymansbox}{From a Swing to a Shot}{lay:launch_primacy}
...
\begin{definition}{A Plane Requires More Than One Direction}{swing_plane}
...
\citep{Trackman2024SwingPlane,Trackman2020Parameters}.

\subsection{Why the Swing Plane Tilts}

\begin{principle}{Geometry Constrains the Available Motion}{prin:plane_tilt}
...
\begin{definition}{Attack Angle and Path From the Same Velocity}{attack_angle}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/swing_plane_geometry.pdf}
\caption{Fixed-Plane Geometry Links Path and Attack Angle. Curves Use the Declared Positive-Left Frame and Forward Tangent Branch; They Are Geometric Calculations, Not Measured Swings.}
...
\hline
\textbf{Quantity} & \textbf{Required Comparison} \\
\hline
...
\hline
\end{tabular}
\caption{Separate Definitions Before Comparing Clubs or Golfers. None of These Quantities Substitutes for Another.}
...
\begin{principle}{Face Orientation and Joint Contributions}{prin:face_three_params}
...
\begin{example}{A Joint's Axis Changes Its Face-Angle Gain}{forearm_sensitivity}
...
\begin{definition}{A Gate Is an Inverse Image of an Objective}{gate}
...
\subsection{Drift vs. Control: Remaining Time Before Impact}

\begin{principle}{Feedback Timing and Continuing Actuation}{prin:face_control_window}
...
different inputs. The computational-brain chapter derives a delayed-activation
...
\subsection{Launch Direction: Measurements and Their Limits}
...
\begin{definition}{The D-Plane as a Geometric Construction}{d_plane}
...
$t=(\hat v-(n^T\hat v)n)/\|\hat v-(n^T\hat v)n\|$. Then
\begin{equation}
...
to a freely rotating head \citep{henrikson2020friction}.
...
\begin{principle}{Launch State and the Flight Model}{prin:three_launch}
...
aerodynamics chapter develops these boundaries. A coefficient table from one
...
\begin{tabular}{cccc}
\hline
\textbf{Ball Speed} & \textbf{Launch} & \textbf{Spin} & \textbf{Model Carry} \\
\hline
...
\hline
\end{tabular}
\caption{Manufactured Local Surface: Coupled Launch and Spin Change Which Angle Is Best. These Are Calculated Examples, Not Golf Fitting Data.}
...
\begin{principle}{A Claimed Optimum Must Follow From the Model}{prin:spin_tradeoff}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/swing_launch_optimum.pdf}
\caption{A Manufactured Launch Surface Shows Coupling and Curvature. The Elliptical One-Yard Loss Region and Conditional Optima Follow From the Stated Quadratic, Not a Golf Fitting Dataset.}
\label{fig:swing_launch_optimum}
\end{figure}
...
\hline
\textbf{Window Choice} & \textbf{Meaning} \\
\hline
...
\hline
\end{tabular}
\caption{Define the Purpose of a Launch Window Before Using It as a Target.}
...
launch fitting table cannot establish it. The impact chapter treats the
...
is for a one-degree \emph{launch} change, not a one-degree face change under
this weighting.
...
At fixed club speed 100 mph, a smash-factor \emph{standard deviation} of
...
\section[The Zero-Torque Counterfactual Perspective on Launch]{The Zero-Torque Counterfactual\\Perspective on Launch}
\label{sec:ztcf_launch}

\begin{principle}{Define the Counterfactual Intervention}{prin:launch_drift}
...
\subsection{What a Late Correction Can Change}

\begin{example}{New Corrections and Continuing Force}{control_illusion}
...
\subsection{Determinants of Delivery: State, Inputs and Constraints}
...
\section{Problems and Thought Experiments}
\label{ex:launch}
\label{exercises:ex:launch}

\begin{enumerate}[leftmargin=*]
\item \textbf{Swing Plane Geometry.} Use a fixed $60^\circ$ plane and
...
\item \textbf{Face Angle Sensitivity.} At $\phi=0$, $\ell=15^\circ$,
...
\item \textbf{Launch Optimization.} In the manufactured quadratic model,
...
\item \textbf{D-Plane and Impulse.} For a path $2^\circ$ right and square
...
\item \textbf{Control and Event Timing.} A new correction is delayed
...
\item \textbf{Equipment and Effective Mass.} Use the masses, inertia
...
\item \textbf{Wind and Feasible Launch.} Specify the additional inputs
...
\item \textbf{Consistency and Objective.} Reproduce the quadratic mean
...
\subsection{Worked Checks and Interpretation}

\begin{enumerate}[leftmargin=*]
\item The path offset is
$\arcsin(\tan(-5^\circ)/\tan60^\circ)=-2.8953^\circ$, or right of the
...
\item The gains are $-0.26795$, $0$ and $1$ respectively. The face-to-offset
...
\item The two endpoint carries are both 228.8 yards. The conditional
...
\item Launch azimuth is $-0.48^\circ$, or $0.48^\circ$ right. With the
...
\item The stated new delayed command arrives after contact. Earlier
...
\item The extra inverse mass is $(0.020)^2/0.0004=1$ kg$^{-1}$.
...
\item Supply initial state, ball aerodynamic and spin-decay model, wind
...
\item The covariance terms give
