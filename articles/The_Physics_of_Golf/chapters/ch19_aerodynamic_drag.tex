% Canonical aerodynamic chapter; paired web edition is synchronized.
\chapter[Aerodynamic Loads in Swing and Flight]{Aerodynamic Loads\\in Swing and Flight}
...
\section{Define the Relative Airflow}
...
\href{https://www1.grc.nasa.gov/beginners-guide-to-aeronautics/drag-equation/}{NASA drag-equation explanation}
...
\subsection{Quadratic Scaling and Its Limits}
...
\section{A Force Estimate With a Clear Scope}
...
\section{Distributed Shaft Loads and the Actual Pivot}
...
\section{From Air Loads to Generalized Loads}
...
\begin{equation}
 ...
 \label{eq:drag_constrained_projection}
\end{equation}
...
\section{Power, Frames, and System Boundaries}
...
\section{The Sign of an Inverse-Dynamics Correction}
...
Chapter~\ref{ch:inverse_dynamics_parallel}. Net hand force, net hand couple,
...
\section{A Reproducible Forward Counterfactual}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/aerodynamic_counterfactual.png}
\caption{Illustrative Zero-Input Pendulum Motion and Energy With and Without Quadratic Drag. Each Curve Ends at Its First Bottom Crossing.}
...
\section{Ball Flight Connects the Swing to the Outcome}
...
\href{https://doi.org/10.1017/S0001925900007617}{Bearman and Harvey, Golf Ball Aerodynamics (1976)}
...
\section{Environment and Model Uncertainty}

For dry ideal-gas air, $\rho=p/(R_{\rm air}T)$ with absolute temperature $T$.
The \href{https://www1.grc.nasa.gov/beginners-guide-to-aeronautics/equation-of-state/}{NASA equation-of-state treatment}
...
\begin{equation}
 ...
 \label{eq:air_density_temperature}
\end{equation}
...
\section{Choosing the Aerodynamic Model}
...
\begin{enumerate}
\item Specify the measured or predicted output, system boundary, coordinate
      conventions, and tolerable discrepancy in physical units.
\item Begin with a transparent force and moment model. Record coefficient-area
      pairs, incidence and Reynolds-number ranges, air properties, wind, and
      the evidence supporting each parameter.
\item Use compatible segment velocities and map loads by virtual power. Check
      force and moment signs, zero-relative-speed behavior, units, and energy
      balance before interpreting the output.
\item Compare omitted, nominal, and plausible alternative aerodynamic models
...
\item Check numerical convergence and compare predictions with independent
...
\item Report the resulting sensitivity and remaining uncertainty. Treat a
...
\item Reproduce the 3.0625 N example. Change the reporting area while preserving
      force; calculate the new coefficient.
\item Derive shaft force and moment for pivot offsets $a=0$ and $a=0.6$ m.
      Explain why a near-end pivot is not a shoulder model.
\item Derive generalized aerodynamic load from virtual power, including the
      single revolute-axis moment as a special case.
\item Verify the tailwind example in both velocity frames and identify the
      energy supplied by the moving air.
\item Reproduce the 10 versus 6 N m inverse example. Explain what changes
      when contact reactions are also unknown.
\item Verify the pendulum equilibrium, bottom-crossing speeds, and energy loss.
      Explain why peak speed can occur before the bottom.
\item Compute the temperature and headwind examples. Distinguish the factors
      held fixed from those that can change outdoors.
\item Compare ball flights with equal launch states and different coefficient
