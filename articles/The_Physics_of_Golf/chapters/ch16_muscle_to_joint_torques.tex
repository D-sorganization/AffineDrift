...
\begin{tikzpicture}[scale=1.0, thick]
  ...
  % Muscle space (left)
  \node[draw, fill=blue!10, rounded corners, minimum width=2cm, minimum height=3cm] at (1, 3) {\begin{tabular}{c} Muscle Forces \\ (space $\mathbb{R}^{n_m}$) \\ \\ $F_1, F_2, \ldots$ \\ (often $n_m > n_j$) \end{tabular}};

  % Joint torque space (right)
  \node[draw, fill=green!10, rounded corners, minimum width=2cm, minimum height=3cm] at (7, 3) {\begin{tabular}{c} Joint Torques \\ (space $\mathbb{R}^{n_j}$) \\ \\ $\tau_1, \tau_2, \ldots$ \\ (fewer DOF) \end{tabular}};
  ...
  % Redundancy note
  \node[draw, rounded corners, fill=yellow!20, text width=5cm, font=\small] at (4, 0.8) {
  ...
  \node[draw, rounded corners, fill=orange!20, text width=5cm, font=\small] at (4, -0.8) {
...
\begin{example}{Moment Arms and Joint Torques in a Two-Joint Arm}{moment-arm-arm}
Consider a simplified arm with two joints (shoulder and elbow) and two muscles (an extensor muscle acting at both joints and a flexor muscle acting at the elbow).
