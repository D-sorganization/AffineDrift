...
description: "Single-file edition of the Drifter Manifesto: the control-affine framework for golf swing analysis (drift/input decomposition, ZTCF, ZVCF, drift invariance, numerical implementation). The maintained canonical version is the Theory Part 1-5 series."
author: "Dieter Olson"
date: today
...
This is the **single-file edition** of the Drifter Manifesto, kept for readers who want the whole derivation on one page. The **maintained canonical version** is the multi-part series: [Theory Part 1](theory-part1.html) (state definition and affine derivation), [Part 2](theory-part2.html), [Part 3](theory-part3.html), [Part 4](theory-part4.html), and [Part 5](theory-part5.html). The series is indexed at [The Drifter Manifesto — Series Index](../pages/drifter-manifesto.html). Where this page and the series differ, **the series is authoritative**; substantive edits should be made there and mirrored here.
:::
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      ...
      <div class="laymans-item">
        ...
        <p>
          When watching a golf swing in slow motion, you can see how fast the club moves. But here's the puzzle: how much of that movement is the golfer actively creating, and how much is just physics (momentum, gravity) doing its thing?
        </p>
        <div class="analogy">
      ...
      <div class="laymans-item">
        ...
        <p>
          The "state" is a complete snapshot of the golfer-club system at any moment—it includes where every joint is and how fast everything is moving. Think of it as pausing a video and noting every detail about the pose and motion.
        </p>
      ...
      <div class="laymans-item">
        ...
        <p>
          To make the math tractable, this framework assumes that body parts are rigid (not squishy), that the club shaft bends in predictable ways, and that we can ignore air resistance. These are standard engineering approximations.
        </p>
        <div class="analogy">
  ...
  <div class="critics-comments-content">
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> This is a crucial distinction. The decomposition does not claim drift is "free" in a metabolic sense; it claims it is "passive" in a <em>causal</em> sense at the moment of analysis. The golfer's skill lies in <em>actively</em> positioning the system (the setup) so that the <em>passive</em> drift field generates the desired high-speed release. The framework highlights this "effort-to-ease" transition, rather than obscuring it.
        </div>
      </div>
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> Standard inverse dynamics provides the <em>sum</em> of forces required, but it is causally ambiguous: it cannot distinguish between a force required to <em>overcome</em> inertia and a force required to <em>exploit</em> it. The affine structure rigorously separates the system's "Inertial Memory" (Drift) from the player's "Current Agency" (Input). Without this separation, we cannot formulate the counterfactuals (e.g., "What if I stopped pushing?") that are essential for understanding control strategy.
        </div>
      </div>
...
Traditional biomechanical analyses of the golf swing rely heavily on inverse dynamics to estimate generalized joint torques and hand forces from measured kinematics and external forces. Inverse dynamics provides the *total* generalized forces consistent with a given motion, but it does not reveal the causal structure of those forces. It cannot say how much arises from the golfer's active muscular effort and how much is generated passively by inertia, gravity, elastic shaft deformation, and motion history.
...
In plainer terms: the state vector $x$ captures both *where* every part of the system is (the joint angles $q$ and shaft bend $\eta$) and *how fast* each is moving ($\dot{q}$ and $\dot{\eta}$). Just as you need both a car's position on a highway and its speed to predict where it will be in five seconds, the physics needs both configuration and velocity to predict what comes next. The tangent bundle is the mathematical name for the space of all possible "where + how fast" combinations.
...
The non-zero block $M_{q\eta}$ is the physical mechanism by which hand acceleration ($\ddot{q}$) creates an instantaneous inertial load on the shaft (via $-M_{q\eta}^T \ddot{q}$). The same block runs in reverse: shaft vibration recoil exerts reaction torques on the hands. This coupling arises purely from the mass distribution of the shaft and is independent of stiffness.
...
$$ Physically, $\Delta$ represents the **Articulated Body Inertia** projected into the joint space. It is the effective inertia felt by the golfer's joints when the shaft modes are free to accelerate. This value is strictly less than the "locked-mode" inertia $M_{qq}$ (in the sense that $M_{qq} - \Delta$ is positive semi-definite), reflecting that the shaft "gives way" under load and so reduces the resistance to hand acceleration.
...
The derivation above transforms the equations of motion from a description of *how the system moves* to a description of *why it moves*. By isolating the input $u$ in a separate linear term, we have created a causal map. This mathematical separation forces a philosophical shift in how we view the swing. In traditional inverse dynamics, the "net moment" is a monolithic quantity. In the affine framework, it is an aggregate of two distinct physical species: the *inertial memory* of the system (drift) and the *current agency* of the player (input). The derivation above shows that these two species add without mixing: within the model they are **mechanically separable**.
  ...
  <div class="resource-card video-card has-media">
    ...
    <div class="resource-content">
      ...
      <p class="resource-description">
        ...
        rather than canceling them—the core philosophy of the AffineDrift framework.
      </p>
      <div class="resource-details" style="font-size: 0.9em; margin-top: 1rem;">
        <strong>Prerequisites:</strong>
        <ul style="margin-bottom: 0.5rem;">
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      ...
      <div class="laymans-item">
        ...
        <p>
          We introduce the <strong>Zero Torque Counterfactual (ZTCF)</strong>. This is a fancy name for a simple question: "What would happen if the golfer suddenly stopped trying?"
        </p>
        <p>
      ...
      <div class="laymans-item">
        ...
        <div class="key-takeaway">
          By comparing the actual swing to these two baselines (Drift and Static Load), we can figure out how much "extra" work the golfer is doing to bend the shaft or speed up the club.
        </div>
      </div>
...
The **Mechanical Separability** derived in Part I enables this shift. In that additive decomposition of drift and input terms, the inertial memory of the system (drift) is structurally distinct from the current agency of the player (input). We now exploit this structure to answer the central causal question of the swing: *which forces are the result of the golfer's immediate effort, and which are the result of the system's passive physics?*
...
It does *not* claim that such a motion could be achieved by a real golfer, nor that the nervous system ever selects "zero torque" as a control policy during a swing. Instead, ZTCF is a mathematical device that:
...
The ZTCF isolates the system's passive dynamics from the golfer's active input, but the passive drift itself remains a composite phenomenon. It aggregates both *motion-dependent* forces (such as centrifugal, Coriolis, and inertial coupling) and *configuration-dependent* forces (such as gravity and elastic stiffness). In high-speed motions like the golf swing, velocity-driven terms often obscure the underlying static loads. To fully deconstruct the drift, we require a second analytical slice—one that freezes motion to reveal the forces arising purely from the system's instantaneous shape.
...
We have now constructed two powerful counterfactual baselines: the ZTCF to isolate passive momentum history, and the ZVCF to isolate passive configuration loads. However, the validity of these diagnostic tools relies on a subtle but critical mathematical condition. For the subtraction $\tau_{\text{input}} = \tau_{\text{total}} - \tau_{\text{drift}}$ to be causally meaningful, the term being subtracted ($\tau_{\text{drift}}$) must be independent of the term being isolated ($\tau_{\text{input}}$). If the act of applying torque altered the passive dynamics of the system—changing the effective inertia or stiffness—our baselines would be invalid.
...
These constructions rely on a fundamental premise: that the 'passive' dynamics of the system are robust to the application of 'active' force. If the act of applying torque altered the nature of the drift field $f(x)$—for instance, by implicitly modifying the effective inertia or stiffness matrix—then our baselines would be moving targets, dependent on the very variable we seek to isolate.
...
The control input $u$ appears *exclusively* in the term $M^{-1} [B(q)u; 0]^T$. This independence is structural: it is not an approximation but a property of the Lagrangian formulation, in which generalized forces are dual to generalized velocities. It guarantees that the "passive" dynamics identified by the model are structurally unpolluted by the "active" control inputs.
...
This subtraction is valid precisely because of the affine structure derived in Part I. Recall that the input acceleration is given by $A_{\text{input}}(x)u$. The 'Input Force' defined here is the dynamic realization of that acceleration term, scaled by the system mass matrix. It captures not just the direct drive on the joints, but also the forces induced by the **Inertial Coupling Ratio** ($\Gamma$) acting on the flexible shaft. Thus, $\tau_{\text{input}}$ represents the complete mechanical footprint of the golfer's agency.
...
This taxonomy completes the theoretical framework, providing a deterministic map from observed forces to their mechanical causes. However, the exactness of that map is bounded by the fidelity of the model that generates it. The decomposition is algebraically exact within the model, but its application to reality is constrained by the simplifications made in Part I. We now catalog these limitations to define the scope of validity for our causal claims.
...
- **Part V: Numerical Validation (Simulink).**
  This section provides the **numerical stress test**. By implementing the full forward dynamics in a stiff solver environment, Part V shows that the delicate counterfactual subtractions remain stable under integration error, so the theory survives the transition from the page to the processor.
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      ...
      <div class="laymans-item">
        ...
        <div class="analogy">
          <strong>Analogy:</strong> If Parts I-III were the user manual for a car, Part IV is the technical blueprint the engineers used to build it. You don't need to read it to drive, but it's proof that the car won't fall apart.
        </div>
      </div>
...
and the system is control-affine.
...
<section class="laymans-terms">
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        Theory is great, but does it work? Part V tests the math in a computer simulation.
      </p>

      <div class="laymans-item">
        <h3>The "Flight Simulator" for Golf</h3>
        <p>
          We built a digital model of a golfer and club (using a tool called Simulink) to act as a proving ground. It lets us run experiments that would be impossible in real life—like instantaneously turning off a golfer's muscles to see what happens.
        </p>
        <div class="analogy">
      ...
      <div class="laymans-item">
        ...
        <div class="key-takeaway">
          The result? The club kept moving violently due to momentum. In the simulation, the "passive" forces (Drift) late in the swing are large—often larger than what the golfer can actively produce.
        </div>
      </div>
...
When the model is configured correctly, then exactly at the freeze instant $t_0$ the difference between the total force and the ZTCF force equals the **input** force (for $t > t_0$ the equality is only approximate, as the ZTCF trajectory diverges):

$$F_{\text{total}}(t_0) - F_{\mathrm{ZTCF}}(t_0) = F_{\mathrm{input}}(t_0).$$

At $t_0$ the ZTCF run records the full drift force $\tau_{\mathrm{drift}}(x(t_0))$, so $F_{\text{total}} - F_{\mathrm{ZTCF}} = \tau_{\mathrm{total}} - \tau_{\mathrm{drift}} = \tau_{\mathrm{input}}$. The configuration-only (ZVCF) force is instead the slice $F_{\mathrm{drift}}(t_0) - F_{\mathrm{vel.drift}}(t_0) = F_{\mathrm{ZVCF}}(t_0)$; it is **not** equal to $F_{\text{total}} - F_{\mathrm{ZTCF}}$.

This is a **Numerical Consistency Check**—a verification that discrete-time integration algorithms and "kill-switch" logic correctly preserve the algebraic structure of the theory without introducing numerical artifacts. It does NOT validate the control-affine model itself against reality. The identity follows from the Lagrangian formulation; its numerical truth in simulation proves only that the implementation is correct, not that the model accurately predicts actual golf swings. Model validation requires comparison with experimental data from real athletes.
:::
...
Future research will incorporate these counterfactual calculations into high-fidelity 3D multibody simulations. The most critical next step is the extension to 3D. The current planar validation shows that the ZTCF subtraction logic works for $M(q)\ddot{q}$, but it does not stress-test the drift invariance assumption against the complex $\omega \times I \omega$ gyroscopic terms present in the full spatial swing.
...
$$\tau_{\mathrm{total}}(t_0) - \tau_{\mathrm{ZTCF}}(t_0)
  =
  \tau_{\mathrm{input}}(t_0),$$
...
The identity $\tau_{\mathrm{total}}(t_0) - \tau_{\mathrm{ZTCF}}(t_0) = \tau_{\mathrm{input}}(t_0)$ is **exact only at $t = t_0$** (the initial instant). After $t_0$, the ZTCF trajectory diverges from the actual trajectory — $F_{\mathrm{ZTCF}}(t)$ is evaluated along a *different* state path than $F_{\mathrm{total}}(t)$. The subtraction mixes forces from two trajectories at different states, which is not generally equal to $F_{\mathrm{input}}(t)$ at the *actual* trajectory state. The numerical near-equality observed in simulation arises because the ZTCF trajectory remains close to the actual trajectory over short time windows, which makes the approximation tight. This is not a flaw in the method — it is inherent to counterfactual reasoning — but it means the result should be interpreted as "the instantaneous counterfactual attribution at each step," not as a globally exact algebraic identity.
:::
...
All inverse-dynamics evaluations must use the exact same inertia, stiffness, and damping parameters as the forward simulation to ensure the recovered input matches the applied input and the configuration slice matches the ZVCF:

$$F_{\mathrm{total}} - F_{\mathrm{ZTCF}} = F_{\mathrm{input}}
  \quad\text{and}\quad
  F_{\mathrm{drift}} - F_{\mathrm{vel.drift}} = F_{\mathrm{ZVCF}}.$$
...
The theoretical framework is complete. It offers a mechanical decomposition of the golfer--club system that replaces the single "net moment" returned by inverse dynamics with a causally articulated account of how forces arise. The framework makes three testable predictions that would cause us to abandon or revise it. First, in simulation, the numerical identity $\tau_{\mathrm{total}} - \tau_{\mathrm{ZTCF}} \approx \tau_{\mathrm{ZVCF}}$ should hold within the bound derived from Gronwall's inequality applied to the drift flow. Second, trajectories with matching drift fields and different inputs should evolve into the same future state if integrated long enough, up to controllable directions in the sense of the Lie algebra rank condition. Third, when applied to experimental motion-capture data of a range of golfers, the Drift-Control Ratio should separate swings dominated by passive physics from swings dominated by muscular correction. That separation should match independent biomechanical markers of effort (EMG amplitudes, grip-force traces). Any of these predictions being violated on clean data would falsify the framework as stated. The next phase of the project leaves the page and the simulator to confront the noisy, variable reality of human motion capture.
