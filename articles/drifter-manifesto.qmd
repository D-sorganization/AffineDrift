...
The identity $\tau_{\mathrm{total}}(t_0) - \tau_{\mathrm{ZTCF}}(t_0) = \tau_{\mathrm{input}}(t_0)$ is **exact only at $t = t_0$** (the initial instant). After $t_0$, the ZTCF trajectory diverges from the actual trajectory — $F_{\mathrm{ZTCF}}(t)$ is evaluated along a *different* state path than $F_{\mathrm{total}}(t)$. The subtraction mixes forces from two trajectories at different states, which is not generally equal to $F_{\mathrm{input}}(t)$ at the *actual* trajectory state. The numerical near-equality observed in simulation is explained by the fact that the ZTCF trajectory remains close to the actual trajectory over short time windows, making the approximation tight. This is not a flaw in the method — it is inherent to counterfactual reasoning — but it means the result should be interpreted as "the instantaneous counterfactual attribution at each step," not as a globally exact algebraic identity.
:::
...
The theoretical framework is complete. It offers a mechanical decomposition of the golfer--club system that replaces the single "net moment" returned by inverse dynamics with a causally articulated account of how forces arise. The framework makes three testable predictions that would cause us to abandon or revise it: (i) in simulation, the numerical identity $\tau_{\mathrm{total}} - \tau_{\mathrm{ZTCF}} \approx \tau_{\mathrm{ZVCF}}$ should hold within the bound derived from Gronwall's inequality applied to the drift flow; (ii) trajectories with matching drift fields and different inputs should evolve into the same future state if integrated long enough, up to controllable directions in the sense of the Lie algebra rank condition; and (iii) when applied to experimental motion-capture data of a range of golfers, the Drift-Control Ratio should separate swings that are dominated by passive physics from swings that are dominated by muscular correction in a way that matches independent biomechanical markers of effort (EMG amplitudes, grip-force traces). Any of these predictions being violated on clean data would falsify the framework as stated. The next phase of the project leaves the page and the simulator to confront the noisy, variable reality of human motion capture.
