...
::: {.callout-important}
## Context: Parametric Counterfactuals
Because the "Stiffness Pulse" introduces time-varying impedance parameters during the impact window, a naive single-mode affine model would appear to violate **Drift Invariance**. As resolved in Section 8, we rigorously model this as a controlled hybrid automaton. Consequently, the ZTCF in this regime must be interpreted as a **Parametric Counterfactual** within the high-impedance mode ("What if continuous torque ceased but the discrete impedance schedule continued?") rather than a globally passive mechanical baseline. This preserves the mathematical validity of the ZTCF while acknowledging the active nature of the constraint collapse.
:::
