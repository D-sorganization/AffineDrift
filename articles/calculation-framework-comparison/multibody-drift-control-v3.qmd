...
description: "A unified tutorial showing how standard rigid-body dynamics frameworks expose a drift-control split under stated assumptions, with a planar double-pendulum worked example throughout."
author: "Dieter Olson"
date: "2026-03-06"
format:
  html:
...
What is more, this clean separation survives across several standard derivation languages. One can write the equations using energy methods, force balance on individual bodies, spatial screws, task-space coordinates, or differential-geometric notation. The notation and assumptions differ, but you can still recover the same bookkeeping split between passive dynamics and applied inputs within the stated model class.
...
But right now, right here, the ZTCF tells you exactly what the physics is contributing. And the difference between what is happening and the ZTCF tells you exactly what the controller is contributing.
...
Under these assumptions, the equations of motion have the control-affine structure shown in @eq-affine-master. Systems with nonsmooth contacts, actuator dynamics, flexible bodies, or hysteresis may require modified treatments that go beyond this article's scope.
...
Three kinds of pushes act on any mechanical system:
...
Here is a question that sounds simple but runs deep: "If I push on this joint, how much does it accelerate?"
...
The ZTCF is the drift evaluated at a specific moment in time:
...
So what appears as "drift" right now is the accumulated legacy of all past inputs, all past gravitational pulls, all past Coriolis interactions.
...
The new term $J_c^T\lambda$ is the **generalized constraint force**---the guardrail's push, expressed in generalized coordinates. The multiplier $\lambda$ tells you the *magnitude* of the push. You do not prescribe it---the requirement that the constraint equation hold determines it. The constraint force is *whatever it needs to be* to keep the system on the constraint surface.

## The Drift--Control Split Survives

Here is the important point: the drift--control decomposition **still works** when constraints are present. It operates within the constrained dynamics.
...
**A note on interpretation:** Per-coordinate power decompositions are useful diagnostic tools, but they depend on the choice of generalized coordinates. Only the total power balance is coordinate-invariant. Treat these decompositions as bookkeeping aids, not as absolute physical partitions.
...
The ZTCF is evaluated exactly as before: freeze the state and set actuation to zero. The resulting joint-level drift acceleration is the same number as from Euler--Lagrange; the screw formulation expresses the spatial accelerations and wrenches in its own language.
...
Every framework we have seen so far reasons about *joints*. Operational-space analysis asks a different question entirely: **"What does the world look like from the endpoint's perspective?"**
...
**An important subtlety:** We write $\Lambda$, $\mu$, and $p$ as if they are task-space primitives, but they are *induced from joint-space dynamics* through the Jacobian and mass matrix. They depend on the full joint configuration $q$, not just the task coordinates.
...
Here is a clear example of hidden energy transfer, and it happens in the humble double pendulum.
...
4. **Link 2 accelerates.** Its kinetic energy is increasing, and it can rise rapidly.
...
The river analogy is useful only if you keep its limits visible: current and paddling separate cleanly in the model, the train follows the modeled track, and the coordinate map should not change the physical prediction it represents.
