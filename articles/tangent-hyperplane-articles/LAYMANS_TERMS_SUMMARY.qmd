...
It is like solving many local problems while tracking the error that appears when you assemble those local pieces.
...
Explains what is exact about the tangent construction and what remains approximate once you use it over a finite step. Uses pictures and analogies to show:
...
**Takeaway:** Linearization is not a hack; it is the local first-order structure of smooth systems. Whenever you use it over a finite step, check it against residuals.
...
**Key point:** Even though each local effect is linear at its own moment, integrating over time produces nonlinear behavior. You can estimate or bound the residuals only after stating the required smoothness, curvature, and step-size assumptions.
...
**Nonlinear systems are only nonlinear globally.** At every instant, their first-order behavior is exactly linear. By exploiting this local linearity—linearize, optimize, move, repeat—we can control systems that seemed impossibly complex.
