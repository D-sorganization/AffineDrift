...
A critic could say: _"The authors claim linearization is exact, but obviously $f(x_0 + \delta x) \neq f(x_0) + A\delta x$ for finite $\delta x$. This is either sloppy terminology or conceptual confusion."_
...
_"You claim residuals are $O(\epsilon^2)$, but you never compute the constant. How do I know if $\epsilon = 0.1$ is 'small'? What if $C = 10^6$?"_
...
An optimization researcher will note: _"They claim quadratic convergence but don't mention the need for regularization or convexity. This is textbook material—why is it missing?"_
...
The text acknowledges this ("already linear...perfect test case") but a critic will say: _"Your first example doesn't use your framework. This undermines the claim that your method enables nonlinear control."_
...
A computational reviewer will ask: _"You claim 8 iterations, 12.4 N·s fuel—did you run this? Where's the code? Where are the plots?"_
...
A reviewer will ask: _"What's new here? EKF has been using tangent space linearization for 60 years."_
...
**After defense:** 4. Implement code repository (Critique 6) 5. Add "Related Work" section (Critique 7) 6. Extend case studies (Critique 5)
