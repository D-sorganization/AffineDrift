...
- **Primary sections of concern**:
  - [Chapter 3](Hybrid_Tangent_Spaces.html#sec-tangent-jumps): Saltation matrix derivation
  - [Chapter 2](Hybrid_Tangent_Spaces.html#sec-hybrid-automata): Measure-theoretic treatment
  - [Chapter 6](Hybrid_Tangent_Spaces.html#sec-variation-through-jumps): Zeno behavior
  - [Chapter 8](Hybrid_Tangent_Spaces.html#sec-contact-implicit): Complementarity constraints
  - [Chapter 10](Hybrid_Tangent_Spaces.html#sec-hs-implementation): JAX custom VJP claims
  - [Chapter 9](Hybrid_Tangent_Spaces.html#sec-hs-applications): Real system applicability
...
7. **Missing failure mode analysis** (when does this break?)
...
### Location
Chapter 3, equations [saltation matrix](Hybrid_Tangent_Spaces.html#eq-saltation-matrix) through [saltation general](Hybrid_Tangent_Spaces.html#eq-saltation-general)
...
- Kong et al. (2023): "Saltation Matrices: The Essential Tool for Linearizing Hybrid Dynamical Systems" provides a complete modern derivation and implementation-oriented discussion of the saltation formula
...
1. **Add explicit assumptions section** before equation [saltation matrix](Hybrid_Tangent_Spaces.html#eq-saltation-matrix):
...
1. **Replace vague appeals to measure theory with precise statements:**
   ...
   Before equation [hs hybrid cost jump](Hybrid_Tangent_Spaces.html#eq-hs-hybrid-cost-jump), add:
...
5. **Clarify when Filippov solutions are needed:**
...
The bouncing ball Zeno example is presented as if it **resolves** the mathematical difficulties, but it **illustrates why Zeno is hard**:
...
### Location
[Chapter 8](Hybrid_Tangent_Spaces.html#sec-contact-implicit), lines 2457-2598
...
**For roboticists:** Complementarity is the **central challenge** in contact simulation. Merely stating "enforce $\phi \lambda = 0$" is like saying "solve the Navier-Stokes equations" — technically correct, but useless without numerical methods.
...
6. **State when complementarity is solved exactly:**
...
1. **Custom VJP definition** (lines 2862-2904):
   ...
   - From equation [saltation matrix](Hybrid_Tangent_Spaces.html#eq-saltation-matrix): $S_j = P_j + (\text{correction term})$. The code uses `S` directly without computing the correction.
...
> This term is **included** in the saltation matrix $S_j$ (the correction term in equation [saltation matrix](Hybrid_Tangent_Spaces.html#eq-saltation-matrix)), so the VJP `g_minus = S.T @ g_plus` is correct **if S is the saltation matrix, not just the reset Jacobian**.

4. **Validate with a test case:**

> **Validation:** We implemented the bouncing ball system and compared gradients:
> - **Analytical:** Using equation [ball saltation](Hybrid_Tangent_Spaces.html#eq-ball-saltation), $\frac{\partial v_{\text{final}}}{\partial v_0} = (-e)^N$ (for $N$ bounces)
...
### Location
[Chapter 9](Hybrid_Tangent_Spaces.html#sec-hs-applications), lines 2600-2792
...
1. **Add a "Relationship to Prior Work" section** before the Conclusion:
   ...
   >
   > Our treatment (Chapter 7) emphasizes the **backward pass through jumps** (adjoint jump condition, equation [value gradient jump](Hybrid_Tangent_Spaces.html#eq-value-gradient-jump)) as a geometric consequence of the tangent space perspective.
   >
...
2. **Cite specific results where claimed:**
   ...
   > This formula is derived carefully in the hybrid-systems sensitivity literature. We provide a sketch here for completeness; see @Kong2023saltation for a modern derivation and implementation discussion.
    ...
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          ...
          <h3>Measure Theory Limitations in Hybrid Environments</h3>
        </div>
        <p class="critic-argument">While the attempt to provide measure-theoretic grounding (claiming jump sets as measure-zero) handles simpler cases effectively, rigid assumptions about bounded variation break down entirely for systems with non-trivial chattering, singular reset maps, and Zeno behaviors without straightforward closure paths. Bounding jump contribution costs strictly to jump duration neglects impulsive costs intrinsic to real-world transitions (like cumulative energy loss in stochastic, rather than isolated, impacts), undermining generalization claims.</p>
        <div class="author-response">
          <strong>Our Response:</strong> This is a fair concern; our assumptions inherently model simple, isolated impacts correctly but over-index on ideal environments. More extensive formal theorems handling infinite-cost scenarios without arbitrary bounded variation closures should be explored in future extensions to maintain accuracy in generalized cases.
        </div>
      </div>
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          <span class="critic-label">Alternative View:</span>
          <h3>Assumed Generalization of Saltation Matrices</h3>
        </div>
        <p class="critic-argument">The derivation of the saltation matrix depends fundamentally on scalar, affine guard functions mapping smoothly separated regions. Critics from non-smooth control theory would argue the model inappropriately glosses over multi-constraint configurations (e.g., intersection states requiring pseudoinverse techniques) and configuration-dependent guards exhibiting curvature, leaving practitioners implementing these equations for complex robots with unpredictable sensitivity issues.</p>
        <div class="author-response">
          <strong>Our Response:</strong> We acknowledge this limitation. We've structured the mathematical foundation for single-constraint, basic affine systems specifically for pedagogical accessibility, but bridging the framework to robust multibody system applications requires defining explicit constraints on curvature and transversality, which are missing here.
        </div>
      </div>
      ...
      <div class="critic-item">
        <div class="critic-perspective">
          <span class="critic-label">Alternative View:</span>
          <h3>Applicability vs Theoretical Optimism</h3>
        </div>
        <p class="critic-argument">Practical implementations in the paper rely heavily on custom unverified gradients within JAX and generalizations from over-simplified systems (like bouncing balls) to infer usefulness in high-degree-of-freedom contexts. Observers highlight the absence of empirical proofs mapping optimal bounds from idealized states to dynamic physical robots where parameter uncertainty is prevalent.</p>
        <div class="author-response">
          <strong>Our Response:</strong> True. Presenting unverified or conceptual autodiff code can be construed as preemptive. The framework establishes a structural template, but demands significant real-world convergence plotting and baseline benchmarking before substantiating robust physical implementation claims.
        </div>
      </div>
