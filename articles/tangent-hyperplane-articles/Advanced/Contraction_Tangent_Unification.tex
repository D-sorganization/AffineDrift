...
For infinite-horizon LTI LQR with \(N=0\), standard stabilizability and
detectability assumptions with \(Q\succeq0\), \(R\succ0\) yield a
stabilizing positive-semidefinite solution. Positive definiteness and a
strict decay bound in that particular matrix require their own checks.
For example, \(A=-1\), \(B=1\), \(Q=0\), \(R=1\) has optimal cost
\(S=0\) and a stable uncontrolled plant, but zero is not a Riemannian
metric. With \(Q\succ0\) the usual stabilizing solution is positive
definite. The \href{https://underactuated.mit.edu/lqr.html}{MIT LQR
treatment} develops the finite- and infinite-horizon optimization
problems.
