...
**"Residuals from simplified plans are not just errors to minimize; they are diagnostic signals that flag when a local model may need smaller steps, a richer controller, or new validation."**
...
The **residual** is the drift: the difference between where the simplified (linear) model says you should be and where the complex (nonlinear) reality puts you.
...
**Illustrative simulation summary:** ATRIAS robot (60kg, 1.2m tall)
- Fixed controller: 15% fall rate (3 out of 20 trials)
- Residual-aware controller: 5% fall rate (1 out of 20 trials)

**Analogy:** It's like a driver who notices "the steering wheel is shaking way more than it should" and immediately pulls over. The other driver ignores the warning signs until they're in a ditch.
...
**Residual-aware approach:** A validated threshold can let the simple controller run most of the time, leaving the complex controller for high-residual phases only. The average cost can then be lower than running the complex controller everywhere. The actual speedup is task- and implementation-dependent.
...
**New:** "Error ≤ (M/2) × δx² × Δt" as a conditional upper bound, once you have established a valid curvature constant M
...
- State tracking-accuracy bounds only after verifying the assumptions behind them
