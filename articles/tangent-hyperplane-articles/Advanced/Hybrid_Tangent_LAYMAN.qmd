...
**"Many real-world systems flow smoothly for a while, then jump, collide, or switch modes; tangent-space reasoning can still help, but only when the jump conditions and mode changes are modeled explicitly."**

---

## The Problem With Smoothness

Remember the main tangent space idea? A smooth system has a linear tangent description at each instant, like a winding road that looks nearly flat if you zoom in close enough.
...
In a hypothetical hybrid-DDP study, the optimizer would test how pre-impact torque, clubhead speed, shaft vibration, and impact restitution interact. A claim that slight pre-impact deceleration improves the outcome would need the objective, model parameters, and validation plots before it should be treated as a golf result.

What the hybrid model adds is not a guaranteed coaching conclusion; it is the ability to represent the impact discontinuity instead of smoothing it away.
...
- **Reset:** Velocity jumps according to the soft-capture mechanism (like a squishy collision)
...
**Result:** NASA's trajectory planners use hybrid models for rendezvous and docking, where the approach must be gentle and the capture reliable.
...
**Result:** A validated hybrid machining model can represent the switch from free motion to cutting contact and can be used to study feed-rate and vibration tradeoffs. Claims about production CNC controllers or safe operating limits require machine-specific evidence.
...
If the eigenvalues of $S$ are less than 1 (dissipative impact), then $S^5$ is small—perturbations die out quickly. The ball's trajectory is stable.
...
**Idea:** Instead of a discontinuous jump, replace the impact with a stiff spring:

$$F_{contact}(x) = \begin{cases} 0 & \text{if } x > 0 \\ -k \cdot x & \text{if } x \leq 0 \end{cases}, \quad k \to \infty$$

With $k$ large, this approximates a hard collision.
...
**Pros:**
- Can optimize contact timings instead of requiring every jump time to be pre-specified
...
- You need local convergence or stability certificates and can state the assumptions behind them
...
**"Smooth nonlinear systems have linear tangent descriptions at each instant."**
...
- **Critical review:** See [Critiques and Responses](../../../critiques/index.html) for limitations and edge cases
