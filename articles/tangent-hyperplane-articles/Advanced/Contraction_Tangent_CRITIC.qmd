...
description: "Formal critique of Contraction Theory Meets Tangent Spaces — Unifying Stability and Optimality."
bibliography: ../references.bib
---

::: {.callout-note}
## Revision Status: September 5, 2026

This critique records the earlier reference and its proposed repairs. The [revised reference](Contraction_Tangent_Unification.html) now replaces the incorrect metric, Riccati, task-space, hybrid, and implementation arguments with explicit derivations and counterexamples. Historical quotations below should be checked against the [pre-correction source](https://github.com/D-sorganization/AffineDrift/blob/c270ea67fa94c96e4d1a6c441bc5965d3a75a4a4/articles/tangent-hyperplane-articles/Advanced/Contraction_Tangent_Unification.qmd).

Some suggested repairs in this critique also require correction: endpoint inertia alone is not a metric for the full position/velocity error state; critical damping does not certify an exact rate from stiffness alone; and a finite-horizon Riccati solution needs uniform definiteness and a closed-loop inequality. The revised reference gives the applicable tests. This historical critique has not received a complete independent technical clearance.
:::
...
This article claims a close relationship between contraction theory and tangent-space methods, asserting that the Riccati solution can serve as both a contraction metric and a value-function object. The mathematical observations are often useful in restricted settings, but the article needs clearer conditions:
...
## Location
- **Equation ([differential riccati](Contraction_Tangent_Unification.html#eq-differential-riccati))**: Differential Riccati Equation
...
The contraction condition ([generalized jacobian](Contraction_Tangent_Unification.html#eq-generalized-jacobian)) for a time-varying metric $\mathbf{M}(t)$ is:
$$
...
The article claims (page 10, after [differential riccati](Contraction_Tangent_Unification.html#eq-differential-riccati)):
...
1. **([tangent bundle dynamics](Contraction_Tangent_Unification.html#eq-tangent-bundle-dynamics))**: Writes dynamics as $\frac{D}{dt} \delta \mathbf{x} = \frac{\partial \mathbf{f}}{\partial \mathbf{x}} \delta \mathbf{x}$.
...
2. **([metric tensor](Contraction_Tangent_Unification.html#eq-metric-tensor))**: Defines metric as $g_{\mathbf{x}}(\delta \mathbf{x}_1, \delta \mathbf{x}_2) = \delta \mathbf{x}_1^\top \mathbf{M}(\mathbf{x}) \delta \mathbf{x}_2$.
...
3. **Contraction condition ([contraction lmi](Contraction_Tangent_Unification.html#eq-contraction-lmi))**: $\mathbf{A}^\top \mathbf{M} + \mathbf{M} \mathbf{A} \preceq -2\lambda \mathbf{M}$.
...
4. **([christoffel](Contraction_Tangent_Unification.html#eq-christoffel))**: Writes Christoffel symbols $\Gamma^i_{jk}$.
...
This section correctly describes pullback metrics ([pullback metric](Contraction_Tangent_Unification.html#eq-pullback-metric)):
$$
...
### The Stochastic Riccati Equation (Equation “Eq-Stochastic-Riccati”)
...
The "proof sketch":
> "The penalty forces $\mathcal{C}(\mathbf{x}^*_t, \mathbf{u}^*_t) \to 0$ as $\mu \to \infty$. By continuity, there exists a neighborhood where ([contraction condition](Contraction_Tangent_Unification.html#eq-contraction-condition)) holds."
...
Claims:
> "If there exists a common metric $\mathbf{M}$ satisfying ([common lyapunov](Contraction_Tangent_Unification.html#eq-common-lyapunov)), then the hybrid system is exponentially stable across mode switches."
...
> For **regulation** to a fixed setpoint $\mathbf{x}_d$ (constant), the controller ([task force](Contraction_Tangent_Unification.html#eq-task-force)) renders the error dynamics contracting with metric $\mathbf{M} = \mathbf{\Lambda}(\mathbf{x}_d)$ (evaluated at setpoint) and rate $\lambda = \sqrt{\lambda_{\min}(\mathbf{K}_p) / \lambda_{\max}(\mathbf{\Lambda})}$.

**Option B** (remove for trajectory tracking):
> **Remark**: For **trajectory tracking** with time-varying $\mathbf{x}_d(t)$, the inertia matrix $\mathbf{\Lambda}(\mathbf{x}(t))$ varies along the trajectory. Rigorous contraction analysis requires computing a time-varying metric $\mathbf{M}(t) = \mathbf{\Lambda}(\mathbf{x}(t))$ and verifying the generalized contraction condition ([generalized jacobian](Contraction_Tangent_Unification.html#eq-generalized-jacobian)). This is left to future work.
...
1. Any metric $\mathbf{M}$ satisfying the contraction condition ([linear contraction](Contraction_Tangent_Unification.html#eq-linear-contraction)).
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        This critique highlights places where a useful local control argument is stated too broadly or without the needed assumptions.
      </p>

      <div class="laymans-item">
        <h3>A One-Way Street, Not a Two-Way Street</h3>
        <p>The original paper presents optimality and stability as if they were interchangeable. The safer statement is that some optimal-control calculations can produce local stability certificates, while the reverse direction is generally non-unique.</p>
        <div class="analogy">
<strong>Think of it like:</strong> wearing a seatbelt. All good drivers wear seatbelts, but just wearing a seatbelt doesn't automatically make you a good driver.
        </div>
      </div>

      <div class="laymans-item">
        <h3>Changing the Rules Mid-Game</h3>
        <p>The author tries to prove safety by measuring how things change over time using a specific mathematical tool. The problem is that the tool itself keeps changing shape, making it impossible to guarantee anything long-term without stricter rules.</p>
        <div class="analogy">
<strong>Think of it like:</strong> trying to build a stable house using a tape measure made of elastic. If your measuring tool keeps stretching, you can't guarantee the walls will stay straight.
        </div>
      </div>

      <div class="laymans-item">
        <h3>Humans Aren't Perfect Robots</h3>
        <p>The paper treats human movement as if a stability-focused control model were sufficient. Real movement also reflects physiology, fatigue, sensory delay, learning history, comfort, and task strategy.</p>
        <div class="analogy">
<strong>Think of it like:</strong> assuming every commuter will take the exact mathematically shortest route to work. In reality, people take the scenic route, avoid bad intersections, or just stick to the path they know best.
        </div>
      </div>

      <div class="laymans-item">
        <h3>Missing the Fine Print</h3>
        <p>The article uses language associated with certified safety, but the proof obligations are not fully documented. A safety claim needs explicit assumptions, residual bounds, basin estimates, and independent verification.</p>
        <div class="analogy">
<strong>Think of it like:</strong> a car salesman promising a vehicle is completely crash-proof but refusing to show you the safety crash test results. You can't just take their word for it when safety is on the line.
        </div>
      </div>

      <div class="key-takeaway">
        <strong>Key Takeaway:</strong> The connection between efficiency and local stability can be useful, but broad safety claims need complete proofs and system-specific evidence.
      </div>
    </div>
  </div>
</section>
  ...
  <div class="critics-comments-content">
    <div class="critics-comments-inner">
      <p class="critics-intro">
        These alternative views identify where the critique itself should be stated carefully:
      </p>

      <div class="critic-item">
        <div class="critic-perspective">
<span class="critic-label">Alternative View:</span>
<h3>The Pragmatic Value of the "Duality" Heuristic</h3>
        </div>
        <p class="critic-argument">The critique that the stability-optimality mapping is strictly unidirectional is mathematically important, but the relationship can still be useful as an engineering heuristic. Riccati-like structures can guide metric searches and controller design even when they do not establish a formal equivalence. The key is to label the result as a heuristic or sufficient-condition argument, not as a bidirectional theorem.</p>
        <div class="author-response">
<strong>Our Response:</strong> Agreed. Riccati-like structures are useful for numerical metric searches, but the article should distinguish a design heuristic from a proved equivalence. Reframing the claim as a sufficient local construction preserves the method's utility without overstating the theorem.
        </div>
      </div>
      ...
      <div class="critic-item">
        <div class="critic-perspective">
<span class="critic-label">Alternative View:</span>
<h3>Local Stability is Sufficient for Receding Horizon Methods</h3>
        </div>
        <p class="critic-argument">The objection regarding time-varying metrics and the Differential Riccati Equation can be too strong if it demands global, infinite-horizon guarantees from a finite-horizon method. In a receding-horizon setting, local tubes, recursive feasibility, and monitored metric conditioning may be the relevant proof objects.</p>
        <div class="author-response">
<strong>Our Response:</strong> This is a valid limitation on the critique. The issue is not that finite-horizon or receding-horizon methods are invalid; it is that the article must state the required recursive-feasibility, tube, and conditioning assumptions if it wants to claim a certified local guarantee.
        </div>
      </div>

      <div class="academic-note">
        <strong>Note:</strong> These comments narrow both the article's claims and this critique's claims.
      </div>
    </div>
  </div>
</section>
