...
1. **Overstated duality claims**: The "stability ↔ optimality duality" is presented as bidirectional and universal, when it is unidirectional (LQR → contraction) and limited to specific problem classes.
...
**Severity**: High. Core claims exceed what the mathematics establishes.
...
- **Answer**: Not in general! The contraction condition is a **Lyapunov inequality**, which has infinitely many solutions. Only a narrow subset corresponds to Riccati solutions.
...
This is a **narrow relationship** between $\mathbf{Q}$, $\mathbf{R}$, $\mathbf{M}$, and $\lambda$, not a general identity.
...
- **Time-varying metric** $\mathbf{M}(t)$: The "distance" $\|\delta \mathbf{x}(t)\|_{\mathbf{M}(t)}^2$ changes meaning over time. A trajectory could be "contracting" in this metric while diverging in Euclidean norm!
...
3. **Practical design**: Engineers cannot use time-varying metrics without tracking the condition number $\kappa(M(t)) = \lambda_{\max} / \lambda_{\min}$. If $\kappa(M(t)) \to \infty$, the analysis breaks down.
...
- A well-tuned PD can achieve low tracking error on smooth trajectories.
