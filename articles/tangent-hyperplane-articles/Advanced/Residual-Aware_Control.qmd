...
description: "Core Thesis: Residuals in tangent space linearization are not approximation errors to be minimized—they are geometric signals encoding system curvature..."
...
4. **Simulation studies** on quadrotor aerobatics, humanoid walking, and golf swing optimization (experimental validation on physical hardware is a direction for future work)
...
$$
\|H_f\|_{\max} = \frac{g}{L} \approx 9.8 \text{ s}^{-2} \quad \text{(for } L = 1 \text{ m)}
$$ {#eq-pendulum-bound}
...
The residual is **0.0049 rad** (≈ 0.28°), which is indeed second-order in the 0.1 rad perturbation.
...
- **Highly curved** (e.g., pendulum near $\theta = \pi/2$, where the Hessian $H_{f} = -(g/L)\sin\theta$ is maximal): Large $\|H_f\|$ → residuals grow rapidly. (Distinct from the *linearization-instability* region near $\theta = \pi$, the inverted equilibrium, where the curvature magnitude is actually zero.)
:::
...
Standard DDP uses a fixed timestep $\Delta t$. But from @eq-rac-residual-bound, we know residuals depend on:
...
**Derivation:** From @eq-rac-residual-bound with constant $\delta x$ over $\Delta t$:
...
2. **Adaptive rule**: The timestep is updated from local curvature and perturbation estimates rather than kept fixed
3. **Nodewise residual target**: If the Hessian bound and perturbation estimates are valid, the construction targets $\|r(t_i)\| \leq \epsilon_r$ at the sampled nodes

### Convergence Status and Assumptions

::: {.callout-warning}
## Status of the Adaptive DDP Claim

This article does **not** prove a new convergence theorem for Adaptive DDP. The intended claim is narrower. Suppose the residual estimator is valid, the regridded discretization stays inside the regime covered by standard DDP assumptions, and the solver tolerances are handled carefully. Under those conditions one expects behavior similar to standard DDP on a better-chosen mesh. Establishing that result rigorously would require a dedicated proof that is outside the scope of this article.
:::

**Practical reading:** The adaptive rule should be treated as an algorithmic proposal motivated by residual control, not as a proved convergence result in its current form.
...
For a moderate 12-state example with a short horizon, the LQR update is typically much cheaper than repeatedly solving a nonlinear MPC problem. The actual timing ratio depends on horizon length, solver choice, warm starts, hardware, and implementation details.

**Potential adaptive benefit:** Concentrate nonlinear-optimization effort in the high-residual phases and use cheaper linear feedback elsewhere. Quantifying any speedup requires benchmark data for the specific controller implementation.
...
**Key idea:** Set $\delta_{\max}(t)$ based on residual bound @eq-rac-residual-bound:
...
### Tube-MPC Implication Under a Valid Bound

::: {.callout-note}
## Standard Tube-MPC Logic With a Residual-Derived Radius

Suppose the nominal trajectory satisfies the tightened constraints, and the feedback controller does in fact maintain $\|x(t) - \bar{x}(t)\| \leq \delta_{\max}(t)$. The usual tube-MPC containment argument then implies that the true trajectory remains inside the original constraint set $x(t) \in \mathcal{X}$.
:::

**What is new here:** not the logic of tube MPC itself, but the proposal to size $\delta_{\max}(t)$ from residual-derived quantities. The burden of proof therefore shifts to whether those residual-derived bounds are valid for the system and operating region at hand.

**Possible advantage over standard tube MPC:** If the residual-derived tube radius is materially smaller than a worst-case disturbance envelope, the resulting tightened constraints may be less conservative. Whether that happens is system- and validation-dependent.
...
**Interpretation of this example:** On this simplified flip setup, the adaptive grid uses fewer nodes than a uniformly fine discretization while targeting the same nodewise residual threshold. The exact savings depend on the bound estimator, solver tolerances, and implementation.
...
$$
\Delta \dot{x} = -M^{-1} J^T (J M^{-1} J^T)^{-1} J \dot{x}^-
$$ {#eq-impact-map}

where $J$ is the contact Jacobian and $M$ is the mass matrix. (Equivalently $\dot{x}^+ = (I - M^{-1} J^T (J M^{-1} J^T)^{-1} J)\,\dot{x}^-$, the projection that zeroes the contact-normal velocity.)
...
**Illustrative simulation summary:**
...
**Interpretation:** In this simulated controller-switching example, residual monitoring acts as an early warning signal for departures from the nominal smooth model. That signal can then trigger a different control mode. The table is scenario-specific and should not be read as a benchmark guarantee.
...
**Interpretation:** In this worked example, the adaptive grid stays close to the fine-grid objective while using fewer nodes. The objective gap and timing savings are example-specific rather than universal performance guarantees.
...
**Illustrative output from one run:**
```
...
✅ You need explicit bounds or diagnostics on linearization accuracy
...
❌ You have **unlimited computation** and can afford fine uniform discretization
...
This article developed a framework for **residual-aware control**, transforming linearization residuals from "errors to minimize" into "signals to exploit."

**Key results:**

1. **Quantitative residual bounds** (@eq-rac-residual-bound): $\|r\| \leq \frac{M}{2} \int \|\delta x\|^2 dt$
2. **Adaptive algorithms**:
   - Curvature-dependent timestep DDP (Algorithm 2.1)
   - Residual-triggered LQR ↔ MPC switching
   - Geometric tube MPC (Algorithm 2.2)
3. **Worked examples and simulations**: quadrotor, humanoid, and golf-motivated scenarios showing how residual estimates can inform grid selection, controller switching, and trajectory optimization

**Impact:**

Residual-aware methods offer a way to organize local error budgeting in nonlinear control. They do not, by themselves, establish real-time feasibility or closed-loop guarantees; those require implementation-specific benchmarks, validated bounds, and in some cases additional proofs.
