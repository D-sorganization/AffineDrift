...
This article does **not** prove a new convergence theorem for Adaptive DDP. The intended claim is narrower: if the residual estimator is valid, the regridded discretization remains inside the regime covered by standard DDP assumptions, and the solver tolerances are handled carefully, one expects behavior similar to standard DDP on a better-chosen mesh. Establishing that result rigorously would require a dedicated proof that is outside the scope of this article.
:::
...
If the nominal trajectory satisfies tightened constraints and $\|x(t) - \bar{x}(t)\| \leq \delta_{\max}(t)$ is in fact maintained by the feedback controller, then the usual tube-MPC containment argument implies that the true trajectory remains inside the original constraint set $x(t) \in \mathcal{X}$.
:::
...
**Interpretation:** In this simulated controller-switching example, residual monitoring acts as an early warning signal for departures from the nominal smooth model and can be used to trigger a different control mode. The table is scenario-specific and should not be read as a benchmark guarantee.
...
3. **Worked examples and simulations**: quadrotor, humanoid, and golf-motivated scenarios showing how residual estimates can be used in grid selection, controller switching, and trajectory optimization
