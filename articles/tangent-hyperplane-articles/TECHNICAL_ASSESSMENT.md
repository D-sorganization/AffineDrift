...
- "Tangent hyperplane" is slightly non-standard; "tangent space" is more common
  - _Justification_: Hyperplane emphasizes the affine structure, acceptable but unusual
- "Exact superposition in the infinitesimal" could be misread as implying finite-time exactness
  - _Clarification needed_: Emphasis should be "exact in the limit $\delta x \to 0$"

**Scope Claims:**

- Claim that LQR "captures exact infinitesimal dynamics" is true but could be misinterpreted
  - _Clarification_: LQR solves the exact local problem; global performance depends on residuals
- "Linearization is not an approximation" is provocative but potentially confusing
  - _Clarification_: It's not an approximation _of the derivative_; it is an approximation _of the function_
