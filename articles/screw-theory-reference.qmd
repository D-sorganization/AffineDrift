...
::: {.callout-note}
## Frame Convention
All 6-vectors (twists $\mathcal{V}$, wrenches $\mathcal{F}$) and matrices ($J$, $B$) in this article are expressed in the **space frame** (fixed world frame $s$) unless explicitly subscripted $_b$ for the body frame. Body-frame quantities transform via the adjoint: $\mathcal{V}_s = \mathrm{Ad}_{T_{sb}} \mathcal{V}_b$ and $\mathcal{F}_s = \mathrm{Ad}_{T_{sb}}^\top \mathcal{F}_b$.
:::
...
::: {.callout-note}
## Spatial Twist Convention
This article adopts the **spatial (space-frame) twist** convention of Lynch and Park, *Modern Robotics* [@lynch2017modern, §3.3.2]. A spatial twist $\mathcal{V}_s \in \mathbb{R}^6$ is expressed in the fixed world frame $s$, not in the moving body frame. Body-frame twists $\mathcal{V}_b$ relate to spatial twists by $\mathcal{V}_s = \mathrm{Ad}_{T_{sb}}\,\mathcal{V}_b$, where $T_{sb} \in SE(3)$ is the body configuration relative to the space frame.
:::

A (spatial) twist is a 6-vector
...
representing the angular velocity $\omega$ and the linear velocity $v$ of the origin of the body frame, both expressed in the space frame. For a reference point $q \in \mathbb{R}^3$ on the instantaneous screw axis expressed in the space frame, the no-slip condition gives

$$
v = -\omega \times q.
$$

This is the **spatial twist identity** in the Lynch--Park convention [@lynch2017modern, §3.3.2] and yields the Plücker representation of screws.
...
Joint torques $\tau \in \mathbb{R}^n$ and task-space wrenches $\mathcal{F}_{\mathrm{input}} \in \mathbb{R}^6$ are related by the screw Jacobian $J_s(q) \in \mathbb{R}^{6 \times n}$ via the virtual-work duality (pull-back):

$$
\tau = J_s(q)^T \mathcal{F}_{\mathrm{input}}.
$$

Because the system is kinematically redundant ($n \gg 6$), the Jacobian $J_s$ is wide and $J_s^{-T}$ is **undefined**. Recovery of a wrench estimate from measured joint torques requires the pseudoinverse:

$$
\mathcal{F}_{\mathrm{est}}
=
\bigl(J_s J_s^T\bigr)^{-1} J_s\, \tau
=
\bigl(J_s J_s^T\bigr)^{-1} J_s\, B u,
$$

where $B$ is the input distribution matrix (expressed in the **space frame**, following the convention fixed in the introduction) and $u$ is the vector of joint inputs. A body-frame input distribution $B_b$ --- natural for grip-force measurements taken in the hand frame --- relates to the spatial form as $B_s = \mathrm{Ad}_{T_{sb}}\, B_b$. The total wrench at a given point is then
...
In the pull-back form, the affine decomposition in joint space reads $\tau = J_s^T \mathcal{F}_{\mathrm{drift}} + J_s^T \mathcal{F}_{\mathrm{input}}$, making explicit that each wrench component contributes to joint torque through the transpose Jacobian. This reproduces the affine decomposition: passive drift wrenches plus active input wrenches.
...
In the Zero Velocity Counterfactual (ZVCF), velocity is set to zero while configuration and internal state are held at their current values. Only the configuration-dependent subset of drift wrenches survives; velocity-dependent terms vanish:

$$
\mathcal{F}_{\mathrm{ZVCF}}(t)
=
\mathcal{F}_{\mathrm{drift}}\!\bigl(q(t),\, \eta(t),\, 0,\, 0\bigr)
=
\mathcal{F}_{g}(q)
+
\mathcal{F}_{\mathrm{shaft}}(q, \eta)
+
\mathcal{F}_{\mathrm{config}}(q).
$$

Crucially, $\mathcal{F}_{\mathrm{ZVCF}} \subseteq \mathcal{F}_{\mathrm{drift}}$: it is a subset of drift, not a form of input wrench. $\mathcal{F}_{\mathrm{input}}$ does **not** appear in the ZVCF. These constructions mirror the affine decomposition in screw space.
...
- **[Notation and Conventions](/pages/notation-conventions.html)** — Twist/wrench stacking convention and adjoint notation adopted here
- **[Lagrangian Reference](lagrangian-reference.html)** — Companion reference for the energy-based derivation of the same equations of motion
- **[Rotation Representations Reference](rotation-representations-reference.html)** — Exponential coordinates and quaternion conversions for $SO(3)$
- **[Force and Mobility Matrices](force-mobility-matrices.html)** — Wrench-space view of directional force transmission in the golf swing
- **[Null Space of the Constraint Jacobian](null-space-constraint-jacobian.html)** — Null-space projections complementing the twist-space decomposition
- **[Rotation Converter](rotation-converter.html)** — Interactive tool implementing the rotation conversions this reference cross-indexes
:::
