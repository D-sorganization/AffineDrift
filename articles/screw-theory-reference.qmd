...
where $B$ is the input distribution matrix, $u$ is the vector of joint inputs, and $(J_s^\top)^+$ denotes the Moore-Penrose pseudoinverse of the transpose of the screw Jacobian. This pseudoinverse is necessary because $J_s$ is generally a $6 \times n$ matrix and is not square for systems with $n \neq 6$ (redundant or underactuated). The pseudoinverse provides the minimum-norm wrench that satisfies the fundamental duality relationship $\tau = J_s^\top \mathcal{F}_{\mathrm{input}}$. The total wrench at a given point is then
...
In the Zero Velocity Counterfactual (ZVCF), all velocities (twists) are set to zero, exposing purely configuration-dependent passive forces (such as gravity and shaft elasticity) within the drift:

$$
\mathcal{F}_{\mathrm{ZVCF}} = \left. \mathcal{F}_{\mathrm{drift}} \right|_{\mathcal{V}=0}.
$$
