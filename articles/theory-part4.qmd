  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        Part 4 is the "engine room" of the theory. It contains the raw mathematics that prove the ideas in Parts 1–3 are solid.
      </p>
      ...
      <div class="laymans-item">
        ...
        <div class="analogy">
          <strong>Analogy:</strong> If Parts 1–3 were the user manual for a car, Part 4 is the technical blueprint the engineers used to build it. You don't need to read it to drive, but it's proof that the car won't fall apart.
        </div>
      </div>
...
The same affine structure extends to the spatial (3D) case: the Coriolis and gyroscopic terms become more complex, but the torque input still appears linearly. The detailed 3D derivation is deferred to future work.
...
**Superposition still holds at a fixed state.** Even though the dynamics are now richer and nonlinear in $(\theta,\eta)$, the torque input still appears linearly as $A_{\mathrm{input}}(x)\,u$. The drift term contains all flexible-shaft effects; evaluated at the same state $x$, two inputs $u_1,u_2$ produce accelerations that differ by the same linear map $A_{\mathrm{input}}(x)$ applied to the difference in inputs. Whole trajectories do not superpose this way: each input carries the system to a different state, where the drift term itself takes a different value.

::: {.callout-tip}
## Worked 2-DOF Example: Concrete Numbers for Rigid + Flexible-Mode Coupling
To make the block structure reproducible, fix representative parameters: rod length $L = 1.1$ m, rod/hand mass $m_h = 0.40$ kg concentrated at the wrist, clubhead mass $m_c = 0.200$ kg at the tip. A single first-bending mode carries modal mass $m_\eta = 0.10$ kg, modal stiffness $k_\eta = 2500$ N/m (first-mode frequency $f_1 = \tfrac{1}{2\pi}\sqrt{k_\eta/m_\eta} \approx 25.2$ Hz --- typical for a steel iron shaft), modal damping $c_\eta = 0.8$ N·s/m, and shape function $\Phi(L) = 1.0$ at the clubhead.
...
**Part 5** addresses this practical challenge [@hairer1996solving]. It documents the Simulink forward-dynamics implementation used to validate these concepts, serving as a numerical proof-of-concept that bridges the gap between the derivations of Part 4 and the empirical reality of the experimental applications.
