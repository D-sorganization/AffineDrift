...
To make this concrete, consider a snapshot during mid-downswing. Inverse dynamics computed from motion capture yields a total torque magnitude of $\tau_{\text{total}} = 100 \text{ Nm}$ at the lead shoulder. The drift torque—evaluated from the passive terms at the actual state $x(t)$—is $\tau_{\text{drift}} = 85 \text{ Nm}$. The input torque is therefore:
