...
Let $\pi: T\mathcal{Q} \to \mathcal{Q}$ be the canonical bundle projection that maps a state vector $(q, \dot{q})$ to its base configuration $q$. Let $\zeta_0: \mathcal{Q} \to T\mathcal{Q}$ be the *zero section* of the tangent bundle, which embeds the configuration manifold into the state space as the locus of zero velocities: $\zeta_0(q) = (q, 0)$.

The *Zero Velocity Counterfactual state* is formally defined by the composition of these operators:

$$
  x^{\mathrm{ZVCF}}(t) = (\zeta_0 \circ \pi)(x(t)) = \zeta_0(q(t), \eta(t)) = (q(t), \eta(t), 0, 0).
$$

Only the configuration variables $q(t)$ and $\eta(t)$ are retained; all generalized velocities are set to zero. This geometric operation projects the system state onto the submanifold of static states.
