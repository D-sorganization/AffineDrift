  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        Part 1 showed us <em>how</em> the swing moves. Part 2 asks <em>why</em> it moves, introducing tools to separate the golfer's effort from physics.
      </p>
      ...
      <div class="laymans-item">
        ...
        <div class="key-takeaway">
          At any one instant, comparing the forces in the actual swing with these two baselines (Drift and Static Load) shows how much of the load at that moment comes from the golfer's own effort to bend the shaft or speed up the club. That comparison holds instant by instant, at one shared state; it does not split a whole swing into a drift swing plus a golfer's swing.
        </div>
      </div>
...
Recall the control-affine form $\dot{x} = f(x) + g(x)u$ established in Part 1, where $f$ encodes drift and $g$ encodes control directions [@isidori1995nonlinear]. In Part 2, we pivot from description to **diagnostics**. We seek to transform this equation of motion into a causal map that can attribute specific features of the swing trajectory to either the golfer's intent or the club's inertia.

The additive separability derived in Part 1 enables that shift: the velocity-dependent drift is structurally distinct from the active control of the player. We now exploit this structure to answer the central causal question of the swing: *which forces are the result of the golfer's immediate effort, and which are the result of the system's passive physics?*
...
To operationalize this decomposition, we cannot look only at the equations; we must observe the behavior they dictate. The drift field $f(x)$ is not static; it describes a flow. To understand the burden of "passive dynamics," we must follow this flow over time. It is not enough to know the instantaneous passive force vector; we must integrate that vector to see where it takes the club. We need a counterfactual history—a timeline of what *would have happened* if the golfer had ceased to intervene. This logic compels us to move from the tangent space (velocities/forces) to the integral curves (trajectories).
...
By comparing total forces to these counterfactual constructions via inverse dynamics, we can separate drift and input forces [@nesbit2005three; @koike2019dynamic]. This process moves us from the instantaneous vector fields of Part 1 to the temporal domain of Part 2. It allows us to simulate the **counterfactual trajectories**—the unobserved swings that *would* have occurred had the golfer chosen differently—exposing the contribution of inertial coupling to the observed motion.

## Zero Torque Counterfactual (ZTCF)

::: {.callout-note}
## Canonical ZTCF Convention (This Part)
This part uses the **Branched ZTCF**: the solution of $\dot x = f(x)$ initialized from the *observed* swing state at a branch time $t_0$, and compared against the observed swing for $t > t_0$. The full glossary (Trajectory ZTCF, drift vector $f(x)$ which is **not** ZTCF, and Branched ZTCF) is in the standalone [Zero-Torque Counterfactual](zero-torque-counterfactual.html#sec-ztcf-canonical) article.
:::
...
where $\text{ID}(\cdot)$ denotes the inverse dynamics operator for the coupled rigid--flexible model [@featherstone2008rigid; @spong2005robot].
...
How useful the ZTCF is as a reference depends on where in the swing it is taken. In phases of low velocity (takeaway), the ZTCF is merely one of many possible paths, and the golfer retains high authority to deviate from it. As swing speed increases, the passive drift field $f(x)$ grows energetically while the input authority $g(x)u$ remains bounded by physiological limits. In this high-energy regime, the ZTCF becomes a useful reference for diagnosing how much of the observed motion is passive versus controlled [@koike2019dynamic]. As the **Drift-Control Ratio** (defined in subsequent work) rises, the passive terms account for a growing share of the observed joint acceleration, and velocity-dependent drift contributes more of the late-swing mechanics than active control does.
...
In this illustrative snapshot, the golfer applies only 15 Nm of active torque, while passive momentum, gravity, and elastic shaft forces contribute 85 Nm—*85%* of the mechanical load. This is the essence of "surfing on momentum." At this instant the muscles are not driving the downswing; they are *steering* it. The drift dominates. In the takeaway, this ratio would be reversed: the golfer might apply 60 Nm of active torque against only 10 Nm of drift, because the system has less momentum and requires more muscular steering. This shift from "steering a momentum-driven system" to "actively powering the early motion" is invisible in raw inverse dynamics but transparent in the drift--input decomposition.
...
**Practical conclusion.** ZTCF is quantitatively trustworthy for horizons on the order of **20--50 ms** from the branch point under typical release-phase Jacobians. At 100 ms, initial-state uncertainty is already amplified by roughly $5\times$--$20\times$, so the trajectory should be used only for qualitative insight. Across the full 250 ms downswing the exponential envelope is large enough that quantitative ZTCF claims are not defensible without error bars propagated from the state uncertainty. The exact $\bar\lambda$ for a given model should be recomputed by integrating the variational equation along the specific trajectory of interest; the numbers above are order-of-magnitude, not universal.
:::
...
To diagnose the mechanical load, we need a sharper instrument—one that freezes the system's velocities to separate the "static" weight of the club from the "dynamic" weight of its motion. This motivates the **Zero Velocity Counterfactual**.
...
We have now constructed two counterfactual baselines: the ZTCF to isolate passive momentum history, and the ZVCF to isolate passive configuration loads. However, the validity of these diagnostic tools relies on a subtle but critical mathematical condition. For the subtraction $\tau_{\text{input}} = \tau_{\text{total}} - \tau_{\text{drift}}$ to be causally meaningful, the term being subtracted ($\tau_{\text{drift}}$) must be independent of the term being isolated ($\tau_{\text{input}}$). If the act of applying torque fundamentally altered the passive dynamics of the system—changing the effective inertia or stiffness—our baselines would be invalid.

But is this separation scientifically valid? A skeptic might argue that in a biological system, the "passive" plant changes when the "active" input changes (e.g., via stiffness modulation). To defend our counterfactuals, we must prove the property of *Drift Invariance*—showing that the passive drift field is structurally immune to the active input. Part 3 supplies that proof and builds the force taxonomy that rests on it.
...
- **[Part 5: Numerical Consistency Check](theory-part5.html)** — Simulink validation of the ZTCF and ZVCF computations
