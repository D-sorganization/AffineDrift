...
for finite-dimensional mechanical systems actuated by generalized forces, and prove that the mapping from inputs to generalized accelerations, generalized forces, and wrenches is exactly **affine**, and its incremental part (relative to $u=0$) is exactly **affine**, and its incremental part (relative to $u=0$) is exactly linear: This yields a precise superposition principle: although *trajectories in state space do not superpose* in general (due to nonlinearity), the *effects of distinct input channels on generalized forces and accelerations do*. We provide worked examples in each formulation (a single pendulum, a planar 2R manipulator, and a spatial single-link model) and show how to decompose motion into contributions from gravity, Coriolis/centrifugal effects, and individual actuators.

In the broader context of the AffineDrift framework, this linearity explains why the **Zero Torque Counterfactual (ZTCF)** is well-defined inside a control-affine model. The counterfactual $u=0$ is well defined for any model with an identified exogenous input. What the affine structure adds is that the resulting difference is exactly linear in $u$: $\dot x(u) - \dot x(0) = G(x)u$, with no cross terms and no $u$-dependence hidden in $f$. This supports a model-level separation between the history-dependent passive layer (Drift) and the instantaneous active layer (Input).
:::
...
This additive separability is the engine behind the **Drift–Input Decomposition**. It implies that the "Input Acceleration" is *additively separable* from the "Drift Acceleration"—the two combine by simple vector addition at a fixed state, with no cross-coupling term (this is separability, not geometric orthogonality in any inner-product sense). Consequently, the operation of subtracting the passive drift acceleration from the total observed acceleration to recover the input term $G(x)u$ is an exact algebraic identity, not an approximation. Recovering the input *vector* $u$ additionally requires $G(x)$ to have full column rank; where it does not, only the projection of $u$ onto $\text{range}(G^\top)$ is identifiable.
...
Let $f^\text{ext,B} \in \mathbb{R}^3$ be the net **external force** on the body, expressed in the body frame, and $n^\text{ext,B} \in \mathbb{R}^3$ the net **external moment** about the body frame origin.

The Newton–Euler equations in body coordinates can be written as

\begin{align}
m (\dot v^B + \omega^B \times v^B) &= m g^B + f^\text{ext,B}, \tag{3.1a} \\
...
* Dynamics:
  \begin{align}
  \dot v &= g^B - \omega^B \times v^B + \frac{1}{m} f^\text{ext,B}, \tag{3.4a} \\
...
\quad
H(q,\dot q) := M(q)^{-1} B(q,\dot q).
$$

Then

$$
\ddot q = A(q,\dot q) + H(q,\dot q) u.
$$
...
* note $(c^\times)^\top = -c^\times$, so the matrix is symmetric,
* $m$ is the mass,
...
Consider a 2-link planar robot arm with joint angles $q_1,q_2$, link lengths $l_1,l_2$, link masses $m_1,m_2$, and moments of inertia $I_1,I_2$ about their proximal joint axes. The angles are measured from the horizontal. Gravity acts in $-y$.
