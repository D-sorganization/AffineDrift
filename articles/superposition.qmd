...
::: {.callout-important}
## Scope of Superposition in This Article --- Read First
...
The rest of this paper shows that **standard rigid-body dynamics models admit the structure (2.1)**, with the controls entering as generalized forces. In each formulation, the map from those generalized forces to accelerations is linear.
...
This is of the form (2.1) with $m=1$ input.
...
This is a concrete illustration of Newton–Euler superposition: *the gravitational component and control component of acceleration add linearly*.
...
Superposition does not extend to trajectories:
...
Still, this local superposition is useful in practice: at each time it lets you calculate how much of the instantaneous acceleration or joint torque is attributable to gravity, inertial coupling, specific actuators, or external loads.
...
**Why this matters for control:** When you design a controller, you often ask: "At the current state, if I apply force $u_1$, what happens?" or "How do I decompose the current acceleration into what gravity is doing versus what my actuators need to do?" These are instantaneous questions, and superposition answers them exactly. A nonlinear long-term trajectory does not invalidate that answer—it means only that you must re-evaluate the decomposition as the state changes.
...
**Example:** In a 2-link robot, you might measure joint angles in degrees or radians. The equations look different numerically. But the geometric fact—"there is a drift from gravity and Coriolis; the control input adds torque additively"—is true in both coordinate systems. The decomposition is real, even though its numerical form is coordinate-dependent.

---

## 7. The Modeling Question: Why Control-Affine?

At this point, a critical reader should ask: **Is the affine structure true, or is it a convenient fiction?** The equations $\dot x = f(x) + G(x)u$ look clean and mathematically beautiful, but does nature obey them?
...
Torque is torque. It enters the equations linearly. Whether a particular $\tau$ value is hard or impossible for the muscle to produce is a separate question—one about the constraints on the input space, not about the structure of the differential equation itself.
...
And you'd be right. But consider: when an engineer asks "What torque is the shoulder joint producing?" they don't care about the molecular details of actin-myosin sliding. The torque is *defined* at the joint, and it enters the equations of motion linearly. The complex biology underlying that torque is irrelevant to the question being asked.
