---
title: "Launch Monitor Technology: Measurement, Inference, and the Screw-Theoretic Opportunity"
  ...
  <div class="laymans-terms-content">
    <div class="laymans-terms-inner">
      <p class="laymans-terms-intro">
        A launch monitor sits behind you, watches for a few thousandths of a second, and then tells you two dozen numbers about your swing. Some of those numbers are things it genuinely saw. Others are things it worked out from what it saw, using physics and some assumptions. The device does not tell you which is which — and that distinction turns out to explain almost every disagreement between brands.
      </p>
      ...
      <div class="key-takeaway">
        <strong>Key Takeaway:</strong> Radar measures the flight and infers the impact. Cameras measure the impact and infer the flight. Neither can see everything, which is why the best modern units bolt a camera onto a radar or a radar onto a camera.
      </div>

      <div class="laymans-item">
        <h3>The clubface problem</h3>
        <p>
          Here is the awkward part. At the moment of impact, the clubface is pointing directly away from a radar sitting behind the golfer. You cannot see a mirror's front by standing behind it. So on radar-only devices, "face angle" is not observed at all — it is calculated backwards from where the ball actually went, using a model of the collision. That works well for a clean strike and less well for a toe or heel hit, because an off-center strike twists the clubhead in a way the model cannot see.
        </p>
        <div class="analogy">
      ...
      <div class="laymans-item">
        ...
        <p>
          There is an old theorem which says that any movement of a rigid object, however complicated, is at every instant just a rotation about some line in space combined with a slide along that same line — like a screw turning. Describing the clubhead this way turns out to match exactly what a radar measures, and it lets you do something clever: see the clubface <em>once</em>, at any convenient moment before impact, then let the measured rotation carry that snapshot forward to the moment of contact. You no longer need to see the face at impact at all.
        </p>
      </div>
    </div>
  </div>
...
At the 24 GHz band used by most consumer units, each 1 mph of radial speed produces roughly 71.7 Hz of Doppler shift; at the ~10.5 GHz X-band used for long-range ball tracking in premium units, about 31.3 Hz per mph. Band choice is a range-versus-resolution trade: X-band supports tracking the full six-second flight of a drive, while 24 GHz gives finer velocity resolution per unit observation time in compact, low-power hardware.

A single-channel Doppler radar measures speed and nothing else. Every serious launch monitor therefore uses **multiple receive antennas** and extracts the angular position of a target from the *phase difference* of its echo across receiver pairs — phase-comparison monopulse, or interferometry. For a baseline $\mathbf{d}$ and arrival direction $\hat{\mathbf{u}}$, the measured phase difference is
...
— with the axis as the unit-eigenvalue eigenvector and the rate from the per-frame rotation angle. Reported accuracy is 1–3% of true spin, independent of ball speed, and robot testing consistently shows camera systems with roughly half the shot-to-shot spin variability of radar.
...
The single most instructive fact about the market is that every high-end vendor has independently concluded that neither modality suffices. TrackMan added cameras to radar; FlightScope added camera fusion tracking; SkyTrak added radar to a camera system; Rapsodo pairs radar with impact cameras reading marked balls. Each was solving the same problem from opposite ends: radar cannot see the impact, and cameras cannot see the flight.

## Part II: The directness hierarchy

The organising idea of this article is that every reported parameter sits somewhere on a spectrum from *measured* to *modelled*, and that inter-device disagreement tracks position on that spectrum almost perfectly.

| Parameter | Radar (outdoor) | Radar (indoor) | Camera | Hybrid |
|---|---|---|---|---|
| Ball speed | Measured | Measured | Measured | Measured |
| Launch angles | Measured | Measured | Measured | Measured |
| Spin rate | Measured | Measured / estimated | Measured | Measured |
| Spin axis | Derived (trajectory) | Estimated | Measured | Measured |
| Carry | Measured | Modelled | Modelled | Modelled |
| Club speed | Measured | Measured | Measured (markers) | Measured |
| Club path, attack angle | Measured | Measured | Measured (markers) | Measured |
| **Face angle** | **Derived** | **Derived** | Measured (markers/overhead) | Measured |
| **Dynamic loft** | **Derived** | **Derived** | Measured (markers/overhead) | Measured |
| Impact location | Unavailable | Unavailable | Measured | Measured |
...
A full catalogue of which vendor uses which convention, with sources, is in the [Launch Monitor Vendor Reference](launch-monitor-vendor-reference.qmd). The short version is that FlightScope and Uneekor also reference the geometric centre, Foresight publicly criticises geometric-centre referencing while declining to name its own point for path, and GEARS ships both reference points --- but only for speed, not for path.
...
The weight $w_f$ is usually quoted as 0.85 for a driver and 0.75 for a wedge. **Both figures are too high**, because they are *vertical*-plane numbers applied to a horizontal-plane equation. The only peer-reviewed measurement — [Wood, Henrikson & Broadie (2018)](https://doi.org/10.3390/proceedings2060249), 157 golfers, 1,575 shots, Vicon at 720 fps, filtered to centred strikes — gives horizontal weights of **0.76 ± 0.08 for a driver, 0.69 for a 7-iron, and 0.61 for a wedge**, with a robot test returning 0.63 for the 7-iron. Nor are these separate per-club constants: the weight declines monotonically with spin loft along a single curve, and the club enters only through its obliqueness.
...
This is the physical claim behind the sceptical position: at impact the face normal points away from any rear-mounted sensor, and a metal clubhead is opaque at microwave frequencies. Nobody measures face orientation from behind the golfer. They either mark the face, view it from above, or infer it from the ball.
...
This is the **reciprocal product** of the sight line, treated as a Plücker line, with the twist — and it is *exactly linear* in the six twist coordinates. Each detection contributes one row of a linear system; the per-frame twist estimate is weighted least squares, with a covariance obtained for free.

That is not an aesthetic observation. It means the sensor's native observable is already a linear functional of the object we want, so the estimator is well-posed, non-iterative, and yields honest uncertainty. Camera systems enter the same framework from the other side: a tracked face gives a pose in $SE(3)$ per frame, and the matrix logarithm of the frame-to-frame relative pose is a finite twist. One representation serves both modalities.
...
- **Club speed at a declared reference point** — $\lVert\mathbf{v}_O + \boldsymbol{\omega}\times\mathbf{r}_{gc}\rVert$ evaluated wherever you choose. The reference-point ambiguity that dominates inter-device disagreement becomes an explicit, documented convention rather than an artefact of which scatterer was brightest. This is a genuine *repeatability* improvement and, to the extent current devices use fastest-return heuristics, a *bias* removal.
- **Club path and attack angle** — direction angles of that same velocity vector.
- **Closure rate** — the component of $\boldsymbol{\omega}$ about the shaft axis. This is currently a premium camera-system exclusive requiring face markers; in twist coordinates it is a free projection.
...
- **Honest covariances** on every parameter, because the estimator is a linear least-squares problem.

Applied to the *swing* rather than the club, the same machinery is not new: Vena and colleagues used instantaneous screw axis theory on optical motion capture to show that at least 71% of marker velocity during the downswing is attributable to rotation about the segment's ISA, and that ISA smoothness distinguishes skilled from novice golfers ([Part 1](https://link.springer.com/article/10.1007/s12283-010-0058-8), [Part 2](https://link.springer.com/article/10.1007/s12283-010-0059-7)). What appears not to exist in the literature is estimating the twist *directly from Doppler observables*.
...
$$
\sigma^2_{\text{face}}(t_{\text{impact}}) \approx \sigma^2_{\text{anchor}} + \int_{t_0}^{t_{\text{impact}}}\sigma^2_{\omega}\,dt
$$

giving a clean design rule: **anchor as late as geometry allows, and measure angular velocity as well as possible throughout.**
...
2. **Linearity where it counts** — Doppler and line observables are linear in twist coordinates, giving tractable estimators with genuine covariances.
3. **Reference-point invariance** — eliminating the largest definitional variance in club data.
4. **The anchor-and-propagate principle** — converting the face problem from "observe the face at impact" (geometrically impossible from behind) into "fix the face once, integrate angular velocity."
5. **New measurands** — closure rate from radar, ISA-defined swing plane, ISA pitch and smoothness as skill metrics, gear-effect recoil, and — on the ball side — true roll percentage in putting, since the skid-to-roll transition is literally the instantaneous screw axis migrating from ball centre to ground contact.
...
The honest summary: same hardware information, extracted into better-defined, reference-point-invariant, uncertainty-quantified quantities, plus a family of measurands that currently either require premium optical hardware or do not exist at all.
...
**Scattering centres migrate.** Specular glints wander across a curved metal crown as aspect changes, violating the fixed-scatterer assumption at the centimetre level. Robust losses are mandatory, and the fitted covariance should be trusted over the point estimate.
...
The measurement content of a launch monitor is smaller than its output table suggests, and the gap is filled by physical models of varying quality. Ball data is genuinely measured and agrees well across devices; club data is a mixture of measurement, convention, and inversion, and diverges accordingly. The published validation literature has said as much for nearly a decade — criterion studies consistently endorse ball parameters and advise caution on club parameters.

Screw theory does not conjure information that the sensors did not collect. What it does is give the estimation problem its natural shape: a single twist trajectory that all sensors constrain and all parameters project from, with linear measurement models, honest covariances, and an explicit reference-point convention. Most importantly, it supplies the propagation machinery that makes the clubface tractable without impact-model inversion — turning a geometrically impossible observation into a solvable anchoring problem, for which at least six independent physical mechanisms are available.
...
- (2024). Within- and between-session reliability of TrackMan 4 launch monitor variables in high-level golfers. *Journal of Sports Sciences*. [DOI](https://doi.org/10.1080/02640414.2024.2314864) — indoor reliability, with spin as the fragile channel.
