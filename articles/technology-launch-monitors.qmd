...
\qquad
w_f \approx \begin{cases} 0.85 & \text{driver} \\ 0.75 & \text{wedge} \end{cases}
$$

Inverting this to recover $\varphi_f$ from measured launch direction and path is exactly what radar units do — and it has two structural consequences. First, because $1/w_f \approx 1.15$, any systematic bias in measured launch direction is *amplified* by about 15% in the reported face angle. Second, the model is blind to **gear effect**: an off-centre strike torques the head about its centre of gravity, and the resulting spin-axis tilt is misattributed to face-to-path. A strike one dimple width off-centre tilts the spin axis about 6° on a driver.
