...
::: {.callout-note}
## Figure Pending
**Figure: Force-couple equivalence.** Diagram showing that a force $\mathbf{F}$ applied at an offset point is dynamically equivalent to the same force applied at the reference point plus a couple $\mathbf{C}_{\text{eq}} = \mathbf{r} \times \mathbf{F}$. Visualization pending.
:::
...
::: {.callout-note}
## Figure Pending
**Figure: Golf club grip schematic.** Diagram showing the club with labeled reference points (top of grip, midpoint $m$, bottom of grip) and signed distances $d_i$ from the center of mass (COM). COM is located closer to the clubhead than to the hands. Visualization pending.
:::
