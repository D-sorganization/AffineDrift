...
The general relation for transforming a force-couple pair from reference point $m$ to point $i$ is:

$$
C_i = C_m + (\mathbf{r}_m - \mathbf{r}_i) \times \mathbf{F},
$$

where $\mathbf{r}_m$ and $\mathbf{r}_i$ are position vectors of points $m$ and $i$ respectively and $\mathbf{F}$ is the resultant force. For a one-dimensional grip with the force **perpendicular to the grip axis** (a standard simplifying assumption for this scalar form), $|(\mathbf{r}_m - \mathbf{r}_i) \times \mathbf{F}| = F \cdot (d_m - d_i)$, which gives:
...
- $M_m$ = moment of force about the midpoint ($M_m = F \cdot d_m$, valid when $\mathbf{F} \perp$ grip axis)
...
**Note:** The scalar formula above assumes the resultant grip force is perpendicular to the grip axis. When the force is oblique to the axis, the full vector form $C_i = C_m + (\mathbf{r}_m - \mathbf{r}_i) \times \mathbf{F}$ must be used.
\end{proposition}
...
Accounting for drag reduces the required hand couple in our inverse-dynamics solution by $\approx 7.3$ Nm (about 40%) --- **not because drag removes effort**, but because the naive calculation mis-attributes an aerodynamic moment arm to the golfer's hands. The drag moment itself ($\approx 2.0$ Nm at the grip) is smaller than the re-attribution ($\approx 7.3$ Nm); the larger number reflects the $F_{\text{drag}} \times r_{\text{hand}}$ lever arm that disappears from the hand-couple side of the ledger once it is correctly assigned to aerodynamics.
