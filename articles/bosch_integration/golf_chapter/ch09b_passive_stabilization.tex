...
The uncontrolled manifold (UCM) hypothesis, developed by \citet{Scholz1999} and extended by \citet{Schöner2003}, provides an empirical way to test the attractor-fluctuation framework. The hypothesis states that variability in movement is not uniformly distributed across all degrees of freedom. Instead, variance is \emph{concentrated in task-irrelevant directions} (the uncontrolled manifold) and minimized in task-relevant directions.
...
If the ZTCF is close to the desired trajectory $\configvec_d(t)$, then the control inputs $u_i(t)$ need only make small corrections. This is an elegant design principle: \emph{let the body's natural dynamics do most of the work}.
...
Recall from Chapter~\ref{ch:zero_torque_counterfactual} the Drift--Control Ratio (DCR):
\begin{equation}
...
fraction bounded in $[0,1]$: $\text{DCR} > 1$ means the drift term exceeds what the bounded
control can produce. See \texttt{NOTATION.md}, which reserves the bare acronym site-wide for
this quantity.
...
UCM decomposition \citep{Scholz1999} quantifies this by partitioning variance into:
\begin{equation}
...
(a) If $K = 100 \, \text{N·m/rad}$, compute the restoring torque for a deviation of $\theta = 0.05 \, \text{rad}$ (about 3 degrees).
...
\end{equation}
where $M = 0.02 \, \text{kg·m}^2$ (moment of inertia of the hand and club), $D = 0.5 \, \text{N·m·s/rad}$ (damping), $K$ is the stiffness (variable), $\tau$ is the muscle torque, and $\tau_{\text{dist}}$ is a disturbance (e.g., impact force).
...
(b) For stability, what is the minimum value of $K_c$ (in units of N·m/rad)?

(c) If $K_c = 100 \, \text{N·m/rad}$, compute the natural frequency $\omega_n$ and damping ratio $\zeta$ of the wrist.

(d) If a disturbance $\tau_{\text{dist}} = 5 \, \text{N·m}$ (impact force) is suddenly applied, what is the steady-state deflection $\theta_{\infty}$? How does it depend on $K_c$?
...
\end{equation}
where $M_1 = 0.5, M_2 = 0.1 \, \text{kg·m}^2$, $D_i = 0.5$ (Ns/rad), and $K_i(\alpha) = K_{0,i} + \alpha \Delta K_i$ with $K_{0,1} = 50, K_{0,2} = 20, \Delta K_1 = 50, \Delta K_2 = 30$ (N·m/rad).
...
(c) What are the expected stiffness values (in N·m/rad) at each phase?
...
\bibitem[Bosch(2020)]{Bosch2020}
Bosch, F. (2020). \emph{Anatomy of Agility: A Biomechanical Approach to Movement Design}. Routledge.

\bibitem[Bosch(2015)]{Bosch2015}
Bosch, F. (2015). \emph{Strength Training and Coordination: An Integrative Approach}. Routledge.

\bibitem[Bernstein(1967)]{Bernstein1967}
Bernstein, N. A. (1967). \emph{The Co-Ordination and Regulation of Movements}. Pergamon Press.

\bibitem[Scholz and Sch\"{o}ner(1999)]{Scholz1999}
Scholz, J. P., \& Schöner, G. (1999). The uncontrolled manifold concept: identifying control variables for a functional task. \emph{Experimental Brain Research}, 126(3), 289-306.

\bibitem[Schöner(2003)]{Schöner2003}
Schöner, G. (2003). Dynamic field theory: the cognitive neuroscience of attention. \emph{Biological Cybernetics}, 89(3), 175-188.

\bibitem[Turvey(1990)]{Turvey1990}
Turvey, M. T. (1990). Coordination. \emph{American Psychologist}, 45(8), 938-953.

\bibitem[Todorov(2004)]{Todorov2004}
Todorov, E. (2004). Optimality principles in sensorimotor control. \emph{Nature Neuroscience}, 7(9), 907-915.

\bibitem[Latash(2012)]{Latash2012}
Latash, M. L. (2012). \emph{The Bliss (not the Problem) of Motor Abundance}. Elsevier.
