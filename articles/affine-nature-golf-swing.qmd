...
where each $\eta_{i}$ represents the amplitude of a particular bending or deformation mode. The total configuration manifold is the product space $\mathcal{Q} = \mathcal{Q}_r \times \mathbb{R}^m$ of dimension $N = n+m$.

We define the full state vector as an element of the **tangent bundle** $T\mathcal{Q}$:
...
The total kinetic energy $T$ is obtained by integrating the squared velocity over the shaft's mass distribution $\rho(s)$:
$$
T_{\text{shaft}} = \frac{1}{2} \int_0^L \rho(s) \| v(s) \|^2 ds.
$$
Substituting the velocity decomposition yields:
$$
T_{\text{shaft}} = \frac{1}{2} \dot{q}^T \left( \int \rho J^T J ds \right) \dot{q} + \dot{q}^T \left( \int \rho J^T \Phi ds \right) \dot{\eta} + \frac{1}{2} \dot{\eta}^T \left( \int \rho \Phi^T \Phi ds \right) \dot{\eta}.
$$
This explicit integration reveals the structure of the mass matrix blocks:
...
To isolate the accelerations, we require the inverse inertia matrix $H(q) = M^{-1}(q)$. Using the **Schur Complement** decomposition, we can explicitly write the blockwise inverse:
$$
H = \begin{bmatrix} H_{qq} & H_{q\eta} \\ H_{\eta q} & H_{\eta\eta} \end{bmatrix},
$$
where the diagonal block $H_{qq}$ is given by the inverse of the **Articulated Body Inertia** $\Delta$:
$$ \Delta = M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q}, \quad H_{qq} = \Delta^{-1}. $$
Crucially, the off-diagonal block $H_{\eta q}$ determines how joint torques affect shaft modes:
$$ H_{\eta q} = - M_{\eta\eta}^{-1} M_{\eta q} H_{qq}. $$

Multiplying the equation of motion by $M^{-1}$ isolates the acceleration vector:
$$
\begin{bmatrix} \ddot{q} \\ \ddot{\eta} \end{bmatrix} =
