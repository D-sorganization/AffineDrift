...
The control-affine formulation assumes that the state vector $x = (q, \eta, \dot{q}, \dot{\eta})$ contains all information needed to predict future evolution—the Markov property. This is mathematically exact for ideal rigid-body and linear-elastic systems. However, biological systems may contain hidden states not captured in our model: muscle fatigue, calcium buffering dynamics, proprioceptive adaptation, and short-term motor learning.
...
Equation~@eq-affine_repeat represents the **physics** of the swing—a descriptive statement of how forces produce motion. We now pivot from description to **diagnostics**. We seek to transform this equation of motion into a causal map that can attribute specific features of the swing trajectory to either the golfer's intent or the club's inertia.

This shift is enabled by the **Mechanical Orthogonality** derived in Section~@sec-affine, where the inertial memory of the system (drift) is shown to be structurally distinct from the current agency of the player (input). Given the control-affine form~@eq-affine_repeat, we now formalize this physical separation of passive (drift) and active (input) contributions to the dynamics.
