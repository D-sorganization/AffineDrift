...
The input-driven acceleration is the part proportional to \(u\). We can expand the inverse inertia matrix $M^{-1}$ in block form using the Schur complement. Let $H = M^{-1}$. Its structure reflects the full coupling:
$$
H(q,\eta) = \begin{bmatrix} H_{qq} & H_{q\eta} \\ H_{\eta q} & H_{\eta\eta} \end{bmatrix}.
$$
Using the block inversion lemma, the rigid-body effective inverse inertia $H_{qq}$ and the coupling block $H_{\eta q}$ are given explicitly by:
$$
...
$$
Physically, $H_{qq}$ represents the articulated inertia of the golfer seen at the joints, "stiffened" or modified by the reaction of the flexible shaft (reflected inertia). $H_{\eta q}$ defines the *transmission* of joint torque to shaft mode acceleration.
