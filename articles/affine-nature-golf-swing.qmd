...
The derivation in Section~[@sec:affine] yielded a mathematical structure, $\dot{x} = f(x) + g(x)u$, where the control input $u$ appears linearly and distinct from the drift term $f(x)$.
...
We have now constructed two powerful counterfactual baselines: the ZTCF (Section~[@sec:ztcf]) to isolate passive momentum history, and the ZVCF (Section~[@sec:zvcf]) to isolate passive configuration loads. These constructions rely on a fundamental premise: that the 'passive' dynamics of the system are robust to the application of 'active' force. If the act of applying torque fundamentally altered the nature of the drift field $f(x)$—for instance, by implicitly modifying the effective inertia or stiffness matrix—then our baselines would be moving targets, dependent on the very variable we seek to isolate.
