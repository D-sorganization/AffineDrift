...
Let $\zeta: \mathcal{Q} \to T\mathcal{Q}$ be the *zero section* of the tangent bundle, which embeds the configuration manifold into the state space as the locus of zero velocities: $\zeta(q) = (q, 0)$.

The ZVCF operator $\mathcal{Z}: T\mathcal{Q} \to T\mathcal{Q}$ is the composition of projection and zero-section embedding:
$$
\mathcal{Z}(x) = (\zeta \circ \pi)(x).
$$
...
**Note on Dynamic Relevance.** A common objection is that static forces (gravity, elastic stiffness) are negligible compared to inertial forces in a high-speed swing (e.g., $1g$ vs $100g$). This misses the point of the ZVCF. We do not calculate ZVCF because we believe gravity "steers" the downswing; we calculate it to mathematically subtract the configuration-dependent baseline from the total passive force. Without the ZVCF, one cannot analytically distinguish "Geometric Stiffness" (a velocity-dependent stiffening effect) from "Elastic Stiffness" (a static material property). The ZVCF is the necessary "tare" operation for the dynamic scale.
...
1.  The kinematic mapping $\dot{q}, \dot{\eta}$ depends only on the state vector $x$.
2.  The mass matrix $M(q,\eta)$ and its inverse depend only on configuration coordinates.
3.  The Coriolis matrix $C$ is quadratic in velocities and depends on configuration.
...
None of these terms contain the variable $u$. The control input $u$ appears *exclusively* in the term $M^{-1} [B(q)u; 0]^T$. Therefore, the Jacobian of the drift field with respect to the input is the zero matrix.
$$ \nabla_u f(x) = 0. $$
