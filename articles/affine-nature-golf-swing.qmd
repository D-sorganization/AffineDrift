...
To see this rigorously, recall the definition of $f(x)$ derived in Section~[@sec:affine]:
$$
f(x) = \begin{bmatrix} \dot{q} \\ \dot{\eta} \\ -M^{-1} \left( C \dot{q}_{\text{sys}} + G + F_s \right) \end{bmatrix}.
$$
The inertia matrix $M$, Coriolis matrix $C$, gravity vector $G$, and shaft forces $F_s$ depend exclusively on $q, \eta, \dot{q}, \dot{\eta}$. The inverse inertia matrix $M^{-1}$ is similarly state-dependent but input-independent. Since the term $B(q)u$ appears only in the input field $g(x)$ and not in the drift field, the partial derivative with respect to $u$ vanishes identically.
