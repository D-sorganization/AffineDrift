...
where $J_{\text{rigid}}(s, q)$ transforms joint rates into the rigid-body velocity at point $s$, and $\Phi(s)$ is the matrix of mode shape values at $s$.

This decomposition is fundamental because the kinetic energy of the system is the integral of these squared velocities over the mass distribution. The cross-terms in the square of this sum ($ \dot{q}^T J^T \Phi \dot{\eta}$) are precisely what generate the off-diagonal inertial coupling $M_{q\eta}$ in the mass matrix. This coupling is not a "linkage" artifact; it is the physical mechanism by which hand acceleration instantaneously loads the shaft (inertial forcing) and by which shaft recoil exerts forces on the hands.
...
Since $M$ is positive-definite, $M^{-1}$ exists. We denote the inverse inertia matrix by $H(q_{\text{sys}}) = M^{-1}$. Using the **Schur Complement** decomposition, we can explicitly solve for the blockwise inverses. Let $\Delta$ be the Schur complement of $M_{\eta\eta}$ in $M$:
$$ \Delta = M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q}. $$
In robotic mechanics, $\Delta$ is known as the **Articulated Body Inertia** of the handle. It represents the effective inertia felt by the hands when the shaft modes are free to accelerate (i.e., not locked).
The inverse block $H_{qq}$ is simply $\Delta^{-1}$. This leads to a crucial physical insight: the effective mobility of the hands depends on the flexibility of the club.

Multiplying the equation of motion by $M^{-1}$ isolates the acceleration:
$$
\begin{bmatrix} \ddot{q} \\ \ddot{\eta} \end{bmatrix} =
...
2.  **Transmission to Flexible Modes:** $\ddot{\eta}_{in} = H_{\eta q} B(q) u$. This term is non-zero only because $M_{\eta q} \neq 0$. It proves that "lag" or "lead" deflection can be actively induced by hand torque, mediated strictly by the inertial coupling matrix.
