...
The golfer's anatomy is modeled as a multibody kinematic chain consisting of \( N_b \) rigid bodies connected by ideal revolute or spherical joints. We define the configuration manifold for the rigid segments as \( \mathcal{Q}_r \), a smooth manifold of dimension \( n \). Typically, \( \mathcal{Q}_r \) is a Lie group or a product of Lie groups (e.g., \( SE(3) \times \mathbb{T}^k \)), but for local coordinate representations, we assume a coordinate chart \( q \in \mathbb{R}^n \) representing the generalized joint angles (e.g., shoulder rotations, elbow flexion, wrist deviations, spinal rotations).

The flexible shaft is modeled via a finite-dimensional modal approximation. The deformation state lies in a vector space \( \mathcal{V} \cong \mathbb{R}^m \), with modal coordinates

$$
  \eta = [\eta_{1}, \dots, \eta_{m}]^{T} \in \mathbb{R}^{m},
$$

where each \( \eta_{i} \) represents the amplitude of a particular bending mode shape \( \phi_i(s) \).

The total configuration manifold for the coupled system is the product manifold

$$
  \mathcal{Q} = \mathcal{Q}_r \times \mathbb{R}^m,
$$

of dimension \( N = n + m \). A point in this manifold is denoted by the tuple \( (q, \eta) \).

The state of the system evolves on the tangent bundle \( T\mathcal{Q} \), the vector bundle containing all possible velocities at each configuration. We define the full state vector formally as a point in the tangent bundle:

$$
  x = (q, \eta, \qd, \etad) \in T\mathcal{Q}.
$$

In local coordinates, we represent the state as a vector in \( \mathbb{R}^{2N} \):

$$
  x = [q^{T}, \eta^{T}, \qd^{T}, \etad^{T}]^{T}.
$$

This geometric structure—a dynamical system evolving on the tangent bundle of a configuration manifold—is fundamental. It allows us to distinguish between forces that depend on the base configuration (vector fields on \( \mathcal{Q} \)) and forces that depend on velocity (vector fields on \( T\mathcal{Q} \)).
...
Defining the system's components is only the first step. To understand the causal relationship between the golfer's effort and the club's motion, we must derive the equations of motion in a form that explicitly isolates the control inputs. Standard Kane's method or Newton-Euler formulations often yield a single implicit system of equations. Here, we employ the Lagrangian formalism specifically to reveal the *control-affine* structure—the energetic architecture that guarantees the golfer's torque enters the dynamics linearly.
...
To recast this into a control-theoretic framework, we solve for the accelerations. Since the inertia matrix $M(q,\eta)$ is symmetric and positive-definite, it admits a unique inverse $H(q,\eta) = M^{-1}(q,\eta)$. While $M$ is defined by physical mass distributions, its inverse $H$ represents the *mobility* or *admittance* of the system---how easy it is to accelerate given applied forces. Multiplying the equations of motion by $M^{-1}$ isolates the highest derivatives:

Multiplying the equations of motion by $M^{-1}$ isolates the highest derivatives:

$$\begin{bmatrix} \ddot{q} \\ \ddot{\eta} \end{bmatrix} = M^{-1} \left( \begin{bmatrix} B(q)u \\ 0 \end{bmatrix} - \underbrace{\left( C \begin{bmatrix} \dot{q} \\ \dot{\eta} \end{bmatrix} + G + \begin{bmatrix} 0 \\ K_s \eta + C_s \dot{\eta} \end{bmatrix} \right)}_{\text{Total Passive Forces}} \right).$$

Crucially, because the flexible modes are underactuated (zero torque in the lower block), the control input $u$ enters the dynamics *only* through the rigid-body channel. However, due to the off-diagonal coupling in $M^{-1}$, this rigid-body torque instantaneously induces accelerations in the flexible modes.

To see this explicit coupling, consider the block inverse of the inertia matrix $H = M^{-1}$. Using the Schur complement $\Delta = M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q}$, the inverse blocks are given by:
$$
H = \begin{bmatrix} H_{qq} & H_{q\eta} \\ H_{\eta q} & H_{\eta\eta} \end{bmatrix}
= \begin{bmatrix} \Delta^{-1} & -\Delta^{-1} M_{q\eta} M_{\eta\eta}^{-1} \\ -M_{\eta\eta}^{-1} M_{\eta q} \Delta^{-1} & M_{\eta\eta}^{-1} + M_{\eta\eta}^{-1} M_{\eta q} \Delta^{-1} M_{q\eta} M_{\eta\eta}^{-1} \end{bmatrix}.
$$
The acceleration induced by the input torque $\tau = B(q)u$ is then:
$$
...
$$
This derivation reveals two critical mechanical insights:
1.  **Effective Rigid Inertia:** The term $H_{qq} = \Delta^{-1}$ represents the rigid-body mobility modified by the "reflected mass" of the flexible shaft. The golfer does not accelerate a rigid club, but rather a rigid handle coupled to a flexible load.
2.  **Inertial Transmission:** The term $H_{\eta q} = -M_{\eta\eta}^{-1} M_{\eta q} H_{qq}$ acts as a transmission gain. It dictates how torque applied at the hands creates instantaneous acceleration in the shaft modes via the inertial coupling block $M_{\eta q}$. If the system were decoupled ($M_{\eta q} = 0$), joint torque would have no immediate effect on shaft bending.

Let the state vector be $x = [q^T, \eta^T, \dot{q}^T, \dot{\eta}^T]^T \in \mathbb{R}^{2(n+m)}$. We define the **drift vector field** $f(x)$ and the **control input field** $g(x)$ by expanding this inverse relationship.
...
The **control field** $g(x)$ is derived by applying the inverse inertia matrix to the actuation vector:
$$
g(x) = \begin{bmatrix} 0 \\ 0 \\ H_{qq} B(q) \\ H_{\eta q} B(q) \end{bmatrix}.
$$
Here, $H_{qq}$ represents the effective inverse inertia seen at the joints, while $H_{\eta q}$ defines the *transmission* of joint torque to shaft mode acceleration (see Appendix A for explicit block matrix derivations).
...
The derivation above is not merely a symbolic exercise; it exposes the fundamental causality of the system. The affine form $\dot{x} = f(x) + g(x)u$ asserts that the system's evolution is a superposition of two distinct processes: a passive drift along the manifold's geodesic fabric, and an active deviation forced by the inputs. This mathematical separability forces a physical decomposition: we can no longer view the swing as a monolithic 'result' of the golfer, but rather as a dialogue between the system's inertial memory and the golfer's current intent.

This algebraic separation is the key to unlocking the causal structure of the swing. Because the equations of motion are linear in torque, we can physically interpret the terms not just as mathematical components, but as distinct mechanical sources of motion. The term $f(x)$ represents the system's *inertial memory* and *potential field*—the forces that exist simply because the system has a state—while $g(x)u$ represents the *current agency* of the golfer.

Given the control-affine form~\eqref{eq:affine_state}, we now formalize this physical separation of passive (drift) and active (input) contributions to the dynamics.
...
The ZTCF successfully purifies the motion of active torque, leaving us with the 'skeletal baseline' of the swing. However, this baseline is still dynamically dense: it entangles static loads (gravity, stiffness) with velocity-dependent effects (Coriolis, centrifugal). A golfer feeling a heavy load at the top of the backswing (static) experiences a fundamentally different mechanical challenge than one fighting centrifugal force at impact (dynamic). To resolve this ambiguity, we must dissect the drift term itself. We require an analytical scalpel that separates the *configuration-driven* loads from the *motion-driven* loads.
...
*Proof.* By construction in Section~@sec-affine, the state derivative is given by:
$$ \dot{x} = \mathcal{A}(x, u) = f(x) + g(x)u. $$
The drift term $f(x)$ is defined as:
$$
...
This property is structural for control-affine systems. Consider the general mechanical form:

$$
  \ddot{z} = M^{-1}(z) \big( \mathcal{F}_{ext} - \mathcal{C}(z,\dot{z})\dot{z} - \mathcal{G}(z) \big).
$$

Since the external force decomposes additively into \( \mathcal{F}_{ext} = \tau_{input} + \mathcal{F}_{passive} \), and \( \tau_{input} \) enters linearly, the acceleration splits cleanly:

$$
  \ddot{z} = \underbrace{M^{-1}(\mathcal{F}_{passive} - \mathcal{C}\dot{z} - \mathcal{G})}_{\text{Drift (Passive)}} + \underbrace{M^{-1}\tau_{input}}_{\text{Input (Active)}}.
$$

The drift term is entirely free of \( \tau_{input} \). Thus,
...
The formal proof of drift invariance in the preceding section provides the necessary license to classify forces. Because the passive dynamics $f(x)$ do not change when the golfer applies torque, we can rigorously define 'Drift Forces' as a stable category of mechanical load that exists independently of the golfer’s immediate action. This independence permits the construction of a comprehensive **Force Taxonomy**, which maps the algebraic terms of the affine structure directly to mechanical interpretations.
...
Theoretically, the stiffness of the grip—and thus the boundary condition of the shaft—varies with muscle activation $u$. This would make the mode shapes $\phi_i$ and mass matrix $M$ dependent on $u$, formally breaking the affine structure $\dot{x} = f(x) + g(x)u$. We adopt the **Constant Impedance Assumption**, treating the grip as a fixed mechanical constraint (clamped or finite-stiffness) that defines the "Effective Plant". The input $u$ is then the torque applied *to* this effective plant, rather than a parameter that reshapes it.
