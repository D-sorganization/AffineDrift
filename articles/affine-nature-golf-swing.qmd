...
Let \( \mathcal{L}: T\mathcal{Q} \to \mathbb{R} \) be the Lagrangian of the system, defined as the difference between kinetic energy \( T \) and potential energy \( V \):

$$
  \mathcal{L}(q, \eta, \qd, \etad) = T(q, \eta, \qd, \etad) - V(q, \eta).
$$

The kinetic energy is defined by a Riemannian metric \( M(q, \eta) \) on the configuration manifold \( \mathcal{Q} \). For the coupled rigid--flexible system, the kinetic energy decomposes into block components:

$$
  T = \frac{1}{2} \begin{bmatrix} \qd^T & \etad^T \end{bmatrix}
  \begin{bmatrix}
    M_{rr}(q,\eta) & M_{rf}(q,\eta) \\
    M_{fr}(q,\eta) & M_{ff}(q,\eta)
  \end{bmatrix}
  \begin{bmatrix} \qd \\ \etad \end{bmatrix},
$$

where:
- \( M_{rr} \in \mathbb{R}^{n \times n} \) is the effective inertia of the rigid links (modified by shaft deformation),
- \( M_{ff} \in \mathbb{R}^{m \times m} \) is the generalized inertia of the shaft modes,
- \( M_{rf} = M_{fr}^T \) represents the inertial coupling between rigid body motion and shaft bending.

The potential energy \( V(q, \eta) \) is the sum of gravitational potential and elastic strain energy:

$$
  V(q, \eta) = V_g(q, \eta) + \frac{1}{2} \eta^T K_s \eta,
$$

where \( K_s \) is the diagonal matrix of modal stiffnesses. Note that \( V_g \) may depend on \( \eta \) if shaft deformation significantly shifts the center of mass.

The equations of motion are derived via the Euler-Lagrange operator acting on the generalized coordinates \( z = [q^T, \eta^T]^T \):

$$
  \frac{d}{dt} \frac{\partial \mathcal{L}}{\partial \dot{z}} - \frac{\partial \mathcal{L}}{\partial z} = \mathcal{F}_{ext},
$$

where \( \mathcal{F}_{ext} \) contains the non-conservative forces. We separate these forces into active control inputs and passive dissipative terms:

$$
  \mathcal{F}_{ext} = \underbrace{\begin{bmatrix} \tau \\ 0 \end{bmatrix}}_{\text{Active Input}} - \underbrace{\begin{bmatrix} 0 \\ C_s \etad \end{bmatrix}}_{\text{Passive Damping}},
$$

where \( \tau = B(q)u \) is the vector of generalized joint torques, and \( C_s \) is the modal damping matrix.

Applying the differential operators yields the coupled equations of motion in block matrix form:
...
where \( C(\dots) \) is the Coriolis/centrifugal matrix derived from the Christoffel symbols of \( M \), and \( G = \nabla V_g \). The zero in the input vector reflects the **underactuated** nature of the system: the golfer cannot directly apply torque to the shaft modes; they must influence the shaft indirectly through the coupling terms \( M_{rf} \) and \( C_{rf} \).
