...
1.  **Rigid-Body Jacobian:** $J_{\text{rigid}}(s, q) \equiv \frac{\partial p}{\partial q}$. This captures the transport velocity of the point $s$ induced by the articulation of the golfer's joints.
2.  **Modal Jacobian:** $\Phi(s) \equiv \frac{\partial p}{\partial \eta}$. This is simply the matrix of mode shapes, mapping generalized modal rates $\dot{\eta}$ to spatial velocities.

Thus, the velocity field is the superposition of rigid-body transport and local elastic deformation:
$$ v(s) = \underbrace{J_{\text{rigid}}(s, q) \dot{q}}_{\text{Rigid Transport}} + \underbrace{\Phi(s) \dot{\eta}}_{\text{Elastic Velocity}}. $$
Here:
- $J_{\text{rigid}}(s, q) \in \mathbb{R}^{3 \times n}$ is the geometric Jacobian transforming joint rates $\dot{q}$ into the rigid-body velocity field at point $s$.
- $\Phi(s) = [\phi_1(s), \dots, \phi_m(s)] \in \mathbb{R}^{3 \times m}$ is the matrix of spatial mode shapes.
...
From (2), we isolate the off-diagonal block $H_{\eta q}$ by pre-multiplying by $M_{\eta\eta}^{-1}$:
...
Substituting this expression for $H_{\eta q}$ back into (1):
...
This derivation reveals the mechanical structure of the transmission. $H_{qq}$ represents the **Effective Mobility** of the joints *after* accounting for the inertial load of the flexible shaft. It is physically distinct from the inverse of the locked-mode inertia ($M_{qq}^{-1}$); instead, it captures the enhanced or diminished responsiveness of the hands due to the dynamic compliance of the club. Crucially, the off-diagonal block $H_{\eta q}$ dictates how joint torques propagate to the flexible modes via the **Inertial Coupling Ratio**.
