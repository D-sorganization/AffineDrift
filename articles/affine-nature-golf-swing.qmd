...
$$r_{\text{club}}(q,\eta) = r_{h}(q) + \sum_{i=1}^{m} \phi_{i}(s) \, \eta_{i},$$

where $\phi_{i}(s)$ are the mode-shape vector fields defined on the beam domain. This coupling modifies the inertia, Coriolis, and elastic terms in the dynamics but does not introduce multiplicative torque terms; the control input still enters linearly through the joint torques.
...
where $M(q_{\text{sys}})$ is the symmetric, positive-definite inertia matrix (Riemannian metric on $\mathcal{Q}$). Due to the mechanical coupling between the golfer's hand and the clubhead, this matrix has a dense block structure:
...
$$
to the Lagrangian $\mathcal{L} = \frac{1}{2} \dot{q}_{\text{sys}}^T M \dot{q}_{\text{sys}} - V(q_{\text{sys}})$ yields the dynamic equations. We explicitly expand the time derivative of the generalized momentum $p = M(q_{\text{sys}}) \dot{q}_{\text{sys}}$:
$$
\frac{d}{dt}\left( \frac{\partial T}{\partial \dot{q}_{\text{sys}}} \right) = \frac{d}{dt} (M \dot{q}_{\text{sys}}) = M \ddot{q}_{\text{sys}} + \dot{M} \dot{q}_{\text{sys}}.
$$
The term $-\frac{\partial \mathcal{L}}{\partial q_{\text{sys}}}$ contributes the potential gradient $\nabla V$ and the gradient of the kinetic energy with respect to configuration:
$$
-\frac{\partial T}{\partial q_k} = -\frac{1}{2} \dot{q}_{\text{sys}}^T \frac{\partial M}{\partial q_k} \dot{q}_{\text{sys}}.
$$
Grouping the velocity-dependent terms $\dot{M}\dot{q}_{\text{sys}} - \nabla_q T$ into the Coriolis and centrifugal matrix $C(q_{\text{sys}}, \dot{q}_{\text{sys}})$ defined by Christoffel symbols of the first kind:
$$
c_{ijk} = \frac{1}{2} \left( \frac{\partial M_{ij}}{\partial q_k} + \frac{\partial M_{ik}}{\partial q_j} - \frac{\partial M_{jk}}{\partial q_i} \right),
$$
we arrive at the standard second-order form:
...
- $C(q,\dot{q},\eta,\dot{\eta})$ represents the Coriolis and centrifugal matrix. A key structural property, crucial for passivity-based analysis, is that the matrix $\dot{M} - 2C$ is skew-symmetric, implying $\dot{q}_{\text{sys}}^T (\dot{M} - 2C) \dot{q}_{\text{sys}} = 0$ (conservation of energy in the absence of non-conservative forces).
...
1.  **Effective Mobility ($H_{qq} = \Delta^{-1}$):** The term $H_{qq}$ represents the effective mobility of the joints. Note that $H_{qq} \neq M_{qq}^{-1}$. The term $\Delta = M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q}$ is the *articulated-body inertia* (or Schur complement inertia), which represents the effective inertia of the rigid body when the flexible modes are free to accelerate. This accounts for the reduction in effective inertia due to the compliance of the shaft (the "recoil" effect).
2.  **Inertial Transmission ($H_{\eta q}$):** The term $H_{\eta q} = -M_{\eta\eta}^{-1} M_{\eta q} H_{qq}$ acts as a transmission gain. It dictates how torque applied at the hands creates instantaneous acceleration in the shaft modes via the inertial coupling block $M_{\eta q}$. If the system were decoupled ($M_{\eta q} = 0$), joint torque would have no immediate effect on shaft bending.
