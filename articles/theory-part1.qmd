  ...
  <div class="critics-comments-content">
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> Standard inverse dynamics provides the <em>sum</em> of forces required, but it does not by itself separate state-dependent dynamics from the modeled input channel. A control-affine formulation makes that separation explicit <em>within the mechanical model</em>. The gain is therefore diagnostic, not metaphysical: it lets later chapters pose counterfactual questions such as "What would this model predict if the torque input were removed here?" while leaving muscle-level identification to other methods and measurements.
        </div>
      </div>
...
Traditional biomechanical analyses of the golf swing rely heavily on inverse dynamics and forward multibody models to estimate generalized joint torques, powers, and interaction forces from measured or simulated motion [@winter2009biomechanics; @nesbit2005three; @mackenzie2009three]. These methods are indispensable, but the quantities they return are net mechanical quantities at the chosen model boundary. They do not, by themselves, identify muscle-level causation, and they can aggregate active torques, passive tissue effects, and interaction terms into the same generalized coordinates [@zajac1993muscle].

This manuscript asks a narrower question. If the golfer--club--shaft system is idealized as a smooth control-affine mechanical system, which parts of the modeled dynamics belong to the drift field and which enter through the selected torque inputs? In that setting, the separation is mathematically exact for the model class; it is not a new law of biomechanics, and it does not turn generalized torques into direct measurements of physiology [@isidori1995nonlinear; @AgrachevSachkov2004; @bullo2004geometric].
...
The key mathematical observation is that the golfer--club--shaft system can be modeled as a *nonlinear control-affine mechanical system*. Control-affine systems admit an additive decomposition of their dynamics into passive drift and linear control input channels. Once the affine structure is established, later parts of the series can define counterfactual tools such as the *Zero Torque Counterfactual* (ZTCF) and *Zero Velocity Counterfactual* (ZVCF). Those constructions are best understood as model-relative diagnostics whose value depends on whether they clarify questions not already answered by inverse dynamics or dynamic contribution analysis.

This paper is *Part I* of a broader project. It develops the theoretical architecture only; no simulation, numerical results, or empirical data are presented here. Subsequent parts examine the counterfactual constructions, implementation choices, and failure modes needed before any empirical claims could be defended. By isolating the theory in this manuscript, the mathematical structure can be inspected on its own terms before stronger interpretive claims are made.

Let us pause and say plainly what we are doing. Imagine you are sitting in a rowboat on a river. Part of your motion downstream comes from the river's current---that is the "drift." Part comes from your rowing---that is the "input." If you stop rowing, the current carries you along at some speed. If the river is still, only your rowing moves you. The total motion is the sum of both. This paper builds the mathematical machinery to pose the golf-swing analogue <em>within a specified mechanical model</em>: what would the club do if the modeled driving torques were removed, and what incremental motion do those torques add? The mathematical framework is sophisticated, but the interpretive claim is deliberately narrow.
...
To rigorously distinguish 'drift' from 'input', we must first define the mechanical universe in which these forces exist. The concepts of active and passive are not absolute; they are defined relative to the system boundary and the level at which the inputs are modeled. A force that is 'external' to a sub-component may be 'internal' to the whole. Therefore, before deriving equations, we must explicitly fix the boundaries of the golfer--club--shaft system, specifying which degrees of freedom are modeled as flexible, which are directly actuated, and which effects are simply left outside the manuscript's scope.
