...
The total kinetic energy of the shaft, $T_{\text{shaft}}$, is the integral of the squared velocity norm over the mass distribution:
$$
T_{\text{shaft}} = \frac{1}{2} \int_0^L \rho(s) \| v(s) \|^2 ds.
$$
Substituting the velocity decomposition $v(s) = J\dot{q} + \Phi\dot{\eta}$ and expanding the quadratic form yields three distinct energy terms:
$$
T_{\text{shaft}} =
\underbrace{\frac{1}{2} \dot{q}^T \left( \int_0^L \rho(s) J(s)^T J(s) \, ds \right) \dot{q}}_{\text{Rigid-Body Kinetic Energy}}
+ \underbrace{\dot{q}^T \left( \int_0^L \rho(s) J(s)^T \Phi(s) \, ds \right) \dot{\eta}}_{\text{Inertial Coupling Energy}}
+ \underbrace{\frac{1}{2} \dot{\eta}^T \left( \int_0^L \rho(s) \Phi(s)^T \Phi(s) \, ds \right) \dot{\eta}}_{\text{Elastic Kinetic Energy}}.
$$
This explicit integration reveals the rigorous definition of the mass matrix blocks:
...
$$
H = M^{-1} = \begin{bmatrix} H_{qq} & H_{q\eta} \\ H_{\eta q} & H_{\eta\eta} \end{bmatrix}.
$$
Let $\Delta = M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q}$ be the **Schur Complement** of the block $M_{\eta\eta}$ in $M$. By the Block Matrix Inversion Lemma, the diagonal block $H_{qq}$ is given by:
$$ H_{qq} = \Delta^{-1} = (M_{qq} - M_{q\eta} M_{\eta\eta}^{-1} M_{\eta q})^{-1}. $$
This matrix $\Delta$ is physically interpreted as the **Articulated Body Inertia**: it represents the effective inertia of the rigid system *after* accounting for the fact that the flexible modes will accelerate in response to any applied force.

Crucially, the off-diagonal block $H_{\eta q}$ determines how joint torques affect shaft modes. The inversion lemma gives:
$$ H_{\eta q} = - M_{\eta\eta}^{-1} M_{\eta q} H_{qq} = - M_{\eta\eta}^{-1} M_{\eta q} \Delta^{-1}. $$
