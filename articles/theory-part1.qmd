...
The dynamics of the system evolve on the **tangent bundle** $T\mathcal{Q}$, the smooth manifold formed by the union of all tangent spaces: $$T\mathcal{Q} = \bigcup_{p \in \mathcal{Q}} T_p \mathcal{Q}.$$ We define the full state vector $x$ as a point in this bundle. In local coordinates: $$x = [q^{T}, \eta^{T}, \dot{q}^{T}, \dot{\eta}^{T}]^{T} \in T\mathcal{Q} \cong \mathbb{R}^{2(n+m)}.$$
...
This explicit derivation proves the **Mechanical Orthogonality** of the system: the golfer's only "handle" on the system is the torque vector $u$, and its influence is strictly constrained by the matrices $H_{qq}$ and $\Gamma$. The term $H_{qq} B(q)$ represents the instantaneous **Control Efficacy**—or **Mobility Ellipsoid**—which defines the geometric capacity of torque to alter the trajectory.
...
Within these constraints, the affine decomposition is exact. Outside them, causal interpretations must be qualified.
:::
