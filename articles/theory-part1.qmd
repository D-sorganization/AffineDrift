...
- **Novel application:** Applying the drift/input decomposition to golf biomechanics; the frozen-coefficient assumption for biological impedance; defining ZTCF and ZVCF as diagnostic tools for swing analysis.
- **Terminology:** The terms "drift" and "input" are standard in nonlinear control. "Zero Torque Counterfactual (ZTCF)" and "Zero Velocity Counterfactual (ZVCF)" are the authors' naming for these constructions in the golf context. For the site-wide canonical ZTCF glossary (Trajectory ZTCF, drift vector $f(x)$ — **not** called ZTCF, and Branched ZTCF), see [Zero-Torque Counterfactual](zero-torque-counterfactual.html#sec-ztcf-canonical).
:::
  ...
  <div class="critics-comments-content">
    <div class="critics-comments-inner">
      ...
      <div class="critic-item">
        ...
        <div class="author-response">
          <strong>Our Response:</strong> Standard inverse dynamics provides the <em>sum</em> of forces required, but it is causally ambiguous: it cannot distinguish between a force required to <em>overcome</em> inertia and a force required to <em>exploit</em> it. The affine structure is a mathematical framework that separates the system's velocity-dependent drift from the player's active control. Without this separation, we cannot formulate the counterfactuals (e.g., "What if I stopped pushing?") that are essential for understanding control strategy.
        </div>
      </div>
...
2.  **Flexible shaft via finite-dimensional modal approximation.** The shaft is modeled using $m$ vibration modes obtained from standard beam theory (e.g., Euler--Bernoulli or Timoshenko) [@meirovitch2010methods; @shabana2020dynamics]. Modal truncation approximates the distributed dynamics with a finite-dimensional representation.
...
The clubshaft is modeled as a flexible beam clamped at the hands. The position of a material point $s \in [0, L]$ along the shaft in the world frame, denoted $p(s, q, \eta)$, is the superposition of the rigid hand motion and the local elastic deformation. Using the **Assumed Modes Method** (AMM) [@meirovitch2010methods; @shabana2020dynamics], the spatial deformation relative to the hand frame is expressed as $w(s, t) = \sum_{i=1}^m \phi_i(s) \eta_i(t)$, where $\phi_i(s) \in \mathbb{R}^3$ are the spatial mode shapes. (The symbol $w$ is used here for the transverse deformation field to avoid collision with the control input $u \in \mathbb{R}^{m_u}$ defined in the notation table.)
...
2.  **Modal Inertia** $M_{\eta\eta} = \int \rho \Phi^T \Phi ds + m_{head}\Phi(L)^T\Phi(L)$: The mass matrix of the flexible coordinates. Because $M_{\eta\eta}$ includes the discrete tip-mass term, standard cantilever mode shapes are not mass-orthogonal under this augmented metric; $M_{\eta\eta}$ is a full positive-definite matrix. The Schur-complement inversion argument requires only positive-definiteness, not identity.
...
To rigorously identify the causal contribution of muscular torque, we must derive the equations of motion in a form that explicitly separates active inputs from passive dynamics. We employ the **Lagrangian formalism** [@goldstein2002classical; @murray1994mathematical] to generate the dynamics, ensuring energy consistency, and then transform the result into **control-affine state space form** [@isidori1995nonlinear; @bullo2004geometric] to isolate the input channels.

### Lagrangian formulation

We define the composite configuration vector $q_{\text{sys}} = [q^T, \eta^T]^T \in \mathbb{R}^{n+m}$. The system dynamics are governed by the Euler-Lagrange equations [@goldstein2002classical]: $$
...
::: callout-note
## Note on Biological Impedance (The Frozen-Coefficient Assumption)

A common critique is that muscle stiffness is input-dependent ($K(u)$), which would violate the affine structure. We defend this by adopting the **frozen-coefficient assumption**: we define the "Drift" $f(x)$ as the dynamics of the system with the golfer's impedance *fixed* at its operational level, but with the *net driving torque* removed ($u=0$). This distinguishes the **Effective Plant** (which acts like a stiff, damped mechanism) from a "flaccid" ragdoll, ensuring the mathematical drift corresponds to the physically relevant "Zero Torque" baseline used in our simulations [@hogan1985impedance].
:::
...
To isolate the accelerations, we require the inverse inertia matrix $H(q) = M^{-1}(q)$. We derive the blockwise inverse explicitly using the **Schur Complement** decomposition [@featherstone2008rigid]. This step is critical because it reveals how the motion of the rigid golfer ($q$) is dynamically coupled to the motion of the flexible shaft ($\eta$).
...
$$ We define $\Gamma \in \mathbb{R}^{m \times n}$ as the **Inertial Coupling Ratio** (this is the off-diagonal block of the inverse mass matrix; we adopt the term "coupling ratio" for brevity). It acts as the system's mechanical 'gear ratio,' dictating how much acceleration is transmitted to the flexible modes for every unit of acceleration the golfer produces at the joints. Crucially, this transmission depends solely on the mass distribution ($M_{q\eta}$ and $M_{\eta\eta}$), making it a fixed property of the club's design that the golfer cannot alter during the swing.

::: {.callout-tip}
## Sign convention for $\Gamma$ --- worked 1-DOF rigid-on-spring example
Consider the minimal rigid-on-spring system: a hand mass $m_h$ at coordinate $q$ connected through a stiff spring of stiffness $k$ (massless) to a shaft mass $m_s$ at coordinate $q + \eta$, where $\eta$ is the shaft deflection relative to the hand. Using $(q, \eta)$ as generalized coordinates, the kinetic energy is
$$
T = \tfrac{1}{2} m_h \dot q^2 + \tfrac{1}{2} m_s (\dot q + \dot\eta)^2,
$$
so the mass matrix is
$$
M = \begin{bmatrix} m_h + m_s & m_s \\ m_s & m_s \end{bmatrix}, \qquad M_{qq} = m_h + m_s,\; M_{q\eta} = M_{\eta q} = m_s,\; M_{\eta\eta} = m_s.
$$
The coupling ratio evaluates to
$$
\Gamma = - M_{\eta\eta}^{-1} M_{\eta q} = - \frac{m_s}{m_s} = -1.
$$
**Physical check.** With the spring soft ($k$ small, elastic force transient), apply a unit upward hand acceleration $\ddot q = +1$. From the block inverse relation, $\ddot\eta_{\text{input}} = \Gamma H_{qq} B u$. With $\Gamma = -1$, $\ddot\eta = -1$ relative to $q$, i.e. the shaft lags the hand --- the shaft mass is left behind by the acceleration, exactly as a freely suspended shaft would bend when the hands jerk upward. This is the correct "shaft droops away from the direction of hand acceleration" behavior.

The sign in $\Gamma = -M_{\eta\eta}^{-1}M_{\eta q}$ is therefore the **only** sign consistent with the intuition that flexible modes lag rigid modes under positive drive. An accidental sign flip would predict the shaft leading the hand --- immediately falsifiable against any strobed swing video. The convention used throughout this article is the one above; no sign flip is present.
:::
...
::: {.callout-note}
## Formal definition: Drift-to-Control Ratio $\rho(x, t)$
To avoid informal appeals to "drift dominance" throughout Parts II--V, we fix a single formal definition here. For a control-affine system $\dot x = f(x) + G(x) u$ with instantaneous input-magnitude bound $\lVert u \rVert_\infty \leq u_{\max}$, define
$$
\boxed{\;\rho(x) \;:=\; \frac{\lVert f(x) \rVert_{\bm{M}}}{\lVert G(x)\, u_{\max}\mathbf{1} \rVert_{\bm{M}}}\;}
$$
where $\lVert v \rVert_{\bm{M}} := \sqrt{v^\top \bm{M}(q) v}$ is the kinetic-energy-weighted norm induced by the configuration-dependent mass matrix $\bm{M}(q)$. (This norm measures "dynamic effort": it weights each mode by its effective inertia so that comparing drift to control is comparing like with like, in units of acceleration × $\sqrt{\text{inertia}}$.)

- $\rho \gg 1$ --- **drift-dominated regime**: the passive vector field is much larger (in kinetic-energy norm) than the largest instantaneously achievable control perturbation. ZTCF is a useful baseline here.
- $\rho \ll 1$ --- **control-dominated regime**: inputs can steer the state freely in one time step.
- $\rho \sim 1$ --- the delicate regime where drift and control compete; counterfactual baselines must be interpreted carefully.

This definition is the one referenced by the standalone article [Drift--Control Ratio](controllability-drift-ratio.html) and is used consistently in Parts II--V, the Physics-of-Golf glossary, and the `controllability-drift-ratio.qmd` reference.
:::
...
The derivation above transforms the equations of motion from a description of *how the system moves* to a description of *why it moves*. By isolating the input $u$ in a separate linear term, we have created a causal map. This mathematical separation forces a shift in how we view the swing. In traditional inverse dynamics, the "net moment" is a monolithic quantity. In the affine framework, it is an aggregate of two distinct contributions: the velocity-dependent drift of the system and the active control of the player. The derivation above proves that these contributions do not mix; they are **additively separable**. The drift and input terms are not orthogonal in any inner-product sense; they are additively separable in the equations of motion [@isidori1995nonlinear].

This separability is what allows us to perform the "surgical" operations defined in the next part. If the drift field $f(x)$ encapsulates the system's passive evolution, we can theoretically allow it to play out uninterrupted. By mathematically setting the input term to zero ($u \equiv 0$), we can expose the underlying passive dynamics that the golfer is steering. This leads us directly to the concept of **counterfactual simulation**. However, the affine equation $\dot{x} = f(x) + g(x)u$ describes only instantaneous tendencies (a vector field), not an outcome. To see what the drift field $f(x)$ prescribes, we must **integrate** it over time. Part II performs this integration, using the affine structure not just to describe the swing that happened, but to computationally reveal the **counterfactual trajectories**—the unobserved trajectories that *would* have occurred had the golfer chosen differently.
...
(f) **Co-contraction not captured:** When antagonist muscles co-contract, they produce zero net torque but modulate joint stiffness in the null space of the torque map. This stiffness modulation is classified as part of the drift term $f(x)$ under the frozen-coefficient assumption (see §Note on Biological Impedance), and is not separately identifiable from the drift without additional measurements or models.
...
- **[Lagrangian Reference for Control-Affine Multibody Dynamics](lagrangian-reference.html)** — Reference derivations behind the Lagrangian formulation in this part
- **[Notation and Conventions](/pages/notation-conventions.html)** — Site-wide symbol and sign conventions used throughout the series
- **[Physics of Golf Glossary](The_Physics_of_Golf/quarto/glossary.html)** — Plain-language definitions of the swing terminology used here
:::
