...
**Companion material.** Appendix E of `articles/Launch_Monitor_Technology_Review/` (screw-theoretic clubhead
...
hence _linear_ in the twist coordinates. The directness hierarchy (tech-review
...
- **(D1) Reference-point ambiguity** — "club speed" depends on which point of
  ...
  _definitional_ variance, not sensor noise.
...
- **(D3) No uncertainty reporting** — all vendors report naked numbers; the
  ...
  true covariances), and _enables_ several routes to D2 (Sections 5–6) by
  providing the propagation machinery that lets a face-orientation _anchor_
...
contribute residuals against this _single_ trajectory at their own native
...
- (c) Dual-quaternion interpolation for pose composition where numerical
  ...
  to impact). The twist trajectory _is_ that integral, with covariance. Every
  ...
  σ²*face(t_impact) ≈ σ²_anchor + ∫ σ²*ω dt — so the design rule is _anchor as
  late as geometry allows, measure ω as well as possible throughout_.
...
- Smoothness of the ISA path (skilled-swing characteristic — usable as a prior
  _and_, inverted, as a skill metric).
...
geometry identifies _where_ on the head each Doppler row originated, breaking
...
That timestamp is a _direct orientation fix_: face normal ∥ line of sight at
...
closed-form (Kabsch on Doppler-consistent positions), i.e., _radar-measured face
orientation_ with sticker-level intrusiveness. Precedent: Titleist RCT balls
...
geometrically a bundle of high-contrast _lines_: shaft axis, topline, leading
...
_on the face itself_. If a camera resolves even two grooves for even one frame,
...
reflect structured illumination; observing a _known pattern's reflection_ in the
...
continuous-time prior, which lets _fewer, noisier_ frames yield a stable pose —
...
| F10 | Well-posed inversion (fallback)       | ball launch + _measured impact location_ → face | model                  | impact                    | upgrade of status quo: with impact location known, gear effect is corrected and the inversion becomes well-conditioned; retain as cross-check |
...
transition = ISA migrating from center height to surface — so _true roll
percentage_ is a direct screw observable from any sensor that resolves both v
and ω early in the putt (camera dimple tracking does). Novel measurand;
putter-fitting relevance; distinct from bounce/slide/roll _classification_
approaches.
...
7.2. Use the twist measurement matrix as a _design tool_: sensor placement,
