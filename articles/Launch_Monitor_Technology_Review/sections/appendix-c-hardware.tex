...
This appendix is a specification-level reference for the commodity sensing
components a launch monitor can be built from, drawn from vendor datasheets
and application notes (all linked). The parts covered here are
representative rather than prescriptive: they are the modules for which
manufacturers publish enough detail to reason about performance, which makes
them useful worked examples whether or not they end up in a given design.

Two of them are documented for this application by their own manufacturers.
OmniPreSense's sports application note AN-029 covers a golf configuration
directly~\cite{an029}, which is unusual and worth exploiting --- most radar
modules are documented for traffic and presence sensing, leaving the
sports-specific parameters to be derived from first principles.
...
split (default 8 $\to$ 1024 pre + 3072 post). At 30\,ksps
...
Gate output directly to HOST\_INT} (the standard hardware-trigger wiring) and
...
club-speed work; ball speed must come from a separate CW module. Frame time at
...
Rx2@$f_A$, Rx1@$f_B$ --- the payload an interferometric angle solution
...
frequency (three channels for multi-module coexistence --- set
co-located units to different channels), \texttt{TRFT} tracking filter,
...
TI's forums confirm no golf-specific lab exists. The natural
role: a single sensor that measures range, angle, and radial speed
simultaneously --- replacing a pair of single-baseline angle modules ---
once the custom chirp work is done.
...
launch-monitor protocol and is the sensible native output
...
A typical CFAR setting (SNR $>15$ over a 150-bin DC mask) is a
