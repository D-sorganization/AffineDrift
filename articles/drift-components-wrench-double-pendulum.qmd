...
This decomposition allows us to answer: **"Is the club gaining energy because I am pulling on it (drift-induced transfer) or because I am twisting it (active transfer)?"** Drift-induced transfer is passive from the viewpoint of active input, but it is mediated by the state history $q(t), \dot q(t)$.
...
This derivation assumes a **Rigid Body** model for all links. Consequently, the "Active Wrench" $w_{\text{act}}$ is assumed to transmit instantaneously from the hands to the clubhead. This omits the **elastic potential energy** storage and release dynamics characteristic of a flexible shaft (the Stretch-Shortening Cycle). In a flexible model, the active input $u$ would first do work on the shaft stiffness ($V_{elastic}$), introducing a phase delay before appearing as kinetic energy at the clubhead. Thus, this rigid-body decomposition represents the "Stiff Shaft Limit" of the true dynamics.

### Suggested Cross-References
*   **Proximal-to-Distal Energy Transfer:** For the applied experimental timing sweep and Robertson–Winter interface power accounting on this same 2-DOF double pendulum model ([Proximal-to-Distal Energy Transfer](/articles/proximal-distal-energy-transfer.html)).
