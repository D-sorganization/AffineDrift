...
- **ZTCF** corresponds to the *trajectory* obtained by setting $u = 0$ in the Euler--Lagrange equations and integrating forward from the actual initial condition, so that
$$
M_{\mathrm{ext}}\ddot{q}_{\mathrm{ext}} = \tau_{\mathrm{drift,ext}},
$$
and the motion is governed solely by the drift.

- **ZVCF** is the *instantaneous-acceleration* computation performed at $\dot{q}_{\mathrm{ext}} = 0$, in which all velocity-dependent drift terms vanish while configuration-dependent forces (gravity and elastic forces) are retained. Input is removed in this construction, so the only remaining generalized forces are the configuration-dependent subset of drift: $\tau_{\mathrm{ZVCF}} = G_{\mathrm{ext}}(q_{\mathrm{ext}}) + \tau_{\mathrm{elastic,ext}}(q_{\mathrm{ext}})$. It is a pointwise diagnostic of the drift field at zero velocity, not a trajectory. (Cross-reference: this definition agrees with the ZVCF in theory-part2 and screw-theory-reference, where $\mathcal{F}_{\mathrm{ZVCF}} = \mathcal{F}_{\mathrm{drift}}(q,\eta,0,0) \subseteq \mathcal{F}_{\mathrm{drift}}$.)
...
<strong>Think of it like:</strong> Riding a bicycle downhill. The "Drift" is the bike speeding up just because of the slope and your current speed. The "Input" is you deciding to pedal harder. Your total speed is the combination of the hill's pull plus your leg power.
      ...
      <div class="laymans-item">
        ...
        <p>This math proves that your muscle effort enters the equations of motion in a direct, linear way — it doesn't get tangled up with other forces. However, because the system as a whole is nonlinear, doubling your muscle effort does <em>not</em> simply double your clubhead speed. The swing's response to effort depends on configuration, velocity, and timing.</p>
        <div class="analogy">
<strong>Think of it like:</strong> A volume knob on a stereo connected to a complex equalizer. Turning the knob up increases the signal linearly, but the actual sound you hear also depends on the equalizer settings — which change throughout the song. Your muscles "turn up the volume" linearly on the input forces, but the final clubhead speed reflects the whole nonlinear system.
        </div>
      </div>
...
- **[Notation and Conventions](/pages/notation-conventions.html)** — Canonical symbol and sign conventions; defines the Lagrangian equations-of-motion form and gravity covector
- **[Screw Theory Reference](screw-theory-reference.html)** — Companion reference covering twists, wrenches, and spatial (6-D) operators for the same multibody setting
- **[Rotation Representations Reference](rotation-representations-reference.html)** — Companion reference for the $SO(3)$ parameterizations used by the spatial operators
- **[Theory Part 1: Affine Control Interpretation of the Golf Swing](theory-part1.html)** — Derives the control-affine swing model via the Lagrangian formalism
- **[Sources of Nonlinearity](sources-of-nonlinearity.html)** — Where the Lagrangian-derived affine model breaks down (biological and physical nonlinearities)
- **[Ch. 3: The Double Pendulum](The_Physics_of_Golf/quarto/ch03_double_pendulum.html)** — Physics-of-Golf chapter deriving the canonical double-pendulum swing with this formalism
:::
