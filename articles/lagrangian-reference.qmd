...
- **ZVCF** is the *instantaneous-acceleration* computation performed at $\dot{q}_{\mathrm{ext}} = 0$, in which all velocity-dependent drift terms vanish and (by convention) gravity is also held off, so the only remaining generalized forces come from $\tau_{\mathrm{input,ext}} = [B u; 0]^\top$. It is a pointwise diagnostic, not a trajectory.
