...
- ZVCF corresponds to freezing $\dot{q}_{\mathrm{ext}} = 0$ (zero velocity) and removing input $u = 0$, so that the only remaining generalized forces are the configuration-dependent drift contributions (such as gravity and elasticity), isolating them from velocity-dependent effects.
