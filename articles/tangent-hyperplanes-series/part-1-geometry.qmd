...
What does "exact" mean? Suppose you move by a perturbation $\delta x$ with $\|\delta x\| = \epsilon$ in a direction in the tangent space. The residual difference between where you land and where the constraint surface lies then decays as $O(\epsilon^2)$ — quadratically in perturbation size:

$$
\|x + \delta x - \text{(closest point on } M)\| = O(\|\delta x\|^2)
$$

This is not an approximation statement. It is a precise asymptotic characterization. By "exact," we mean in the **infinitesimal limit**: the tangent hyperplane captures the local geometry with no error as $\|\delta x\| \to 0$. The Fréchet derivative **is** the linear map, not an approximation to one. For finite perturbations, second-order residuals arise; they are what make the tangent hyperplane an "excellent approximation" rather than a perfect one. [Part 4](./part-4-residuals-curvature.qmd) bounds them quantitatively. The term "exact" refers to the infinitesimal limit, not to finite-step accuracy.
...
With tangent spaces established, we can ask: what happens to the dynamics when the system evolves along a manifold? In [Part 2](./part-2-dynamics.qmd), we will see how the equations of motion decompose into a linear system on the moving tangent space.
