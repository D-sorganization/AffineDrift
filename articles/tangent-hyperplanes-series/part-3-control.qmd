...
aliases:
  - ../tangent-hyperplane-contraction/chapters/03-local-optimal-control.html
  - ../tangent-hyperplane-articles/Drafts_Original_Articles/Tangent_Hyperplanes_Series_Package/03_Tangent_Hyperplanes_III_Control.html
---

## The Linearization-Based Control Principle

If nonlinear dynamics are exactly linear at each instant, then we can use linear control tools — LQR, Riccati equations, all the machinery of linear systems theory — at each instant along a trajectory. **This is the key insight behind differential dynamic programming (DDP), the iterative linear-quadratic regulator (iLQR), and virtually every practical nonlinear control method.**
...
The power of linearization becomes clear in DDP and iLQR. These algorithms work as follows:
...
**LQR is exact for the linearized system.** That is: solve the Riccati equation and apply the feedback control to the linear system $\delta\dot{x} = A(t)\delta x + B(t)\delta u$. You will get the optimal trajectory for that linear system, to machine precision.

The linearized LQR solution does not capture residuals from nonlinearity — the $O(\|\delta x\|^2)$ terms we dropped. Iteration *handles* those residuals instead. You apply the control, integrate the nonlinear system forward, see what trajectory you get, linearize around it, solve LQR again, and repeat. Each iteration accounts for the mismatch between linear and nonlinear dynamics.

This is why DDP/iLQR works so well: it is not trying to solve the nonlinear problem in one shot. Instead, it leverages the infinitesimal exactness of linearization and uses iteration to handle the nonlinear corrections systematically.

## Connection to Stability and Optimality

The site's [Stability-Optimality Duality](../tangent-hyperplane-contraction/chapters/04-stability-optimality-duality.qmd) framework reveals a deep connection: **The Riccati matrix $S(t)$ serves a dual role.**
