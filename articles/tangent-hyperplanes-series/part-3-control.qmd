...
Because the first variation of a smooth nonlinear system is linear along a specified trajectory, linear control tools can be used locally around that trajectory. This is one of the main ideas behind DDP, iLQR, and related trajectory-optimization methods.

The computational pattern is: linearize around a nominal trajectory, solve a local linear-quadratic problem to get a feedback gain, apply or test that update on the nonlinear system, and repeat. Under appropriate smoothness, initialization, and regularization conditions, these iterations can converge to a local solution of the nonlinear optimal-control problem. Those conditions matter; the method is not a global guarantee.
...
The role of linearization becomes clear in DDP (Differential Dynamic Programming) and iLQR (iterative Linear-Quadratic Regulator). A simplified version of these algorithms is:
...
This is why DDP/iLQR can be effective on well-conditioned local problems: the method does not try to solve the nonlinear problem in one linearized step. It uses repeated local models and line-search or regularization logic to manage the nonlinear corrections.

## Connection to Stability and Optimality

The site's [stability-optimality chapter](../tangent-hyperplane-contraction/chapters/04-stability-optimality-duality.html) discusses a narrower connection: in linear-quadratic settings, the Riccati matrix $S(t)$ can be interpreted both as a value-function Hessian and as a metric-like object for local perturbation dynamics.
...
This does not make optimality and stability equivalent in all nonlinear problems. It is a local relationship that is strongest for LQR-like assumptions and becomes more delicate when constraints, strong nonlinearities, mode switches, or poor conditioning are present.
...
We have now seen the chain from tangent spaces to variational dynamics and local optimal control. The final piece is understanding the error: what happens when the linearized model stops being predictive? In [Part 4](./part-4-residuals-curvature.html), we examine residuals and curvature, the mechanisms by which nonlinearity limits the range of validity of linear tools.
