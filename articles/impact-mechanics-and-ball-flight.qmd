...
Right-handed coordinate frame for a right-handed golfer: $x$ along the target line, $y$ vertical, $z$ to the right of the target line looking down it. Angles positive right and up. Unless a measurement source specifies otherwise, club-delivery quantities refer to **immediately before first contact** and ball launch quantities to **separation**. Compression changes club motion; maximum-compression values must not be substituted into pre-impact momentum or restitution formulas. Where a quantity depends on which point of the clubhead is used as reference — and several do — the reference point is stated.
:::
...
| Closure rate | Time derivative of the face-normal azimuth; depends on attitude and angular velocity |
| Swing plane, swing direction | A declared trajectory-plane fit or local osculating-plane construction; not determined by the instantaneous twist alone |
...
Spin loft $\Phi$ is the angle between the local face unit normal $\hat n$ and the incident contact-point velocity direction $\hat v$. Let face azimuth be $\phi$, path azimuth $\psi$, dynamic loft $\lambda$, and attack angle $\alpha$. In the forward/up/right frame,
...
For zero face-to-path, $\Phi=|\lambda-\alpha|$ within the relevant angle range. In general, $\cos(\phi-\psi)\cos(\lambda-\alpha)$ is **not** the exact dot product. At face-to-path $30^\circ$, loft $40^\circ$, and attack $20^\circ$, the expressions already differ materially. Near a common elevation $\alpha$, small separations give $\Phi^2\simeq(\lambda-\alpha)^2+\cos^2\alpha(\phi-\psi)^2$, with angles in radians.

Spin loft sets the obliqueness of compression and shear. Spin also depends on contact speed, friction, tangential compliance, strike position, and incident club rotation. A three-dimensional angle organizes those inputs; it does not replace the contact model.
...
The rotational contribution is nonnegative: it depends on the perpendicular moment arm to the impulse line, not simply the distance to the centre of gravity. This rigid-body mechanism can reduce ball speed and contribute to gear effect. Real face compliance and local restitution can also vary with strike location.
...
For an initially stationary ball in a one-dimensional collision, the [USGA's separation-speed definition](https://www.usga.org/content/usga/home-page/articles/2025/05/what-happens-collision-between-club-ball.html) is the ordinary relative-speed COR: $e=(v_b^+-v_c^+)/v_c^-$. An apparent rebound coefficient from a different experimental setup must be defined separately. Neither COR nor its square is a general fraction of the club's energy delivered to the ball.
...
Contact time is not a constant. It **decreases with impact speed** ([Arakawa et al. 2009](https://doi.org/10.1007/s11340-008-9156-y)), but differing reported durations can also reflect ball construction, face compliance, and measurement method. The two quoted durations cannot be assigned to the same ball at different speeds without the underlying experiment. This matters more than it looks: finite-element golf-ball models are calibrated by fitting maximum load, contact time, deformation history and rebound velocity *simultaneously*, so contact time is a model-identification target, not merely a prediction.
...
**Loft and speed.** In the stationary-ball, normal-only model, the outgoing velocity is $\mathbf v_b=C(\mathbf v_c\cdot\hat n)\hat n$, with $C=(1+e)/(1+m/M_{\rm eff})$. Consequently, the **speed ratio** is $C\cos\Phi$. Its projection on the incoming club direction is $C\cos^2\Phi$; that projection is not smash factor. Tangential impulse changes both speed and launch direction. Restitution can depend on impact speed and the interacting ball/face construction, so a fixed-$e$ example is not a universal equipment ceiling.
...
Three limitations are worth stating, because the D-plane is often presented as more complete than it is. The two vectors define a geometric plane whenever they are nonparallel, including off-centre strikes, but its simple spin prediction neglects off-centre gear effect (Part III). It does not specify *when* "just before impact" is, though both vectors rotate measurably through the ~0.5 ms of contact (Part VI). And practitioners routinely assume a straight-line path tangent that Jorgensen never asserted.
...
There is a second correction that matters more conceptually. **The "85/15 driver" and "75/25 wedge" rules are not two laws — they are one curve evaluated at two spin lofts.** Wood et al. show the ratio declining monotonically and non-linearly with obliqueness $\Phi$; a published fit $DA_x = A(0.96 - 0.0071\Phi)$ reproduces the folklore exactly, giving 85.3% at $\Phi = 15°$ and 74.7% at $\Phi = 30°$. The fitted relation describes the tested conditions. Club construction, strike location, and contact properties can still matter at matched spin loft.
...
For an initially nonspinning spherical ball in a centred, symmetric oblique-impact model, the spin direction is parallel to $\hat v\times\hat n$. This follows from the moment of the tangential impulse at the ball contact point. The direction degenerates when the two vectors are parallel: the ideal model then generates no oblique-impact spin. Gear effect, initial spin, asymmetric contact, and finite face rotation require additional mechanics.

Compute spin tilt from vectors in a declared launch frame. Given outgoing unit velocity $\hat v_b$ and world-up vector $\hat e_y$, define

$$
\hat e_b=\frac{\hat v_b\times\hat e_y}{\|\hat v_b\times\hat e_y\|},
\qquad \hat e_u=\hat e_b\times\hat v_b,
\qquad \mathbf s=\hat v\times\hat n.
$$

Here $\hat e_b$ is the positive backspin axis for the declared forward/up/right convention. One signed tilt convention is

$$
\gamma=\operatorname{atan2}(-\mathbf s\cdot\hat e_u,\ \mathbf s\cdot\hat e_b).
$$

State the sign convention when comparing monitor outputs. A vertical launch needs a different frame construction. The cross product gives a direction, not the spin magnitude; the collision model supplies the magnitude.

The frequently repeated $\arctan[\tan A/\tan L]$ relation is not a general identity for horizontal face-to-path $A$ and loft-minus-attack $L$. The full vectors retain both elevations and specify the reference axis. Small-angle sensitivities can explain why a low-loft strike has a large tilt response, but matching a vendor rule of thumb does not identify the vendor's internal algorithm.

Tilt alone does not determine yards of curve. Launch speed, total spin, aerodynamic coefficients, wind, and flight duration also matter. In particular, **yards per degree of face-to-path** and **yards per degree of spin-axis tilt** have different denominators. The earlier comparison of 9.5 with 2.7 yards per degree did not establish a factor-of-three physical disagreement without matching those variables and flight conditions.
...
### IV.6 Launch Weighting and Contact Mechanics

Launch direction and spin are coupled outputs of the same normal and tangential impulses. For an initially stationary ball, let $J_n$ and $J_t$ be their scalar components in the obliquity plane. Then

$$
\tan\delta_b=J_t/J_n,
\qquad m\mathbf v_b=J_n\hat n+J_t\hat t.
$$

Here $\delta_b$ is launch deflection from the normal and $\hat t$ is the in-plane tangent. A tangential-compliance model changes $J_t$, so it changes both launch weighting and spin. This is the physical connection between those measurements.

Quantifying that connection requires matched ball, face, impact speed, obliqueness, effective mass, and coefficient definitions. A tangential restitution coefficient measured on granite at low speed cannot simply be inserted alongside an ambiguously defined driver COR to explain a high-speed population average. The previous 84/81/77-percent table mixed those assumptions and did not establish why a particular experiment found 76-percent face weighting. A useful next test measures both outgoing spin and launch direction under the same controlled impacts and fits one contact model to both.
...
The measurement side of this section — how far the impact point's path differs from the reported reference-point path for a given delivery — is treated separately in [The Reference-Point Problem in Club Delivery Data](reference-point-problem.qmd), with an interactive tool that computes it for any combination of speed, rotation rate, lie angle and strike location.
:::

### VI.1 Reference-Point Velocity and Direction

The exact relation is $\Delta\mathbf v=\boldsymbol\omega\times\mathbf r$. For small offsets, the change in the unit velocity direction is

$$
\delta\hat v\simeq\frac{(I-\hat v\hat v^T)(\boldsymbol\omega\times\mathbf r)}{\|\mathbf v\|}.
$$

Only the component perpendicular to the original velocity changes its direction to first order. Horizontal path differences need the corresponding horizontal projection; a vertical or along-path velocity change cannot simply be counted as horizontal path. Angular velocity and the offset vector must be expressed in the same frame.

For $\boldsymbol\omega\ne0$, distance to the instantaneous screw axis is

$$
R_{\rm ISA}=\frac{\|\mathbf v_\perp\|}{\|\boldsymbol\omega\|},\qquad
\mathbf v_\perp=\mathbf v-(\mathbf v\cdot\hat\omega)\hat\omega.
$$

Using total speed $v/\omega$ requires zero axial translation. The estimate $\Delta\theta\simeq d/R_{\rm ISA}$ additionally requires a suitably oriented offset and a velocity difference perpendicular to the original velocity. A scalar closure rate is the derivative of face azimuth; it is not generally the magnitude of the full club angular velocity. Consequently, a three-degree path offset alone does not locate the screw axis or identify release technique.

### VI.2 Scaling and Illustrative Magnitudes

If the full twist scales by the same factor while attitude and reference-point separation remain fixed, the direction offset is unchanged. Increasing speed alone does not specify how the rotational part changes. For a deliberately simplified case with $d=0.04$ m, $v=50$ m/s, and a perpendicular angular contribution of $34.9$ rad/s, the transverse speed difference is $1.396$ m/s and the small-angle direction difference is about $1.6^\circ$. These assumptions explain the estimate; they do not establish a population-average offset.

### VI.3 Horizontal and Vertical Components

Evaluate both components of the same cross product for the measured delivery. Shaft lean, lie, strike location, and all components of angular velocity can change their signs and magnitudes. A driver and an iron have different reference-point offsets, but the equipment label alone does not determine the correction. The linked reference-point tool provides a specified geometry for investigating these dependencies.
...
These are constant-rate estimates of face-angle change during contact. Actual club rotation changes under the collision impulse; the resulting launch bias requires integrating the contact forces and local normals. It is not a universal draw correction.
...
Face-angle error affects launch direction (see @sec-weighting), and obliquity affects spin direction. Whether timing sensitivity dominates other sources of shot variation depends on their measured magnitudes and covariances. A high-closure release is a **high-gain system**: it will square the face, and it converts small timing errors into large directional ones.
...
A between-player accuracy prediction requires assumptions about timing-error distributions and other delivery variables. The evidence below does not establish a clear accuracy disadvantage for faster handle twist; a nonsignificant association is not proof of equivalence.
...
**Adapted timing is one possible explanation, not an identified mechanism.** Other possibilities include covariance among delivery variables, differences between handle twist and face closure, limited statistical power, and selection of skilled players. The population association does not measure each player's timing-error distribution or prove that gain and precision co-adapt.
