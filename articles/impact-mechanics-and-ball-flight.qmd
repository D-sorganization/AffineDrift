...
::: {.callout-important}
## The 85/15 rule conflates two different planes

The 83–87% figures that circulate are **vertical**-plane numbers — launch angle relative to dynamic loft. The **horizontal** ratio, which is what "85% face, 15% path" is always used to describe, measures **76% ± 8% for a driver and 69% for a 7-iron**.

Jorgensen's "just over 80%" was likewise a *vertical* derivation. The number migrated planes somewhere in transmission, exactly as the old laws did.
...
There is also a cost that is rarely acknowledged. [Beratlis, Balaras & Squires (2019)](https://doi.org/10.1017/jfm.2019.647) show dimples **impose a local drag penalty** accounting for up to 60% of total drag in the post-critical regime, which is why a dimpled ball's $C_D \approx 0.25$ never approaches a smooth sphere's post-critical minimum of 0.07–0.1. Dimples are not free; they buy separation delay by paying surface drag, and the trade is favourable only because the ball would otherwise be stuck subcritical.
