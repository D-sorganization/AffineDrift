...
class OptimizeResult:
    """Result returned by :meth:`ILQRSolver.optimize`.

    Attributes
    ----------
    x_traj:
        State trajectory of shape ``(N+1, n_x)``.
    u_traj:
        Control trajectory of shape ``(N, n_u)``.
    t_traj:
        Time vector of shape ``(N+1,)``.
    converged:
        ``True`` when the solver met the tolerance criterion before exhausting
        *max_iters*; ``False`` otherwise.
    iterations:
        Number of iLQR iterations actually performed.
    final_cost:
        Quadratic tracking cost evaluated on the returned trajectories.
    message:
        Human-readable summary of the termination reason.
    """
...
class ILQRSolver:
    ...
    def optimize(
        ...
        """Runs the iLQR algorithm and returns an :class:`OptimizeResult`."""
        ...
        converged = False
        iteration = 0
        message = f"Reached maximum iterations ({max_iters}) without convergence."
        ...
        for iteration in range(max_iters):
            ...
            if not accepted:
                message = f"Line search failed to find a descent step at iteration {iteration + 1}."
                break
            if max_k < tol:
                converged = True
                message = (
                    f"Converged: max feedforward gain {max_k:.2e} < tol {tol:.2e}"
