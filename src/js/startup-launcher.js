...
(function () {
  ...
  function createSplashScreen() {
    ...
    if (document.body) {
      ...
      // Progress element can only be found after splash is in DOM
      state.progressElement = document.getElementById('ad-splash-progress-bar');
    } else {
    ...
    // Note: state.progressElement is assigned inside the blocks above to ensure DOM presence
  ...
  function logFinalMetrics() {
    ...
    const summary = {
      'Navigation Start to DOM Ready': metrics.domContentLoaded
        ? (metrics.domContentLoaded - metrics.navigationStart).toFixed(2) + 'ms'
        : 'N/A',
      'Navigation Start to All Resources': metrics.resourcesLoaded
        ? (metrics.resourcesLoaded - metrics.navigationStart).toFixed(2) + 'ms'
        : 'N/A',
      ...
      'Splash Duration': (metrics.splashHidden && metrics.splashShown)
        ? (metrics.splashHidden - metrics.splashShown).toFixed(2) + 'ms'
        : 'N/A'
    };
