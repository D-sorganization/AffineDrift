...
# Canonical double-pendulum dynamics live in ``rl_funnel_dynamics`` and the
# shared constants/helpers in ``rl_funnel_support``. They are re-exported here
# for backward compatibility so existing imports of e.g.
# ``rl_funnel_benchmark.double_pendulum_drift`` keep working after the
# duplicated bodies were removed (issue #3180 DRY cleanup).
__all__ = [
    "CONTROL_SATURATION_DEFAULT",
    "DEFAULT_CONTROL_SATURATION",
    "GRAVITY_M_S2",
