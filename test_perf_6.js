...
const script5 = testScript.replace(`        filtered = state.entries.filter((entry) => {
...
`        filtered = [];
        for (let i = 0; i < state.entries.length; i++) {
            ...
            for (let j = 0; j < queryTerms.length; j++) {
                ...
                // ⚡ Bolt Optimization: Score during filter to avoid second pass and use indexOf instead of includes for faster substring search
...
runTest(script5, 'Combined Loop with indexOf');
