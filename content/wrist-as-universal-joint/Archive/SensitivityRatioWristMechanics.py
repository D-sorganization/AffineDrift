...
# Calculate Errors
error_finger_grip = calculate_angular_displacement(
    torque_range, I_alpha, dt
)  # Displaces swing path (Speed/Direction)
error_palm_grip = calculate_angular_displacement(
    torque_range, I_beta, dt
)  # Displaces Face Angle (Spin/Slice)
...
plt.plot(
    torque_range,
    error_finger_grip,
    label=f"Finger Grip (Routing to Swing Plane)\nInertia: {I_alpha:.3f} kg·m²",
    color="green",
    linewidth=2,
)
...
plt.plot(
    torque_range,
    error_palm_grip,
    label=f"Palm Grip (Routing to Shaft Axis)\nInertia: {I_beta:.4f} kg·m²",
    color="red",
    linewidth=2,
)
...
plt.annotate(
    f"Face twists ~{error_palm_grip[50]:.1f}°\nwith 5Nm torque!",
...
plt.annotate(
    f"Path varies only ~{error_finger_grip[50]:.2f}°",
...
print(
    f"{'Resulting Displacement':<25} | {error_finger_grip[50]:.2f} degrees         | {error_palm_grip[50]:.2f} degrees"
)
