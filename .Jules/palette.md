...
## 2024-03-31 - Accessible Collapsible Sections on the Home Page
**Learning:** Found that the sidebar collapsible sections on the home page (`.sidebar-section-toggle`) used `aria-expanded` but were missing the corresponding `aria-controls` on the toggle button and the dynamic `aria-hidden` attribute on the collapsible content. This pattern was missing in the homepage sidebar logic, even though it is present elsewhere (e.g. `js/ui-components.js`). Proper ARIA attributes are critical for screen readers to understand the relationship between a button and the section it reveals/hides.
**Action:** When implementing or fixing collapsible sections, always ensure the toggle button has both `aria-expanded` and `aria-controls` pointing to the ID of the content element, and that the content element has an `aria-hidden` attribute that dynamically updates to match the inverse of `aria-expanded`.
