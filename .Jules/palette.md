...
## 2025-03-31 - Static UI Accessibility requires dynamic JS state management
**Learning:** Found a pattern where visually appealing, static HTML interactive components (like collapsible sidebars) had initial ARIA attributes, but lacked `aria-controls` links and dynamic updates to `aria-hidden` when toggled.
**Action:** Always ensure that statically defined UI widgets that toggle state are accompanied by JavaScript to dynamically manage ARIA states (like `aria-hidden` and `aria-expanded`). Also ensure control elements use `aria-controls` to link to their target container.
