...
## 2026-07-21 - Add Loading Spinner to Mailto Forms
**Learning:** Sighted users rely on visual momentum for asynchronous operations (like submitting a mailto form that opens an external email client). Merely changing the text to 'Opening Email Client...' lacks expected motion feedback.
**Action:** Explicitly inject an SVG loading spinner with aria-hidden into the submit button to provide visual momentum. Restoring the original content via textContent is a safe and efficient way to remove appended children.
