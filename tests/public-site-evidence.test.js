...
describe("public-site visual evidence contracts (SITE-UX)", () => {
  ...
  test("builds a bounded 18-cell supplemental scenario plan", () => {
    const manifest = fixtureManifest();
    const plan = supplementalScenarioPlan(manifest);

    expect(plan).toHaveLength(18);
    expect(new Set(plan.map((item) => item.scenarioId))).toEqual(
      new Set([
        "footer",
        "dense-content",
        "keyboard-focus",
    ...
    expect(
      scenarioScreenshotName("/home.html", "footer", "mobile", "dark"),
    ).toBe("home__footer__mobile__dark.png");
  });

  test("supplemental scenarios fail closed on duplicate ids and unsupported inputs", () => {
    const duplicate = fixtureManifest();
    duplicate.verification.supplemental.scenarios[1].id = "footer";
    expect(() => supplementalScenarioPlan(duplicate)).toThrow(/scenario id/);

    const missingRoute = fixtureManifest();
    missingRoute.verification.supplemental.scenarios[0].route = "/missing.html";
    expect(() => supplementalScenarioPlan(missingRoute)).toThrow(/route/);

    const unknownViewport = fixtureManifest();
    unknownViewport.verification.supplemental.scenarios[0].viewports = [
      "cinema",
    ];
    expect(() => supplementalScenarioPlan(unknownViewport)).toThrow(/viewport/);
