...
def test_deploy_workflow_integrity():
    ...
    assert "tools/check_site_health.py" in content, "Post-build site health check missing from workflow"
...
def test_requirements_integrity():
    ...
    assert "beautifulsoup4" in reqs, "beautifulsoup4 missing from requirements (needed for health check)"
