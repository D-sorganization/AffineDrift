...
def test_deferred_route_partition_is_exhaustive_and_exact() -> None:
    ...
    reviewed_completed_batches = [
        ...
        and (
            ...
            or record["route"].startswith("/critiques/")
            or record["route"].startswith(
                (
                    "/articles/tangent-hyperplane-articles/",
    ...
    assert [record["route"] for record in deferred] == ["/articles/putting-roll-models.html"]
    ...
    assert companion["status"] == "reviewed"
    assert companion["review"]["source_path"] == (
        "articles/null-space-constraint-jacobian-bibliography.qmd"
    )
