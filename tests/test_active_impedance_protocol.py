...
def test_preregistration_freezes_sources_safety_phases_models_and_hypotheses() -> None:
    ...
    assert protocol.human_gate.eligible is False
...
def test_full_rank_synthetic_fixture_recovers_phase_specific_effective_parameters(
    ...
    report = assess_identifiability(design, model.maximum_condition_number)
...
def test_emg_co_contraction_is_a_normalization_sensitive_proxy_not_stiffness() -> None:
    agonist, antagonist = manufactured_emg_pair()
    baseline_proxy = co_contraction_proxy(agonist, antagonist)
    rescaled_proxy = co_contraction_proxy(agonist, 0.5 * antagonist)

    assert 0.0 <= baseline_proxy <= 1.0
    assert np.isclose(baseline_proxy, co_contraction_proxy(antagonist, agonist))
    assert not np.isclose(baseline_proxy, rescaled_proxy)
    with pytest.raises(ValueError, match="aligned"):
        co_contraction_proxy(agonist[:-1], antagonist)
    with pytest.raises(ValueError, match="nonnegative"):
        co_contraction_proxy(agonist, -antagonist)
