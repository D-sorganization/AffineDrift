...
test.describe("Article Pages", () => {
  ...
  test("should render MathJax equations", async ({ page }) => {
    ...
    const mathElements = page.locator(
      '.MathJax, mjx-container, .mjx-chtml, svg[class*="MathJax"]',
    );
    await expect(mathElements.first()).toBeVisible({ timeout: 10000 });
  ...
  test("should display table of contents on article pages", async ({
    page,
  }) => {
    await page.goto("/articles/inverse-dynamics.html");
    ...
    const toc = page.locator(
      '#TOC, .toc, nav[aria-label*="Table of Contents"]',
    );
  ...
  test("should have proper article metadata", async ({ page }) => {
    ...
    const metaElements = page.locator(
      ".author, .date, .quarto-title-meta, time",
    );
