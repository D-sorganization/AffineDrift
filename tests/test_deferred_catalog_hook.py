...
def _copy_gate(tmp_path: Path) -> None:
    ...
    destination.mkdir()
    for name in ("deferred_validation.py", "deferred_planning.py", "handoff_validator.py"):
...
@pytest.mark.parametrize("fault", ["premature_activation", "missing_checker", "dual_catalog"])
def test_configured_hook_rejects_invalid_publication(tmp_path: Path, fault: str) -> None:
    _hook()
    _copy_gate(tmp_path)
    if fault == "premature_activation":
        ...
        data["entries"][0][
            "activation_issue"
        ] = "https://github.com/D-sorganization/AffineDrift/issues/4445"
...
def test_installed_bundle_matches_published_receipt() -> None:
    ...
    receipt = json.loads(
        (ROOT / "docs/development/deferred-catalog-bundle.json").read_text(encoding="utf-8")
    )
