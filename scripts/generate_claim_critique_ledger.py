...
def _route(source_path: str, from_dir: str = ".") -> str:
    """Route to a rendered critique page relative to the including file."""
    target = Path(str(source_path)).with_suffix(".html").as_posix()
    return Path(os.path.relpath(target, start=from_dir)).as_posix()


def _page_links(pages: list[object], from_dir: str = ".") -> str:
    links: list[str] = []
    for page in pages:
        label = Path(str(page)).stem.replace("-", " ")
        label = re.sub(r"(?<=[A-Za-z])(?=\d)", " ", label).title()
        links.append(f"[{html.escape(label)}]({_route(str(page), from_dir)})")
    return ", ".join(links)
...
def _render_status(rows: list[dict[str, object]], digest: str) -> str:
    ...
    for row in rows:
        lines.append(
            f"| [{html.escape(str(row['title']))}]({_route(str(row['source_path']), STATUS_DIR)}) "
            f"| {str(row['severity']).title()} | {str(row['status']).title()} "
            f"| {_page_links(_list_field(row, 'affected_pages'), STATUS_DIR)} | {row['verification']} |"
        )
    lines.append("")
    return "\n".join(lines)
...
def _render_annotation(page: str, rows: list[dict[str, object]], digest: str) -> str:
    ...
    for row in rows:
        lines.append(
            ...
            f"[{html.escape(str(row['title']))}]({_route(str(row['source_path']), from_dir)}) "
            f"(`{row['critique_id']}`)"
        )
    lines.extend(["", ":::", ""])
    return "\n".join(lines)
