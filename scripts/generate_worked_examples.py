#!/usr/bin/env python3
"""Generate print and web fragments for textbook worked examples with numbers.
...
includes the result in its print or web source. Run with ``--check`` to fail when a committed
...
# Illustrative discrete pendulum-like model, nominal dt = 0.1 s, g/l = 9.81.
# A retains first-order drift while B retains a second-order position response;
# this historical example is not an exact ZOH discretization of a pendulum.
...
def render_ch06_web(solution: LQRSolution) -> str:
    """Publish the checked ch06 matrices and actual trajectory in the web edition."""
    # Reuse the print validation before publishing any web numbers.
    render_ch06(solution)
    stage = solution.stage_cost(CH06_Q, CH06_R)
    rho = solution.contraction_rate(CH06_Q, CH06_R)
    matrices = [
        ("S", solution.riccati),
        ("K", solution.gain),
        (r"A_{\mathrm{cl}}", solution.closed_loop),
        (r"D=Q+K^TRK", stage),
    ]
    lines = [
    ...
    for symbol, matrix in matrices:
        lines.extend(["$$", rf"{symbol}=\begin{{bmatrix}}{_matrix(matrix)}\end{{bmatrix}},", "$$"])
    lines.extend(
        [
            "The full-precision DARE residual is below "
            r"$10^{-10}$; the displayed matrices are rounded.",
            f"The closed-loop spectral radius is ${solution.spectral_radius():.4f}<1$.",
            rf"The metric condition number is $\kappa_2(S)={solution.condition_number():.4f}$,",
            rf"and the conservative value factor is $\rho={rho:.4f}$.",
            "The corresponding norm factor is the square root of this value.",
        ]
    )
    return "\n".join(lines) + "\n" + _ch06_web_trajectory(solution, rho)


def _ch06_web_trajectory(solution: LQRSolution, rho: float) -> str:
    """Format actual state propagation separately from the conservative bound."""
    lines = [
        "",
        ...
        "",
        r"| Step $k$ | $V_k$ | $\|x_k\|_2$ | $V_k/V_{k-1}$ | $\rho^k$ |",
        "| --- | --- | --- | --- | --- |",
    ]
    previous: float | None = None
    for step, value, norm in solution.value_trajectory(CH06_X0, CH06_STEPS):
        ratio = "---" if previous is None else f"{value / previous:.4f}"
        lines.append(f"| {step} | {value:.4f} | {norm:.4f} | {ratio} | {rho**step:.4f} |")
        previous = value
    lines.extend(
        [
            "",
...
def build() -> dict[Path, str]:
    ...
    return {
        ...
        GENERATED_DIR.parent.parent
        / "quarto/_includes/ch06_lqr_example.qmd": render_ch06_web(solution),
