...
def generate_sample_torque(noise_type, t, polynomial_expression='t**2 - t'):
...
def plot_torque(t, input_torque, grip_angle_deg, wrist_angle_deg, I_alpha, I_gamma,
...
def plot_acceleration(t, input_torque, grip_angle_deg, wrist_angle_deg, I_alpha, I_gamma,
    ...
    torque_transmitted = input_torque * tau_ratio
    torque_alpha, torque_gamma = distribute_torque_by_grip_angle(torque_transmitted, theta_grip_rad)
    accel_alpha = torque_alpha / I_alpha if I_alpha > 1e-6 else np.zeros_like(torque_alpha)
    accel_gamma = torque_gamma / I_gamma if I_gamma > 1e-6 else np.zeros_like(torque_gamma)
...
def plot_transmission_sweep(grip_angle_deg, wrist_angle_deg, I_alpha, I_gamma,
    ...
    for phi_rad in phi_sweep_rad:
        ...
        accel_alpha_ratios.append(t_alpha / I_alpha if I_alpha > 1e-6 else 0)
        accel_gamma_ratios.append(t_gamma / I_gamma if I_gamma > 1e-6 else 0)
...
with st.sidebar:
    ...
    **Moments of Inertia:**
    ...
    if plot_type == 'Torque':
        ...
        show_alpha = st.checkbox("Acceleration α", True)
        show_gamma = st.checkbox("Acceleration γ", True)
        show_velocity = False
        show_accel_alpha = False
        show_accel_gamma = False
        ...
        show_alpha = False
        show_gamma = False
        show_transmission = st.checkbox("Transmission Ratio", True)
        show_velocity = st.checkbox("Velocity Ratio", False)
        show_accel_alpha = st.checkbox("Accel α Ratio", False)
        show_accel_gamma = st.checkbox("Accel γ Ratio", False)
