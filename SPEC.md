...
|------|---------|---------|
| 2026-03-31 | 1.0.10 | perf(frontend): Optimized ARIA label generation in `script.js` by replacing O(N) `querySelectorAll` DOM traversals with O(1) live `HTMLCollection` lookups via `getElementsByTagName`, combining input attribute filtering loops to reduce main thread blocking during initialization |
