...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-10 | 1.0.122 | quality(testing/CI): Removed the dead `rust/golf_physics` crate and its never-firing `rust-quality-gate` CI job (issue #3231 — crate was unbuilt, unimported, and a divergence-prone duplicate of the tested Python `src/golf_simulation`). Added pytest coverage for the previously-unverified CI-gate scripts `check_styles_budget.py` and `check_local_only_workflows.py` (extracting pure `evaluate_budget`/`scan_workflow_text` helpers) and for `optimizer_factory.py` (issue #3230), plus Jest coverage for the ESM `js/forms.js` and `js/ui-components.js` modules via a test-only babel-jest transform (issue #3233).                                                                                                       |
