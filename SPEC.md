...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-10 | 1.0.123 | refactor(DRY): Removed the root-level `fix_html.py` post-render patcher (issue #3234) — relocated its pure normalization rules into `src.tools.utils.html_utils.normalize_html_content` and replaced the repair tool with a `tests/test_html_content_lint.py` regression gate on the static wrist article. Unified the dual color-token systems (issue #3224): the legacy brand palette in `styles.css` `:root` now aliases the canonical `css/tokens/colors.css` token set (added `--color-accent`/`--color-highlight-yellow` for the two genuinely-unique values), a zero-visual-change single-source-of-truth refactor guarded by `tests/design-tokens.test.js`. Full dark-mode-block consolidation deferred (touches the dark CSS surface).                                                                                                       |
