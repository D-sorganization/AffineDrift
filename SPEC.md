...
| 2026-05-23 | 1.0.111 | perf(frontend): Optimized global `keydown` and `mousedown` event listeners in `js/accessibility.js` by caching the keyboard active state and only mutating the DOM attribute when the state actually changes, preventing redundant layout invalidations on every keystroke. |
