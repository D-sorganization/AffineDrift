...
| 2026-05-01 | 1.0.97 | perf(frontend): optimized `id` suffix queries by replacing `querySelectorAll('[id$=\"-history-list\"]')` with `getElementsByTagName('ul')` live collections across JS bundles, and synchronized missing `document.links` optimization in `script.js`. |
