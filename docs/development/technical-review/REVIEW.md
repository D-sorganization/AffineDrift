...
The next longer-first pass addresses #4158 in the contraction/tangent reference and its generated LaTeX edition. It replaces unsupported numerical tables and a nonfunctional proposed optimizer with a reproducible linear certificate, full differential derivations, and a golf outcome investigation. Eight independent/regression checks pass after the source guard failed on the previous editions. Its revised HTML renders 189 math expressions without MathJax errors, and historical critique anchors are preserved. The critique receives a dated revision note identifying the old source and limitations of its own proposed repairs; that note does not certify the entire critique.
