...
**Project Design Guidelines**: `IMPLEMENTATION_CHECKLIST.md`, `DEVELOPMENT_GUIDE.md` - **MANDATORY reference for scientific requirements**

Context: The domain is scientific modeling (e.g., biomechanics, control theory, universal joint mechanics).

### **PRIMARY OBJECTIVE: Validate Scientific Requirements from Design Guidelines**

You **MUST** validate implementation against the scientific requirements in the project documentation:

- **Mathematical Notation**: MathJax/LaTeX rendering correctness
- **Physical Models**: Universal joint mechanics, moment of inertia calculations
- **Coordinate Systems**: Frame consistency and transformation correctness
...
4. **Priority Gaps**: What breaks scientific credibility?
...
Coordinate Systems: Are frames of reference (World vs. Local) clear? Are transformations (quaternions, rotation matrices) handled via robust libraries (e.g., scipy.spatial) or hand-rolled implementation?
...
Singularities: Are division-by-zero or sqrt(negative) cases handled in the math? Check for gimbal lock handling in universal joint calculations.
...
Vectorization vs. Loops: CRITICAL: Identify explicit Python for loops performing math that should be NumPy vector operations. This is a performance and readability killer.
