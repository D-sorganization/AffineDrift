...
## 2026-06-24 - Add rel="noopener" to generated links
**Vulnerability:** Reverse tabnabbing vulnerability in links generated by `src/tools/utils/latex_utils.py` due to missing `rel="noopener"` on `target="_blank"` anchors.
**Learning:** Automatically generated content, such as links produced by LaTeX conversion scripts, must adhere to the same security standards as manually written HTML to prevent newly opened tabs from accessing the origin's `window.opener` object.
**Prevention:** Always include the `rel="noopener"` attribute whenever generating HTML anchors (`<a>`) with `target="_blank"`.
