...
## 2026-07-06 - Prevent Layout Thrashing with DocumentFragment
**Learning:** When dynamically generating and appending multiple child elements within a loop (such as rendering lists of search results), appending directly to a live DOM container triggers a layout recalculation and repaint on every iteration, leading to micro-stutters.
**Action:** Batch the DOM insertions by appending the elements to a `DocumentFragment` first, and then appending the fragment to the live DOM container once outside the loop to eliminate O(N) layout thrashing.
