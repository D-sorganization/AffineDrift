...
## 2026-06-24 - Batching DOM insertions in loops
**Learning:** Dynamically generating and appending multiple child elements directly to a live DOM container within a loop triggers a layout recalculation on every iteration. This can cause significant micro-stutters.
**Action:** Batch the DOM insertions by appending the elements to a `DocumentFragment` first, and then append the fragment to the live DOM container once outside the loop.
