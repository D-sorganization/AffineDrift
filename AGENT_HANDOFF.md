...
- Next: commit canonical paths, compare final affine head/squash trees and replay #4351 onto protected main, push and open ready PR, then update its single SPEC row to the PR key. Verify protected checks/auto-squash and revision-matched live evidence. Six proven render-only files restored; stage explicit paths because scratch files are untracked. Central development-log checker has pre-existing peer #3903/#3902 metadata gaps; preserve peer entries.
