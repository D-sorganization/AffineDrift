...
- Active #4349 under #4009/#4021/#4054, branch `fix/4349-affine-structure-rigor`, worktree `C:/Users/diete/Repositories/AffineDrift-technical-review`. Current implementation commit SELF; PR not created. Initially stacked on brain head 4f73040fa9ecccd1b9f25f84b4db7168e4e895b9; brain merged as 688dda81c3f0c38759d9994cbc0d5c0cd0478bd2. Complete paired affine rewrite is ready. Audit `docs/development/technical-review/affine-structure-review.md` records all technical decisions, independent examples, primary-source reading limits and failure history.
...
- Next: commit with normal hooks, fetch brain squash and compare its tree against 4f73040f before replaying only the affine commit. Preserve the validated implementation tree. Push and submit a focused PR closing #4349, enable protected squash auto-merge, then inspect exact live artifacts. The single SPEC row is keyed to issue #4349 until the PR exists. All QA sessions reaped; idle preview 8767 remains. Six proven render-only files restored byte-for-byte; stage explicit canonical paths only, never scratch artifacts en masse.
