...
- Next: enable/verify protected squash auto-merge for PR #4350, then inspect exact live artifacts. Normal commit/push hooks passed. The single SPEC row is keyed to PR #4350. All QA sessions reaped; idle preview 8767 remains. Six proven render-only files restored byte-for-byte; stage explicit canonical paths only. Central development-log check now reports only pre-existing peer #3903/#3902 missing metadata; preserve those entries. Complete constraint-forces print/web sources have now been read and contain independent mechanics, power, impulse and biological-inference errors; create a focused child issue and lease before implementation.
