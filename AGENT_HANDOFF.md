...
- PR #4162 for #4160 on `fix/physics-parallel-mechanisms` rebuilds the Physics of Golf closed-loop chapter and paired Quarto edition, with nine independent/regression checks, a corrected mechanical graph, and targeted anatomy and inverse-dynamics exercise corrections. Local validation passes 3,745 Python tests (92.32% source coverage), 128 content checks, and 34 static contracts. The 593-page PDF and changed website pages are inspected. The longer inverse-dynamics chapter is next under #4161; do not treat adjacent routes as cleared.
