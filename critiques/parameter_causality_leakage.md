...
2.  **Causal Leakage**: The "Drift" term (using high $K$) effectively includes the active stiffening. The "Input" term (residual) will show *less* input than actual.
...
## Suggested Remedies
1.  **Strict Identification Protocol**: Mandate that passive parameters (especially stiffness/damping) be identified from **passive experiments** (e.g., static deflection tests, pendulum drop tests) or physics-based estimation, *never* from the active swing data itself.
