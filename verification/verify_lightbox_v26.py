...
def verify_lightbox_focus():
    with sync_playwright() as p:
        ...
        # Now verify my logic.
        # It relies on .active class to set visibility.
        # But playwright kept saying 'not visible' because styles.css might be applying styles that override?
        # Or styles.css defines .lightbox-overlay with opacity: 0, visibility: hidden.
        # Adding .active should change that.
        # But if my manual injection didn't use the exact class or structure, maybe it failed.
        ...
        page.evaluate("""
           ...
           lightbox.className = 'lightbox-overlay'; // styles.css applies here
           ...
           window.triggerLightbox = function() {
             ...
             // The critical part: adding active class
             lightbox.classList.add('active');
             // styles.css: .lightbox-overlay.active { opacity: 1; visibility: visible; }
        ...
        # Verify visibility state via computed style
        opacity = page.evaluate('getComputedStyle(document.querySelector(.lightbox-overlay)).opacity')
        visibility = page.evaluate('getComputedStyle(document.querySelector(.lightbox-overlay)).visibility')
        print(f'Computed opacity: {opacity}, visibility: {visibility}')

        page.screenshot(path='verification/lightbox_open.png')

        # Close
        # Click center. If visible, it should intercept.
